...
export class LintValidator {
  ...
  private readonly config: LintValidationConfig;
   ...
   * Check if linting tools are available
   */
  private async checkLintToolsAvailable(): Promise<void> {
   ...
   */
  private analyzeLintResults(eslintResults: ESLintResult[]): LintSummary {
   ...
   */
  private calculateLintScore(summary: LintSummary): number {
   ...
   */
  private determineLintStatus(summary: LintSummary): ValidationStatus {
   ...
   */
  private generateLintMessage(summary: LintSummary): string {
