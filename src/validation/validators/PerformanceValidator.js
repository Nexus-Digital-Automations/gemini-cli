...
import { ValidationContext, ValidationResult, ValidationStatus, ValidationSeverity, } from '../core/ValidationEngine.js';
...
export class PerformanceValidator {
     ...
     */
    async gatherPerformanceMetrics(context) {
        const metrics = {};
        // Bundle analysis
     ...
     */
    async findBundleFiles(distPath) {
        const bundleFiles = [];
        try {
     ...
     */
    async runWebpackBundleAnalyzer(_result) {
        ...
        try {
            ...
            if (totalDuration > this.config.thresholds.buildTime.warningDurationMs) {
                metrics.bottlenecks.push({
                    ...
                    impact: totalDuration > this.config.thresholds.buildTime.maxDurationMs
                        ? 'high'
                        : 'medium',
     ...
     */
    assessUpdateImpact(current, latest) {
        try {
     ...
     */
    analyzePerformanceResults(metrics) {
        const issues = [];
        const recommendations = [];
        // Analyze bundle size
        if (metrics.bundleAnalysis) {
            const bundle = metrics.bundleAnalysis;
        ...
        if (metrics.buildPerformance) {
            const build = metrics.buildPerformance;
        ...
        if (metrics.codeComplexity) {
            const complexity = metrics.codeComplexity;
     ...
     */
    calculateOverallPerformanceScore(issues) {
        let score = 100;
     ...
     */
    determinePerformanceStatus(summary) {
     ...
     */
    determinePerformanceSeverity(summary) {
        const criticalIssues = summary.issues.filter((issue) => issue.severity === 'critical').length;
        const highIssues = summary.issues.filter((issue) => issue.severity === 'high').length;
        if (criticalIssues > 0) {
     ...
     */
    generatePerformanceMessage(summary) {
     ...
     */
    generatePerformanceDetails(summary) {
     ...
     */
    generatePerformanceSuggestions(summary) {
        const suggestions = [];
     ...
     */
    createPerformanceEvidence(summary) {
        const evidence = [];
