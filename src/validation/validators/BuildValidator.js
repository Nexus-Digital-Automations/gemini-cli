...
export class BuildValidator {
     ...
     */
    async checkIfBuildRequired(artifacts) {
        ...
        const hasTypeScript = artifacts.some(artifact => artifact.type === 'file' && artifact.path.endsWith('.ts'));
        ...
        const hasBuildConfig = await Promise.all(buildConfigs.map(async (config) => {
            try {
    ...
    async runBuild() {
        ...
        try {
            ...
            return {
                success: false,
                exitCode: error && typeof error === 'object' && 'code' in error ? error.code : 1,
                stdout: error && typeof error === 'object' && 'stdout' in error ? error.stdout : '',
                stderr: error && typeof error === 'object' && 'stderr' in error ? error.stderr : String(error),
                duration,
                outputFiles: [],
                errors: [{
     ...
     * Validate build outputs
     */
    async validateBuildOutputs(buildResult) {
        const expectedOutputs = this.config.expectedOutputs || [];
        const actualOutputs = buildResult.outputFiles;
        const expectedFound = expectedOutputs.filter(expected => actualOutputs.some(actual => actual.includes(expected)));
        const expectedMissing = expectedOutputs.filter(expected => !actualOutputs.some(actual => actual.includes(expected)));
     ...
     */
    analyzeBuildResults(buildResult, outputValidation) {
        return {
     ...
     */
    generateBuildDetails(summary, buildResult) {
     ...
     */
    generateBuildSuggestions(summary, buildResult) {
        const suggestions = [];
        if (!summary.success) {
