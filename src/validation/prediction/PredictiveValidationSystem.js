...
export class PredictiveValidationSystem extends EventEmitter {
     ...
     */
    getActiveAlerts() {
        return Array.from(this.activeAlerts.values()).sort((a, b) => b.confidence - a.confidence);
    }
    /**
     * Get prediction statistics
     */
    getPredictionStatistics() {
        const totalPredictions = Array.from(this.modelPerformance.values()).reduce((sum, perf) => sum + perf.predictions, 0);
        const totalCorrect = Array.from(this.modelPerformance.values()).reduce((sum, perf) => sum + perf.correctPredictions, 0);
        const totalConfidence = Array.from(this.modelPerformance.values()).reduce((sum, perf) => sum + perf.totalConfidence, 0);
        const recentMetrics = this.validationHistory.filter((m) => Date.now() - m.timestamp.getTime() <
    ...
    runMovingAverageModel(config) {
        ...
        const currentFailureRate = currentWindow.filter((m) => m.status === 'failed').length /
            currentWindow.length;
        const previousFailureRate = previousWindow.filter((m) => m.status === 'failed').length /
            previousWindow.length;
        const trend = currentFailureRate > previousFailureRate
            ? 'declining'
    ...
    runPatternMatchingModel(config) {
        ...
        for (let i = patternLength; i <= this.validationHistory.length - patternLength; i++) {
            ...
            if (this.calculatePatternSimilarity(currentPattern, pattern) >=
                similarityThreshold) {
        ...
        const failureProbability = historicalPatterns.filter((p) => p.nextResult.status === 'failed')
            .length / historicalPatterns.length;
        const avgNextScore = historicalPatterns.reduce((sum, p) => sum + p.nextResult.score, 0) /
    ...
    runAnomalyDetectionModel(config) {
        ...
        const anomalies = [];
        // Detect anomalies
        if (Math.abs(latest.score - scoreStats.mean) >
        ...
        if (Math.abs(latest.duration - durationStats.mean) >
        ...
        if (Math.abs(latest.memoryUsage - memoryStats.mean) >
        ...
        const trend = anomalies.some((a) => a.direction === 'declining' || a.direction === 'increasing')
            ? 'declining'
            : anomalies.length === 0
                ? 'stable'
                : 'improving';
    ...
    cleanupHistoricalData() {
        ...
        if (this.validationHistory.length >
    ...
    cleanupAlerts() {
        ...
        if (this.activeAlerts.size > this.config.alerting.maxActiveAlerts) {
            const sorted = Array.from(this.activeAlerts.entries()).sort(([, a], [, b]) => b.timestamp.getTime() - a.timestamp.getTime());
