...
export class ValidationIntegration {
    ...
    async validateFeatureCompletion(featureId) {
        try {
            ...
            return await this.validationSystem.validateTaskCompletion(TaskType.FEATURE_IMPLEMENTATION, context);
        }
        catch (error) {
            throw new Error(`Failed to validate feature completion: ${error.message}`);
        }
    }
    /**
    ...
    async validateProjectCompletion() {
        ...
        return await this.validationSystem.validateTaskCompletion(TaskType.REFACTORING, context);
    }
    /**
    ...
    shouldBlockContinuation(validationResult) {
        ...
        const errorGatesFailed = validationResult.qualityGateResults.some((gate) => !gate.passed && gate.severity === 'error');
    ...
    generateStopAuthorizationRecommendation(validationResult) {
        const canAuthorizeStop = validationResult.passed &&
...
export class ValidationCLI {
    ...
    async executeCommand(command, args) {
        try {
            ...
            switch (command) {
                case 'validate-task':
                    result = await this.integration.validateTaskCompletion(args.description || 'Task completion validation', args.category || 'feature');
                    break;
                ...
                case 'validate-commit':
                    result = await this.integration.validateCommitReadiness(args.commitMessage || 'feat: task completion');
                    break;
                default:
 ...
 */
export async function validateTaskCompletion(projectRoot, taskDescription, taskCategory = 'feature') {
 ...
 */
export async function getStopAuthorizationStatus(projectRoot, taskDescription = 'Project completion check') {
    const integration = new ValidationIntegration(projectRoot);
