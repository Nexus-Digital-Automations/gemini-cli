...
export class AutomaticValidationSystem {
   ...
   */
  private getTaskSpecificGates(taskType: TaskType): QualityGateDefinition[] {
    switch (taskType) {
   ...
   */
  private determineOverallResult(gateResults: QualityGateResult[]): {
   ...
   */
  private estimateEffortForGate(gate: QualityGateResult): EffortEstimate {
   ...
   */
  private createErrorReport(error: Error): ValidationReport {
    return {
      id: `error-${Date.now()}`,
      sessionId: 'unknown',
...
abstract class QualityGateExecutor {
  constructor(protected logger: ValidationLogger) {}
