...
export class FailureRecoverySystem extends EventEmitter {
  ...
  private readonly recoveryActions: Map<string, RecoveryAction>;
  private readonly failureHistory: Map<string, ValidationFailure[]>;
  private readonly recoveryHistory: Map<string, RecoveryResult[]>;
   ...
   */
  private extractFailures(report: ValidationReport): ValidationFailure[] {
  ...
  private classifyFailureCategory(result: ValidationResult): 'build' | 'test' | 'lint' | 'security' | 'performance' | 'other' {
    ...
    if (criteriaId.includes('test')) return 'test';
    if (criteriaId.includes('lint')) return 'lint';
    if (criteriaId.includes('security')) return 'security';
    if (criteriaId.includes('performance')) return 'performance';
   ...
   */
  private storeFailure(failure: ValidationFailure): void {
   ...
   */
  public getRecoveryStatistics(): RecoveryStats {
