...
export class ValidationReporter extends EventEmitter {
    ...
    getStorageStats() {
        ...
        const oldestReport = Array.from(this.reports.values())
            .reduce((oldest, report) => !oldest || report.timestamp < oldest.timestamp ? report : oldest, null);
        const newestReport = Array.from(this.reports.values())
            .reduce((newest, report) => !newest || report.timestamp > newest.timestamp ? report : newest, null);
        return {
            totalReports,
            totalSize,
            compressionEnabled: this.config.compressionEnabled,
