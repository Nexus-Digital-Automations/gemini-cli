...
export class ValidationReporter extends EventEmitter {
    ...
    async queryReports(filters = {}) {
        ...
        if (filters.categories && filters.categories.length > 0) {
            results = results.filter((r) => r.results.some((result) => filters.categories.some((cat) => result.criteriaId.includes(cat))));
        }
    ...
    getStorageStats() {
        ...
        const totalSize = Array.from(this.reports.values()).reduce((sum, report) => sum + JSON.stringify(report).length, 0);
        const oldestReport = Array.from(this.reports.values()).reduce((oldest, report) => !oldest || report.timestamp < oldest.timestamp ? report : oldest, null);
        const newestReport = Array.from(this.reports.values()).reduce((newest, report) => !newest || report.timestamp > newest.timestamp ? report : newest, null);
        return {
            totalReports,
            totalSize,
            compressionEnabled: this.config.compressionEnabled,
    ...
    calculatePerformanceTrend(recent, older) {
        if (recent.length === 0 || older.length === 0)
            return 'stable';
        const recentAvgDuration = recent.reduce((sum, r) => sum + r.performance.totalDuration, 0) /
            recent.length;
        const olderAvgDuration = older.reduce((sum, r) => sum + r.performance.totalDuration, 0) /
            older.length;
        const difference = recentAvgDuration - olderAvgDuration;
        if (difference < -5000)
            return 'improving'; // 5 seconds improvement
        if (difference > 5000)
            return 'declining'; // 5 seconds worse
        return 'stable';
    }
    /**
...
`;
        reports.forEach((report) => {
            const scoreClass = report.overallScore >= 80
                ? 'score-excellent'
...
`;
     ...
     */
    async exportAsPdf(reports, options) {
    ...
    async performCleanup() {
        ...
        if (this.reports.size > this.config.maxReports) {
            const sortedReports = Array.from(this.reports.entries()).sort(([, a], [, b]) => a.timestamp.getTime() - b.timestamp.getTime());
