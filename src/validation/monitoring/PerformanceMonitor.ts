...
export class PerformanceMonitor extends EventEmitter {
  ...
  private readonly alerts: Map<string, PerformanceAlert>;
  private readonly validationHistory: ValidationReport[];
   ...
   */
  public recordValidation(report: ValidationReport): void {
    this.logger.debug(
      `Recording validation performance for task: ${report.taskId}`,
      {
  ...
  private getValidationMetrics() {
    ...
    const durations = this.validationHistory.map(
      (v) => v.performance.totalDuration,
    );
  ...
  private calculateTrends(): PerformanceTrends {
    ...
    return {
      validationDuration: this.calculateDurationTrend(
        recentHistory,
        olderHistory,
      ),
  ...
  private calculateSuccessRateTrend(
    ...
    const olderSuccess = older.filter(
      (v) => v.overallStatus === 'passed',
    ).length;
  ...
  private generateOptimizationRecommendations() {
    ...
    const slowValidations = this.validationHistory.filter(
      (v) => v.performance.totalDuration > 30000,
    );

    if (slowValidations.length > this.validationHistory.length * 0.2) {
      recommendations.push(
        'Consider optimizing validation criteria execution time',
      );
      estimatedImprovements.duration_improvement = 25;
    }

    const highFailureRate =
      this.validationHistory.filter((v) => v.overallStatus === 'failed')
        .length / Math.max(1, this.validationHistory.length);

    if (highFailureRate > 0.3) {
      recommendations.push(
        'High failure rate detected - review task complexity',
      );
    ...
    if (memoryUsage.heapUsed / memoryUsage.heapTotal > 0.8) {
      recommendations.push(
        'High memory usage - consider implementing memory optimization',
      );
   ...
   */
  private shouldTriggerAlert(alert: PerformanceAlert): boolean {
    if (!alert.lastTriggered) return true;

    const cooldownExpired =
      Date.now() - alert.lastTriggered.getTime() > alert.cooldownPeriod;
  ...
  private trimMetrics(): void {
    ...
    retentionCutoff.setDate(
      retentionCutoff.getDate() - this.metricsRetentionDays,
    );
