...
export class PerformanceMonitor extends EventEmitter {
   ...
   */
  private calculateDurationTrend(recent: ValidationReport[], older: ValidationReport[]) {
    const recentAvg = recent.length > 0 ?
      recent.reduce((sum, v) => sum + v.performance.totalDuration, 0) / recent.length : 0;
    const olderAvg = older.length > 0 ?
      older.reduce((sum, v) => sum + v.performance.totalDuration, 0) / older.length : 0;
   ...
   */
  private calculateSuccessRateTrend(recent: ValidationReport[], older: ValidationReport[]) {
  ...
  private getActiveAlerts() {
    const activeAlerts = Array.from(this.alerts.values()).filter(
      alert => this.shouldTriggerAlert(alert)
    );
  ...
  private generateOptimizationRecommendations() {
    ...
    const highFailureRate = this.validationHistory.filter(
      v => v.overallStatus === 'failed'
    ...
    return {
      ...
      validations: {
        ...
        successRate: (metrics.validation.successfulValidations /
          Math.max(1, metrics.validation.totalValidations)) * 100,
