...
import { DependencyAnalyzer, TaskNode, TaskDependency, DependencyType as _DependencyType, ExecutionReadiness as _ExecutionReadiness, SchedulingResult as _SchedulingResult, DependencyAnalysisResult, } from './DependencyAnalyzer.js';
/**
...
export class IntelligentTaskScheduler extends EventEmitter {
    ...
    getPerformanceAnalytics() {
        ...
        const averageQueueTime = completedTasksArray.reduce((sum, ctx) => sum + ctx.metrics.queueTime, 0) /
    ...
    getReadyTasks() {
        return this.executionQueue.filter((ctx) => {
            ...
            const dependenciesSatisfied = ctx.dependencies.every((depId) => this.completedTasks.has(depId));
    ...
    canAllocateResources(requested) {
        ...
        return (requested.cpu <= available.cpu &&
    ...
    calculateResourceEfficiencyScore(resources) {
        ...
        let score = 0;
        if (utilization.cpu < 50)
            score += resources.cpu;
        if (utilization.memory < 50)
            score += resources.memory / 100;
        if (utilization.disk < 50)
            score += resources.disk;
        if (utilization.network < 50)
            score += resources.network / 10;
        return score;
    }
    ...
    calculateThroughput() {
        ...
        const recentHistory = this.performanceHistory.filter((metric) => Date.now() - metric.timestamp.getTime() < timeWindow);
        if (recentHistory.length === 0) {
            return 0;
        }
        const completedInWindow = Array.from(this.completedTasks.values()).filter((ctx) => ctx.actualEndTime &&
            Date.now() - ctx.actualEndTime.getTime() < timeWindow).length;
    ...
    calculateResourceEfficiency() {
        const utilization = this.calculateCurrentResourceUtilization();
        const averageUtilization = (utilization.cpu +
            utilization.memory +
            utilization.disk +
            utilization.network) /
            4;
    ...
    identifyBottlenecks() {
        ...
        if (utilization.cpu > 90)
            bottlenecks.push('CPU constraint');
        if (utilization.memory > 90)
            bottlenecks.push('Memory constraint');
        if (utilization.disk > 90)
            bottlenecks.push('Disk constraint');
        if (utilization.network > 90)
            bottlenecks.push('Network constraint');
        return bottlenecks;
    }
