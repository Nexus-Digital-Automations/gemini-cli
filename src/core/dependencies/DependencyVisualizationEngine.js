...
export class DependencyVisualizationEngine extends EventEmitter {
    ...
    getTimelineData(timeRange) {
        ...
        if (timeRange) {
            timeline = timeline.filter(event => event.timestamp >= timeRange.start && event.timestamp <= timeRange.end);
        }
    ...
    handleTaskEvent(event) {
        ...
        if (node) {
            ...
            node.color = this.calculateNodeColor({ status: node.status }, { readiness: node.readiness });
        }
        // Add to timeline
        const timelineEvent = {
