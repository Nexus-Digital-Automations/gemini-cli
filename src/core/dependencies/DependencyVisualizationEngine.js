...
import { DependencyAnalyzer, TaskNode, TaskDependency, DependencyType, DependencyStatus, ExecutionReadiness, DependencyConflict, } from './DependencyAnalyzer.js';
import { IntelligentTaskScheduler, ExecutionContext, SystemMetrics, SchedulingEvent, } from './IntelligentTaskScheduler.js';
/**
...
export class DependencyVisualizationEngine extends EventEmitter {
    ...
    getVisualizationData() {
        ...
        const conflicts = Array.from(this.alerts.values()).filter((alert) => alert.type === 'conflict' && !alert.resolved).length;
        return {
            nodes,
            edges,
    ...
    getTimelineData(timeRange) {
        ...
        if (timeRange) {
            timeline = timeline.filter((event) => event.timestamp >= timeRange.start &&
    ...
    calculateNodePosition(nodeData) {
        ...
        return {
            ...
            z: this.visualizationData.layout.dimensions === '3d'
                ? Math.random() * 100
                : undefined,
        };
    }
    ...
    calculateEdgeStyle(edgeData) {
        if (edgeData.status === DependencyStatus.FAILED)
            return 'dashed';
        if (edgeData.weight > 0.8)
            return 'thick';
        if (edgeData.type === DependencyType.CONDITIONAL)
            return 'dotted';
        return 'solid';
    }
    // =================== EVENT HANDLERS ===================
    ...
    calculateAverageExecutionTime(nodes) {
        const executedNodes = nodes.filter((node) => node.metadata.actualDuration !== undefined);
        if (executedNodes.length === 0)
            return 0;
        const totalTime = executedNodes.reduce((sum, node) => sum + (node.metadata.actualDuration || 0), 0);
    ...
    calculatePerformanceTrends() {
        ...
        const recentAvg = {
            ...
            resourceUtilization: recent.reduce((sum, m) => sum + m.resourceEfficiency, 0) /
                recent.length,
        };
        const olderAvg = {
            throughput: older.reduce((sum, m) => sum + m.taskThroughput, 0) /
                Math.max(older.length, 1),
            successRate: older.reduce((sum, m) => sum + m.successRate, 0) /
                Math.max(older.length, 1),
            resourceUtilization: older.reduce((sum, m) => sum + m.resourceEfficiency, 0) /
                Math.max(older.length, 1),
        };
        return {
