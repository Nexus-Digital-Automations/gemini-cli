...
export class CrossSessionPersistenceEngine extends EventEmitter {
  private readonly config: CrossSessionConfig;
   ...
   */
  async saveTaskState(taskState: CrossSessionTaskState): Promise<void> {
    const startTime = performance.now();
   ...
   */
  async loadTaskState(taskId: string): Promise<CrossSessionTaskState | null> {
    const startTime = performance.now();
    ...
    try {
      ...
      if (cached && this.isCacheValid(cached.timestamp)) {
        ...
        return cached.data as CrossSessionTaskState;
      }
   ...
   * Get all task states
   */
  async getAllTaskStates(): Promise<Map<string, CrossSessionTaskState>> {
    this.ensureInitialized();
  ...
  private validateTaskState(taskState: CrossSessionTaskState): void {
      ...
      for (const file of files) {
        if (file.startsWith('task-') && file.endsWith('.json')) {
          try {
            ...
            const taskState: CrossSessionTaskState = JSON.parse(data);
  ...
  private isSessionCrashed(sessionInfo: SessionInfo): boolean {
    const now = Date.now();
