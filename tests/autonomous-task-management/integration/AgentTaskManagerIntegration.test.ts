...
class MockTaskManager {
  ...
  async assignAgentToTask(taskId: string, agent: SubAgentScope): Promise<void> {
...
describe('Agent-TaskManager Integration Tests', () => {
  ...
  describe('Basic Task-Agent Integration', () => {
    it('should create and assign tasks to agents', async () => {
      ...
      const mockAgentConfig: MockAgentConfig = {
  ...
  describe('Multi-Agent Coordination', () => {
    it('should coordinate multiple agents working on related tasks', async () => {
      ...
      const preparationAgent: MockAgentConfig = {
      ...
      const processingAgent: MockAgentConfig = {
      ...
      // Create agents
      const prepAgent = await MockAgentFactory.createMockAgent(
        preparationAgent,
        mockConfig,
        preparationTask.promptConfig,
        preparationTask.modelConfig,
        preparationTask.runConfig,
        preparationTask.options,
      );

      const procAgent = await MockAgentFactory.createMockAgent(
        processingAgent,
        mockConfig,
        processingTask.promptConfig,
        processingTask.modelConfig,
        processingTask.runConfig,
        processingTask.options,
      );
    ...
    it('should handle concurrent task execution', async () => {
      ...
      for (let i = 0; i < taskConfigs.length; i++) {
        ...
        const agent = await MockAgentFactory.createMockAgent(
          agentConfigs[i],
          mockConfig,
          taskConfigs[i].promptConfig,
          taskConfigs[i].modelConfig,
          taskConfigs[i].runConfig,
          taskConfigs[i].options,
        );
  ...
  describe('Error Handling and Recovery', () => {
    it('should handle partial failures in multi-agent scenarios', async () => {
      ...
      const successAgent: MockAgentConfig = {
        name: 'success-agent',
        state: MockAgentState.SUCCESS,
      ...
      const successAgentInstance = await MockAgentFactory.createMockAgent(
        successAgent,
        mockConfig,
        successTask.promptConfig,
        successTask.modelConfig,
        successTask.runConfig,
        successTask.options,
      );

      const failureAgentInstance = await MockAgentFactory.createMockAgent(
        failureAgent,
        mockConfig,
        failureTask.promptConfig,
        failureTask.modelConfig,
        failureTask.runConfig,
        failureTask.options,
      );
    ...
    it('should provide detailed error information for debugging', async () => {
      ...
      const agent = await MockAgentFactory.createMockAgent(
        failureAgent,
        mockConfig,
        complexTask.promptConfig,
        complexTask.modelConfig,
        complexTask.runConfig,
        complexTask.options,
      );
  ...
  describe('Performance and Resource Management', () => {
    it('should track execution metrics', async () => {
      ...
      const performanceAgent: MockAgentConfig = {
      ...
      const agent = await MockAgentFactory.createMockAgent(
        performanceAgent,
        mockConfig,
        performanceTask.promptConfig,
        performanceTask.modelConfig,
        performanceTask.runConfig,
        performanceTask.options,
      );
    ...
    it('should handle resource constraints gracefully', async () => {
      ...
      const intensiveTasks = Array.from({ length: 5 }, (_, i) =>
        new TaskBuilder()
      ...
      for (let i = 0; i < intensiveTasks.length; i++) {
        ...
        const agent = await MockAgentFactory.createMockAgent(
          agentConfigs[i],
          mockConfig,
          intensiveTasks[i].promptConfig,
          intensiveTasks[i].modelConfig,
          intensiveTasks[i].runConfig,
          intensiveTasks[i].options,
        );
