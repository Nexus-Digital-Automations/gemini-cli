...
class MockSessionStorage {
    ...
    async saveContextSnapshot(snapshotId, snapshot) {
        this.contextSnapshots.set(snapshotId, snapshot);
    }
    async loadContextSnapshot(snapshotId) {
        return this.contextSnapshots.get(snapshotId) || null;
    }
    async getAllActiveSessions() {
        const now = Date.now();
        const activeThreshold = 24 * 60 * 60 * 1000; // 24 hours
        return Array.from(this.sessions.values()).filter((session) => now - session.lastUpdated < activeThreshold);
    }
    ...
    async cleanup() {
        ...
        for (const [snapshotId] of this.contextSnapshots.entries()) {
            ...
            const referenced = Array.from(this.sessions.values()).some((session) => session.contextSnapshots.includes(snapshotId));
            if (!referenced) {
...
class CrossSessionPersistenceManager {
    ...
    async createTaskCheckpoint(taskId, step, progress, data) {
    ...
    async saveContextSnapshot(context, metadata) {
        if (!this.activeSession) {
    ...
    async restoreContextSnapshot(snapshotId) {
    ...
    async migrateTaskToNewSession(taskId, newSessionId) {
        const taskState = await this.storage.loadTaskState(taskId);
        if (!taskState) {
            return false;
        }
    ...
    async getSessionMetrics() {
        ...
        const allTaskStates = await Promise.all(this.activeSession.taskIds.map((taskId) => this.storage.loadTaskState(taskId)));
        const validTaskStates = allTaskStates.filter((ts) => ts !== null);
...
describe('Cross-Session Task Persistence Tests', () => {
    ...
    describe('Task State Persistence', () => {
        it('should create and restore task checkpoints', async () => {
            ...
            const checkpoint1 = await persistenceManager.createTaskCheckpoint(taskId, 'initialization', 0.1, { initialized: true, config: 'loaded' });
            const checkpoint2 = await persistenceManager.createTaskCheckpoint(taskId, 'data_processing', 0.5, { processed_items: 50, total_items: 100 });
            const checkpoint3 = await persistenceManager.createTaskCheckpoint(taskId, 'finalization', 0.9, { results: 'generated', cleanup: 'completed' });
        ...
        it('should persist and restore complex task states', async () => {
            ...
            }, mockConfig, task.promptConfig, task.modelConfig, task.runConfig, task.options);
    ...
    describe('Context Persistence and Restoration', () => {
        ...
        it('should maintain context integrity during concurrent modifications', async () => {
            ...
            const modificationPromises = Array.from({ length: 5 }, async (_, i) => {
                ...
                const operations = modifiedContext.get('concurrent_operations');
    ...
    describe('Cross-Session Workflow Continuity', () => {
        it('should resume complex workflows across sessions', async () => {
            ...
            }, mockConfig, step1Task.promptConfig, step1Task.modelConfig, step1Task.runConfig, step1Task.options);
            await step1Agent.runNonInteractive(workflowContext);
            // Update context and create checkpoint
            const completedSteps = workflowContext.get('completed_steps');
            ...
            }, mockConfig, step2Task.promptConfig, step2Task.modelConfig, step2Task.runConfig, step2Task.options);
            ...
            // Update progress
            const updatedCompletedSteps = restoredContext.get('completed_steps');
        ...
        it('should handle workflow failure and recovery across sessions', async () => {
            ...
            }, mockConfig, { systemPrompt: 'Failing task' }, { model: 'gemini-1.5-flash', temp: 0.5, top_p: 1 }, { max_time_minutes: 1, max_turns: 3 });
            ...
            }, mockConfig, { systemPrompt: 'Recovery from failure' }, { model: 'gemini-1.5-flash', temp: 0.5, top_p: 1 }, { max_time_minutes: 2, max_turns: 5 });
    ...
    describe('Performance and Data Integrity', () => {
        it('should handle large context data efficiently', async () => {
            ...
            const originalDataset = largeContext.get('large_dataset');
            const restoredDataset = restoredContext.get('large_dataset');
...
//# sourceMappingURL=TaskPersistence.test.js.map