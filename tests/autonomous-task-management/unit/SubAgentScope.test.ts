...
describe('SubAgentScope - Autonomous Task Management Tests', () => {
  ...
  describe('Task Creation and Configuration', () => {
    it('should create agents with different complexity levels', async () => {
      ...
      const mockSimple: MockAgentConfig = {
        name: 'simple-task',
        state: MockAgentState.SUCCESS,
        executionTime: 500,
      };

      const mockComplex: MockAgentConfig = {
        name: 'complex-task',
        state: MockAgentState.SUCCESS,
        executionTime: 2000,
      };

      const simpleAgent = await MockAgentFactory.createMockAgent(
        mockSimple,
        mockConfig,
        simpleTask.promptConfig,
        simpleTask.modelConfig,
        simpleTask.runConfig,
        simpleTask.options,
      );

      const complexAgent = await MockAgentFactory.createMockAgent(
        mockComplex,
        mockConfig,
    ...
    it('should validate required output variables', async () => {
      ...
      expect(agent.output.terminate_reason).toBe(SubagentTerminateMode.GOAL);
      expect(Object.keys(agent.output.emitted_vars)).toEqual(
        expect.arrayContaining(['result', 'status', 'metrics']),
      );
    });
  });
  ...
  describe('Priority and Scheduling', () => {
    it('should handle different task priorities', async () => {
      ...
      const agents = await Promise.all(
        mockConfigs.map((config, index) =>
          MockAgentFactory.createMockAgent(
            config,
            mockConfig,
            tasks[index].promptConfig,
            tasks[index].modelConfig,
            tasks[index].runConfig,
            tasks[index].options,
          ),
        ),
      );
    ...
    it('should handle task dependencies', async () => {
      ...
      const dependencyMock: MockAgentConfig = {
      ...
      const mainMock: MockAgentConfig = {
        name: 'main-task',
        state: MockAgentState.SUCCESS,
        executionTime: 500,
      ...
      const dependencyAgent = await MockAgentFactory.createMockAgent(
        dependencyMock,
        mockConfig,
        dependencyTask.promptConfig,
        dependencyTask.modelConfig,
        dependencyTask.runConfig,
        dependencyTask.options,
      );
      ...
      const mainAgent = await MockAgentFactory.createMockAgent(
        mainMock,
        mockConfig,
        mainTask.promptConfig,
        mainTask.modelConfig,
        mainTask.runConfig,
        mainTask.options,
      );
  ...
  describe('Error Handling and Recovery', () => {
    it('should handle agent failures gracefully', async () => {
      ...
      await expect(agent.runNonInteractive(new ContextState())).rejects.toThrow(
  ...
  describe('Performance Characteristics', () => {
    ...
    it('should handle different complexity performance characteristics', async () => {
      ...
      expect(executionTimes[TaskComplexity.SIMPLE]).toBeLessThan(
        executionTimes[TaskComplexity.VERY_COMPLEX],
      );
    });
  });
