...
export type PIIType = 'email' | 'phone' | 'ssn' | 'credit_card' | 'ip_address' | 'passport' | 'driver_license' | 'medical_id' | 'bank_account' | 'crypto_address' | 'api_key' | 'password';
...
export declare class DataProtectionManager extends EventEmitter {
     ...
     */
    encryptData(data: string | Buffer, classification: DataClassification, options?: {
        keyId?: string;
        algorithm?: string;
     ...
     */
    redactPII(content: string, options?: {
     ...
     */
    classifyData(content: string, metadata?: Record<string, unknown>): Promise<DataClassification>;
    /**
     * Store data securely with automatic classification and encryption.
     */
    storeSecurely(identifier: string, data: string | Buffer, options: SecureStorageOptions): Promise<string>;
    /**
