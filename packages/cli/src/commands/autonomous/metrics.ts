...
function getSuccessRateDisplay(rate: number): string {
...
function getUsageDisplay(usage: number, warningThreshold: number): string {
...
function getUtilizationDisplay(utilization: number): string {
...
function getErrorRateDisplay(rate: number): string {
...
function getQualityDisplay(rate: number): string {
  const percentage = (rate * 100).toFixed(1);
  if (rate >= 0.95) return chalk.green(`${percentage}%`);
  if (rate >= 0.85) return chalk.yellow(`${percentage}%`);
  return chalk.red(`${percentage}%`);
