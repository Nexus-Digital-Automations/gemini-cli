...
export const retryTaskCommand: CommandModule<{}, RetryTaskOptions> = {
  ...
  builder: (yargs) =>
    yargs
      ...
      .example('gemini autonomous tasks retry task_456 --reset-progress --new-priority high', 'Retry with reset and high priority')
      .example('gemini autonomous tasks retry task_789 --reason "Environment fixed"', 'Retry with reason'),

  handler: async (argv) => {
    try {
      console.log(chalk.cyan(`🔄 Preparing to retry task: ${chalk.bold(argv.taskId)}...`));
      ...
      const mockCurrentStatus = ['failed', 'cancelled', 'completed', 'in_progress'][Math.floor(Math.random() * 4)];
      ...
      console.log(`   Status: ${getStatusColor(mockCurrentStatus)(mockCurrentStatus.toUpperCase())}`);
      ...
      if (mockCurrentStatus === 'in_progress') {
        ...
        console.log(chalk.blue('💡 Cancel the task first if you need to restart it'));
        process.exit(1);
      }

      if (mockCurrentStatus === 'completed') {
        console.log(chalk.yellow('⚠️  Task is already completed'));
        console.log(chalk.blue('💡 Use "add" command to create a new similar task if needed'));
        process.exit(1);
      }

      if (mockRetryCount >= mockMaxRetries && !argv['max-retries']) {
        console.log(chalk.red('❌ Task has reached maximum retry attempts'));
        console.log(chalk.blue('💡 Use --max-retries flag to override the limit'));
        process.exit(1);
      }

      // Show retry configuration
      console.log(chalk.blue('\n⚙️  Retry Configuration:'));
      console.log(`   Reset Progress: ${argv['reset-progress'] ? chalk.green('Yes') : chalk.yellow('No')}`);

      if (argv['new-priority']) {
        console.log(`   Priority Change: ${getPriorityColor(argv['new-priority'])(argv['new-priority'].toUpperCase())}`);
      }

      if (argv['max-retries']) {
        console.log(`   Max Retries Override: ${chalk.cyan(argv['max-retries'])}`);
      }

      console.log(`   Wait for Dependencies: ${argv['wait-for-dependencies'] ? chalk.green('Yes') : chalk.yellow('No')}`);
      ...
      if (argv['wait-for-dependencies']) {
        ...
        mockDepStatuses.forEach(dep => {
          const statusColor = dep.status === 'completed' ? chalk.green :
                            dep.status === 'failed' ? chalk.red : chalk.yellow;
        ...
        if (!allDepsComplete) {
          ...
          console.log(chalk.blue('💡 Task will be queued and start when dependencies are ready'));
        }
      }
      ...
      console.log(chalk.blue('\n📊 Updated Task Status:'));
      console.log(`   Status: ${chalk.yellow('QUEUED')} → ${chalk.blue('PENDING_RETRY')}`);
      console.log(`   Retry Count: ${mockRetryCount} → ${chalk.green(newRetryCount)}`);
      console.log(`   Queue Position: ${chalk.cyan('#' + queuePosition)}`);

      if (argv['new-priority']) {
        console.log(`   Priority Updated: ${getPriorityColor(argv['new-priority'])(argv['new-priority'].toUpperCase())}`);
      }

      if (argv['reset-progress']) {
        console.log(`   Progress Reset: ${chalk.yellow('65%')} → ${chalk.gray('0%')}`);
      }
      ...
      const selectedImprovements = improvements.slice(0, Math.floor(Math.random() * 3) + 1);
      selectedImprovements.forEach(improvement => {
        console.log(`   • ${improvement}`);
      });

      // Estimated timeline
      const estimatedStart = argv['wait-for-dependencies'] && !allDepsComplete ?
        'When dependencies complete' :
        `${Math.floor(Math.random() * 5) + 1} minutes`;

      const estimatedDuration = argv['reset-progress'] ? '45-60 minutes' : '20-30 minutes';
      ...
      // Show next steps
      console.log(chalk.blue('\n🔄 Next Steps:'));
      console.log('   • Monitor progress: gemini autonomous tasks show ' + argv.taskId);
