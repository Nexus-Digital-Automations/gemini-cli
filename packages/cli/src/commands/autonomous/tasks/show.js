...
export const showTaskCommand = {
    ...
    builder: (yargs) => yargs
        ...
        .example('gemini autonomous tasks show task_123', 'Show basic task details')
        .example('gemini autonomous tasks show task_456 --verbose --logs', 'Show detailed info with logs')
        .example('gemini autonomous tasks show task_789 --show-subtasks --show-dependencies', 'Show complete task tree'),
    handler: async (argv) => {
        try {
            console.log(chalk.cyan(`📋 Loading task details: ${chalk.bold(argv.taskId)}...`));
            ...
            const mockTask = {
                ...
                description: 'Create comprehensive authentication system with OAuth2, JWT tokens, password reset, and user session management.',
                status: 'IN_PROGRESS',
                priority: 'HIGH',
                category: 'FEATURE',
                type: 'IMPLEMENTATION',
            ...
            console.log(`   Status: ${getStatusColor(mockTask.status)(mockTask.status)}`);
            console.log(`   Priority: ${getPriorityColor(mockTask.priority)(mockTask.priority)}`);
            console.log(`   Category: ${chalk.cyan(mockTask.category)}`);
            console.log(`   Type: ${chalk.magenta(mockTask.type)}`);
            // Progress information
            console.log(chalk.blue('\n⏱️  Progress & Timing:'));
            console.log(`   Overall Progress: ${getProgressBar(mockTask.progress)} ${mockTask.progress}%`);
            console.log(`   Subtasks: ${chalk.green(mockTask.completedSubtasks)}/${mockTask.totalSubtasks} completed`);
            console.log(`   Est. Time Remaining: ${chalk.yellow(mockTask.estimatedTimeRemaining)}`);
            console.log(`   Created: ${chalk.gray(mockTask.createdAt.toLocaleString())}`);
            console.log(`   Started: ${chalk.gray(mockTask.startedAt.toLocaleString())}`);
            console.log(`   Last Updated: ${chalk.gray(mockTask.updatedAt.toLocaleString())}`);
            // Agent information
            console.log(chalk.blue('\n🤖 Agent Assignment:'));
            console.log(`   Assigned Agent: ${chalk.bold(mockTask.assignedAgent)}`);
            console.log(`   Agent Status: ${getAgentStatusColor(mockTask.agentStatus)(mockTask.agentStatus)}`);
            console.log(`   Retry Count: ${mockTask.retryCount}/${mockTask.maxRetries}`);
            // Show description
            console.log(chalk.blue('\n📝 Description:'));
            ...
            if (argv.verbose) {
                ...
                console.log(`   Max Execution Time: ${mockTask.maxExecutionTime} minutes`);
            }
            // Show dependencies if requested
            if (argv['show-dependencies']) {
                console.log(chalk.blue('\n🔗 Dependencies:'));
                if (mockTask.dependencies.length > 0) {
                    console.log(`   Depends on: ${chalk.yellow(mockTask.dependencies.join(', '))}`);
                }
                else {
                    console.log(`   No dependencies`);
                }
                if (mockTask.dependents.length > 0) {
                    console.log(`   Required by: ${chalk.cyan(mockTask.dependents.join(', '))}`);
                }
                else {
            ...
            if (argv['show-subtasks']) {
                console.log(chalk.blue('\n📋 Subtasks:'));
                const mockSubtasks = [
                    { id: 'sub_1', title: 'Set up JWT token generation', status: 'COMPLETED', progress: 100 },
                    { id: 'sub_2', title: 'Implement password hashing', status: 'COMPLETED', progress: 100 },
                    { id: 'sub_3', title: 'Create authentication middleware', status: 'COMPLETED', progress: 100 },
                    { id: 'sub_4', title: 'Build OAuth2 integration', status: 'IN_PROGRESS', progress: 80 },
                    { id: 'sub_5', title: 'Add session management', status: 'IN_PROGRESS', progress: 40 },
                    { id: 'sub_6', title: 'Implement password reset', status: 'QUEUED', progress: 0 },
                    { id: 'sub_7', title: 'Write comprehensive tests', status: 'QUEUED', progress: 0 }
                ];
                mockSubtasks.forEach((subtask, index) => {
                    ...
                    console.log(`      Status: ${getStatusColor(subtask.status)(subtask.status)} (${subtask.progress}%)`);
                });
            }
            ...
            if (argv.logs) {
                ...
                mockLogs.forEach(log => {
                    ...
                    if (level) {
                        ...
                        const color = logLevel === 'ERROR' ? chalk.red :
                            logLevel === 'WARN' ? chalk.yellow :
                                logLevel === 'DEBUG' ? chalk.gray : chalk.white;
            ...
            // Show next steps
            console.log(chalk.blue('\n🔄 Available Actions:'));
            console.log('   • Cancel task: gemini autonomous tasks cancel ' + argv.taskId);
            if (mockTask.status === 'FAILED') {
                console.log('   • Retry task: gemini autonomous tasks retry ' + argv.taskId);
            }
...
function getPriorityColor(priority) {
    switch (priority.toLowerCase()) {
        ...
        case 'background': return chalk.gray;
        default: return chalk.white;
    }
}
