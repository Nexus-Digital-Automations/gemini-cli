...
export const compatibilityCheckCommand = {
    command: 'compatibility-check',
    aliases: ['compat', 'check'],
    describe: 'Run backward compatibility checks for autonomous task management integration',
    builder: (yargs) => yargs
        .option('quick', {
        type: 'boolean',
        ...
        .example('gemini autonomous compatibility-check', 'Run all compatibility checks')
        ...
        .example('gemini autonomous compat --fix-issues', 'Attempt to fix issues automatically'),
    handler: async (argv) => {
        try {
            const validator = new CompatibilityValidator();
            if (argv.quick) {
                console.log(chalk.cyan('🚀 Running Quick Compatibility Check (Critical Only)...'));
                console.log(chalk.gray('─'.repeat(60)));
                const success = await validator.quickHealthCheck();
                if (argv.json) {
                    console.log(JSON.stringify({
                        success,
                        message: success ? 'All critical checks passed' : 'Critical issues detected',
                    ...
                    if (success) {
                        ...
                        console.log(chalk.blue('Autonomous system is ready for basic operation.'));
                    }
                    else {
                        console.log(chalk.red('\n❌ Quick compatibility check failed!'));
                        console.log(chalk.blue('Run full compatibility check for detailed information.'));
                        process.exit(1);
                    }
                }
                return;
            ...
            if (argv.json) {
                ...
                console.log(JSON.stringify({
                    summary: {
                        ...
                        overallScore: Math.round((report.passed / (report.passed + report.failed + report.warnings)) * 100)
                    },
                ...
                if (report.criticalFailures > 0) {
                    console.log('   • Address critical failures before using autonomous features');
                    console.log('   • Run: gemini autonomous compatibility-check --fix-issues');
                    ...
                    console.log('   • Consider fixing non-critical issues when convenient');
                    ...
                    console.log('   • Add your first task: gemini autonomous tasks add "Your task"');
                }
                // Show documentation links
                console.log(chalk.blue('\n📚 Documentation:'));
                console.log('   • Integration Guide: docs/autonomous-task-management-architecture.md');
                console.log('   • Troubleshooting: development/essentials/troubleshooting.md');
                console.log('   • API Reference: gemini autonomous help');
            }
            // Attempt to fix issues if requested
            if (argv['fix-issues']) {
                console.log(chalk.cyan('\n🔧 Attempting to fix detected issues...'));
                const fixableIssues = report.checks.filter(c => c.status !== 'PASS' &&
                    c.remediation &&
                    (c.name.includes('dependency') || c.name.includes('configuration')));
                if (fixableIssues.length > 0) {
                    console.log(chalk.yellow('⚠️  Automatic issue fixing is limited in this version.'));
                    console.log(chalk.blue('Please manually address the following issues:'));
                    ...
                    console.log(chalk.green('✅ No automatically fixable issues detected.'));
                }
            }
...
//# sourceMappingURL=compatibility-check.js.map