...
import { createBudgetTracker, createAnalyticsEngine, createBudgetDashboard, } from '@google/gemini-cli-core';
/**
...
export const visualizeCommand = {
    ...
    describe: 'Visualize budget usage with analytics and optimization recommendations',
    builder: ((yargs) => yargs
        .positional('format', {
        ...
        .example('gemini budget visualize', 'Show interactive budget visualization with analytics')
        .example('gemini budget visualize ascii --period month', 'Show ASCII chart for monthly budget usage')
        .example('gemini budget visualize json --analytics --recommendations', 'Export comprehensive analytics data in JSON format')),
    handler: async (args) => {
        try {
            ...
            if (args.format === 'json') {
                await displayJSONVisualization(stats, mockMetrics, analyticsEngine, args.analytics, args.recommendations);
            }
            else if (args.format === 'ascii') {
                await displayASCIIVisualization(stats, mockMetrics, args.period || 'week');
            }
            else {
                // Use the new BudgetDashboard system for interactive mode
                await displayInteractiveDashboard(tracker, analyticsEngine, args.period || 'week', args.analytics, args.recommendations);
            }
        }
        catch (error) {
 ...
 */
async function displayInteractiveDashboard(tracker, analyticsEngine, period, includeAnalytics, includeRecommendations) {
    try {
        ...
        const dashboard = createBudgetDashboard(tracker, analyticsEngine, dashboardConfig);
 ...
 */
async function displayLegacyVisualization(stats, mockMetrics, analyticsEngine, period, includeAnalytics, includeRecommendations) {
    ...
    if (includeAnalytics) {
        ...
        try {
            ...
            if (includeRecommendations &&
                ...
                console.log('─────────────────────────────────────────────────────────');
                displayOptimizationRecommendations(report.optimizationRecommendations.slice(0, 5));
                console.log('\n💸 Potential Savings');
                console.log('─────────────────────────────────────────────────────────');
 ...
 */
async function displayASCIIVisualization(stats, mockMetrics, period) {
 ...
 */
async function displayJSONVisualization(stats, mockMetrics, analyticsEngine, includeAnalytics, includeRecommendations) {
    const jsonOutput = {
 ...
 */
function displayCurrentStatus(stats) {
    const percentage = stats.usagePercentage;
    const statusIcon = percentage >= 90 ? '🔴' : percentage >= 75 ? '🟡' : '🟢';
    console.log(`${statusIcon} Current Status: ${stats.requestCount}/${stats.dailyLimit} requests (${percentage.toFixed(1)}%)`);
 ...
 */
function displayUsageTrend(mockMetrics, period) {
    const trend = calculateTrend(mockMetrics);
    const trendIcon = trend > 0 ? '📈' : trend < 0 ? '📉' : '➡️';
    const trendText = trend > 0 ? 'increasing' : trend < 0 ? 'decreasing' : 'stable';
    console.log(`${trendIcon} Trend: ${trendText} (${Math.abs(trend).toFixed(1)}% change)`);
 ...
 */
function displayCostAnalysis(mockMetrics) {
    const totalCost = mockMetrics.reduce((sum, m) => sum + (m.cost || 0), 0);
    ...
    Object.entries(featureCosts).forEach(([feature, cost]) => {
        ...
        console.log(`   ${feature}: $${cost.toFixed(2)} (${percentage.toFixed(1)}%)`);
    });
}
/**
 * Display analytics insights
 */
function displayAnalyticsInsights(report) {
    console.log(`🔍 Analysis period: ${report.period.start.split('T')[0]} to ${report.period.end.split('T')[0]}`);
    console.log(`📈 Cost trend: ${report.summary.costTrend}`);
    console.log(`📊 Budget utilization: ${(report.summary.budgetUtilization * 100).toFixed(1)}%`);
    console.log(`🔮 Projected monthly spend: $${report.summary.projectedMonthlySpend.toFixed(2)}`);
...
function displayOptimizationRecommendations(recommendations) {
    recommendations.forEach((rec, index) => {
        ...
        console.log(`   💰 Potential savings: $${rec.potentialSavings.toFixed(2)} (${rec.savingsPercentage}%)`);
        console.log(`   ${complexityColor} Complexity: ${rec.implementationComplexity} | ⏱️  Time: ${rec.timeToImplement}`);
        console.log(`   📋 ${rec.description}`);
        if (rec.actionItems && rec.actionItems.length > 0) {
            console.log('   🎯 Action items:');
            rec.actionItems.slice(0, 2).forEach((item) => {
                console.log(`      • ${item}`);
            });
        }
 ...
 */
function displaySavingsBreakdown(savings) {
    console.log(`💸 Total potential savings: $${savings.total.toFixed(2)} (${savings.percentage.toFixed(1)}%)`);
    console.log('');
    console.log('📊 Savings timeline:');
    console.log(`   🟢 Immediate (low complexity): $${savings.immediate.toFixed(2)}`);
    console.log(`   🟡 Short-term (medium complexity): $${savings.shortTerm.toFixed(2)}`);
    console.log(`   🔴 Long-term (high complexity): $${savings.longTerm.toFixed(2)}`);
}
/**
 * Display action items and next steps
 */
function displayActionItems(stats) {
    const items = [];
    if (stats.usagePercentage >= 80) {
        items.push('⚠️  Monitor usage closely - approaching budget limit');
        items.push('💡 Consider increasing daily limit or implementing rate limiting');
    }
    if (stats.remainingRequests < 50) {
        items.push('🔴 Low remaining requests - consider extending budget');
    }
    items.push('📊 Run "gemini budget visualize --analytics" for detailed insights');
 ...
 */
function displayASCIIChart(data, unit) {
    ...
    data.forEach((point) => {
        ...
        console.log(`${point.label}${padding} │${bar} ${point.value.toFixed(2)} ${unit}`);
    });
}
/**
 * Display quick statistics summary
 */
function displayQuickStats(mockMetrics, stats) {
    const totalRequests = mockMetrics.length;
    const totalCost = mockMetrics.reduce((sum, m) => sum + (m.cost || 0), 0);
    const avgResponseTime = mockMetrics.reduce((sum, m) => sum + (m.responseTime || 0), 0) / totalRequests;
    console.log(`📊 Total requests analyzed: ${totalRequests}`);
    console.log(`💰 Total cost: $${totalCost.toFixed(2)}`);
    console.log(`⚡ Average response time: ${avgResponseTime.toFixed(0)}ms`);
    console.log(`🎯 Efficiency score: ${calculateEfficiencyScore(mockMetrics, stats).toFixed(1)}/10`);
}
// Helper functions
function generateMockUsageData(period) {
    ...
    for (let i = 0; i < dataPoints; i++) {
        ...
        const weekendMultiplier = timestamp.getDay() === 0 || timestamp.getDay() === 6 ? 0.5 : 1.0;
        const baseRequests = Math.random() * 10 + 2;
        const requests = Math.round(baseRequests * businessHoursMultiplier * weekendMultiplier);
        for (let j = 0; j < requests; j++) {
            metrics.push({
                timestamp: timestamp.toISOString(),
                cost: Math.random() * 0.05 + 0.01, // $0.01 to $0.06 per request
                feature: ['chat-completion', 'embeddings', 'image-analysis'][Math.floor(Math.random() * 3)],
...
function identifyPeakHours(metrics) {
...
function generateHourlyData(metrics, period) {
    const hours = period === 'day' ? 24 : 12; // Show 12 hours for week/month
    const data = [];
    for (let i = 0; i < hours; i++) {
        const hour = i;
        const count = metrics.filter((m) => new Date(m.timestamp).getHours() === hour).length;
        data.push({
...
function generateHourlyCostData(metrics, period) {
    const hours = period === 'day' ? 24 : 12;
    const data = [];
    for (let i = 0; i < hours; i++) {
        const hour = i;
        const hourMetrics = metrics.filter((m) => new Date(m.timestamp).getHours() === hour);
...
function calculateEfficiencyScore(metrics, stats) {
    ...
    const costScore = metrics.length > 0 ? Math.min(10, 10 - metrics[0].cost / 0.1) : 5;
