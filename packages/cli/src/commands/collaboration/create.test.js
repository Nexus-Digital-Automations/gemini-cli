...
describe('collaboration create command', () => {
    ...
    test('should have correct command structure', () => {
        ...
        expect(createCommand.describe).toContain('Create a new collaborative programming session');
    ...
    test('should handle session creation errors gracefully', async () => {
        ...
        const { SessionManager } = await import('@gemini-cli/core/src/collaboration/SessionManager.js');
    ...
    test('should provide helpful troubleshooting information', async () => {
        ...
        const { SessionManager } = await import('@gemini-cli/core/src/collaboration/SessionManager.js');
        SessionManager.mockImplementation(() => ({
            createSession: jest.fn().mockRejectedValue(new Error('Permission denied'))
        }));
    ...
    test('should validate session type choices', () => {
        ...
        const typeOption = mockYargs.option.mock.calls.find(call => call[0] === 'type');
        expect(typeOption).toBeDefined();
        expect(typeOption[1].choices).toEqual(['PAIR', 'MOB', 'REVIEW', 'MENTOR', 'ASYNC']);
    ...
    test('should validate conflict strategy choices', () => {
        ...
        const strategyOption = mockYargs.option.mock.calls.find(call => call[0] === 'conflict-strategy');
        expect(strategyOption).toBeDefined();
        expect(strategyOption[1].choices).toEqual(['LAST_WRITE_WINS', 'ROLE_PRIORITY', 'MANUAL', 'VERSION_BRANCHING']);
