...
import { EXTENSIONS_CONFIG_FILENAME, INSTALL_METADATA_FILENAME, annotateActiveExtensions, disableExtension, enableExtension, installExtension, loadExtension, loadExtensions, performWorkspaceExtensionMigration, uninstallExtension, } from './extension.js';
import { GEMINI_DIR, ExtensionUninstallEvent, ExtensionEnableEvent, } from '@google/gemini-cli-core';
...
describe('extension tests', () => {
    ...
    describe('loadExtensions', () => {
        ...
        it('should filter out disabled extensions', () => {
            ...
            const extensions = loadExtensions();
            const activeExtensions = annotateActiveExtensions(extensions, [], tempWorkspaceDir).filter((e) => e.isActive);
    ...
    describe('annotateActiveExtensions', () => {
        ...
        it('should mark all extensions as active if no enabled extensions are provided', () => {
            const activeExtensions = annotateActiveExtensions(extensions, [], '/path/to/workspace');
            expect(activeExtensions).toHaveLength(3);
            expect(activeExtensions.every((e) => e.isActive)).toBe(true);
        });
        it('should mark only the enabled extensions as active', () => {
            const activeExtensions = annotateActiveExtensions(extensions, ['ext1', 'ext3'], '/path/to/workspace');
            expect(activeExtensions).toHaveLength(3);
        ...
        it('should mark all extensions as inactive when "none" is provided', () => {
            const activeExtensions = annotateActiveExtensions(extensions, ['none'], '/path/to/workspace');
            expect(activeExtensions).toHaveLength(3);
            expect(activeExtensions.every((e) => !e.isActive)).toBe(true);
        });
        it('should handle case-insensitivity', () => {
            const activeExtensions = annotateActiveExtensions(extensions, ['EXT1'], '/path/to/workspace');
            expect(activeExtensions.find((e) => e.name === 'ext1')?.isActive).toBe(true);
        });
        ...
        describe('autoUpdate', () => {
            it('should be false if autoUpdate is not set in install metadata', () => {
                const activeExtensions = annotateActiveExtensions(extensions, [], tempHomeDir);
            ...
            it('should be true if autoUpdate is true in install metadata', () => {
                ...
                const activeExtensions = annotateActiveExtensions(extensionsWithAutoUpdate, [], tempHomeDir);
    ...
    describe('installExtension', () => {
        ...
        it('should throw an error if the extension already exists', async () => {
            ...
            await installExtension({ source: sourceExtDir, type: 'local' });
            await expect(installExtension({ source: sourceExtDir, type: 'local' })).rejects.toThrow('Extension "my-local-extension" is already installed. Please uninstall it first.');
        });
        it('should throw an error and cleanup if gemini-extension.json is missing', async () => {
            const sourceExtDir = path.join(tempHomeDir, 'bad-extension');
            fs.mkdirSync(sourceExtDir, { recursive: true });
            const configPath = path.join(sourceExtDir, EXTENSIONS_CONFIG_FILENAME);
            await expect(installExtension({ source: sourceExtDir, type: 'local' })).rejects.toThrow(`Configuration file not found at ${configPath}`);
        ...
        it('should throw an error for invalid JSON in gemini-extension.json', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' })).rejects.toThrow(new RegExp(`^Failed to load extension config from ${configPath.replace(/\\/g, '\\\\')}`));
        });
        it('should throw an error for missing name in gemini-extension.json', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' })).rejects.toThrow(`Invalid configuration in ${configPath}: missing "name"`);
        });
        ...
        it('should show users information on their mcp server when installing', async () => {
            ...
            mockQuestion.mockImplementation((_query, callback) => callback('y'));
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, true)).resolves.toBe('my-local-extension');
            expect(consoleInfoSpy).toHaveBeenCalledWith(`Extensions may introduce unexpected behavior.
...
This extension will run the following MCP servers:
  ...
  * test-server-2 (remote): https://google.com`);
        ...
        it('should cancel installation if user declines prompt for local extension with mcp servers', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, true)).rejects.toThrow('Installation cancelled by user.');
            expect(mockQuestion).toHaveBeenCalledWith(expect.stringContaining('Do you want to continue? [Y/n]: '), expect.any(Function));
        });
        ...
        it('should ignore consent flow if not required', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, false)).resolves.toBe('my-local-extension');
        });
    });
    ...
    describe('enableExtension', () => {
        ...
        const getActiveExtensions = () => {
            const extensions = loadExtensions();
            const activeExtensions = annotateActiveExtensions(extensions, [], tempWorkspaceDir);
