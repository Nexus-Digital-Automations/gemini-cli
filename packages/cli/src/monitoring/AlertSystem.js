...
export class AlertSystem extends EventEmitter {
    ...
    async evaluateAgentPerformanceAlerts(agent, context) {
        await this.createAlert({
            ...
            severity: agent.performance.successRate < 50
                ? AlertSeverity.ERROR
                : AlertSeverity.WARNING,
    ...
    async evaluateRules(eventType, context) {
        for (const [ruleId, rule] of this.alertRules) {
            ...
            if (!rule.triggers.eventTypes.includes(eventType))
                continue;
    ...
    evaluateCombinationCondition(params, context) {
        ...
        const results = conditionsArray.map((condition) => {
    ...
    async sendNotifications(alert, config) {
        for (const channel of config.channels) {
            try {
                ...
                const success = await delivery.send(alert, config.recipients || [], config.template);
    ...
    calculateHourlyTrends(alerts, hours) {
        ...
        for (let i = hours - 1; i >= 0; i--) {
            ...
            const hourAlerts = alerts.filter((alert) => alert.timestamp.getTime() >= hourStart &&
    ...
    calculateAutoRemediationSuccessRate(alerts) {
        const autoRemediatedAlerts = alerts.filter((alert) => alert.remediationActions.some((action) => action.status === 'completed'));
        const attemptedAutoRemediationAlerts = alerts.filter((alert) => alert.remediationActions.length > 0);
