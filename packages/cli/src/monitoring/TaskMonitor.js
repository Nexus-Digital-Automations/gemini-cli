...
export class TaskMonitor extends EventEmitter {
     ...
     */
    async startMonitoring(taskId, config = {}) {
        const defaultConfig = {
            scope: {
                taskId,
     ...
     */
    async getIntegratedTaskStatus(taskId) {
        ...
        const integratedStatus = {
            ...
            progress: {
                ...
                timeRemaining: progressEstimate
                    ? Math.max(0, progressEstimate.estimatedCompletion.getTime() - Date.now())
                    : 0,
            },
            performance: {
    ...
    async updateTaskProgress(taskId, progress, options = {}) {
        ...
        await taskStatusMonitor.updateTaskStatus(taskId, 'in_progress', {
            progress,
     ...
     */
    async completeTask(taskId, result, details) {
        const config = this.monitoredTasks.get(taskId);
        if (!config)
            return;
        const task = taskStatusMonitor.getTaskStatus(taskId);
        ...
        if (config.scope.enablePerformanceAnalytics) {
            performanceAnalyticsDashboard.onTaskEvent(result === 'success' ? 'completed' : 'failed', {
                task,
    ...
    async calculateTaskAnalytics(taskId, task, progressMetrics, config) {
    ...
    async updateHealthScore(taskId) {
        const task = taskStatusMonitor.getTaskStatus(taskId);
        const config = this.monitoredTasks.get(taskId);
        if (!task || !config)
            return;
    ...
    async generateTaskAlerts(taskId, status) {
        const config = this.monitoredTasks.get(taskId);
        if (!config)
            return [];
        const alerts = [];
        ...
        if (status.performance.efficiency <
    ...
    generateSystemRecommendations(tasks, alerts) {
    ...
    mergeConfig(defaultConfig, override) {
        return {
