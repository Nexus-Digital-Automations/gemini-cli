    ...
    for (const [subscriberId, config] of Array.from(
      ...
      if (config.filters) {
        ...
        if (config.filters.agentIds && event.data.agentId) {
          if (!config.filters.agentIds.includes(event.data.agentId)) continue;
        }
      }
