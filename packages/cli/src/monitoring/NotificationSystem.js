...
export class NotificationSystem extends EventEmitter {
     ...
     */
    registerUserPreferences(preferences) {
     ...
     */
    async sendImmediateNotification(userId, event, options = {}) {
     ...
     */
    async sendBroadcastNotification(event, options = {}) {
     ...
     */
    acknowledgeNotification(notificationId, userId, actionTaken) {
     ...
     */
    getNotificationHistory(userId, filters, limit = 50) {
        let notifications = Array.from(this.deliveredNotifications.values()).filter((n) => n.userId === userId);
        if (filters) {
            if (filters.eventTypes) {
                notifications = notifications.filter((n) => filters.eventTypes.includes(n.eventType));
            }
            if (filters.since) {
                notifications = notifications.filter((n) => n.deliveredAt >= filters.since);
            }
            if (filters.acknowledged !== undefined) {
                notifications = notifications.filter((n) => filters.acknowledged ? !!n.acknowledged : !n.acknowledged);
            }
        }
     ...
     */
    getMetrics() {
        const totalNotificationsPerUser = Array.from(this.deliveredNotifications.values()).reduce((acc, n) => {
    ...
    async handleStatusEvent(userId, event) {
        const preferences = this.userPreferences.get(userId);
        if (!preferences)
            return;
        if (this.isInQuietHours(preferences)) {
    ...
    async formatNotification(event, preferences, options = {}) {
    ...
    async deliverNotification(userId, notification, channel, event) {
