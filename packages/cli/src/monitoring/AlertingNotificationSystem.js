...
export class AlertingNotificationSystem extends EventEmitter {
    ...
    getDashboardData() {
        ...
        const mttr = resolvedAlerts.length > 0
            ? resolvedAlerts.reduce((sum, alert) => sum + (alert.resolvedAt.getTime() - alert.triggeredAt.getTime()), 0) / resolvedAlerts.length / 1000 / 60 // minutes
            : 0;
        return {
            activeAlerts,
    ...
    getAnalytics(timeRange) {
        const alerts = timeRange
            ? this.alertHistory.filter(alert => alert.triggeredAt >= timeRange.start && alert.triggeredAt <= timeRange.end)
            : this.alertHistory;
        return {
...
*Affected:* {{affectedResources}}
    ...
    triggerAlert(rule, sourceType, sourceData) {
        ...
        const alert = {
            ...
            source: {
                type: sourceType,
                id: sourceType === 'metric' ? sourceData.metric.id : 'unknown',
                data: sourceData,
            },
            context: {
