...
export class MonitoringIntegrationHub extends EventEmitter {
     ...
     */
    registerCoreExecutionMonitoring(executionMonitoring) {
     ...
     */
    correlateEvents(correlationId, event) {
    ...
    getAggregatedData(timeRange = 'last_hour') {
        ...
        return {
            ...
            agentMetrics: {
                ...
                averagePerformance: allAgents.reduce((sum, a) => sum + a.performance.successRate, 0) /
                    Math.max(allAgents.length, 1),
                utilization: (allAgents.filter((a) => a.status === 'busy').length /
                    Math.max(allAgents.length, 1)) *
                    100,
            },
            correlatedEvents,
     ...
     */
    async exportData(format, timeRange = 'last_hour') {
    ...
    setupCoreExecutionEventForwarding() {
        if (!this.coreExecutionMonitoring)
            return;
    ...
    async performDataSynchronization() {
        ...
        performanceAnalyticsDashboard.recordMetric('system_sync', 1, 'count', 'throughput', { timestamp: this.lastSyncTimestamp.toISOString() });
    ...
    getComponentHealthStatus(component) {
...
//# sourceMappingURL=MonitoringIntegrationHub.js.map