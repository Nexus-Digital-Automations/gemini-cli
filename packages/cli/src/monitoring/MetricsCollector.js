...
export class MetricsCollector extends EventEmitter {
    ...
    async analyzePerformanceBottlenecks() {
        ...
        const avgCpuUsage = recentMetrics.reduce((sum, m) => sum + m.cpu.usage, 0) /
            recentMetrics.length;
        if (avgCpuUsage > 80) {
            const cpuBottleneck = this.createBottleneckAnalysis('cpu', avgCpuUsage > 95 ? 'critical' : avgCpuUsage > 90 ? 'high' : 'medium', `High CPU usage detected: ${avgCpuUsage.toFixed(1)}%`, [
        ...
        const avgMemoryUsage = recentMetrics.reduce((sum, m) => sum + m.memory.usagePercent, 0) /
            recentMetrics.length;
        if (avgMemoryUsage > 85) {
            const memoryBottleneck = this.createBottleneckAnalysis('memory', avgMemoryUsage > 95
                ? 'critical'
                : avgMemoryUsage > 90
                    ? 'high'
                    : 'medium', `High memory usage detected: ${avgMemoryUsage.toFixed(1)}%`, [
        ...
        const avgEventLoopLag = recentMetrics.reduce((sum, m) => sum + m.performance.eventLoopLag, 0) /
            recentMetrics.length;
        if (avgEventLoopLag > 100) {
            const lagBottleneck = this.createBottleneckAnalysis('io', avgEventLoopLag > 500
                ? 'critical'
                : avgEventLoopLag > 200
                    ? 'high'
                    : 'medium', `High event loop lag detected: ${avgEventLoopLag.toFixed(1)}ms`, [
        ...
        const activeTasks = Array.from(this.taskMetrics.values()).filter((t) => !t.endTime).length;
        const activeAgents = Array.from(this.agentMetrics.values()).filter((a) => !a.activePeriod.end).length;
        if (activeTasks > activeAgents * 3) {
            // More than 3 tasks per agent
            const concurrencyBottleneck = this.createBottleneckAnalysis('concurrency', activeTasks > activeAgents * 5 ? 'high' : 'medium', `High task-to-agent ratio detected: ${(activeTasks / Math.max(activeAgents, 1)).toFixed(1)} tasks per agent`, [
                'Scale agent pool',
    ...
    async getSystemAnalytics(periodHours = 24) {
        ...
        const periodMetrics = this.systemMetricsHistory.filter((m) => m.timestamp >= periodStart);
        const periodTasks = Array.from(this.taskMetrics.values()).filter((t) => t.startTime >= periodStart);
        const periodBottlenecks = this.bottleneckHistory.filter((b) => b.detectionTime >= periodStart);
        const analytics = {
            period: {
                start: periodStart,
                end: periodEnd,
            ...
            systemOverview: {
                ...
                activeAgents: Array.from(this.agentMetrics.values()).filter((a) => !a.activePeriod.end).length,
    ...
    calculateOverallThroughput(tasks) {
        if (tasks.length === 0)
            return 0;
        const totalOperations = tasks.reduce((sum, task) => sum + Object.values(task.operationCounts).reduce((a, b) => a + b, 0), 0);
    ...
    cleanupOldMetrics() {
        ...
        this.systemMetricsHistory = this.systemMetricsHistory.filter((metrics) => metrics.timestamp.getTime() > cutoff);
    }
    cleanupOldBottlenecks() {
        const cutoff = Date.now() - this.metricsRetentionPeriod;
        this.bottleneckHistory = this.bottleneckHistory.filter((bottleneck) => bottleneck.detectionTime.getTime() > cutoff);
    }
    /**
     * Cleanup resources
     */
