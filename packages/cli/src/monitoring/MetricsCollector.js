...
export class MetricsCollector extends EventEmitter {
    ...
    async analyzePerformanceBottlenecks() {
        ...
        if (avgCpuUsage > 80) {
            const cpuBottleneck = this.createBottleneckAnalysis('cpu', avgCpuUsage > 95 ? 'critical' : avgCpuUsage > 90 ? 'high' : 'medium', `High CPU usage detected: ${avgCpuUsage.toFixed(1)}%`, ['Consider load balancing', 'Optimize CPU-intensive operations', 'Scale horizontally']);
        ...
        if (avgMemoryUsage > 85) {
            const memoryBottleneck = this.createBottleneckAnalysis('memory', avgMemoryUsage > 95 ? 'critical' : avgMemoryUsage > 90 ? 'high' : 'medium', `High memory usage detected: ${avgMemoryUsage.toFixed(1)}%`, ['Optimize memory usage', 'Implement garbage collection tuning', 'Increase available memory']);
        ...
        if (avgEventLoopLag > 100) {
            const lagBottleneck = this.createBottleneckAnalysis('io', avgEventLoopLag > 500 ? 'critical' : avgEventLoopLag > 200 ? 'high' : 'medium', `High event loop lag detected: ${avgEventLoopLag.toFixed(1)}ms`, ['Optimize async operations', 'Reduce blocking operations', 'Implement proper queueing']);
        ...
        if (activeTasks > activeAgents * 3) { // More than 3 tasks per agent
            const concurrencyBottleneck = this.createBottleneckAnalysis('concurrency', activeTasks > activeAgents * 5 ? 'high' : 'medium', `High task-to-agent ratio detected: ${(activeTasks / Math.max(activeAgents, 1)).toFixed(1)} tasks per agent`, ['Scale agent pool', 'Implement task prioritization', 'Optimize task distribution']);
