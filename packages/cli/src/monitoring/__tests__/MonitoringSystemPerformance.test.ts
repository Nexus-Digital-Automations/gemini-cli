...
describe('Monitoring System Performance Tests', () => {
  ...
  describe('Response Time Performance', () => {
    it('should collect monitoring snapshots within performance targets', () => {
      ...
      expect(maxTime).toBeLessThan(PERFORMANCE_TARGETS.snapshotCollectionTime * 2); // Allow 2x spike
      expect(times.filter(t => t < PERFORMANCE_TARGETS.snapshotCollectionTime).length / iterations)
    ...
    it('should process alerts within performance targets', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        const rule = {
          ...
          condition: (data: any) => data.taskMetrics.total > i,
      ...
      if (alertTimes.length > 0) {
        const avgAlertTime = alertTimes.reduce((sum, time) => sum + time, 0) / alertTimes.length;
        ...
        expect(avgAlertTime).toBeLessThan(PERFORMANCE_TARGETS.alertProcessingTime);
        expect(maxAlertTime).toBeLessThan(PERFORMANCE_TARGETS.alertProcessingTime * 3);
      }
    ...
    it('should generate dashboard data within performance targets', () => {
      ...
      const layoutId = dashboard.createLayout('Performance Test Layout', 'Layout for performance testing');

      const widgetTypes: Array<'metric' | 'chart' | 'gauge' | 'table'> = ['metric', 'chart', 'gauge', 'table'];
  ...
  describe('Throughput Performance', () => {
    it('should handle high-frequency task operations', async () => {
      ...
      expect(throughput).toBeGreaterThan(PERFORMANCE_TARGETS.throughputTarget * 0.5); // At least 50% of target
    ...
    it('should handle concurrent monitoring operations', async () => {
      ...
      const avgOperationTime = allResults.reduce((sum, result) => sum + result.duration, 0) / totalOperations;
      ...
      expect(maxOperationTime).toBeLessThan(PERFORMANCE_TARGETS.maxResponseTime);
    ...
    it('should maintain performance under sustained load', async () => {
      ...
      // Performance assertions
      expect(avgOperationTime).toBeLessThan(PERFORMANCE_TARGETS.snapshotCollectionTime);
      ...
      const firstHalfAvg = operationTimes.slice(0, Math.floor(operationTimes.length / 2))
        .reduce((sum, time) => sum + time, 0) / Math.floor(operationTimes.length / 2);
      const secondHalfAvg = operationTimes.slice(Math.floor(operationTimes.length / 2))
        .reduce((sum, time) => sum + time, 0) / Math.ceil(operationTimes.length / 2);
  ...
  describe('Memory Efficiency', () => {
    it('should maintain stable memory usage', async () => {
      ...
      // Memory assertions
      expect(finalMemory.heapUsed).toBeLessThan(PERFORMANCE_TARGETS.memoryUsageLimit);
    ...
    it('should efficiently handle large datasets', async () => {
      ...
      for (let batch = 0; batch < largeDatasetSize / batchSize; batch++) {
        ...
        if (batch % 5 === 0) {
          ...
          console.log(`Batch ${batch}: Memory usage: ${memoryUsage.toFixed(2)}MB`);

          // Memory should not grow unbounded
          expect(currentMemory.heapUsed).toBeLessThan(PERFORMANCE_TARGETS.memoryUsageLimit * 2);
        }
      }
      ...
      expect(endMemory.heapUsed).toBeLessThan(PERFORMANCE_TARGETS.memoryUsageLimit * 1.5);
    });
  });
    ...
    it('should maintain performance as data volume increases', async () => {
      ...
      performanceResults.forEach((result, index) => {
        if (index > 0) {
          ...
          const snapshotGrowthFactor = result.snapshotTime / prevResult.snapshotTime;
          const aggGrowthFactor = result.aggregationTime / prevResult.aggregationTime;
      ...
      expect(largestResult.snapshotTime).toBeLessThan(PERFORMANCE_TARGETS.snapshotCollectionTime * 2);
      expect(largestResult.memoryUsage).toBeLessThan(PERFORMANCE_TARGETS.memoryUsageLimit);
    });
  });
});