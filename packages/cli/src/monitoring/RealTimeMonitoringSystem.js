...
export class RealTimeMonitoringSystem extends EventEmitter {
     ...
     */
    async initializeSystem(httpServer) {
        try {
     ...
     */
    async exportMonitoringData(format = 'json', hours = 24) {
    ...
    async collectAndProcessSnapshot() {
        try {
    ...
    setupDefaultAlertRules() {
        const defaultRules = [
            ...
            },
            {
                ...
                condition: (data) => data.systemHealth.memoryUsageMB >
            ...
            },
            {
                ...
                condition: (data) => data.taskMetrics.queued >
    ...
    triggerAlert(rule, snapshot, action) {
        const alert = {
            type: 'alert',
            timestamp: new Date(),
            data: {
    ...
    executeAlertAction(actionConfig, rule, snapshot, action) {
    ...
    performAnomalyDetection(snapshot) {
    ...
    updateAnomalyBaseline(key, value) {
    ...
    async generatePredictiveInsights() {
        if (this.monitoringSnapshots.length < 10)
            return;
        try {
    ...
    async analyzeSystemTrends() {
        ...
        // Memory usage trend
        const memoryTrend = this.calculateLinearTrend(recentSnapshots.map((s) => ({
        ...
        const failureTrend = this.calculateLinearTrend(failureRates.map((rate, i) => ({ x: i, y: rate })));
    ...
    calculateSystemHealth(taskMetrics, allTasks, allAgents) {
    ...
    calculatePerformanceMetrics() {
        ...
        const avgThroughput = recentSnapshots.reduce((sum, s) => sum + s.taskMetrics.throughputPerHour, 0) / recentSnapshots.length;
        const avgErrorRate = recentSnapshots.reduce((sum, s) => sum + s.taskMetrics.failed / Math.max(1, s.taskMetrics.total), 0) / recentSnapshots.length;
        return {
    ...
    calculateTrends() {
        ...
        const firstCompletion = firstHalf.reduce((sum, s) => sum + s.taskMetrics.throughputPerHour, 0) /
            firstHalf.length;
        const secondCompletion = secondHalf.reduce((sum, s) => sum + s.taskMetrics.throughputPerHour, 0) /
            secondHalf.length;
        ...
        const firstMemory = firstHalf.reduce((sum, s) => sum + s.systemHealth.memoryUsageMB, 0) /
            firstHalf.length;
        const secondMemory = secondHalf.reduce((sum, s) => sum + s.systemHealth.memoryUsageMB, 0) /
            secondHalf.length;
        return {
    ...
    async loadPersistedData() {
        try {
            ...
            try {
                ...
                this.predictiveInsights = (parsed.insights || []).map((insight) => ({
                    ...insight,
    ...
    async persistMonitoringData() {
        try {
            ...
            const alertsData = {
                alerts: Object.fromEntries(Array.from(this.activeAlerts.entries()).map(([id, alert]) => [
                    id,
                    {
                        rule: alert.rule,
...
//# sourceMappingURL=RealTimeMonitoringSystem.js.map