...
export class RealTimeMonitoringSystem extends EventEmitter {
    ...
    async analyzeSystemTrends() {
        ...
        // Memory usage trend
        const memoryTrend = this.calculateLinearTrend(recentSnapshots.map(s => ({ x: s.timestamp.getTime(), y: s.systemHealth.memoryUsageMB })));
    ...
    async persistMonitoringData() {
        try {
            ...
            const alertsData = {
                alerts: Object.fromEntries(Array.from(this.activeAlerts.entries()).map(([id, alert]) => [id, {
                        rule: alert.rule,
