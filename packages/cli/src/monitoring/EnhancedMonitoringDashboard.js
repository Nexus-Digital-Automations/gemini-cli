...
export class EnhancedMonitoringDashboard extends EventEmitter {
    ...
    generateSystemHealthChart(snapshots, widget) {
        const labels = snapshots.slice(-20).map(s => s.timestamp.toLocaleTimeString()).reverse();
        return {
            labels,
            datasets: [
                {
    ...
    generateAlertHistoryChart(snapshots, widget) {
        const labels = snapshots.slice(-10).map(s => s.timestamp.toLocaleTimeString()).reverse();
        return {
            labels,
            datasets: [
                {
            ...
            datasets: [
                {
                    ...
                    backgroundColor: confidenceData.map(d => d.impact === 'high' ? '#ef4444' :
