...
export interface ProgressMetrics {
  ...
  velocity: number; // progress points per minute
  ...
  trends: {
    velocityTrend: number[]; // last 10 measurements
    progressTrend: number[]; // last 10 progress points
...
export interface ProgressTrackingConfig {
  ...
  checkpointThreshold: number; // minimum progress change to record
  velocityWindowSize: number; // number of measurements for velocity calculation
  alertThresholds: {
    slowProgress: number; // velocity below this triggers alert
    timeOverrun: number; // percentage over estimate to trigger alert
    blockingDuration: number; // minutes blocked before alert
  };
}
...
export class ProgressTracker extends EventEmitter {
  ...
  async addCheckpoint(
    ...
    const fullCheckpoint: ProgressCheckpoint = {
      ...checkpoint,
      id: checkpointId,
      taskId,
      timestamp: new Date(),
  ...
  predictCompletionTime(taskId: string): {
    ...
    if (!metrics || metrics.velocity <= 0) return null;
  ...
  analyzeBottlenecks(taskId: string): {
    ...
    const bottlenecks: ProgressCheckpoint[] = [];
  ...
  private updateTrends(taskId: string, metrics: ProgressMetrics): void {
    const velocityHistory = this.velocityHistory.get(taskId) || [];
  ...
  private async performPeriodicAnalysis(): Promise<void> {
    try {
      for (const taskId of this.taskConfigs.keys()) {
        const metrics = this.taskMetrics.get(taskId);
        if (!metrics || metrics.overallProgress >= 100) continue;

        // Update metrics
        const dummyCheckpoint: ProgressCheckpoint = {
          id: 'periodic',
          taskId,
