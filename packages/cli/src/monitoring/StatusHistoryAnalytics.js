...
export class StatusHistoryAnalytics {
    ...
    queryHistory(query) {
        ...
        if (query.agentIds && query.agentIds.length > 0) {
            entries = entries.filter(entry => entry.context.agentId && query.agentIds.includes(entry.context.agentId));
        }
        // Sort results
    ...
    async calculateSystemMetrics(entries, timeframe) {
        ...
        const trends = {
            taskCompletionTrend: taskMetrics.completionRate > 0.8 ? 'improving' :
                taskMetrics.completionRate > 0.6 ? 'stable' : 'declining',
            agentUtilizationTrend: 'stable', // Would need more complex trend analysis
            errorRateTrend: taskMetrics.failureRate < 0.1 ? 'improving' :
                taskMetrics.failureRate < 0.2 ? 'stable' : 'declining',
        };
        return {
            systemUptime,
            totalEvents,
