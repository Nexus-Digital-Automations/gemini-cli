...
import { TaskStatusMonitor as _TaskStatusMonitor, TaskStatus, taskStatusMonitor, } from './TaskStatusMonitor.js';
import { StatusUpdateBroker as _StatusUpdateBroker, StatusEventType, statusUpdateBroker, } from './StatusUpdateBroker.js';
import { NotificationSystem as _NotificationSystem, NotificationPreferences as _NotificationPreferences, notificationSystem as _notificationSystem, } from './NotificationSystem.js';
import { StatusHistoryAnalytics as _StatusHistoryAnalytics, statusHistoryAnalytics as _statusHistoryAnalytics, } from './StatusHistoryAnalytics.js';
/**
...
export class MonitoringIntegrations extends EventEmitter {
    ...
    async syncTodoWriteState(todoWriteTasks) {
        ...
        try {
            ...
            await statusUpdateBroker.publishEvent(StatusEventType.SYSTEM_ALERT, {
                type: 'todowrite_sync',
    ...
    async testExternalSystem(systemName) {
        ...
        try {
            const testPayload = this.createWebhookPayload('test_' + Date.now(), StatusEventType.SYSTEM_ALERT, {
    ...
    setupIntegrationListeners() {
        ...
        statusUpdateBroker.on('event:published', async (data) => {
            ...
            const payload = this.createWebhookPayload(event.id, event.type, event.data, {
    ...
    async updateTodoWriteFromTask(task, update) {
        ...
        if (task.metadata?.source !== 'todowrite')
            return;
    ...
    generateActiveForm(status, title) {
        const action = status === TaskStatus.IN_PROGRESS
            ? 'Working on'
