...
import { StatusUpdateBroker, StatusEventType, statusUpdateBroker, } from './StatusUpdateBroker.js';
import { NotificationSystem, NotificationPreferences, notificationSystem, } from './NotificationSystem.js';
...
export class MonitoringIntegrations extends EventEmitter {
    ...
    async testExternalSystem(systemName) {
        ...
        try {
            const testPayload = this.createWebhookPayload('test_' + Date.now(), StatusEventType.SYSTEM_ALERT, { type: 'connection_test', message: 'Testing external system connection' });
