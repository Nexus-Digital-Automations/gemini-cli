...
export class ProgressTracker extends EventEmitter {
    ...
    getTaskCheckpoints(taskId, filters) {
        let checkpoints = Array.from(this.checkpoints.values()).filter((checkpoint) => checkpoint.taskId === taskId);
        if (filters) {
    ...
    analyzeBottlenecks(taskId) {
        ...
        const severity = bottlenecks.length > 3
            ? 'high'
    ...
    async updateTaskMetrics(taskId, _checkpoint) {
        ...
        if (config.enableBottleneckDetection) {
            ...
            metrics.bottlenecks = {
                ...
                timeOverruns: taskCheckpoints.filter((cp) => cp.duration &&
        ...
        if (completedCheckpoints.length > 0) {
            ...
            if (config &&
                ...
                this.emit(ProgressEventType.TIME_OVERRUN, {
                    ...
                    overrunPercent: ((metrics.actualTimeElapsed - metrics.estimatedTotalTime) /
                        metrics.estimatedTotalTime) *
                        100,
                });
            }
