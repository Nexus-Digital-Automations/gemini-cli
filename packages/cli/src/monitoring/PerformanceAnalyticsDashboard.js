...
export class PerformanceAnalyticsDashboard extends EventEmitter {
    ...
    onTaskEvent(event, data) {
        ...
        switch (event) {
            case 'started':
                ...
                this.recordMetric('task_started_rate', 1, 'count', 'throughput', { taskType: task.type, priority: task.priority });
                break;
            case 'completed':
    ...
    onAgentEvent(event, data) {
        ...
        switch (event) {
            case 'heartbeat':
                ...
                break;
            case 'status_changed':
                this.recordMetric('agent_status_change', 1, 'count', 'resource_usage', { agentId: agent.id, newStatus: agent.status });
                break;
        }
    }
    /**
    ...
    getAnalytics(timeRange, metrics) {
        ...
        for (const metricName of metricsToAnalyze) {
            ...
            const filteredHistory = metricHistory.filter(metric => metric.timestamp >= timeRange.start && metric.timestamp <= timeRange.end);
    ...
    handleTaskCompletion(task, agent) {
        ...
        if (startTime) {
            ...
            this.recordMetric('task_execution_time', executionTime, 'milliseconds', 'latency', { taskType: task.type, priority: task.priority, agentId: agent?.id || 'unknown' });
            this.recordMetric('task_completion_rate', 1, 'count', 'success_rate', { taskType: task.type, priority: task.priority });
        }
    }
    handleTaskFailure(task, update, agent) {
        ...
        if (startTime) {
            ...
            this.recordMetric('task_failure_rate', 1, 'count', 'quality', { taskType: task.type, priority: task.priority, error: update?.error || 'unknown' });
        }
    }
