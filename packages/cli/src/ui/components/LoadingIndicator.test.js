...
const renderWithContext = (ui, streamingStateValue, width = 120) => {
    ...
    return render(_jsx(ProgressProvider, { children: _jsx(StreamingContext.Provider, { value: contextValue, children: ui }) }));
};
describe('<LoadingIndicator />', () => {
    ...
    it('should transition correctly between states using rerender', () => {
        ...
        rerender(_jsx(ProgressProvider, { children: _jsx(StreamingContext.Provider, { value: StreamingState.Responding, children: _jsx(LoadingIndicator, { currentLoadingPhrase: "Now Responding", elapsedTime: 2 }) }) }));
        let output = lastFrame();
        expect(output).toContain('MockRespondingSpinner');
        expect(output).toContain('Now Responding');
        expect(output).toContain('(esc to cancel, 2s)');
        // Transition to WaitingForConfirmation
        rerender(_jsx(ProgressProvider, { children: _jsx(StreamingContext.Provider, { value: StreamingState.WaitingForConfirmation, children: _jsx(LoadingIndicator, { currentLoadingPhrase: "Please Confirm", elapsedTime: 15 }) }) }));
        output = lastFrame();
        ...
        rerender(_jsx(ProgressProvider, { children: _jsx(StreamingContext.Provider, { value: StreamingState.Idle, children: _jsx(LoadingIndicator, { ...defaultProps }) }) }));
        expect(lastFrame()).toBe('');
    });
