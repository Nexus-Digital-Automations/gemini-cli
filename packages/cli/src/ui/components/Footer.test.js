...
const renderWithWidth = (width, props = defaultProps) => {
    useTerminalSizeMock.mockReturnValue({ columns: width, rows: 24 });
    return render(_jsx(ProgressProvider, { children: _jsx(Footer, { ...props }) }));
};
describe('<Footer />', () => {
    ...
    describe('sandbox and trust info', () => {
        ...
        it('should display "no sandbox" when SANDBOX is not set and folder is trusted', () => {
            // Clear any SANDBOX env let that might be set.
            vi.stubEnv('SANDBOX', '');
            const { lastFrame } = renderWithWidth(120, {
                ...defaultProps,
                isTrustedFolder: true,
