...
describe('BudgetDisplay', () => {
  let defaultProps: BudgetDisplayProps;
  ...
  beforeEach(() => {
    ...
    vi.mocked(createBudgetTracker).mockReturnValue(
      mockTracker as unknown as ReturnType<typeof createBudgetTracker>,
    );
  ...
  describe('Rendering Conditions', () => {
    it('should not render when budget is disabled', async () => {
      ...
      const { container } = render(
        <BudgetDisplay {...defaultProps} budgetSettings={disabledSettings} />,
      );
    ...
    it('should not render when budget settings are undefined', async () => {
      const { container } = render(
        <BudgetDisplay {...defaultProps} budgetSettings={undefined} />,
      );
    ...
    it('should not render while loading', () => {
      mockTracker.getUsageStats.mockImplementation(
        () => new Promise((resolve) => setTimeout(resolve, 1000)),
      );

      const { container } = render(<BudgetDisplay {...defaultProps} />);
  ...
  describe('Full Display Mode', () => {
    ...
    it('should render full display with budget information', async () => {
      const { getByText } = render(<BudgetDisplay {...defaultProps} />);
  ...
  describe('Compact Display Mode', () => {
    ...
    it('should render compact display with essential information', async () => {
      const { getByText, queryByText } = render(
        <BudgetDisplay {...defaultProps} compact={true} />,
      );
  ...
  describe('Usage Color Coding', () => {
    it('should use success color for low usage (under 75%)', async () => {
      ...
      const hasGreenColor = Array.from(textElements).some(
        (el) => el.getAttribute('color') === 'green',
      );
    ...
    it('should use warning color for medium usage (75-89%)', async () => {
      ...
      const textElements = container.querySelectorAll('*');
      const hasYellowColor = Array.from(textElements).some(
        (el) => el.getAttribute('color') === 'yellow',
      );
    ...
    it('should use error color for high usage (90%+)', async () => {
      ...
      const textElements = container.querySelectorAll('*');
      const hasRedColor = Array.from(textElements).some(
        (el) => el.getAttribute('color') === 'red',
      );
  ...
  describe('Data Refresh and Updates', () => {
    ...
    it('should cleanup interval on unmount', async () => {
      ...
      const { unmount } = render(<BudgetDisplay {...defaultProps} />);
  ...
  describe('Props Updates', () => {
    it('should reload data when budget settings change', async () => {
      ...
      const { rerender } = render(<BudgetDisplay {...defaultProps} />);
      ...
      rerender(
        <BudgetDisplay {...defaultProps} budgetSettings={newSettings} />,
      );
    ...
    it('should reload data when project root changes', async () => {
      ...
      // Change project root
      rerender(<BudgetDisplay {...defaultProps} projectRoot="/new/project" />);
  ...
  describe('Accessibility and Usability', () => {
    ...
    it('should handle rapid prop changes gracefully', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        rerender(<BudgetDisplay {...defaultProps} compact={i % 2 === 0} />);
        await act(async () => {
