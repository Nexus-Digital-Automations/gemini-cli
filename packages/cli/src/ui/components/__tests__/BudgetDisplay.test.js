...
describe('BudgetDisplay', () => {
  ...
  describe('Rendering Conditions', () => {
    it('should not render when budget is disabled', async () => {
      ...
      const { container } = render(
        _jsx(BudgetDisplay, {
          ...defaultProps,
          budgetSettings: disabledSettings,
        }),
      );
    ...
    it('should not render when budget settings are undefined', async () => {
      const { container } = render(
        _jsx(BudgetDisplay, { ...defaultProps, budgetSettings: undefined }),
      );
      // Wait for async loading
      await act(async () => {
        vi.runAllTimers();
  ...
  describe('Full Display Mode', () => {
    ...
    it('should show exceeded status when over budget', async () => {
      ...
      const { getByText, queryByText } = render(
        _jsx(BudgetDisplay, { ...defaultProps }),
      );
      await act(async () => {
        vi.runAllTimers();
      });
  ...
  describe('Compact Display Mode', () => {
    ...
    it('should render compact display with essential information', async () => {
      const { getByText, queryByText } = render(
        _jsx(BudgetDisplay, { ...defaultProps, compact: true }),
      );
      await act(async () => {
        vi.runAllTimers();
      });
  ...
  describe('Props Updates', () => {
    it('should reload data when budget settings change', async () => {
      ...
      rerender(
        _jsx(BudgetDisplay, { ...defaultProps, budgetSettings: newSettings }),
      );
      await act(async () => {
        vi.runAllTimers();
      });
    ...
    it('should reload data when project root changes', async () => {
      ...
      // Change project root
      rerender(
        _jsx(BudgetDisplay, { ...defaultProps, projectRoot: '/new/project' }),
      );
      await act(async () => {
        vi.runAllTimers();
      });
  ...
  describe('Accessibility and Usability', () => {
    ...
    it('should handle rapid prop changes gracefully', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        rerender(
          _jsx(BudgetDisplay, { ...defaultProps, compact: i % 2 === 0 }),
        );
        await act(async () => {
