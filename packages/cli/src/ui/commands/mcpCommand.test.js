...
const isMessageAction = (result) => result !== null &&
...
const createMockMCPTool = (name, serverName, description) => new DiscoveredMCPTool({
    callTool: vi.fn(),
    tool: vi.fn(),
}, serverName, name, description || `Description for ${name}`, { type: Type.OBJECT, properties: {} }, name);
describe('mcpCommand', () => {
    let mockContext;
    let mockConfig;
    beforeEach(() => {
    ...
    describe('no MCP servers configured', () => {
        ...
        it('should display a message with a URL when no MCP servers are configured', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: 'No MCP servers configured. Please view MCP documentation in your browser: https://goo.gle/gemini-cli-docs-mcp or use the cli /docs command',
            });
        });
    });
    describe('with configured MCP servers', () => {
        ...
        it('should display configured MCP servers with status indicators and their tools', async () => {
            ...
            if (isMessageAction(result)) {
                ...
                // Server 2 - Connected
                expect(message).toContain('🟢 \u001b[1mserver2\u001b[0m - Ready (1 tool)');
                expect(message).toContain('server2_tool1');
                // Server 3 - Disconnected but with cached tools, so shows as Ready
                expect(message).toContain('🟢 \u001b[1mserver3\u001b[0m - Ready (1 tool)');
        ...
        it('should indicate when a server has no tools', async () => {
            ...
            if (isMessageAction(result)) {
                const message = result.content;
                expect(message).toContain('🟢 \u001b[1mserver1\u001b[0m - Ready (1 tool)');
        ...
        it('should show startup indicator when servers are connecting', async () => {
            ...
            if (isMessageAction(result)) {
                ...
                expect(message).toContain('🟢 \u001b[1mserver1\u001b[0m - Ready (1 tool)');
                expect(message).toContain('🔄 \u001b[1mserver2\u001b[0m - Starting... (first startup may take longer) (tools and prompts will appear when ready)');
            }
        });
        ...
        it('should display blocked MCP servers', async () => {
            ...
            if (isMessageAction(result)) {
                const message = result.content;
                expect(message).toContain('🔴 \u001b[1mblocked-server (from my-extension)\u001b[0m - Blocked');
            }
        });
        it('should display both active and blocked servers correctly', async () => {
            ...
            if (isMessageAction(result)) {
                const message = result.content;
                expect(message).toContain('server1 (from my-extension)');
                expect(message).toContain('🔴 \u001b[1mblocked-server (from another-extension)\u001b[0m - Blocked');
            }
        });
    });
    describe('schema functionality', () => {
        it('should display tool schemas when schema argument is used', async () => {
            ...
            const mockCallableTool1 = {
                callTool: vi.fn(),
                tool: vi.fn(),
            };
            const mockCallableTool2 = {
                callTool: vi.fn(),
                tool: vi.fn(),
            };
            const tool1 = new DiscoveredMCPTool(mockCallableTool1, 'server1', 'tool1', 'This is tool 1 description', {
                type: Type.OBJECT,
                properties: {
            ...
            const tool2 = new DiscoveredMCPTool(mockCallableTool2, 'server1', 'tool2', 'This is tool 2 description', {
                type: Type.OBJECT,
                properties: {
    ...
    describe('argument parsing', () => {
        ...
        it('should handle "nodesc" overriding both "desc" and "schema" - "desc schema nodesc"', async () => {
            const result = await mcpCommand.action(mockContext, 'desc schema nodesc');
            expect(isMessageAction(result)).toBe(true);
            if (isMessageAction(result)) {
                const message = result.content;
                expect(message).not.toContain('Test tool');
    ...
    describe('auth subcommand', () => {
        ...
        it('should list OAuth-enabled servers when no server name is provided', async () => {
            ...
            const authCommand = mcpCommand.subCommands?.find((cmd) => cmd.name === 'auth');
        ...
        it('should show message when no OAuth servers are configured', async () => {
            ...
            if (isMessageAction(result)) {
                expect(result.messageType).toBe('info');
                expect(result.content).toBe('No MCP servers configured with OAuth authentication.');
            }
        });
        it('should authenticate with a specific server', async () => {
            ...
            expect(mockAuthProvider.authenticate).toHaveBeenCalledWith('test-server', { enabled: true }, 'http://localhost:3000', expect.any(Object));
        ...
        it('should handle authentication errors', async () => {
            ...
            const { MCPOAuthProvider } = await import('@google/gemini-cli-core');
            const mockAuthProvider = new MCPOAuthProvider();
            vi.mocked(mockAuthProvider.authenticate).mockRejectedValue(new Error('Auth failed'));
            const authCommand = mcpCommand.subCommands?.find((cmd) => cmd.name === 'auth');
            const result = await authCommand.action(context, 'test-server');
            expect(isMessageAction(result)).toBe(true);
            if (isMessageAction(result)) {
    ...
    describe('refresh subcommand', () => {
        it('should refresh the list of tools and display the status', async () => {
            ...
            context.ui.reloadCommands = vi.fn();
            const refreshCommand = mcpCommand.subCommands?.find((cmd) => cmd.name === 'refresh');
