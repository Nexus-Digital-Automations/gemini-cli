...
async function updateAction(context, args) {
    ...
    try {
        ...
        if (all) {
            updateInfos = await updateAllUpdatableExtensions(context.services.config.getWorkingDir(), context.services.config.getExtensions(), context.ui.extensionsUpdateState, context.ui.setExtensionsUpdateState);
        }
            ...
            for (const name of names) {
                ...
                const updateInfo = await updateExtension(extension, workingDir, context.ui.extensionsUpdateState.get(extension.name) ??
                    ExtensionUpdateState.UNKNOWN, (updateState) => {
