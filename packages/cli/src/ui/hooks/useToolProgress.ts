...
export const useToolProgress = (
  initialStatus: ToolCallStatus = ToolCallStatus.Pending,
): UseToolProgressResult => {
  const progressContext = useProgress();
  const [operationId, setOperationId] = useState<string>();
  const [status, setStatus] = useState<ToolCallStatus>(initialStatus);
  const [_currentStepIndex, setCurrentStepIndex] = useState(0);

  // Get the current operation from the progress context
  const operation = operationId
    ? progressContext.activeOperations.find(
  ...
  const currentStepDescription =
    operation?.steps[operation.currentStepIndex]?.description;
  ...
  const startTracking = useCallback(
    (toolCall: ToolCallInfo, toolCallId: string) => {
      const newOperationId = progressContext.startOperation(
        toolCall,
        toolCallId,
      );
      ...
      const newOperation = progressContext.activeOperations.find(
        (op) => op.operationId === newOperationId,
      );
  ...
  const updateProgress = useCallback(
    (stepId: string, progress: number, intermediateResult?: unknown) => {
      if (operationId) {
        progressContext.updateOperationProgress(
          operationId,
          stepId,
          progress,
          intermediateResult,
...
export const useSimpleToolProgress = () => {
  const [status, setStatus] = useState<ToolCallStatus>(ToolCallStatus.Pending);
  ...
  const updateProgress = useCallback(
    (newProgress: number, message?: string) => {
