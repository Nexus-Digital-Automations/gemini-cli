...
export interface ContextualPhrase {
  message: string;
  isSpecific: boolean; // true if specific to current operation, false if generic
}

/**
 * Hook that provides contextual loading phrases based on the current operation
 */
export const useContextualPhrases = (
 ...
 */
function getOperationSpecificPhrase(
  operation: OperationProgress,
): string | null {
 ...
 */
function getOperationTypePhrase(operationType: OperationType): string | null {
  const typeSpecificPhrases: Record<OperationType, string[]> = {
    file_operation: [
      'Working with files...',
 ...
 */
function getGenericPhrase(): ContextualPhrase {
