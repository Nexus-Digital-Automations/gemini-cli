...
vi.mock('../charts/TokenUsageChart.js', () => ({
  TokenUsageChart: ({ config }: { config: { title: string } }) => (
    <div data-testid="token-usage-chart">{config.title}</div>
  ),
}));

vi.mock('../charts/CostBreakdownChart.js', () => ({
  CostBreakdownChart: ({ config }: { config: { title: string } }) => (
    <div data-testid="cost-breakdown-chart">{config.title}</div>
  ),
}));

vi.mock('../charts/UsageTrendsGraph.js', () => ({
  UsageTrendsGraph: ({ config }: { config: { title: string } }) => (
    <div data-testid="usage-trends-graph">{config.title}</div>
  ),
}));

vi.mock('../charts/CostProjectionChart.js', () => ({
  CostProjectionChart: ({ config }: { config: { title: string } }) => (
    <div data-testid="cost-projection-chart">{config.title}</div>
  ),
}));
...
const mockUseBudgetDashboard = useBudgetDashboard as Mock;

describe('BudgetDashboard', () => {
  const defaultProps: BudgetDashboardProps = {
  ...
  it('should render loading state when dashboard is loading', () => {
    ...
    const { lastFrame } = render(<BudgetDashboard {...defaultProps} />);
  ...
  it('should initialize with correct hook parameters', () => {
    render(<BudgetDashboard {...defaultProps} />);
