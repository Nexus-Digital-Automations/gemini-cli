 ...
 * ```
 */
export function formatCompact(value, precision = 1) {
 ...
 */
export function convertUsageToChartData(usageData, type) {
    return usageData.map((point) => ({
        ...
        y: type === 'cost'
            ...
            : type === 'tokens'
                ? point.tokens
                : point.requests,
        label: `${formatTime(point.timestamp)}: ${type === 'cost'
 ...
 */
export function convertBreakdownToChartData(breakdown, colorPalette = DEFAULT_PIE_COLORS) {
 ...
 */
export function createTimeSeriesFromMetrics(metrics, field) {
 ...
 */
export function formatTime(timestamp, format = 'short') {
    const now = new Date();
 ...
 */
export function createProgressBar(percentage, width = 10, filled = '█', empty = '░') {
 ...
 */
export function createSparkline(data, width) {
    if (data.length === 0)
        return '';
 ...
 */
export function generateColorPalette(count, theme = 'default') {
    const themes = {
        default: [
            '#3b82f6',
            '#10b981',
 ...
 */
export function aggregateUsageByPeriod(data, period) {
 ...
 */
function getTimePeriodKey(timestamp, period) {
