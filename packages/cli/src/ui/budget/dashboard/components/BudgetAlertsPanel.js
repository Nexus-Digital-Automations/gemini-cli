...
export const BudgetAlertsPanel = ({ alerts, budgetStats, onDismissAlert, onModifyThresholds: _onModifyThresholds, }) => {
    ...
    const renderAlertsList = () => {
        ...
        return (_jsxs(Box, { flexDirection: "column", flexGrow: 1, children: [_jsx(Box, { marginBottom: 1, children: _jsxs(Text, { color: theme.text.muted, children: ["Active Alerts (", sortedAlerts.length, "):"] }) }), sortedAlerts.map((alert, index) => (_jsxs(Box, { flexDirection: "column", paddingX: 2, paddingY: 1, borderStyle: "single", borderColor: index === selectedAlertIndex
                        ? getSeverityColor(alert.severity)
                        : theme.text.muted, marginBottom: 1, backgroundColor: index === selectedAlertIndex
                        ? theme.background.secondary
                        : undefined, children: [_jsxs(Box, { justifyContent: "space-between", alignItems: "center", marginBottom: 1, children: [_jsxs(Box, { gap: 1, alignItems: "center", children: [_jsx(Text, { children: getSeverityIcon(alert.severity) }), _jsx(Text, { children: getTypeIcon(alert.type) }), _jsx(Text, { color: getSeverityColor(alert.severity), bold: true, children: alert.title })] }), _jsx(Text, { color: theme.text.muted, children: formatTime(alert.triggeredAt) })] }), _jsx(Box, { marginBottom: 1, children: _jsx(Text, { color: theme.text.secondary, children: alert.message }) }), _jsxs(Box, { justifyContent: "space-between", marginBottom: 1, children: [_jsxs(Box, { gap: 3, children: [_jsxs(Text, { color: theme.text.muted, children: ["Threshold: ", alert.threshold] }), _jsxs(Text, { color: theme.text.muted, children: ["Current: ", alert.currentValue] }), _jsxs(Text, { color: getSeverityColor(alert.severity), children: ["Severity: ", alert.severity] })] }), index === selectedAlertIndex && (_jsx(Text, { color: theme.text.muted, children: "Press Enter to dismiss" }))] }), index === selectedAlertIndex &&
                            alert.suggestedActions.length > 0 && (_jsxs(Box, { flexDirection: "column", marginTop: 1, children: [_jsx(Text, { color: theme.text.muted, children: "Suggested Actions:" }), alert.suggestedActions.map((action, actionIndex) => (_jsx(Box, { marginTop: 1, children: _jsxs(Text, { color: theme.text.secondary, children: ["\u2022 ", action] }) }, actionIndex)))] }))] }, alert.id)))] }));
    };
    /**
    ...
    const renderThresholdConfig = () => {
        ...
        return (_jsxs(Box, { flexDirection: "column", marginTop: 2, children: [_jsx(Box, { marginBottom: 1, children: _jsx(Text, { color: theme.text.muted, children: "Alert Thresholds:" }) }), _jsx(Box, { gap: 4, marginBottom: 1, children: currentThresholds.map((threshold, index) => (_jsxs(Box, { alignItems: "center", gap: 1, children: [_jsxs(Text, { color: theme.status.warning, children: [threshold, "%"] }), _jsx(Text, { color: theme.text.muted, children: index === 0 ? 'Warning' : index === 1 ? 'Error' : 'Critical' })] }, index))) }), _jsx(Text, { color: theme.text.muted, children: "\uD83D\uDCA1 Configure custom thresholds in dashboard settings" })] }));
    };
    /**
    ...
    const renderAlertStats = () => {
        ...
        return (_jsxs(Box, { flexDirection: "column", marginTop: 2, children: [_jsx(Box, { marginBottom: 1, children: _jsx(Text, { color: theme.text.muted, children: "Alert Statistics:" }) }), _jsxs(Box, { gap: 4, children: [_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: theme.text.muted, children: "Today" }), _jsx(Text, { color: theme.text.primary, children: todaysAlerts.length })] }), _jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: theme.text.muted, children: "Last Hour" }), _jsx(Text, { color: theme.text.primary, children: recentAlerts.length })] }), _jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: theme.text.muted, children: "Total" }), _jsx(Text, { color: theme.text.primary, children: alerts.length })] })] })] }));
    };
    /**
