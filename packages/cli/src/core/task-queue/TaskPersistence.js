...
export class TaskPersistence extends EventEmitter {
    ...
    async recoverQueueState() {
        ...
        try {
            ...
            let recoveredState = await this.attemptRecoveryFromFile(join(this.storageDir, this.fileNames.currentState));
            if (!recoveredState) {
                // Try backup file
                recoveredState = await this.attemptRecoveryFromFile(join(this.storageDir, this.fileNames.backup));
                if (recoveredState) {
    ...
    async verifyWrittenData(filePath, originalState) {
        try {
            ...
            if (parsedState.tasks.registry.length !==
