...
export class PerformanceOptimizer extends EventEmitter {
    ...
    async measureOptimizationImpact(_recommendation) {
    ...
    calculateAgentUtilization(_queueStatus) {
    ...
    updateTrends(history) {
        ...
        history.trends.throughput = this.determineTrend(recentAvg.queueMetrics.queueThroughput, olderAvg.queueMetrics.queueThroughput);
        history.trends.latency = this.determineTrend(olderAvg.systemMetrics.latency.p95, // Inverted - lower latency is better
        recentAvg.systemMetrics.latency.p95);
        history.trends.efficiency = this.determineTrend(recentAvg.queueMetrics.queueEfficiency, olderAvg.queueMetrics.queueEfficiency);
        history.trends.errorRate = this.determineTrend(olderAvg.systemMetrics.errorRate, // Inverted - lower error rate is better
