...
export class DataIntegrityManager {
    ...
    getRecoveryHistory(options = {}) {
        ...
        operations.sort((a, b) => new Date(b.timestamps.initiated).getTime() - new Date(a.timestamps.initiated).getTime());
        // Apply limit
        if (options.limit) {
    ...
    async getStatistics() {
        ...
        const stats = {
            ...
            recentRecoveries: recoveries.filter(recovery => new Date(recovery.timestamps.initiated).getTime() > Date.now() - (24 * 60 * 60 * 1000)).length,
