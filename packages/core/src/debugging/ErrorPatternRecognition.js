 ...
 */
export const DEFAULT_ERROR_PATTERN_RECOGNITION_CONFIG = {
 ...
 * - **Context-Aware Matching**: Considers file _context, project type, and execution environment
...
export class ErrorPatternRecognition {
    ...
    async initialize(projectPath) {
        ...
        try {
            ...
            logger.info(`Error Pattern Recognition Engine initialized in ${duration.toFixed(2)}ms`, {
     ...
     */
    async analyzeError(errorText, _context) {
        if (!this.isInitialized) {
            throw new Error('ErrorPatternRecognition not initialized. Call initialize() first.');
        }
        const startTime = performance.now();
        const cacheKey = this.generateCacheKey(errorText, _context);
        try {
            // Check cache first
            ...
            const matches = await this.findMatches(errorText, _context);
            // Sort by confidence and relevance
            const sortedMatches = this.rankMatches(matches, _context);
            // Filter by confidence threshold
            const filteredMatches = sortedMatches.filter((_match) => match.confidence >= this.config.confidenceThreshold);
     ...
     */
    async learnFromError(errorText, matches, userFeedback) {
     ...
     */
    async loadProjectPatterns(projectPath) {
        try {
    ...
    async initializePatternStats() {
        ...
        logger.debug(`Initialized statistics for ${this.patternStats.size} patterns`);
    }
    /**
     ...
     */
    async findMatches(errorText, _context) {
        const matches = [];
        for (const pattern of this.patterns.values()) {
            ...
            for (const matcher of pattern.matchers) {
                const matchResult = await this.testMatcher(errorText, matcher, _context);
                if (matchResult.isMatch) {
                    const patternMatch = {
                        ...
                        matchDetails: {
                            ...
                            context: matchResult._context,
                        },
                        suggestions: this.generateSuggestions(pattern, _matchResult),
     ...
     */
    async testMatcher(errorText, matcher, _context) {
        try {
            switch (matcher.type) {
                case 'regex': {
                    ...
                    if (_match) {
                        return {
                            isMatch: true,
                            confidence: this.calculateRegexConfidence(_match, errorText),
                            matchedText: match[0],
                ...
                case 'contains': {
                    ...
                    if (index !== -1) {
                        return {
                            isMatch: true,
                            confidence: 0.8,
                            matchedText: errorText.substring(index, index + matcher.pattern.length),
     ...
     */
    calculateRegexConfidence(_match, errorText) {
     ...
     */
    generateSuggestions(pattern, _matchResult) {
        const suggestions = [];
        // Add common causes as suggestions
        if (pattern.metadata?.commonCauses &&
            Array.isArray(pattern.metadata.commonCauses)) {
            suggestions.push(...pattern.metadata.commonCauses.map((cause) => `Check for: ${cause}`));
        }
     ...
     * Update pattern statistics
     */
    async updatePatternStats(matches) {
        const now = new Date();
     ...
     */
    async learnNewPattern(errorText, actualCause) {
        if (!this.config.enableMLPatterns) {
            return;
        }
        try {
     ...
     */
    async updatePatternEffectiveness(matches, feedback) {
        // Mark correct matches as successful
        if (feedback.correctMatch) {
            const stats = this.patternStats.get(feedback.correctMatch);
            if (stats) {
                stats.successfulMatches++;
                stats.effectiveness = Math.min((stats.successfulMatches / stats.matchCount) * 1.2, 1.0);
            }
        }
        ...
        if (feedback.incorrectMatches) {
            for (const incorrectId of feedback.incorrectMatches) {
                ...
                if (stats) {
                    ...
                    stats.effectiveness = Math.max((stats.successfulMatches - stats.falsePositives) / stats.matchCount, 0);
                }
            }
        }
    }
     ...
     */
    generateCacheKey(errorText, _context) {
 ...
 */
export async function createErrorPatternRecognition(config = {}, projectPath) {
    const engine = new ErrorPatternRecognition(config);
    await engine.initialize(projectPath);
    return engine;
}
//# sourceMappingURL=ErrorPatternRecognition.js.map