 ...
 */
export const DEFAULT_REAL_TIME_ERROR_MONITOR_CONFIG = {
    errorAnalysis: {
...
export class RealTimeErrorMonitor {
    ...
    monitoringTimer;
    healthCheckTimer;
    currentMetrics = this.getInitialMetrics();
    isMonitoring = false;
    isInitialized = false;
    constructor(config = {}) {
    ...
    async startMonitoring() {
        ...
        if (this.config.enableRealTimeMonitoring) {
            this.monitoringTimer = setInterval(() => this.performMonitoringCycle().catch((error) => logger.error('Monitoring cycle failed', { error })), this.config.monitoringInterval);
        }
        // Start health monitoring timer
        if (this.config.enableHealthMonitoring) {
            this.healthCheckTimer = setInterval(() => this.performHealthCheck().catch((error) => logger.error('Health check failed', { error })), this.config.healthCheckInterval);
        }
     ...
     * Get error history
     */
    getErrorHistory(limit, applicationId) {
     ...
     */
    async checkErrorPatterns(errorEvent) {
        ...
        for (const [patternId, patternConfig] of Object.entries(REAL_TIME_ERROR_PATTERNS)) {
            ...
            const relevantErrors = this.errorHistory.filter((error) => error.timestamp.getTime() >= windowStart);
            ...
            switch (patternId) {
                ...
                case 'errorSpike': {
                    ...
                    const baselineErrors = this.errorHistory.filter((error) => error.timestamp.getTime() >= baselineWindow &&
                ...
                case 'memoryLeakIndicator': {
                    const memoryErrors = relevantErrors.filter((error) => error.message.toLowerCase().includes('memory') ||
                ...
                case 'infiniteLoop': {
                    const similarErrors = relevantErrors.filter((error) => error.message === errorEvent.message);
     ...
     * Emit error event to subscribers
     */
    async emitErrorEvent(errorEvent) {
     ...
     */
    async triggerPatternAlert(patternId, patternConfig, description, triggerEvent) {
        const alert = {
    ...
    async updateApplicationStatuses() {
        ...
        for (const [appId, app] of this.monitoredApps.entries()) {
            const appErrors = this.errorHistory.filter((error) => error.applicationId === appId && error.timestamp >= fiveMinutesAgo);
            let status = 'healthy';
 ...
 */
export async function createRealTimeErrorMonitor(config = {}) {
    const monitor = new RealTimeErrorMonitor(config);
    await monitor.initialize();
    return monitor;
}
//# sourceMappingURL=RealTimeErrorMonitor.js.map