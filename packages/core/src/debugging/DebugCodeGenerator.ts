 ...
 */
export const DEFAULT_DEBUG_CODE_GENERATOR_CONFIG: DebugCodeGeneratorConfig = {
 ...
 */
const DEBUG_CODE_TEMPLATES: Record<
  string,
  Record<DebugCodeType, DebugCodeTemplate[]>
> = {
  javascript: {
    logging: [
      {
...
export class DebugCodeGenerator {
  private config: DebugCodeGeneratorConfig;
  private templates: Map<string, Map<DebugCodeType, DebugCodeTemplate[]>> =
    new Map();
  private templateCache: Map<string, { template: string; timestamp: number }> =
    new Map();
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.info(
        `Debug Code Generator initialized in ${duration.toFixed(2)}ms`,
        {
  ...
  async generateDebugCode(
    ...
    if (!this.isInitialized) {
      throw new Error(
        'DebugCodeGenerator not initialized. Call initialize() first.',
      );
    }
    ...
    try {
      ...
      const snippet: DebugCodeSnippet = {
   ...
   */
  async generateInstrumentationCode(
    ...
    try {
      ...
      if (
        (instrumentationType === 'performance' ||
      ...
      if (
        instrumentationType === 'error-handling' ||
        instrumentationType === 'all'
      ) {
  ...
  async generateTestCase(
    ...
    const testOptions: DebugGenerationOptions = {
      type: 'testing',
      language,
      context: {
  ...
  async generateValidationCode(
    ...
    return this.generateDebugCode({
      ...
      context: {
        ...
        validationCondition: this.generateValidationCondition(
          expectedType,
          targetLanguage,
        ),
      },
  ...
  getAvailableTemplates(
    ...
    const allTemplates: DebugCodeTemplate[] = [];
  ...
  private loadBuiltinTemplates(): void {
    for (const [language, typeTemplates] of Object.entries(
      DEBUG_CODE_TEMPLATES,
    )) {
      const languageMap = new Map<DebugCodeType, DebugCodeTemplate[]>();

      for (const [type, templates] of Object.entries(typeTemplates)) {
        languageMap.set(type as DebugCodeType, templates);
      }
    ...
    logger.debug(
      `Loaded built-in templates for ${Object.keys(DEBUG_CODE_TEMPLATES).length} languages`,
    );
  }
   ...
   */
  private applyCodeStyle(code: string, language: LanguageSupport): string {
    let styledCode = code;
  ...
  private generateUsageInstructions(
    ...
    switch (template.category) {
      case 'logging':
        instructions.push(
          '2. Place the logging code at the location where you want to inspect variables',
        );
        instructions.push('3. Run your application and check console output');
        break;
      case 'error-handling':
        instructions.push(
          '2. Wrap the error-prone code with the generated try-catch block',
        );
        instructions.push('3. Customize the error handling logic as needed');
        break;
      case 'performance':
        instructions.push(
          '2. Wrap the code you want to measure with the performance instrumentation',
        );
        instructions.push(
          '3. Run your application and check performance metrics',
        );
        break;
      case 'testing':
    ...
    instructions.push(
      `4. Remove debug code before committing to version control (if temporary)`,
    );
   ...
   */
  private extractOperationFromError(errorAnalysis: ErrorAnalysis): string {
  ...
  private generateInstrumentationInstructions(
    instrumentation: InstrumentationCode,
  ): string[] {
    const instructions: string[] = [];

    instructions.push(
      'Generated instrumentation code to help debug the error:',
    );
    instructions.push('');
    ...
    if (instrumentation.loggingInstrumentation) {
      ...
      instructions.push(
        `   ${instrumentation.loggingInstrumentation.description}`,
      );
      instructions.push(
        `   Placement: ${instrumentation.loggingInstrumentation.placement}`,
      );
      instructions.push('');
    }
    ...
    if (instrumentation.performanceInstrumentation) {
      ...
      instructions.push(
        `   ${instrumentation.performanceInstrumentation.description}`,
      );
      instructions.push(
        `   Placement: ${instrumentation.performanceInstrumentation.placement}`,
      );
      instructions.push(
        `   Metrics: ${instrumentation.performanceInstrumentation.metrics?.join(', ')}`,
      );
      instructions.push('');
    }
  ...
  private generateValidationCondition(
    ...
    switch (language) {
      ...
      case 'python':
        if (expectedType === 'str') return 'isinstance(value, str)';
        if (expectedType === 'int') return 'isinstance(value, int)';
        if (expectedType === 'float') return 'isinstance(value, (int, float))';
 ...
 */
export async function createDebugCodeGenerator(
  config: Partial<DebugCodeGeneratorConfig> = {},
): Promise<DebugCodeGenerator> {
