 ...
 * - **Contextual Intelligence**: Considers project _context, file relationships, and execution environment
...
export class ErrorAnalysisEngine {
    ...
    async analyzeError(errorText, _context) {
        ...
        const startTime = performance.now();
        const cacheKey = this.generateAnalysisCacheKey(errorText, _context);
        try {
            // Check cache first
            ...
            const patternMatches = await this.patternRecognition.analyzeError(errorText, _context);
            // Generate error signature for tracking
            const signature = this.generateErrorSignature(errorText, _context);
            ...
            const analysis = {
                ...
                severity: this.calculateSeverity(patternMatches, errorText, _context),
                confidence: this.calculateOverallConfidence(patternMatches),
                rootCause: await this.identifyRootCause(errorText, patternMatches, _context),
                patterns: patternMatches,
                fixSuggestions: await this.generateFixSuggestions(patternMatches, _context),
                insights: await this.generateInsights(errorText, patternMatches, _context),
                contextualFactors: await this.analyzeContextualFactors(_context),
                relatedErrors: this.findRelatedErrors(signature, _context),
                performanceImpact: this.config.enablePerformanceAnalysis
                    ? await this.analyzePerformanceImpact(errorText, _context)
                    : undefined,
                securityImplications: this.config.enableSecurityAnalysis
                    ? await this.analyzeSecurityImplications(errorText, _context)
                    : undefined,
                projectImpact: await this.assessProjectImpact(errorText, _context),
                metadata: {
            ...
            await this.updateErrorFrequency(signature, _analysis);
            // Update related errors
            if (this.config.enableRelatedErrorDetection) {
                await this.updateRelatedErrors(signature, _analysis);
            }
     ...
     */
    async findSimilarErrors(errorText, _context, limit = 5) {
        if (!this.isInitialized) {
     ...
     */
    calculateSeverity(patternMatches, errorText, _context) {
        let severity = 'medium';
    ...
    calculateOverallConfidence(patternMatches) {
        ...
        const weightedSum = patternMatches.reduce((sum, _match) => sum + match.confidence, 0);
     ...
     */
    async generateFixSuggestions(patternMatches, _context) {
        const suggestions = [];
     ...
     */
    async generateInsights(errorText, patternMatches, _context) {
     ...
     */
    async analyzeContextualFactors(_context) {
        const factors = [];
        // Language factor
        if (context.language) {
            factors.push({
     ...
     */
    findRelatedErrors(signature, _context) {
        const related = [];
     ...
     */
    async assessProjectImpact(errorText, _context) {
        ...
        return {
            ...
            estimatedFixTime: this.estimateFixTime(errorText, _context),
        };
    }
    /**
     ...
     */
    generateErrorSignature(errorText, _context) {
     ...
     */
    generateAnalysisCacheKey(errorText, _context) {
