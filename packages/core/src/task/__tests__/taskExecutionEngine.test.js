...
describe('TaskExecutionEngine', () => {
    ...
    const createMockTask = (id, status, toolCalls = [], dependencies = []) => ({
        id,
        name: `Task ${id}`,
    ...
    it('should execute a task with tool calls successfully', async () => {
        ...
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-1', TaskStatus.InProgress);
        expect(mockDefaultApi.tool1).toHaveBeenCalledWith({ a: 1 });
        expect(mockDefaultApi.tool2).toHaveBeenCalledWith({ b: 2 });
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-1', TaskStatus.Done);
    ...
    it('should mark task as failed if a tool call fails', async () => {
        ...
        expect(mockDefaultApi.tool1).toHaveBeenCalledWith({ a: 1 });
        expect(mockDefaultApi.tool2).toHaveBeenCalledWith({ b: 2 });
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-2', TaskStatus.Failed);
    ...
    it('should mark task as blocked if dependencies are not done', async () => {
        ...
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-3', TaskStatus.Blocked);
    ...
    it('should execute task if all dependencies are done', async () => {
        const toolCalls = [{ toolName: 'tool1', args: { a: 1 } }];
        ...
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-4', TaskStatus.InProgress);
        expect(mockDefaultApi.tool1).toHaveBeenCalledWith({ a: 1 });
        expect(resultTask.status).toBe(TaskStatus.Done);
    });
    it('should find and execute runnable tasks', async () => {
        ...
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-5', TaskStatus.InProgress);
        expect(mockDefaultApi.tool1).toHaveBeenCalled();
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-5', TaskStatus.Done);
        // Task2 should be blocked initially, then marked as blocked
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-6', TaskStatus.Blocked);
        // Task3 should be blocked initially, then marked as blocked
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-7', TaskStatus.Blocked);
    });
    it('should handle non-existent tool gracefully', async () => {
        const toolCalls = [{ toolName: 'nonExistentTool', args: {} }];
        ...
        expect(mockTaskManager.updateTaskStatus).toHaveBeenCalledWith('task-8', TaskStatus.Failed);
        expect(resultTask.status).toBe(TaskStatus.Failed);
