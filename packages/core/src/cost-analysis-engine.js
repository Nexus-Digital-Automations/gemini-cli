...
export class CostAnalysisEngine {
     ...
     */
    static async performComprehensiveAnalysis(costData, currentBudget, alertConfigs = []) {
        ...
        try {
            ...
            const activeAlerts = alertConfigs.length > 0
                ? await BudgetAlertSystem.monitorAndAlert(costData, alertConfigs.map((config) => ({
                    ...config,
            ...
            const recommendations = this.generateOptimizationRecommendations(costData, currentBudget, trend, projection, burnRate, variance);
            // Calculate health scores
            const healthScore = this.calculateHealthScore(currentBudget, trend, projection, burnRate, variance);
    ...
    static identifyUsagePattern(costData, trend, variance, seasonal) {
        ...
        try {
            ...
            const significantSpikes = variance.variances.filter((v) => v.varianceType === 'spike' && v.varianceScore > 0.7).length;
    ...
    static calculateBudgetRunway(currentBudget, projection, burnRate) {
        ...
        try {
            ...
            const conservativeDays = conservativeRate > 0
            ...
            const optimisticDays = optimisticRate > 0
            ...
            const maxRunway = Math.max(...scenarios
     ...
     */
    static generateOptimizationRecommendations(costData, currentBudget, trend, projection, burnRate, variance) {
        const recommendations = [];
     ...
     */
    static calculateHealthScore(currentBudget, trend, projection, burnRate, variance) {
        // Calculate component scores (0-100)
        // Trend health: positive trends are bad for budget health
        const trendHealth = trend.direction === 'decreasing'
            ? 90
        ...
        const burnRateHealth = runwayDays > 90
            ? 95
            : runwayDays > 60
                ? 80
                : runwayDays > 30
        ...
        const overall = Math.round(trendHealth * 0.25 +
            varianceHealth * 0.2 +
    ...
    static analyzeDataContinuity(costData) {
        ...
        const totalTimespan = (sortedData[sortedData.length - 1].timestamp.getTime() -
