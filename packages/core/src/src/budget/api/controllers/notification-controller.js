...
const logger = {
    info: (message, meta) => console.info(`[NotificationController] ${message}`, meta),
    warn: (message, meta) => console.warn(`[NotificationController] ${message}`, meta),
    error: (message, meta) => console.error(`[NotificationController] ${message}`, meta),
};
/**
...
export class NotificationController {
    ...
    async testNotification(req, res) {
        ...
        try {
            ...
            const deliveryResults = await this.sendTestNotification(testNotification, channel, alertConfig);
            const responseTime = Date.now() - startTime;
            const response = {
                success: true,
                data: {
            ...
            logger.info('Notification test completed', {
                ...
                alertId,
                channel,
                deliverySuccess: deliveryResults.every((result) => result.success),
            });
            res.status(200).json(response);
        }
        catch (error) {
    ...
    validateAlertConfig(config) {
        ...
        if (!config.type ||
            !['threshold', 'limit_exceeded', 'anomaly', 'custom'].includes(config.type)) {
     ...
     */
    async sendTestNotification(notification, channel, alertConfig) {
        const results = [];
        // Validate and cast channel parameter
        const validChannelTypes = ['email', 'webhook', 'desktop', 'sms'];
        const validatedChannel = channel && validChannelTypes.includes(channel)
            ? channel
            : 'email';
     ...
     */
    async sendDesktopNotification(notification, _config) {
