 ...
 */
export const DEFAULT_UTILIZATION_CONFIG = {
...
export class ResourceUtilizationOptimizer {
     ...
     */
    optimizeResourceUtilization(candidate, historicalData) {
     ...
     */
    optimizePortfolioUtilization(candidates, historicalData) {
        this.logger.info('Starting portfolio utilization optimization', {
            resourceCount: candidates.length,
        });
        // Analyze each resource
        const resources = candidates.map((candidate) => this.optimizeResourceUtilization(candidate, historicalData[candidate.resourceId] || []));
        ...
        this.logger.info('Portfolio utilization optimization completed', {
            overallScore,
            totalOpportunities: resources.reduce((sum, r) => sum + r.opportunities.length, 0) +
     ...
     */
    generateUtilizationRecommendations(candidate, analysis) {
     ...
     * Calculate current utilization metrics
     */
    calculateCurrentUtilization(candidate, historicalData) {
        ...
        const average = utilizationRates.reduce((sum, rate) => sum + rate, 0) /
            utilizationRates.length;
        const peak = Math.max(...utilizationRates);
        const minimum = Math.min(...utilizationRates);
        // Calculate variance
        const variance = utilizationRates.reduce((sum, rate) => sum + Math.pow(rate - average, 2), 0) / utilizationRates.length;
        // Determine trend
     ...
     */
    analyzeUtilizationPatterns(historicalData) {
    ...
    analyzeDailyPattern(historicalData) {
        ...
        for (let hour = 0; hour < 24; hour++) {
            if (hourlyUtilization[hour]) {
                const avg = hourlyUtilization[hour].reduce((sum, rate) => sum + rate, 0) /
        ...
        const mean = utilizationValues.reduce((sum, val) => sum + val, 0) /
            utilizationValues.length;
        const variance = utilizationValues.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) /
            utilizationValues.length;
    ...
    analyzeWeeklyPattern(historicalData) {
        ...
        for (const data of historicalData) {
            ...
            if (!dailyUtilization[dayOfWeek])
                dailyUtilization[dayOfWeek] = [];
        ...
        for (let day = 0; day < 7; day++) {
            if (dailyUtilization[day]) {
                const avg = dailyUtilization[day].reduce((sum, rate) => sum + rate, 0) /
     ...
     */
    forecastUtilization(historicalData, horizon) {
        ...
        const anomalyRisk = Math.min(0.5, Math.abs(slope) +
            Math.sqrt(utilizationData.reduce((sum, point) => sum + Math.pow(point.y - (slope * point.x + intercept), 2), 0) / n) *
                2);
        return {
            horizon: `${horizon} days`,
            predictions,
     ...
     * Analyze capacity requirements
     */
    analyzeCapacityRequirements(candidate, current, predicted) {
     ...
     * Analyze performance correlation
     */
    analyzePerformanceCorrelation(candidate, historicalData) {
    ...
    identifyCrossResourceOpportunities(resources, _candidates) {
        ...
        const utilizationVariance = resources.reduce((sum, r) => sum + r.current.variance, 0) /
            resources.length;
     ...
     */
    calculatePortfolioScore(resources) {
        if (resources.length === 0)
            return 0;
        const avgUtilization = resources.reduce((sum, r) => sum + r.current.average, 0) /
            resources.length;
        const avgEfficiency = resources.reduce((sum, r) => sum + r.current.efficiency, 0) /
            resources.length;
     ...
     * Generate portfolio insights
     */
    generatePortfolioInsights(resources, crossResourceOpportunities) {
        const totalSavings = resources.reduce((sum, r) => sum +
            r.opportunities.reduce((oSum, opp) => oSum + opp.benefits.costSavings, 0), 0) +
            crossResourceOpportunities.reduce((sum, opp) => sum + opp.benefits.costSavings, 0);
        const utilizationImprovement = resources.reduce((sum, r) => sum +
            r.opportunities.reduce((oSum, opp) => oSum + opp.benefits.utilizationGain, 0), 0) / resources.length;
        // Calculate risk-adjusted benefits
        const riskWeights = { low: 0.9, medium: 0.7, high: 0.4 };
        const riskAdjustedBenefits = resources.reduce((sum, r) => sum +
            r.opportunities.reduce((oSum, opp) => {
                const weight = riskWeights[opp.complexity] || 0.5;
     ...
     * Create implementation roadmap
     */
    createImplementationRoadmap(resources, crossResourceOpportunities) {
        const allOpportunities = [
            ...resources.flatMap((r) => r.opportunities),
            ...crossResourceOpportunities,
        ];
        const quickWins = allOpportunities.filter((opp) => opp.complexity === 'low' &&
            (opp.priority === 'high' || opp.priority === 'critical'));
        const strategic = allOpportunities.filter((opp) => opp.complexity === 'medium' && opp.benefits.costSavings > 20);
        const longTerm = allOpportunities.filter((opp) => opp.complexity === 'high' || opp.benefits.costSavings > 50);
        return { quickWins, strategic, longTerm };
    }
    /**
     * Create allocation recommendation from optimization opportunity
     */
    createRecommendationFromOpportunity(candidate, opportunity, analysis) {
        ...
        return {
            ...
            timeToImplement: opportunity.complexity === 'low'
                ? '2 hours'
            ...
            riskAssessment: {
                riskLevel: opportunity.complexity === 'low'
                    ? 'low'
                    : opportunity.complexity === 'high'
                        ? 'high'
                        : 'medium',
            ...
            dependencies: [],
            estimatedTimeToImplement: opportunity.complexity === 'low'
                ? 2
                : opportunity.complexity === 'high'
                    ? 8
                    : 4,
    ...
    validateConfiguration() {
        ...
        if (this.config.targetUtilization < utilizationRange.min ||
 ...
 */
export function createResourceUtilizationOptimizer(config, logger) {
    const defaultLogger = {
        info: () => { },
        warn: () => { },
        error: () => { },
...
//# sourceMappingURL=ResourceUtilizationOptimizer.js.map