...
export class ROIOptimizedAlgorithm extends BaseAllocationAlgorithm {
    ...
    generateROIRecommendations(candidates, distribution) {
        ...
        for (const candidate of candidates) {
            ...
            const recommendation = {
                ...
                dependencies: [],
                priority: candidate.priority === 'deferred'
                    ? 'low'
                    : candidate.priority,
    ...
    ensureBudgetBalance(originalCandidates, recommendations) {
        const totalOriginalBudget = originalCandidates.reduce((sum, candidate) => sum + candidate.currentAllocation, 0);
        const totalRecommendedBudget = recommendations.reduce((sum, rec) => sum + rec.recommendedAllocation, 0);
