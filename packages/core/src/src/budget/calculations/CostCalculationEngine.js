...
export class CostCalculationEngine {
    logger = getComponentLogger('CostCalculationEngine');
    modelPricing;
    calculationHistory = [];
     ...
     */
    calculateUsage(currentUsage, limit, period, periodElapsed) {
        const start = Date.now();
        try {
            ...
            const projectedUsage = periodElapsed > 0 ? currentUsage / periodElapsed : currentUsage;
            ...
            this.logger.error('Usage calculation failed', { error: error });
            throw error;
        }
    }
    /**
     ...
     */
    getTokenUsageSummary(timeframe) {
        const cutoffTime = Date.now() - timeframe;
        // Filter recent calculations
        const recentCalculations = this.calculationHistory.filter((calc) => {
        ...
        const summary = recentCalculations.reduce((acc, calc) => {
     ...
     */
    getModelUsageBreakdown(timeframe) {
        const cutoffTime = Date.now() - timeframe;
        const modelUsage = {};
     ...
     * @returns Usage trend
     */
    calculateUsageTrend(currentUsage, period) {
 ...
 */
export function createCostCalculationEngine(customPricing) {
    return new CostCalculationEngine(customPricing);
}
//# sourceMappingURL=CostCalculationEngine.js.map