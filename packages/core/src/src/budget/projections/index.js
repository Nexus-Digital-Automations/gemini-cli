...
export class CostProjectionSystem {
    ...
    async generateCostProjection(historicalData, projectionDays = 30) {
        ...
        try {
            ...
            const projection = await CostForecastingEngine.generateProjections(historicalData, projectionDays, 0.95);
            ...
            const analysis = await CostAnalysisEngine.performComprehensiveAnalysis(historicalData, defaultBudget);
            // Calculate required parameters for optimization
            const trend = await import('./algorithms/mathematical-algorithms.js').then((m) => m.MathematicalAlgorithms.performTrendAnalysis(historicalData));
            const variance = await import('./algorithms/mathematical-algorithms.js').then((m) => m.MathematicalAlgorithms.detectVariances(historicalData));
            const burnRate = await CostForecastingEngine.calculateBurnRateAnalysis(historicalData, defaultBudget.total);
            // Add optimization recommendations
            const optimizations = await BudgetOptimizationEngine.generateOptimizationPlan(historicalData, defaultBudget, projection, burnRate, trend, variance);
    ...
    async monitorBudgetAndAlert(currentUsage, budgetLimit, alertConfigs) {
        ...
        try {
            ...
            const alerts = await BudgetAlertSystem.monitorAndAlert(currentCostData, alertConfigs, budgetData);
            const duration = Date.now() - startTime;
    ...
    async performCostAnalysis(costData) {
        ...
        try {
            ...
            const analysis = await CostAnalysisEngine.performComprehensiveAnalysis(costData, defaultBudget);
            const duration = Date.now() - startTime;
    ...
    async generateOptimizationRecommendations(historicalData, currentProjection) {
        ...
        try {
            ...
            const projection = currentProjection ||
                (await CostForecastingEngine.generateProjections(historicalData, 30, 0.95));
            const optimizationPlan = await BudgetOptimizationEngine.generateOptimizationPlan(historicalData, defaultBudget, projection, burnRate, trend, variance);
            const duration = Date.now() - startTime;
    ...
    async validateSystemPerformance() {
        ...
        try {
            ...
            const testAlgorithm = (data) => CostForecastingEngine.generateProjections(data, 30, 0.95);
