...
export class AlertTriggerEngine {
     ...
     */
    static evaluateAllTriggers(costData, alertConfigs, currentBudget, projection, variance) {
        ...
        try {
            ...
            for (const config of alertConfigs) {
                const result = this.evaluateTrigger(config, costData, currentBudget, projection, variance);
                results.push(result);
            }
            // Evaluate composite triggers
            const compositeTriggers = this.evaluateCompositeTriggers(results, costData, currentBudget, projection, variance);
     ...
     */
    static evaluateTrigger(config, costData, currentBudget, projection, variance) {
        try {
            let currentValue = 0;
            let confidence = 0.8;
            const context = {};
            ...
            const adjustedThreshold = this.applyDynamicThresholdAdjustments(config.threshold.value, config.threshold.type, costData, variance);
            // Evaluate threshold condition
            const triggered = this.evaluateThresholdCondition(currentValue, adjustedThreshold, config.threshold.operator);
            // Apply temporal conditions
            const temporallyTriggered = this.applyTemporalConditions(triggered, config, costData);
            return {
                triggerId: config.id,
     ...
     */
    static evaluateCompositeTriggers(basicResults, costData, currentBudget, projection, variance) {
        const compositeResults = [];
        // Budget exhaustion composite trigger
        const budgetTriggers = basicResults.filter((r) => r.triggerId.includes('budget') || r.triggerId.includes('percentage'));
        const rateTriggers = basicResults.filter((r) => r.triggerId.includes('rate') || r.triggerId.includes('burn'));
        if (budgetTriggers.some((t) => t.triggered) &&
        ...
        if (variance && variance.summary.significantVariances > 0) {
            const recentSpikes = variance.variances.filter((v) => v.varianceType === 'spike' &&
        ...
        if (projection && projection.summary.burnRatePerDay > 0) {
            const projectedOverrun = currentBudget.used +
    ...
    static calculateCurrentBurnRate(costData) {
        ...
        const timeSpanHours = (dataToAnalyze[dataToAnalyze.length - 1].timestamp.getTime() -
            dataToAnalyze[0].timestamp.getTime()) /
            (60 * 60 * 1000);
        return timeSpanHours > 0 ? (totalCost / timeSpanHours) * 24 : 0; // Convert to daily rate
    }
    static applyDynamicThresholdAdjustments(baseThreshold, thresholdType, costData, variance) {
    ...
    static isRuleApplicable(alert, rule) {
        ...
        const recentAlerts = history.filter((h) => h.timestamp.getTime() >
