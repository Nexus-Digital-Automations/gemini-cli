...
// Event management
export { BudgetEventManager, } from './events.js';
// Quota and rate limiting
export { QuotaManager, } from './quota-manager.js';
// Data aggregation
export { TokenDataAggregator, } from './aggregator.js';
// Real-time streaming
export { RealTimeStreamingService, } from './streaming.js';
// Caching system
export { TokenUsageCache, createTokenUsageCache, CachePriority, CachePresets, CacheKeys, } from './cache.js';
// Integration layer
export { TokenMonitoringIntegration, TokenTrackingContentGenerator, createTokenMonitoringIntegration, createMonitoringEnabledContentGenerator, } from './integration.js';
import { createTokenMonitoringIntegration, createMonitoringEnabledContentGenerator, } from './integration.js';
/**
...
export const MonitoringUtils = {
     ...
     */
    createForEnvironment: async (config, budgetSettings, environment = 'development') => {
        const preset = MonitoringPresets[environment === 'production'
            ? 'Production'
            : environment === 'testing'
                ? 'Testing'
                : 'Development'];
     ...
     */
    setupWithErrorHandling: async (baseContentGenerator, config, budgetSettings, onError) => {
        try {
            return await createMonitoringEnabledContentGenerator(baseContentGenerator, config, budgetSettings, MonitoringPresets.Production);
        }
        catch (error) {
            if (onError) {
                onError(error);
     ...
     */
    createHealthChecker: (integration) => new MonitoringHealthChecker(integration),
};
