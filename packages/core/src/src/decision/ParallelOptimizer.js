...
export class ParallelOptimizer {
    ...
    calculateGroupPriority(taskIds, tasks) {
        ...
        for (const taskId of taskIds) {
            ...
            if (task) {
                totalPriority +=
                    priorities[String(task.priority)];
            }
        }
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            const score = weight *
                (result.projections.parallelizationFactor * 0.4 +
                    ...
                    (Array.from(result.projections.resourceUtilization.values()).reduce((a, b) => a + b, 0) /
    ...
    calculateOptimizationConfidence(result, validation) {
        ...
        const avgUtilization = Array.from(result.projections.resourceUtilization.values()).reduce((sum, util) => sum + util, 0) / result.projections.resourceUtilization.size;
