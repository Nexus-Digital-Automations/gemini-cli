...
export class TaskManagementSystemIntegrator {
        ...
        try {
            ...
            if (this.config.persistence?.enabled !== false) {
                ...
                this.persistence = new CrossSessionPersistenceEngine({
                    storageDir: this.config.persistence?.storageLocation || '.task-management-data',
                    enableCompression: this.config.persistence?.compressionEnabled ?? true,
                    enableMetrics: true,
                    maxBackupVersions: 5,
                });
            ...
            if (this.config.hookIntegration?.enabled !== false) {
                ...
                try {
                    this.hookIntegration = new InfiniteHookIntegration(this.config.core, this.taskEngine, this.monitoring, {
                        agentId: this.config.hookIntegration?.agentId || 'INTEGRATED_SYSTEM',
                        capabilities: this.config.hookIntegration?.capabilities || [
                            'autonomous_execution',
                        ],
                        progressReportingIntervalMs: this.config.hookIntegration?.progressReportingIntervalMs ??
                            30000,
                    });
    ...
    getSystemHealth() {
        ...
        return {
            ...
            metrics: {
                ...
                taskThroughput: Number(metrics.completedTasks) /
    ...
    async getSystemStatus() {
        ...
        const monitoringMetrics = this.monitoring?.getSystemHealth(await this.monitoring?.collectMetrics(this.taskEngine?.getAllTasks() || []));
        return {
            health,
            taskEngineStats,
    ...
    async handleTaskStatusChange(task) {
        ...
        if (this.monitoring) {
            this.monitoring.recordEvent({
                taskId: task.id,
                eventType: task.status === 'in_progress'
                    ? 'started'
