...
export class EnhancedTaskQueue extends TaskQueue {
    ...
    mlOptimizedScheduling(availableSlots) {
        ...
        const predictions = this.predictionModel?.predict?.(taskFeatures) || taskFeatures.map(() => Math.random());
    ...
    boostPriorityForStarvation(originalPriority, waitTime) {
        ...
        const priorityValues = Object.values(TaskPriority).filter((value) => typeof value === 'number');
