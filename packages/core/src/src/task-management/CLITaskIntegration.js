...
import { TaskStatus, TaskPriority, TaskCategory, } from './types.js';
/**
...
export class CLITaskIntegration {
    config;
    taskManager;
    stateManager; // TaskStateManager type not available - will be typed properly later
    executionEngine;
    commands = new Map();
    ...
    async executeCommand(commandName, args = [], options = {}) {
        ...
        try {
            ...
            const taskId = await this.taskManager.addTask(task.title, task.description, {
    ...
    async cancelTask(taskId, reason) {
        ...
        try {
            ...
            await this.stateManager.transitionTaskState(taskId, TaskStatus.CANCELLED, {}, // Would get actual task
            {
    ...
    initializeBuiltinCommands() {
        ...
        this.registerCommand({
            ...
            mapToTask: async ([title, description], options) => ({
                ...
                category: options.category || TaskCategory.FEATURE,
                metadata: {
                    createdAt: new Date(),
                    updatedAt: new Date(),
                    createdBy: 'cli_user',
    ...
    handleStateChange(event) {
        if (this.config.verbosity === 'debug') {
            console.log(`🔄 Task ${event.taskId}: ${event.transition.fromState} → ${event.transition.toState}`);
        }
    ...
    async updateProgressDisplay(taskId) {
        const progress = await this.getTaskProgress(taskId);
        if (!progress)
            return;
        if (this.config.verbosity === 'verbose' ||
            this.config.verbosity === 'debug') {
            console.log(`📈 ${progress.title}: ${progress.progress}% (${progress.state})`);
        }
    }
    updateGlobalProgress() {
        const status = this.getSystemStatus();
        if (this.config.verbosity === 'normal' ||
            this.config.verbosity === 'verbose') {
            if (status.tasksActive > 0) {
                console.log(`📊 System Status: ${status.tasksActive} active, ${status.tasksCompleted} completed, ${status.tasksFailed} failed`);
            }
        }
    }
    /**
    ...
    generateErrorSuggestions(error) {
        ...
        if (error.message.includes('permission')) {
            suggestions.push('Check file permissions', 'Run with elevated privileges');
        }
        if (error.message.includes('memory')) {
            suggestions.push('Increase memory allocation', 'Close other applications');
        }
