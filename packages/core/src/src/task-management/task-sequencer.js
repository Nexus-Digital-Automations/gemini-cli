...
export class TaskSequencer {
     ...
     */
    generateExecutionSequence(tasks, algorithm = 'hybrid') {
     ...
     */
    generatePriorityBasedSequence(tasks) {
     ...
     */
    generateDependencyAwareSequence(tasks) {
     ...
     */
    generateResourceOptimalSequence(tasks) {
        ...
        const criticalPath = this.dependencyGraph
            .calculateCriticalPath()
            .filter((taskId) => tasks.has(taskId));
        const estimatedDuration = this.calculateEstimatedDurationWithParallelization(parallelGroups, tasks);
        return {
            sequence,
            parallelGroups,
            criticalPath,
    ...
    generateHybridSequence(tasks) {
        ...
        for (const dependencyGroup of dependencyGroups) {
            ...
            const resourceOptimizedGroups = this.optimizeGroupForResources(groupTasks.map((item) => item.task));
     ...
     */
    calculateSchedulingFactors(task) {
        ...
        const dependencyWeight = dependencyImpact.totalImpact +
     ...
     */
    optimizeWithinDependencyConstraints(sequence, tasks) {
     ...
     */
    canReorderWithinGroup(taskId, group) {
     ...
     */
    identifyParallelGroups(sequence, tasks) {
        const groups = [];
        let currentGroup = [];
     ...
     */
    canExecuteInParallel(task, groupTaskIds, tasks) {
     ...
     */
    optimizeGroupForResources(tasks) {
        if (tasks.length === 0)
            return [];
        const groups = [];
        let currentGroup = [];
     ...
     */
    calculateEstimatedDuration(sequence, tasks) {
        let totalDuration = 0;
        for (const taskId of sequence) {
            const task = tasks.get(taskId);
            if (task) {
     ...
     */
    calculateEstimatedDurationWithParallelization(parallelGroups, tasks) {
        let totalDuration = 0;
     ...
     */
    generateResourceAllocations(sequence, tasks) {
        const allocations = [];
        const resourceUsage = new Map();
        // Initialize resource tracking
        for (const [resourceType] of this.resourcePools) {
...
//# sourceMappingURL=task-sequencer.js.map