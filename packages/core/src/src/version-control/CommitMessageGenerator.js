...
export class CommitMessageGenerator {
    ...
    generateBody(analysis) {
        ...
        if (analysis.importChanges.length > 0) {
            ...
            if (addedImports.length > 0) {
                bodyParts.push(`New dependencies: ${addedImports
                    .slice(0, 3)
                    .map((i) => i.packageName)
                    .join(', ')}`);
            }
            if (removedImports.length > 0) {
                bodyParts.push(`Removed dependencies: ${removedImports
                    .slice(0, 3)
                    .map((i) => i.packageName)
                    .join(', ')}`);
            }
     ...
     */
    extractIssueReferences(_analysis) {
        const references = [];
        try {
    ...
    isStyleChange(analysis) {
        ...
        const styleFiles = analysis.modifiedFiles.filter((file) => file.includes('.prettier') ||
    ...
    isCIChange(analysis) {
        return analysis.modifiedFiles.some((file) => file.includes('.github/workflows') ||
            file.includes('.gitlab-ci') ||
            file.includes('Jenkinsfile') ||
            file.includes('.travis.yml'));
    }
    isBuildChange(analysis) {
        return analysis.modifiedFiles.some((file) => file.includes('webpack') ||
    ...
    generateDocsDescription(_analysis) {
    ...
    generateTestDescription(_analysis) {
        return `add tests`;
    }
    ...
    generatePerfDescription(_analysis) {
