...
export class ChangelogGenerator {
    ...
    async generateChangelog(options = {}) {
        try {
            ...
            const { fromVersion, toVersion } = await this.resolveVersionRange(options.fromVersion, options.toVersion);
            ...
            const changelog = await this.renderChangelog(entries, releaseNotes, format, template, options);
    ...
    async analyzeCommits(commits) {
        ...
        for (const commit of commits) {
            ...
            if (this.isInternalCommit(commit) &&
        ...
        return entries.sort((a, b) => {
            if (a.significance !== b.significance) {
                return (this.getSignificanceWeight(b.significance) -
     ...
     */
    async renderChangelog(entries, releaseNotes, format, templateName, options) {
     ...
     */
    renderMarkdown(entries, releaseNotes, template, options) {
     ...
     */
    renderJSON(entries, releaseNotes, _options) {
        const data = {
     ...
     */
    renderHTML(entries, releaseNotes, template, options) {
...
<body>
     ...
     */
    renderXML(entries, releaseNotes, _options) {
    ...
    async resolveVersionRange(fromVersion, toVersion) {
        if (!fromVersion) {
    ...
    async parseCommitsInRange(fromVersion, toVersion) {
        try {
            const gitLog = execSync(`git log ${fromVersion}..${toVersion} --pretty=format:"%H|%an|%ae|%ad|%s|%b" --date=iso`, { cwd: this.workingDir, encoding: 'utf-8' });
            return gitLog
                .split('\n')
    ...
    parseConventionalCommit(message, metadata) {
    ...
    async calculateNextVersion(entries) {
    ...
    extractBreakingChanges(entries) {
        return entries
    ...
    async getContributorInfo(username) {
    ...
    generateHighlights(groupedEntries) {
        const highlights = [];
    ...
    calculateReleaseMetrics(entries, commits) {
        return {
            ...
            averageChangeSize: entries.reduce((acc, e) => acc + e.linesAdded + e.linesDeleted, 0) /
                entries.length,
            complexityScore: entries.reduce((acc, e) => acc + (e.metadata.complexity || 1), 0) /
                entries.length,
    ...
    async generateAnalytics(entries, releaseNotes, metrics) {
        return {
            trends: {
    ...
    async analyzeCommitImpact(commit, fileChanges) {
        ...
        return {
            ...
            complexity: fileChanges.files.length +
                (fileChanges.additions + fileChanges.deletions) / 100,
            riskLevel: commit.breakingChange ? 'high' : 'low',
            testCoverage: 85,
            affectedModules: [
                ...new Set(fileChanges.files.map((f) => f.split('/')[0])),
            ],
        };
    }
    enhanceDescription(commit, impact) {
    ...
    async getCommitReviewers(_hash) {
        // Mock implementation - would fetch PR reviewers
        return [];
    }
    generateCommitTags(commit, impact) {
        const tags = [];
    ...
    determineReleaseType(entries, breakingChanges) {
        if (breakingChanges.length > 0)
    ...
    generateReleaseTags(entries, impact) {
        const tags = [];
        if (impact.level === 'high')
            tags.push('major-release');
        if (entries.some((e) => e.type === 'perf'))
            tags.push('performance');
        if (entries.some((e) => e.type === 'feat'))
            tags.push('features');
        if (entries.some((e) => e.type === 'fix'))
            tags.push('bugfixes');
        if (entries.some((e) => e.breakingChange))
            tags.push('breaking-changes');
        return tags;
    }
    generateReleaseRecommendations(entries, impact) {
    ...
    generateReleaseSummary(entries, impact) {
    ...
    calculateDevelopmentVelocity(commits) {
    ...
    analyzeCommitFrequency(_entries) {
        return { trend: 'stable', average: 2.5 };
    }
    analyzeChangeTypes(entries) {
    ...
    analyzeContributorActivity(contributors) {
        return {
    ...
    analyzeComplexityTrends(entries) {
        const avgComplexity = entries.reduce((acc, e) => acc + (e.metadata.complexity || 1), 0) /
            entries.length;
        return { average: avgComplexity, trend: 'stable' };
    }
    analyzeQualityTrends(entries) {
        const avgQuality = entries.reduce((acc, e) => acc + (e.metadata.testCoverage || 80), 0) /
            entries.length;
        return { average: avgQuality, trend: 'improving' };
    }
    identifyHottestComponents(entries) {
    ...
    identifyQualityImprovements(entries) {
        return entries
            .filter((e) => e.type === 'test' || e.type === 'refactor')
            .map((e) => e.subject)
            .slice(0, 5);
    }
    analyzeTechnicalDebt(entries) {
        const debtReduction = entries.filter((e) => e.type === 'refactor' ||
            e.subject.toLowerCase().includes('debt') ||
            e.subject.toLowerCase().includes('cleanup')).length;
        return {
    ...
    estimateAffectedUsers(level, _entries) {
    ...
    assessReleaseRisk(entries, breakingChanges) {
        const riskScore = this.calculateRiskScore(entries);
        if (breakingChanges.length > 0 || riskScore > 70)
            return 'high';
        if (riskScore > 30)
            return 'medium';
        return 'low';
    }
    assessRollbackComplexity(entries) {
        ...
        if (dbChanges > 0 || schemaChanges > 0)
            return 'complex';
        if (entries.length > 20)
            return 'moderate';
        return 'simple';
    }
    assessCompatibility(entries, breakingChanges) {
        return {
