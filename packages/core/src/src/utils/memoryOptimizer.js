...
export class StreamTokenCounter {
     ...
     */
    async countTokensFromFile(filePath) {
        this.tokenCount = 0;
        ...
        try {
            await pipeline(createReadStream(filePath, {
                highWaterMark: this.config.streamChunkSize,
            }), tokenizeTransform);
        }
        catch (error) {
            throw new Error(`Failed to count tokens from file: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
...
export class ContextWindowManager {
     ...
     */
    slideContextWindow(history, maxTokens) {
        if (history.length === 0)
            return history;
        ...
        for (let i = history.length - 1; i >= 0; i--) {
            ...
            if (currentTokens + entryTokens <= maxTokens) {
                ...
                const truncatedEntry = this.truncateEntry(entry, maxTokens - currentTokens, tokenCounter);
                if (truncatedEntry) {
    ...
    estimateEntryTokens(entry, tokenCounter) {
        if (!entry.parts)
            return 0;
        let totalTokens = 0;
    ...
    truncateEntry(entry, maxTokens, tokenCounter) {
        if (!entry.parts || maxTokens <= 0)
            return null;
        ...
        for (const part of entry.parts) {
            if (part.text) {
                ...
                if (partTokens <= remainingTokens) {
                    ...
                    const truncatedText = this.truncateTextToTokens(part.text, remainingTokens);
...
export class MemoryOptimizationUtils {
     ...
     */
    static optimizeStringReplace(text, searchValue, replaceValue) {
        if (text.length < this.config.streamChunkSize) {
            return text.replace(searchValue, replaceValue);
        }
        const builder = new MemoryEfficientStringBuilder(this.config.streamChunkSize);
     ...
     */
    static optimizeArrayConcat(...arrays) {
     ...
     */
    static async measurePerformance(operation, label) {
...
export const memoryOptimizer = {
    ...
    tokenCounter: (config) => new StreamTokenCounter(config),
    contextManager: (config) => new ContextWindowManager(config),
