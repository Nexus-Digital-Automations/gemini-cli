...
import { SuggestionEngine, DEFAULT_SUGGESTION_ENGINE_CONFIG, } from './SuggestionEngine.js';
 ...
 */
export const DEFAULT_ADVANCED_CONTEXT_RETENTION_CONFIG = {
...
export class AdvancedContextRetentionSystem {
    ...
    async initialize() {
        ...
        try {
            ...
            logger.info(`Advanced Context Retention System initialized in ${duration.toFixed(2)}ms`, {
    ...
    async addContext(type, content, metadata = {}) {
        ...
        try {
            ...
            const added = await this.windowManager.addToSection(sectionName, contextItem);
            if (!added) {
                ...
                const retryAdded = await this.windowManager.addToSection(sectionName, contextItem);
                if (!retryAdded) {
    ...
    async getSuggestions(currentContext) {
        ...
        try {
            ...
            const _recentInteractions = [];
            // Generate suggestions
            const suggestions = await this.suggestionEngine.getSuggestions(currentContext || this.getCurrentContextSummary(), 'code', codeContext);
        ...
        try {
            ...
            return {
                ...
                stats: {
                    ...
                    totalTokens: contextItems.reduce((sum, item) => sum + item.tokenCount, 0),
                    averageItemSize: contextItems.reduce((sum, item) => sum + item.tokenCount, 0) /
    ...
    async optimize() {
        ...
        try {
            ...
            const contextItems = Array.from(this.contextItems.values());
            // Prioritize items
            const prioritizationResult = await this.prioritizer.prioritize(contextItems);
    ...
    async loadPreviousSession() {
        try {
            const sessions = await this.storage.getRelatedSessions(this.config.projectPath, 1);
            ...
            if (interaction.files) {
                for (const file of interaction.files) {
                    for (const [itemId, item] of this.contextItems.entries()) {
                        if (item.metadata.filePath === file ||
    ...
    getSystemStats() {
        ...
        return {
            ...
            uptimeHours: (Date.now() - this.systemStartTime.getTime()) / (1000 * 60 * 60),
        };
    }
    /**
    ...
    updateConfig(newConfig) {
        ...
        if (newConfig.autoOptimizationInterval &&
    ...
    startAutoOptimization() {
        ...
        logger.debug(`Auto-optimization scheduled every ${this.config.autoOptimizationInterval} minutes`);
    }
    /**
    ...
    async pruneOldContext() {
        ...
        const sortedItems = itemsArray.sort((a, b) => a.lastAccessed.getTime() - b.lastAccessed.getTime());
        // Remove oldest items to get under limit
        const itemsToRemove = sortedItems.slice(0, this.contextItems.size - this.config.maxContextItems);
    ...
    extractTags(content) {
        ...
        const extensionMatch = content.match(/\.(js|ts|jsx|tsx|py|java|go|rs|c|cpp|h|php|rb|swift|kt|cs)\b/g);
        if (extensionMatch) {
    ...
    extractUserPreferences() {
        const preferences = {};
        const preferenceItems = Array.from(this.contextItems.values()).filter((item) => item.type.toString() === 'user-preference');
 ...
 */
export async function createAdvancedContextRetentionSystem(config = {}) {
    return AdvancedContextRetentionSystem.create(config);
}
//# sourceMappingURL=AdvancedContextRetentionSystem.js.map