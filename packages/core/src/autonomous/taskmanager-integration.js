    ...
    async updateTaskResults(tasks, results) {
        for (const result of results) {
            try {
                ...
                this.logger.error(`Failed to update task result: ${result.taskId}`, error);
            }
        }
    }
 ...
 */
export function createAutonomousIntegration(workspaceContext, availableTools, config) {
    const defaultConfig = {
