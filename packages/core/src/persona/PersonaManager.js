...
const BUILT_IN_PERSONAS = {
    professional: {
        ...
        description: 'Balanced, professional communication focused on efficiency and clarity',
        personality: {
    ...
    mentor: {
        ...
        customization: {
            preferredTerminology: {},
            customPromptAdditions: 'Focus on explaining the reasoning behind suggestions and helping the user understand best practices.',
            learningPreferences: [
                'best-practices',
                'code-quality',
                'debugging',
    ...
    expert: {
        ...
        description: 'Highly knowledgeable expert providing comprehensive technical guidance',
        personality: {
            formality: 'formal',
        ...
        customization: {
            preferredTerminology: {},
            customPromptAdditions: 'Provide deep technical insights, consider edge cases, and suggest advanced optimizations.',
            learningPreferences: [
                'advanced-patterns',
                'performance',
                'security',
    ...
    casual: {
        name: 'Casual Helper',
        description: 'Friendly, approachable assistant for relaxed development sessions',
        personality: {
            formality: 'casual',
            verbosity: 'balanced',
            creativity: 'practical',
        ...
        customization: {
            preferredTerminology: {},
            customPromptAdditions: 'Use a friendly, conversational tone. Keep explanations accessible and encouraging.',
            learningPreferences: [],
        },
        isSystem: true,
        updatedAt: new Date().toISOString(),
    ...
    efficient: {
        ...
        customization: {
            preferredTerminology: {},
            customPromptAdditions: 'Prioritize brevity and actionable solutions. Minimize explanatory text unless specifically requested.',
            learningPreferences: [],
        },
        isSystem: true,
        updatedAt: new Date().toISOString(),
    ...
    creative: {
        ...
        description: 'Innovative problem solver encouraging experimental approaches',
        personality: {
            formality: 'casual',
            verbosity: 'detailed',
        ...
        customization: {
            preferredTerminology: {},
            customPromptAdditions: 'Encourage creative solutions, suggest alternative approaches, and explore innovative techniques.',
            learningPreferences: [
                'emerging-tech',
                'creative-coding',
...
export class PersonaManager {
    logger = logger.child({ component: 'PersonaManager' });
    personas = new Map();
    ...
    getActivePersona() {
        ...
        if (!persona) {
            this.logger.warn('Active persona not found, falling back to professional', {
    ...
    generateSystemPrompt() {
        ...
        if (persona.customization.learningPreferences?.length) {
            ...
            promptParts.push('Pay special attention to these areas when providing guidance:');
        ...
        if (persona.customization.preferredTerminology &&
            ...
            Object.entries(persona.customization.preferredTerminology).forEach(([standard, preferred]) => {
    ...
    generatePersonalityInstructions(personality) {
        ...
        switch (personality.formality) {
            case 'formal':
                instructions.push('- Use formal, professional language with proper titles and structured responses');
                break;
            case 'professional':
                instructions.push('- Maintain professional tone while being approachable and clear');
                break;
            case 'casual':
                instructions.push('- Use conversational, friendly language that feels natural and approachable');
                break;
            case 'relaxed':
                instructions.push('- Adopt a relaxed, informal tone as if speaking with a colleague');
                break;
            default:
                instructions.push('- Maintain professional tone while being approachable and clear');
                break;
        ...
        switch (personality.verbosity) {
            case 'concise':
                instructions.push('- Keep responses brief and to the point, focusing on essential information');
                break;
            case 'balanced':
                instructions.push('- Provide well-balanced responses with appropriate detail for the context');
                break;
            case 'detailed':
                instructions.push('- Include comprehensive explanations and context for better understanding');
                break;
            case 'comprehensive':
                instructions.push('- Provide thorough, extensive responses covering all relevant aspects');
                break;
            default:
                instructions.push('- Provide well-balanced responses with appropriate detail for the context');
                break;
        ...
        switch (personality.creativity) {
            case 'conservative':
                instructions.push('- Stick to well-established, proven solutions and conventional approaches');
                break;
            case 'practical':
                instructions.push('- Focus on practical, reliable solutions while considering some innovative options');
                break;
            case 'innovative':
                instructions.push('- Suggest creative solutions and explore modern approaches when appropriate');
                break;
            case 'experimental':
                instructions.push('- Encourage experimental approaches and cutting-edge techniques');
                break;
            default:
                instructions.push('- Focus on practical, reliable solutions while considering some innovative options');
                break;
        ...
        switch (personality.teachingStyle) {
            case 'direct':
                instructions.push('- Provide direct solutions with minimal explanation unless requested');
                break;
            case 'explanatory':
                instructions.push('- Explain the reasoning behind suggestions and provide educational context');
                break;
            case 'socratic':
                instructions.push('- Guide understanding through questions and encourage independent thinking');
                break;
            case 'mentoring':
                instructions.push('- Act as a patient mentor, focusing on skill development and learning');
                break;
            default:
                instructions.push('- Explain the reasoning behind suggestions and provide educational context');
                break;
     ...
     */
    generateBehaviorInstructions(behavior) {
        const instructions = [];
        // Proactiveness instructions
        switch (behavior.proactiveness) {
            case 'reactive':
                instructions.push('- Wait for explicit requests before suggesting improvements or alternatives');
                break;
            case 'balanced':
                instructions.push('- Offer moderate suggestions and improvements when relevant to the context');
                break;
            case 'proactive':
                instructions.push('- Actively suggest improvements, optimizations, and best practices');
                break;
            case 'assertive':
                instructions.push('- Strongly advocate for best practices and proactively guide toward optimal solutions');
                break;
            default:
                instructions.push('- Offer moderate suggestions and improvements when relevant to the context');
                break;
        ...
        switch (behavior.errorHandling) {
            case 'concise':
                instructions.push('- Provide brief error explanations focused on the immediate solution');
                break;
            case 'helpful':
                instructions.push('- Explain errors clearly and provide guidance for resolution');
                break;
            case 'thorough':
                instructions.push('- Provide comprehensive error analysis including root causes and prevention');
                break;
            case 'preventive':
                instructions.push('- Focus on preventing future errors through education and best practices');
                break;
            default:
                instructions.push('- Explain errors clearly and provide guidance for resolution');
                break;
        ...
        switch (behavior.codeStyle) {
            case 'minimal':
                instructions.push('- Focus on essential code changes only, avoiding unnecessary modifications');
                break;
            case 'constructive':
                instructions.push('- Provide balanced feedback with constructive suggestions for improvement');
                break;
            case 'comprehensive':
                instructions.push('- Offer detailed code review covering style, performance, and best practices');
                break;
            case 'mentoring':
                instructions.push('- Focus on teaching through code review, explaining patterns and principles');
                break;
            default:
                instructions.push('- Provide balanced feedback with constructive suggestions for improvement');
                break;
     ...
     */
    importCustomPersonas(personasJson) {
        try {
            const importedPersonas = JSON.parse(personasJson);
