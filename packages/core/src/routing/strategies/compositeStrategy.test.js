...
describe('CompositeStrategy', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(getComponentLogger).mockReturnValue(mockLogger);
        mockContext = {};
        mockConfig = {};
        mockBaseLlmClient = {};
        mockStrategy1 = {
    ...
    it('should handle errors in non-terminal strategies and continue', async () => {
        ...
        const result = await composite.route(mockContext, mockConfig, mockBaseLlmClient);
        expect(mockLogger.error).toHaveBeenCalledWith("Strategy 'strategy1' failed. Continuing to next strategy.", expect.objectContaining({
            error: testError,
    ...
    it('should re-throw an error from the terminal strategy', async () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith("Critical Error: Terminal strategy 'terminal' failed. Routing cannot proceed.", expect.objectContaining({
            error: terminalError,
