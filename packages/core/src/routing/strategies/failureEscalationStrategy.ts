...
/**
 * Tracks failure patterns for Flash model requests.
 */
interface FailurePattern {
  /** Hash of the request for pattern matching */
  requestHash: string;
  /** Number of times Flash failed for this pattern */
  failureCount: number;
  /** Timestamp of last failure */
  lastFailure: number;
  /** Whether this pattern should escalate to Pro */
 ...
 */
const DEFAULT_ESCALATION_CONFIG: FailureEscalationConfig = {
  failureThreshold: 2,
 ...
 * ```
 */
export class FailureEscalationStrategy implements RoutingStrategy {
  readonly name = 'failure-escalation';

  private config: FailureEscalationConfig;
  private failurePatterns: Map<string, FailurePattern> = new Map();
  private logger = getComponentLogger('FailureEscalationStrategy');
  ...
  getFailureStats(): {
    ...
    const patterns = Array.from(this.failurePatterns.values()).map(
      (pattern) => ({
   ...
   */
  private convertRequestToString(request: PartListUnion): string {
   ...
   */
  private generateRequestHash(request: string): string {
