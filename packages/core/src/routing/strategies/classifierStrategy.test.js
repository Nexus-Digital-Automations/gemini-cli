...
describe('ClassifierStrategy', () => {
    ...
    it('should return null if the classifier API call fails', async () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith('ClassifierStrategy failed', expect.objectContaining({
            error: testError,
        }));
    });
    ...
    it('should use a fallback promptId if not found in context', async () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Could not find promptId in context. This is unexpected. Using a fallback ID:'), expect.objectContaining({
