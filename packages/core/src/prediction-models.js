...
export class PredictionModels {
     ...
     */
    static async linearRegressionModel(trainingData, predictionDays, regularization = 'ridge', alpha = 0.1) {
     ...
     */
    static async exponentialSmoothingModel(trainingData, predictionDays, smoothingParams = { alpha: 0.3, beta: 0.1, gamma: 0.1 }, seasonalPeriod = 7) {
        const startTime = Date.now();
        ...
        try {
            ...
            for (let t = 0; t < values.length; t++) {
                ...
                if (t < values.length - 1) {
                    // Update components
                    const newLevel = alpha * (values[t] / currentSeasonal[seasonalIndex]) +
     ...
     */
    static async arimaModel(trainingData, predictionDays, order = { p: 2, d: 1, q: 1 }) {
        const startTime = Date.now();
     ...
     */
    static async ensembleModel(trainingData, predictionDays, config = {
        models: [
            ...
            for (let day = 0; day < predictionDays; day++) {
                ...
                switch (config.votingStrategy) {
                    case 'weighted_average':
                        ...
                        combinedValue =
                            dayPredictions.reduce((sum, p) => sum + p.value * p.weight, 0) /
                                totalWeight;
                        combinedConfidence =
                            dayPredictions.reduce((sum, p) => sum + p.confidence * p.weight, 0) / totalWeight;
                        break;
                    case 'median':
                    ...
                    case 'confidence_weighted':
                        ...
                        combinedValue =
                            dayPredictions.reduce((sum, p) => sum + p.value * p.confidence, 0) / totalConfidence;
                ...
                const meanValue = dayPredictions.reduce((sum, p) => sum + p.value, 0) /
                    dayPredictions.length;
                const agreement = 1 -
                    Math.sqrt(dayPredictions.reduce((sum, p) => sum + Math.pow(p.value - meanValue, 2), 0) / dayPredictions.length) /
                        meanValue;
    ...
    static calculateRegularizedCoefficients(X, y, regularization, alpha) {
    ...
    static initializeHoltWinters(values, seasonalPeriod) {
    ...
    static estimateARIMAParameters(values, order) {
    ...
    static calculateAutocorrelations(values, lags) {
    ...
    static calculateARIMAFittedValues(values, arParams, maParams, order) {
        const fitted = [];
        const residuals = [];
    ...
    static calculateR2Score(actual, predicted) {
        const n = Math.min(actual.length, predicted.length);
