...
export class ToolRegistry {
    ...
    async discoverAllTools() {
        ...
        try {
            ...
            discoveryPromises.push(this.mcpClientManager.discoverAllMcpTools(this.config));
            await Promise.all(discoveryPromises);
            logger.debug('Tool discovery completed', {
                totalTools: this.getAllToolNames().length,
                mcpToolCount: this.getAllTools().filter((tool) => tool.constructor.name === 'DiscoveredMCPTool').length,
            });
        }
        catch (error) {
