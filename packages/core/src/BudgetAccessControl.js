...
export class BudgetAccessControl {
    ...
    async checkPermissions(context, requiredPermissions, resource, action) {
        ...
        try {
            ...
            if (this.config.rateLimiting && !this.checkRateLimit(context.sessionId)) {
                const auditEntryId = await this.logSecurityEvent(context, action, resource, requiredPermissions, false, 'Rate limit exceeded');
                return {
                    granted: false,
                    requiredPermissions,
            ...
            if (this.config.ipValidation &&
                ...
                const auditEntryId = await this.logSecurityEvent(context, action, resource, requiredPermissions, false, 'IP address not allowed');
                return {
                    granted: false,
                    requiredPermissions,
                    missingPermissions: requiredPermissions,
            ...
            if (userPermissions.includes(BudgetPermission.ADMIN)) {
                const auditEntryId = await this.logSecurityEvent(context, action, resource, requiredPermissions, true, 'Admin permission granted');
                return {
                    granted: true,
                    requiredPermissions,
                    missingPermissions: [],
            ...
            const missingPermissions = finalRequiredPermissions.filter((perm) => !userPermissions.includes(perm));
            const granted = missingPermissions.length === 0;
            const auditEntryId = await this.logSecurityEvent(context, action, resource, finalRequiredPermissions, granted, granted
                ? undefined
            ...
            const auditEntryId = await this.logSecurityEvent(context, action, resource, requiredPermissions, false, `Permission check error: ${error.message}`);
            return {
                granted: false,
                requiredPermissions,
                missingPermissions: requiredPermissions,
     ...
     */
    async logSecurityEvent(context, action, resource, requiredPermissions, accessGranted, denialReason) {
