...
export const usageRequestSchema = (data) => {
    ...
    if (typedData.startDate && !isValidDateString(typedData.startDate)) {
        errors.push('startDate must be a valid ISO date string');
    }
    if (typedData.endDate && !isValidDateString(typedData.endDate)) {
        errors.push('endDate must be a valid ISO date string');
    }
    // Validate pagination
    if (typedData.limit && (typedData.limit < 1 || typedData.limit > 1000)) {
        errors.push('limit must be between 1 and 1000');
    }
    if (typedData.offset && typedData.offset < 0) {
...
export const configurationRequestSchema = (data) => {
    ...
    if (typedData.enforcement) {
        const enumValidation = validateEnum(typedData, 'enforcement', Object.values(BudgetEnforcementLevel));
    ...
    // Validate limits
    if (typedData.dailyLimit && typedData.dailyLimit <= 0) {
        errors.push('dailyLimit must be positive');
    }
    if (typedData.weeklyLimit && typedData.weeklyLimit <= 0) {
        errors.push('weeklyLimit must be positive');
    }
    if (typedData.monthlyLimit && typedData.monthlyLimit <= 0) {
        errors.push('monthlyLimit must be positive');
    }
    // Validate reset time format (HH:MM)
    if (typedData.resetTime &&
        !/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(typedData.resetTime)) {
        errors.push('resetTime must be in HH:MM format');
    }
    // Validate notifications settings
    if (typedData.notifications) {
        const notificationErrors = validateNotificationSettings(typedData.notifications);
...
export const analyticsRequestSchema = (data) => {
    ...
    if (typedData.granularity) {
        ...
        if (!validGranularities.includes(typedData.granularity)) {
            errors.push(`granularity must be one of: ${validGranularities.join(', ')}`);
        }
    }
    // Validate date range
    if (typedData.startDate && typedData.endDate) {
        const start = new Date(typedData.startDate);
        const end = new Date(typedData.endDate);
        if (start >= end) {
...
export const exportRequestSchema = (data) => {
    ...
    if (data.format) {
        ...
        if (!validFormats.includes(typedData.format)) {
    ...
    if (data.compression) {
        ...
        if (!validCompressions.includes(typedData.compression)) {
...
export const notificationRequestSchema = (data) => {
    ...
    // Validate threshold
    if (typedData.threshold !== undefined &&
        (typedData.threshold < 0 || typedData.threshold > 100)) {
        errors.push('threshold must be between 0 and 100');
    }
    // Validate notification settings
    if (typedData.settings) {
        const notificationErrors = validateNotificationSettings(typedData.settings);
        errors.push(...notificationErrors);
    }
    const valid = errors.length === 0;
