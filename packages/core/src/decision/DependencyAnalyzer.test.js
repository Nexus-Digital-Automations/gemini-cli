...
describe('DependencyAnalyzer', () => {
    ...
    describe('Dependency Analysis', () => {
        ...
        test('should detect semantic dependencies between related tasks', async () => {
            ...
            const implementationTestDep = result.suggestedDependencies.find(dep => dep.dependsOnTaskId === 'task-1' && dep.dependentTaskId === 'task-2');
        ...
        test('should identify resource conflicts', async () => {
            ...
            const resourceDep = result.suggestedDependencies.find(dep => (dep.dependsOnTaskId === 'task-4' && dep.dependentTaskId === 'task-5') ||
        ...
        test('should analyze temporal dependencies', async () => {
            ...
            const temporalDep = result.suggestedDependencies.find(dep => dep.type === 'temporal');
            if (temporalDep) {
    ...
    describe('Conflict Detection', () => {
        test('should detect circular dependencies', async () => {
            ...
            const circularConflict = result.conflicts.find(conflict => conflict.type === 'circular');
            if (circularConflict) {
    ...
    describe('Optimization Recommendations', () => {
        ...
        test('should suggest parallel execution optimizations', async () => {
            const result = await analyzer.analyzeDependencies(testTasks, [], testContext);
            const parallelOpt = result.optimizations.find(opt => opt.type === 'parallel_execution');
            if (parallelOpt) {
        ...
        test('should recommend dependency removal for weak dependencies', async () => {
            ...
            const removalOpt = result.optimizations.find(opt => opt.type === 'dependency_removal');
            if (removalOpt) {
    ...
    describe('Configuration Impact', () => {
        test('should respect auto-create threshold', async () => {
            ...
            expect(result.suggestedDependencies.length).toBeLessThanOrEqual(testTasks.size);
        });
