...
export class TaskSequencer {
    ...
    hasDeadline(task) {
        ...
        return !!task.parameters?.deadline || !!task.metadata.custom?.deadline;
    }
    getDeadline(task) {
        // Extract deadline timestamp
        const deadline = task.parameters?.deadline || task.metadata.custom?.deadline;
    ...
    canGroupTogether(task, group, dependencyAnalysis) {
        ...
        for (const groupTask of group) {
            const hasDependency = dependencyAnalysis.suggestedDependencies.some(dep => (dep.dependentTaskId === task.id && dep.dependsOnTaskId === groupTask.id) ||
