...
export class RuleEngine {
     ...
     */
    async evaluate(type, input, context) {
     ...
     */
    async evaluateRule(rule, input, context) {
        ...
        const conditionResults = rule.conditions.map(condition => this.evaluateCondition(condition, evaluationContext));
        // Determine if rule matches based on condition requirements
        const requiredConditions = conditionResults.filter((result, index) => rule.conditions[index].required);
        const optionalConditions = conditionResults.filter((result, index) => !rule.conditions[index].required);
     ...
     */
    evaluateCondition(condition, evaluationContext) {
        try {
     ...
     */
    generateReasoning(rule, conditionResults, matched) {
     ...
     */
    updateRuleStats(ruleId, evaluationTime, matched) {
        ...
        stats.avgEvaluationTime = ((stats.avgEvaluationTime * (stats.evaluations - 1)) + evaluationTime) / stats.evaluations;
