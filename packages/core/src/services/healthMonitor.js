...
export class HealthMonitor extends EventEmitter {
    ...
    async performHealthCheckAll() {
        ...
        const healthChecks = await Promise.all(allAgents.map((agentInfo) => this.performHealthCheck(agentInfo.agent.id)));
    ...
    analyzeTrend(trend) {
        ...
        const correlation = Math.abs(slope) /
            (Math.sqrt(sumX2 / n - Math.pow(sumX / n, 2)) *
                Math.sqrt(points.reduce((sum, p) => sum + Math.pow(p.value, 2), 0) / n -
                    Math.pow(sumY / n, 2)));
    ...
    updateSLAMetrics(healthCheck) {
        ...
        const recentChecks = (this.healthChecks.get(agentId) || []).filter((check) => check.timestamp > periodStart);
        if (recentChecks.length > 0) {
            ...
            // Error rate
            sla.errorRate =
                availableChecks.reduce((sum, check) => sum + check.metrics.errorRate, 0) / availableChecks.length;
