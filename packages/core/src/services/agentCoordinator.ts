...
export class AgentCoordinator extends EventEmitter {
  ...
  private communicationChannels: Map<string, Set<string>> = new Map(); // agentId -> connected agents
  private loadBalanceStrategies: LoadBalanceStrategy[] = [];
  ...
  async rebalanceWorkload(): Promise<void> {
    ...
    if (overloadedAgents.length === 0 || underloadedAgents.length === 0) return;
  ...
  private initializeLoadBalanceStrategies(): void {
  ...
  private getPriorityBonus(priority: TaskPriority): number {
  ...
  private async checkAgentPerformance(agentId: string): Promise<void> {
    const metrics = this.agentMetrics.get(agentId);
    if (!metrics) return;
