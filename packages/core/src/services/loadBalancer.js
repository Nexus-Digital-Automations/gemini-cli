...
export class LoadBalancer extends EventEmitter {
    ...
    updateAgentPerformance(agentId, metrics) {
        ...
        this.context.performanceHistory.set(agentId, history.filter((entry) => entry.timestamp > cutoff));
    }
    /**
    ...
    getLoadBalancingStats() {
        ...
        for (const state of this.context.circuitBreakerStates.values()) {
            circuitBreakerStats[state.state.replace('-', '')]++;
        }
    ...
    calculateAgentScore(agent, task) {
        ...
        const matchedCaps = task.requiredCapabilities.filter((cap) => agent.agent.capabilities.includes(cap)).length;
        if (requiredCaps > 0) {
    ...
    calculatePerformanceScore(agent, task, context) {
        ...
        const avgSuccessRate = recentEntries.reduce((sum, entry) => sum + entry.successRate, 0) /
            recentEntries.length;
        const avgResponseTime = recentEntries.reduce((sum, entry) => sum + entry.responseTime, 0) /
            recentEntries.length;
    ...
    calculateWeightedScore(agent, task, context) {
        const weights = {
            performance: 0.4,
            load: 0.3,
            capability: 0.2,
    ...
    calculateConfidence(selected, allAgents, task) {
        ...
        const confidence = maxScore > avgScore
            ? (selectedScore - avgScore) / (maxScore - avgScore)
            : 0.5;
        return Math.max(0.1, Math.min(1.0, confidence));
    }
    generateReasonCode(agent, task, strategy) {
        const reasons = [];
    ...
    async recordAssignment(decision, task) {
        const assignment = {
    ...
    updateCircuitBreaker(agentId, success, timestamp) {
    ...
    analyzeLoadDistribution(agents) {
        return agents.map((agent) => {
            ...
            const recentAssignments = this.context.recentAssignments.filter((a) => a.agentId === agent.agent.id &&
