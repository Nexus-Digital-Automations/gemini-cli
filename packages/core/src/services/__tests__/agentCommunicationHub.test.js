...
describe('AgentCommunicationHub', () => {
    ...
    describe('Synchronization Barriers', () => {
        ...
        it('should create synchronization barrier', async () => {
            const barrier = await communicationHub.createSynchronizationBarrier('test-barrier', ['agent-1', 'agent-2', 'agent-3']);
        ...
        it('should wait for all agents to reach barrier', async () => {
            await communicationHub.createSynchronizationBarrier('sync-test', ['agent-1', 'agent-2']);
        ...
        it('should handle barrier timeout', async () => {
            await communicationHub.createSynchronizationBarrier('timeout-barrier', ['agent-1', 'agent-2'], 1000 // 1 second timeout
            );
        ...
        it('should release barrier when all agents arrive', async () => {
            await communicationHub.createSynchronizationBarrier('release-test', ['agent-1', 'agent-2']);
            const promises = [
        ...
        it('should handle non-existent barrier gracefully', async () => {
            await expect(communicationHub.waitForBarrier('agent-1', 'non-existent')).rejects.toThrow('Barrier not found');
        });
        it('should handle agent not in barrier participants', async () => {
            await communicationHub.createSynchronizationBarrier('exclusive-barrier', ['agent-1', 'agent-2']);
            await expect(communicationHub.waitForBarrier('agent-3', 'exclusive-barrier')).rejects.toThrow('not a participant');
        });
    });
    describe('Distributed Consensus', () => {
        ...
        it('should start consensus operation', async () => {
            const consensusId = await communicationHub.startConsensusOperation('leader-election', ['agent-1', 'agent-2', 'agent-3'], { candidates: ['agent-1', 'agent-2'] });
        ...
        it('should handle consensus voting', async () => {
            const consensusId = await communicationHub.startConsensusOperation('configuration-change', ['agent-1', 'agent-2', 'agent-3'], { newConfig: { maxLoad: 0.8 } });
            // Agents vote
        ...
        it('should determine consensus result based on majority', async () => {
            const consensusId = await communicationHub.startConsensusOperation('task-priority', ['agent-1', 'agent-2', 'agent-3'], { taskId: 'task-1', newPriority: 'high' });
            // Majority approval
        ...
        it('should handle consensus timeout', async () => {
            const consensusId = await communicationHub.startConsensusOperation('slow-consensus', ['agent-1', 'agent-2', 'agent-3'], { timeout: 500 }, // Short timeout
            500);
            // Only partial votes
            await communicationHub.submitConsensusVote(consensusId, 'agent-1', { vote: 'approve' });
            // Wait for timeout
        ...
        it('should handle unanimous consensus requirement', async () => {
            const consensusId = await communicationHub.startConsensusOperation('critical-change', ['agent-1', 'agent-2'], { requireUnanimous: true });
            await communicationHub.submitConsensusVote(consensusId, 'agent-1', { vote: 'approve' });
