...
class AuditStore {
    ...
    getStats() {
        ...
        const recentErrors = this.events.filter((e) => e.severity === EventSeverity.ERROR && e.timestamp >= oneHourAgo).length;
        return {
    ...
    res.json = function (data) {
        const configChangeEvent = {
            ...
            details: {
                ...
                previousConfig: data?.data && typeof data.data === 'object' ? data.data?.previousConfiguration : undefined,
            },
    ...
    return (req, res, next) => {
        ...
        res.json = function (data) {
            const exportImportEvent = {
                id: generateRequestId(),
                timestamp: new Date(),
                eventType: operationType === 'export'
...
function determineEventType(req, _res) {
