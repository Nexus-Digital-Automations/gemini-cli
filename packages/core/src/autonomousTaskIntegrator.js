...
export class AutonomousTaskIntegrator extends EventEmitter {
    ...
    dependencyGraph = new Map(); // taskId -> dependencies
    scheduler;
    config;
    ...
    async handleTaskCompletion(task, agent) {
        ...
        const event = {
            ...
            data: {
                task,
                completionTime: task.completedAt.getTime() - (task.startedAt?.getTime() || 0),
            },
        };
     ...
     */
    async handleApiCall(endpoint, params = []) {
    ...
    setupEventHandlers() {
        ...
        this.on('agent_registered', (event) => {
    ...
    async initializeTaskManagerConnection() {
    ...
    async createTodoBreakdown(task) {
...
//# sourceMappingURL=autonomousTaskIntegrator.js.map