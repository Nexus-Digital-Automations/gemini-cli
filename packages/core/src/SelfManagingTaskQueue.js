...
import { TaskQueue as _TaskQueue, TaskPriority, TaskCategory, } from './TaskQueue.js';
/**
...
export class SelfManagingTaskQueue extends EventEmitter {
    ...
    async addTask(taskDefinition) {
        ...
        try {
            ...
            const taskId = await this.baseQueue.addTask(this.convertToTaskDefinition(enhancedTaskDefinition));
     ...
     */
    async enhanceTaskWithPredictions(taskDefinition) {
        if (!this.mlModel) {
     ...
     */
    async checkAndReserveResources(taskDefinition) {
     ...
     */
    async queueForLaterExecution(taskDefinition, reason) {
     ...
     */
    async generateTaskPrediction(taskDefinition) {
        const taskId = taskDefinition.id || uuidv4();
     ...
     */
    async shutdown(timeoutMs = 120000) {
    ...
    async predictTaskComplexity(_taskDefinition) {
    ...
    async predictExecutionTime(taskDefinition) {
    ...
    async predictResourceUsage(taskDefinition) {
    ...
    async predictTaskFailureProbability(_taskDefinition) {
    ...
    calculateResourceAvailability(resources) {
    ...
    estimateResourceRequirement(taskDefinition, resourceType) {
    ...
    getCategoryResourceMultiplier(category, resourceType) {
        const multipliers = {
    ...
    calculateQueuePosition(taskDefinition) {
    ...
    async calculateDependencyDelay(taskDefinition) {
    ...
    calculatePredictionConfidence(taskDefinition) {
        let confidence = 0.5; // Base confidence
        // Increase confidence based on available data
        if (taskDefinition.estimatedDuration)
            confidence += 0.1;
        if (taskDefinition.category)
            confidence += 0.1;
        if (taskDefinition.requiredResources?.length)
            confidence += 0.1;
        // Historical data would increase confidence in real implementation
        confidence += 0.2;
        return Math.min(1.0, confidence);
    }
    identifyTaskRiskFactors(taskDefinition) {
        const riskFactors = [];
    ...
    calculateResourceRequirements(taskDefinition) {
    ...
    updateRealTimeMetrics(event, _data) {
    ...
    handleTaskCompletion(task, record, _result) {
    ...
    handleTaskFailure(task, _record, _error) {
        // Release reserved resources
        this.releaseTaskResources(task);
    ...
    updatePredictionAccuracy(prediction, actualDuration) {
    ...
    async performIntelligentOptimization(_strategy) {
    ...
    async performAdaptiveLoadBalancing() {
    ...
    generateHealthRecommendations() {
        ...
        if (this.realTimeMetrics.systemHealth === 'poor' ||
            this.realTimeMetrics.systemHealth === 'critical') {
...
//# sourceMappingURL=SelfManagingTaskQueue.js.map