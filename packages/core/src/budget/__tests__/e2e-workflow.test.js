...
import { BudgetEnforcement, BudgetExceededError, } from '../budget-enforcement.js';
...
describe('Budget Analytics System - End-to-End Workflow Tests', () => {
    ...
    describe('Complete Budget Lifecycle Workflow', () => {
        it('should handle complete budget setup and usage cycle', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(usageFilePath, expect.stringContaining('"requestCount":1'));
            ...
            expect(fs.writeFile).toHaveBeenLastCalledWith(usageFilePath, expect.stringContaining('"requestCount":0'));
        ...
        it('should handle budget extension workflow', async () => {
            ...
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(initialUsageData));
            const tracker = new BudgetTracker(projectRoot, budgetSettings);
            const enforcement = new BudgetEnforcement(projectRoot, budgetSettings);
            // 1. Verify near limit
        ...
        it('should handle daily reset time workflow', async () => {
            ...
            await tracker.recordRequest();
            expect(fs.writeFile).toHaveBeenCalledWith(usageFilePath, expect.stringContaining('"requestCount":76'));
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(usageFilePath, expect.stringContaining('"requestCount":0'));
            expect(fs.writeFile).toHaveBeenLastCalledWith(usageFilePath, expect.stringContaining('"requestCount":1'));
            vi.useRealTimers();
        });
    });
    describe('BudgetContentGenerator E2E Integration', () => {
        it('should handle complete API request lifecycle with budget enforcement', async () => {
            ...
            const result1 = await budgetContentGenerator.generateContent(mockRequest, 'prompt1');
            ...
            for (let i = 1; i <= 75; i++) {
                ...
                if (i === 50 && !usageData.warningsShown.includes(50)) {
                    ...
                    vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
                    const warnResult = await budgetContentGenerator.generateContent(mockRequest, `prompt${i}`);
                    ...
                    const normalResult = await budgetContentGenerator.generateContent(mockRequest, `prompt${i}`);
            ...
            for (let i = 76; i <= 100; i++) {
                ...
                if (i === 90 && !usageData.warningsShown.includes(90)) {
                    ...
                    vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
                    const finalWarnResult = await budgetContentGenerator.generateContent(mockRequest, `prompt${i}`);
                    expect(finalWarnResult).toEqual(mockResponse);
                    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('⚠️'));
                }
                else {
                    const nearLimitResult = await budgetContentGenerator.generateContent(mockRequest, `prompt${i}`);
            ...
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
            await expect(budgetContentGenerator.generateContent(mockRequest, 'over-budget')).rejects.toThrow(BudgetExceededError);
            // Verify underlying API was not called
            expect(mockContentGenerator.generateContent).not.toHaveBeenLastCalledWith(mockRequest, 'over-budget');
        });
        it('should handle streaming API requests with budget enforcement', async () => {
            ...
            const stream = await budgetContentGenerator.generateContentStream(streamRequest, 'stream-prompt');
            const chunks = [];
            ...
            usageData.requestCount = 101;
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
            await expect(budgetContentGenerator.generateContentStream(streamRequest, 'over-budget-stream')).rejects.toThrow(BudgetExceededError);
            expect(mockContentGenerator.generateContentStream).not.toHaveBeenLastCalledWith(streamRequest, 'over-budget-stream');
        });
    });
    describe('Multi-Component System Integration', () => {
        it('should coordinate between tracker, enforcement, and content generator', async () => {
            ...
            const result = await budgetContentGenerator.generateContent(request, 'test');
            expect(result).toEqual(mockResponse);
            // 3. Verify usage was recorded
            expect(fs.writeFile).toHaveBeenCalledWith(usageFilePath, expect.stringContaining('"requestCount":1'));
            // 4. Update settings and verify coordination
            const newSettings = { dailyLimit: 200 };
            ...
            await expect(budgetContentGenerator.generateContent(request, 'blocked')).rejects.toThrow(BudgetExceededError);
        });
    ...
    describe('Error Recovery and Resilience', () => {
        it('should handle and recover from file system errors', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('Failed to save budget usage data:', expect.any(Error));
        ...
        it('should maintain consistency during concurrent operations', async () => {
            ...
            const results = await Promise.allSettled(Array.from({ length: 20 }, (_, i) => operations[i % operations.length]()));
