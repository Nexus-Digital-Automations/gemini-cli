...
class PerformanceTestUtils {
    ...
    static async measureOperation(operationName, operation, threshold) {
    ...
    static async measureBatchOperations(operationName, operations, threshold) {
        const startTime = performance.now();
    ...
    static createLargeUsageData(requestCount) {
        return {
...
describe('Budget Analytics Performance and Load Tests', () => {
    ...
    describe('Single Operation Performance', () => {
        ...
        it('should read usage statistics within performance threshold', async () => {
            ...
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
            const { result, duration } = await PerformanceTestUtils.measureOperation('Usage Stats Read', () => tracker.getUsageStats(), PerformanceTestUtils.getThreshold('USAGE_STATS_READ'));
        ...
        it('should check budget status within performance threshold', async () => {
            ...
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
            const { result, duration } = await PerformanceTestUtils.measureOperation('Budget Check', () => tracker.isOverBudget(), PerformanceTestUtils.getThreshold('BUDGET_CHECK'));
        ...
        it('should check warning thresholds within performance threshold', async () => {
            ...
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
            const { result, duration } = await PerformanceTestUtils.measureOperation('Warning Check', () => tracker.shouldShowWarning(), PerformanceTestUtils.getThreshold('WARNING_CHECK'));
        ...
        it('should perform budget enforcement check within performance threshold', async () => {
            ...
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
            const { result, duration } = await PerformanceTestUtils.measureOperation('Enforcement Check', () => enforcement.checkRequestAllowed(), PerformanceTestUtils.getThreshold('ENFORCEMENT_CHECK'));
    ...
    describe('Batch Operation Performance', () => {
        it('should handle batch request recording within performance threshold', async () => {
            ...
            const batchOperations = Array.from({ length: 100 }, () => () => tracker.recordRequest());
            const { results, totalDuration, averageDuration } = await PerformanceTestUtils.measureBatchOperations('Batch Request Recording', batchOperations, PerformanceTestUtils.getThreshold('BATCH_REQUEST_RECORD'));
        ...
        it('should handle concurrent statistics reads efficiently', async () => {
            ...
            const batchOperations = Array.from({ length: 50 }, () => () => tracker.getUsageStats());
            const { results, totalDuration } = await PerformanceTestUtils.measureBatchOperations('Concurrent Stats Reads', batchOperations, 1000);
        ...
        it('should handle mixed operation workload efficiently', async () => {
            ...
            const { results, totalDuration } = await PerformanceTestUtils.measureBatchOperations('Mixed Operation Workload', mixedOperations, PerformanceTestUtils.getThreshold('CONCURRENT_OPERATIONS'));
    ...
    describe('Large Data Processing Performance', () => {
        it('should handle large usage data files efficiently', async () => {
            ...
            const { result, duration } = await PerformanceTestUtils.measureOperation('Large Data Processing', () => tracker.getUsageStats(), PerformanceTestUtils.getThreshold('LARGE_DATA_PROCESSING'));
        ...
        it('should handle extreme usage values without performance degradation', async () => {
            ...
            for (const operation of operations) {
                const { duration } = await PerformanceTestUtils.measureOperation('Extreme Value Processing', operation, 100);
        ...
        it('should maintain performance with complex warning threshold arrays', async () => {
            ...
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(usageData));
            const { result, duration } = await PerformanceTestUtils.measureOperation('Complex Threshold Processing', () => tracker.shouldShowWarning(), 50);
    ...
    describe('Memory and Resource Efficiency', () => {
        ...
        it('should handle content generator integration efficiently', async () => {
            const budgetContentGenerator = new BudgetContentGenerator(mockContentGenerator, mockConfig, budgetSettings);
            let requestCount = 0;
            vi.mocked(fs.readFile).mockImplementation(() => {
                const usageData = PerformanceTestUtils.createLargeUsageData(requestCount);
                requestCount++;
            ...
            const apiCallOperations = Array.from({ length: 100 }, (_, i) => () => budgetContentGenerator.generateContent(mockRequest, `prompt-${i}`));
            const { results, totalDuration, averageDuration } = await PerformanceTestUtils.measureBatchOperations('Integrated API Calls', apiCallOperations, 5000);
    ...
    describe('Stress Testing and Edge Cases', () => {
        it('should handle file system latency gracefully', async () => {
            ...
            vi.mocked(fs.readFile).mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(JSON.stringify(PerformanceTestUtils.createLargeUsageData(10))), 50)));
            vi.mocked(fs.writeFile).mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(), 30)));
            const operations = [
                async () => {
                    await tracker.recordRequest();
                },
            ...
            for (const operation of operations) {
                const { duration } = await PerformanceTestUtils.measureOperation('Slow FileSystem Operation', operation, 200);
        ...
        it('should maintain performance under high concurrency', async () => {
            ...
            const concurrentOperations = Array.from({ length: 200 }, (_, i) => {
                ...
                switch (opType) {
                    case 0:
                        return () => tracker.recordRequest().then(() => {
                            sharedRequestCount++;
                        });
                    case 1:
        ...
        it('should handle repeated reset operations efficiently', async () => {
            ...
            const resetOperations = Array.from({ length: 50 }, () => () => tracker.resetDailyUsage());
            const { results, totalDuration } = await PerformanceTestUtils.measureBatchOperations('Repeated Reset Operations', resetOperations, 2000);
        ...
        it('should handle settings update operations efficiently', async () => {
            ...
            const updateOperations = Array.from({ length: 100 }, (_, i) => async () => {
                const newLimit = 1000 + i * 10;
                tracker.updateSettings({ dailyLimit: newLimit });
                enforcement.updateSettings({ dailyLimit: newLimit });
            });
            const { totalDuration } = await PerformanceTestUtils.measureBatchOperations('Rapid Settings Updates', updateOperations, 1000);
            expect(totalDuration).toBeLessThan(1000);
            // Verify final state
    ...
    describe('Performance Regression Detection', () => {
        it('should establish performance baseline for future comparison', async () => {
            ...
            for (const [operationName, operation] of Object.entries(baselineOperations)) {
                const { duration } = await PerformanceTestUtils.measureOperation(operationName, operation);
        ...
        it('should measure end-to-end workflow performance', async () => {
            const tracker = new BudgetTracker(projectRoot, budgetSettings);
            const enforcement = new BudgetEnforcement(projectRoot, budgetSettings);
            const contentGenerator = new BudgetContentGenerator(mockContentGenerator, mockConfig, budgetSettings);
