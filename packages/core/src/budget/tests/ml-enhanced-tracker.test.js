...
describe('MLEnhancedBudgetTracker', () => {
    ...
    describe('Enhanced Usage Statistics', () => {
        ...
        test('should include trend analysis in ML predictions', async () => {
            ...
            for (let day = 0; day < 5; day++) {
                ...
                vi.spyOn(Date.prototype, 'toISOString').mockReturnValue(date.toISOString());
            ...
            if (stats.mlPredictions?.trendAnalysis) {
                ...
                expect(stats.mlPredictions.trendAnalysis.confidence).toBeGreaterThanOrEqual(0);
                expect(stats.mlPredictions.trendAnalysis.confidence).toBeLessThanOrEqual(1);
            }
        });
    });
    describe('Budget Forecasting', () => {
        test('should generate comprehensive budget forecast', async () => {
            ...
            for (const point of forecast.forecast) {
                ...
                expect(point.confidenceInterval.lower).toBeLessThanOrEqual(point.predictedValue);
                expect(point.confidenceInterval.upper).toBeGreaterThanOrEqual(point.predictedValue);
            ...
            expect(forecast.riskAssessment.budgetExceedProbability).toBeGreaterThanOrEqual(0);
            expect(forecast.riskAssessment.budgetExceedProbability).toBeLessThanOrEqual(1);
            expect(forecast.riskAssessment.mitigationStrategies.length).toBeGreaterThan(0);
        });
        test('should provide different forecast horizons', async () => {
            ...
            const avg6hConfidence = forecast6h.forecast.reduce((sum, p) => sum + p.confidence, 0) / 6;
            const avg48hConfidence = forecast48h.forecast.reduce((sum, p) => sum + p.confidence, 0) / 48;
    ...
    describe('Optimization Suggestions', () => {
        ...
        test('should provide trend-based recommendations', async () => {
            ...
            const rapidIncreaseRecs = suggestions.immediate.filter((rec) => rec.message.toLowerCase().includes('trend') ||
            ...
            const immediateOptimization = suggestions.immediate.find((rec) => rec.type === 'optimization' && rec.priority === 'high');
    ...
    describe('Anomaly Detection', () => {
        test('should detect usage anomalies and patterns', async () => {
            ...
            const highSeverityAnomalies = anomalies.anomalies.filter((a) => a.severity === 'high');
    ...
    describe('Model Performance Monitoring', () => {
        ...
        test('should classify model performance correctly', async () => {
            ...
            const excellentModels = metrics.models.filter((m) => m.performance === 'excellent');
            const goodOrBetterModels = metrics.models.filter((m) => ['excellent', 'good'].includes(m.performance));
    ...
    describe('Factory Function', () => {
        test('should create tracker with factory function', () => {
            const factoryTracker = createMLEnhancedBudgetTracker(tempDir, defaultSettings);
        ...
        test('should pass settings correctly through factory', async () => {
            ...
            const factoryTracker = createMLEnhancedBudgetTracker(tempDir, customSettings);
