...
describe('MLCostPredictor', () => {
  ...
  describe('Data Recording and Management', () => {
    ...
    test('should limit historical data to 365 days', async () => {
      ...
      // Add old data point
      const oldUsageData: BudgetUsageData = {
      ...
      const recentUsageData: BudgetUsageData = {
  ...
  describe('Budget Predictions', () => {
    ...
    test('should generate accurate predictions with sufficient data', async () => {
      ...
      expect(prediction.predictedWeeklyUsage).toBe(
        prediction.predictedDailyUsage * 7,
      );
      expect(prediction.predictedMonthlyUsage).toBe(
        prediction.predictedDailyUsage * 30,
      );
      expect(prediction.recommendations.length).toBeGreaterThan(0);
      expect(
        prediction.riskAssessment.budgetExceedProbability,
    ...
    test('should detect high risk scenarios', async () => {
      ...
      expect(prediction.riskAssessment.budgetExceedProbability).toBeGreaterThan(
        0.5,
      );
  ...
  describe('Trend Analysis', () => {
    ...
    test('should detect seasonal patterns', async () => {
      ...
      for (let day = 0; day < 3; day++) {
        for (let hour = 0; hour < 24; hour++) {
          ...
          const requestCount =
            baseUsage + businessHourBonus + Math.random() * 10;
    ...
    test('should identify anomalies', async () => {
      ...
      const highSeverityAnomalies = trends.anomalies.filter(
        (a) => a.severity === 'high',
      );
  ...
  describe('Model Performance and Metrics', () => {
    ...
    test('should calculate accurate performance metrics', async () => {
      ...
      // Force training
      const extraUsageData: BudgetUsageData = {
        date: new Date().toISOString().split('T')[0],
        requestCount: 90,
        lastResetTime: new Date().toISOString(),
        warningsShown: [],
  ...
  describe('Advanced ML Features', () => {
    ...
    test('should adapt predictions based on recent patterns', async () => {
      ...
      expect(prediction2.predictedDailyUsage).toBeGreaterThan(
        prediction1.predictedDailyUsage,
      );
    });
  });
});
