...
export class BudgetConfigManager {
   ...
   */
  private validateTimeFormat(field: string, value: string): void {
   ...
   */
  private validateWarningThresholds(field: string, value: number[]): void {
   ...
   */
  private validateCurrencyCode(field: string, value: string): void {
    if (typeof value !== 'string') {
      throw new BudgetConfigValidationError(
        `${field} must be a string`,
        field,
   ...
   */
  private validatePermission(requiredPermission: BudgetPermission): void {
