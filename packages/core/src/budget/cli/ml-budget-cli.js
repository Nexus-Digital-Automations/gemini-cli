...
export class MLBudgetCLI {
    ...
    async forecast(hours = 24, settings) {
        ...
        try {
            ...
            if (forecast.length > 0) {
                ...
                console.log(`📈 Total Predicted Usage: ${Math.round(totalPredicted)} requests`);
                console.log(`📉 Hourly Average: ${Math.round(totalPredicted / hours)} requests`);
                console.log(`🎯 Overall Confidence: ${(avgConfidence * 100).toFixed(1)}% (${confidence})`);
                console.log(`⚠️  Risk Level: ${riskAssessment.riskLevel.toUpperCase()}`);
                console.log(`🎲 Budget Exceed Probability: ${(riskAssessment.budgetExceedProbability * 100).toFixed(1)}%`);
                if (riskAssessment.timeToExceedBudget) {
                    console.log(`⏰ Time to Budget Exceed: ${riskAssessment.timeToExceedBudget.toFixed(1)} hours`);
                }
            }
    ...
    async optimize(settings) {
        console.log('🔧 Analyzing usage patterns for optimization opportunities...\n');
        try {
            ...
            if (!result.success || !result.data) {
                console.error('❌ Failed to get optimization suggestions:', result.error);
                return;
            }
            ...
            console.log('─'.repeat(50));
            console.log(`📊 Potential Savings: ${potentialSavings.percentage.toFixed(1)}% (${potentialSavings.estimatedRequests} requests)`);
            console.log(`🎯 Confidence: ${potentialSavings.confidence.toUpperCase()}`);
            // Display categorized recommendations
            this.displayRecommendationCategory('🚨 Immediate Actions', immediate);
            this.displayRecommendationCategory('📅 Short-term Improvements', shortTerm);
    ...
    async metrics(settings) {
        ...
        try {
            ...
            console.log('─'.repeat(50));
            console.log(`📊 Average Model Accuracy: ${(overallAccuracy * 100).toFixed(1)}%`);
            ...
            if (models.length > 0) {
                ...
                for (const model of models) {
                    ...
                    console.log(`   📅 Last Training: ${new Date(model.lastTraining).toLocaleString()}`);
                    console.log(`   📈 Training Data: ${model.trainingDataPoints} points`);
            ...
            console.log(`🔄 Consistency: ${(dataQuality.consistency * 100).toFixed(1)}%`);
    ...
    async stats(settings) {
        ...
        try {
            ...
            console.log('─'.repeat(50));
            console.log(`🔢 Requests Today: ${current.requestCount}/${current.dailyLimit}`);
            console.log(`📊 Usage Percentage: ${current.usagePercentage.toFixed(1)}%`);
            ...
            if (mlPredictions) {
                console.log('\n🔮 ML Predictions:');
                console.log('─'.repeat(50));
                console.log(`📊 Model Accuracy: ${(mlPredictions.modelAccuracy * 100).toFixed(1)}%`);
                if (mlPredictions.trendAnalysis) {
                    ...
                    console.log(`🔄 Seasonality: ${mlPredictions.trendAnalysis.seasonalityDetected ? 'Detected' : 'Not detected'}`);
                }
                ...
                if (mlPredictions.riskAssessment) {
                    ...
                    console.log(`🎲 Exceed Probability: ${(risk.budgetExceedProbability * 100).toFixed(1)}%`);
                    if (risk.timeToExceedBudget) {
                        console.log(`⏰ Time to Exceed: ${risk.timeToExceedBudget.toFixed(1)} hours`);
                    }
                }
    ...
    async health(settings) {
        ...
        try {
            ...
            if (result.success && result.details) {
                console.log(`✅ Tracker Initialized: ${result.details.trackerInitialized ? 'Yes' : 'No'}`);
                console.log(`📊 Data Available: ${result.details.dataAvailable ? 'Yes' : 'No'}`);
                console.log(`🤖 Models Trained: ${result.details.modelsTrained ? 'Yes' : 'No'}`);
