...
export class CostForecastingEngine {
     ...
     */
    static async generateProjections(historicalData, projectionDays = 30, confidenceLevel = 0.95) {
        ...
        try {
            ...
            const ensembleProjection = this.createEnsembleProjection(linearProjection, exponentialProjection, seasonalProjection, confidenceLevel);
            ...
            const projection = {
                metadata: {
                    algorithm: 'ensemble',
                    projectionPeriod: {
                        start: new Date(sortedData[sortedData.length - 1].timestamp.getTime() +
                            24 * 60 * 60 * 1000),
                        end: new Date(sortedData[sortedData.length - 1].timestamp.getTime() +
    ...
    static calculateBurnRateAnalysis(dataPoints, currentBudget) {
        ...
        try {
            ...
            const averageBurnRate = dailyBurnRates.reduce((sum, day) => sum + day.burnRate, 0) /
                dailyBurnRates.length;
            // Analyze burn rate trend
            const burnRateTrend = MathematicalAlgorithms.performTrendAnalysis(dailyBurnRates.map((day) => ({
                timestamp: day.date,
                cost: day.burnRate,
            ...
            const runway = {
                currentRateDays: currentBurnRate > 0
                    ? Math.floor(remainingBudget / currentBurnRate)
                    : Infinity,
                averageRateDays: averageBurnRate > 0
    ...
    static createEnsembleProjection(linearProjection, exponentialProjection, seasonalProjection, confidenceLevel) {
        ...
        for (let i = 0; i < linearProjection.length; i++) {
            ...
            const weightedCost = linearPoint.projectedCost * linearWeight +
                exponentialPoint.projectedCost * exponentialWeight +
                seasonalPoint.projectedCost * seasonalWeight;
            const weightedConfidence = linearPoint.confidence * linearWeight +
    ...
    static calculateConsistencyScore(data) {
        ...
        for (let i = 1; i < sortedData.length; i++) {
            const gap = (sortedData[i].timestamp.getTime() -
                sortedData[i - 1].timestamp.getTime()) /
                (60 * 60 * 1000); // Gap in hours
            gaps.push(gap);
        }
        const averageGap = gaps.reduce((sum, gap) => sum + gap, 0) / gaps.length;
        const gapVariance = gaps.reduce((sum, gap) => sum + Math.pow(gap - averageGap, 2), 0) /
            gaps.length;
        const gapStdDev = Math.sqrt(gapVariance);
        // Score based on gap consistency (lower variance = higher score)
        const consistencyScore = averageGap > 0 ? Math.max(0.3, 1.0 - gapStdDev / averageGap) : 1.0;
    ...
    static calculateCategoryBurnRates(data) {
        ...
        categoryData.forEach((points, category) => {
            ...
            const trend = points.length > 1 ? this.calculateSimpleTrend(points) : 'stable';
            breakdown.push({
                category,
                burnRate,
                percentage,
