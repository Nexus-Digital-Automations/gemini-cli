...
export class BudgetOptimizationEngine {
     ...
     */
    static async generateOptimizationPlan(costData, currentBudget, projection, burnRate, trend, variance, targetSavings) {
        ...
        try {
            ...
            const calculatedTargetSavings = targetSavings ||
                Math.max(currentBudget.remaining * 0.2, // 20% of remaining budget
                burnRate.currentBurnRate * 7);
            // Generate recommendations
            const recommendations = await this.generateDetailedRecommendations(costData, currentBudget, projection, burnRate, trend, variance, calculatedTargetSavings);
            // Identify optimization opportunities
            const opportunities = this.identifyOptimizationOpportunities(costData, burnRate, variance, calculatedTargetSavings);
            // Create budget allocation strategies
            const allocationStrategies = this.createAllocationStrategies(costData, currentBudget, burnRate, calculatedTargetSavings);
            // Calculate summary metrics
            const totalPotentialSavings = recommendations.reduce((sum, rec) => sum + rec.impact.estimatedSavings, 0) +
                opportunities.reduce((sum, opp) => sum + opp.potentialSavings.amount, 0);
            const implementationScore = this.calculateImplementationScore(recommendations, opportunities);
        ...
        try {
            ...
            if (burnRate.categoryBreakdown) {
                for (const category of burnRate.categoryBreakdown) {
                    ...
                    const categoryData = costData.filter((point) => point.context?.feature === category.category);
                    ...
                    const optimizationPotential = this.calculateOptimizationPotential(categoryData, category.trend);
                    const efficiencyScore = this.calculateEfficiencyScore(costPerUnit, utilizationRate, category.percentage);
                    efficiency.push({
                    ...
                    if (efficiencyScore < 60) {
                        recommendations.push(`${category.category} shows low efficiency (${efficiencyScore.toFixed(1)}%). Consider optimization strategies.`);
                    }
                    if (optimizationPotential > 30) {
                        recommendations.push(`${category.category} has high optimization potential (${optimizationPotential.toFixed(1)}%). Prioritize for cost reduction.`);
                    }
                }
            }
            // Calculate overall efficiency
            const overallEfficiency = efficiency.length > 0
                ? efficiency.reduce((sum, e) => sum +
                    e.efficiency *
                        (burnRate.categoryBreakdown?.find((c) => c.category === e.category)?.percentage || 0), 0) / 100
                : 50;
    ...
    static async generateDetailedRecommendations(costData, currentBudget, projection, burnRate, trend, variance, targetSavings) {
        const recommendations = [];
        let recId = 0;
        ...
        if (variance.summary.significantVariances > 0) {
            ...
            recommendations.push({
                ...
                priority: 'high',
                urgency: variance.summary.significantVariances > 5
                    ? 'this_week'
                    : 'this_month',
                supportingData: {
    ...
    static identifyOptimizationOpportunities(costData, burnRate, variance, targetSavings) {
        const opportunities = [];
        let oppId = 0;
    ...
    static createAllocationStrategies(costData, currentBudget, burnRate, targetSavings) {
        const strategies = [];
        ...
        strategies.push({
            ...
            expectedSavings: (balancedAllocations.reduce((sum, alloc) => sum +
                Math.max(0, alloc.currentAllocation - alloc.recommendedAllocation), 0) *
                currentBudget.total) /
                100,
        ...
        const performanceAllocations = burnRate.categoryBreakdown.map((category) => {
            const performanceScore = category.trend === 'decreasing'
                ? 1.2
                : category.trend === 'stable'
                    ? 1.0
                    : 0.8;
            const recommendedPercentage = Math.max(5, // Minimum 5%
    ...
    static calculateOptimizationPotential(categoryData, trend) {
        const basePotential = trend === 'increasing' ? 40 : trend === 'stable' ? 20 : 10;
    ...
    static calculateEfficiencyScore(costPerUnit, utilizationRate, categoryPercentage) {
        ...
        const efficiencyScore = normalizedUtilization * utilizationWeight +
