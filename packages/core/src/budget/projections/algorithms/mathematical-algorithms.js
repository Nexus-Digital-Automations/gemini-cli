...
export class MathematicalAlgorithms {
     ...
     */
    static performTrendAnalysis(dataPoints) {
        const startTime = Date.now();
        ...
        try {
            ...
            const standardError = Math.sqrt((sumYY - intercept * sumY - slope * sumXY) /
     ...
     */
    static calculateMovingAverage(dataPoints, config) {
        const startTime = Date.now();
        ...
        try {
            ...
            if (recentValues.length >= 2) {
                const trendSlope = this.calculateSlope(recentValues.map((v, i) => ({ x: i, y: v.value })));
     ...
     */
    static analyzeSeasonality(dataPoints) {
        const startTime = Date.now();
     ...
     */
    static detectVariances(dataPoints, algorithm = 'z_score') {
        const startTime = Date.now();
    ...
    static analyzeDailyPattern(dataPoints) {
        ...
        if (hourlyData.size < 12)
            return null; // Need reasonable coverage
        ...
        const peaks = [];
        const troughs = [];
        const averageValue = hourlyAverages.reduce((sum, item) => sum + item.average, 0) /
            hourlyAverages.length;
        hourlyAverages.forEach(({ hour, average }) => {
            if (average > averageValue * 1.2)
                peaks.push(hour);
            if (average < averageValue * 0.8)
                troughs.push(hour);
        });
    ...
    static analyzeWeeklyPattern(dataPoints) {
        ...
        const peaks = [];
        const troughs = [];
        const averageValue = dailyAverages.reduce((sum, item) => sum + item.average, 0) /
            dailyAverages.length;
        dailyAverages.forEach(({ day, average }) => {
            if (average > averageValue * 1.15)
                peaks.push(day);
            if (average < averageValue * 0.85)
                troughs.push(day);
        });
        // Calculate pattern strength
    ...
    static analyzeMonthlyPattern(dataPoints) {
    ...
    static removeSeasonality(dataPoints, patterns) {
    ...
    static calculateSeasonalAdjustment(timestamp, pattern) {
    ...
    static detectVariancesZScore(dataPoints, statistics) {
    ...
    static detectVariancesIQR(dataPoints) {
        const costs = dataPoints.map((point) => point.cost).sort((a, b) => a - b);
        const n = costs.length;
        // Calculate quartiles
    ...
    static detectVariancesMovingAverageDeviation(dataPoints) {
        ...
        for (let i = windowSize; i < sortedPoints.length; i++) {
            ...
            const windowStdDev = Math.sqrt(window.reduce((sum, point) => sum + Math.pow(point.cost - windowAverage, 2), 0) / windowSize);
    ...
    static detectVariancesIsolationForest(dataPoints) {
        // Simplified isolation forest implementation
        ...
        dataPoints.forEach((point) => {
            ...
            if (anomalyScore > 2.5) {
                ...
                variances.push({
                    ...
                    deviation,
                    deviationPercentage,
                    varianceType: Math.abs(deviation) > statistics.standardDeviation
                        ? 'outlier'
                        : 'drift',
...
//# sourceMappingURL=mathematical-algorithms.js.map