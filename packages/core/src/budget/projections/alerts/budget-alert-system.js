...
export class BudgetAlertSystem {
    ...
    static alertSuppressions = new Map();
    static activeAlerts = new Map();
    /**
     * Monitor cost data and trigger alerts based on configured thresholds
     */
    static async monitorAndAlert(costData, alertConfigs, currentBudget) {
        ...
        try {
            ...
            for (const config of alertConfigs) {
                const alert = await this.evaluateAlertCondition(config, costData, currentBudget, recentTrend, varianceDetection, costProjection);
     ...
     */
    static async evaluateAlertCondition(config, costData, currentBudget, recentTrend, varianceDetection, costProjection) {
        const startTime = Date.now();
        try {
            let currentValue = 0;
            ...
            // Generate alert
            const alert = this.createAlert(config, currentValue, currentBudget, recentTrend, varianceDetection, costProjection);
...
createAlert(config, BudgetAlertConfig, currentValue, number, currentBudget, { total: number, used: number, remaining: number }, recentTrend, TrendAnalysis, varianceDetection, VarianceDetection, costProjection, CostProjection);
BudgetAlert;
{
    ...
    const projectedImpact = this.calculateProjectedImpact(currentBudget, costProjection, config.threshold.type);
    // Generate recommendations
    const recommendations = this.generateRecommendations(config, currentValue, currentBudget, recentTrend, varianceDetection, costProjection);
    return {
        id: alertId,
...
generateAlertMessage(config, BudgetAlertConfig, currentValue, number, thresholdPercentage, number);
{
    title: string;
    message: string;
}
...
calculateProjectedImpact(currentBudget, { total: number, used: number, remaining: number }, costProjection, CostProjection, thresholdType, string);
{
...
generateRecommendations(config, BudgetAlertConfig, currentValue, number, currentBudget, { total: number, used: number, remaining: number }, recentTrend, TrendAnalysis, varianceDetection, VarianceDetection, costProjection, CostProjection);
...
shouldTriggerAlert(config, BudgetAlertConfig, alert, BudgetAlert);
boolean;
{
    ...
    const hoursElapsed = (now.getTime() - suppression.hourWindowStart.getTime()) /
        (60 * 60 * 1000);
    if (hoursElapsed >= 1) {
        // Reset hourly window
...
updateAlertSuppression(configId, string);
void {
    const: now = new Date(),
...
calculateCurrentBurnRate(costData, CostDataPoint[]);
number;
{
    ...
    if (recentData.length === 0) {
        ...
        const timeSpanDays = (sortedData[sortedData.length - 1].timestamp.getTime() -
    ...
    const timeSpanHours = (recentData[recentData.length - 1].timestamp.getTime() -
...
//# sourceMappingURL=budget-alert-system.js.map