...
export class AlgorithmValidator {
     ...
     */
    async validateProjectionAccuracy(algorithm, testData, actualOutcomes) {
        const startTime = Date.now();
     ...
     */
    async validatePerformance(algorithm, testData) {
        ...
        try {
            ...
            const cpuUsagePercent = ((endCpuUsage.user + endCpuUsage.system) / 1000 / executionTimeMs) *
                100;
     ...
     */
    async validateRobustness(algorithm, baseData) {
     ...
     */
    async runValidationSuite(algorithm, testCategories = [
        'accuracy',
        'performance',
        'robustness',
        'edge_case',
        ...
        try {
            ...
            const selectedTests = Array.from(this.testCases.values()).filter((test) => testCategories.includes(test.category));
     ...
     */
    async executeTestCase(algorithm, testCase) {
        const startTime = Date.now();
        try {
            // Calculate data quality score
            const dataQualityScore = this.calculateDataQuality(testCase.inputData);
            // Execute algorithm and collect metrics
            const accuracyMetrics = await this.validateProjectionAccuracy(algorithm, testCase.inputData, Array(testCase.inputData.length)
                .fill(0)
                .map((_, i) => i * 10));
     ...
     */
    async generateValidationReport(results, testsPassed, testsFailed) {
    ...
    generateLinearTrendData(count, base, slope) {
        const data = [];
    ...
    generateSeasonalData(count, base, amplitude) {
        const data = [];
        const startDate = new Date();
        for (let i = 0; i < count; i++) {
            const date = new Date(startDate.getTime() + i * 24 * 60 * 60 * 1000);
    ...
    generateDataWithOutliers(count, outlierCount) {
    ...
    addNoiseToData(data, noiseLevel) {
        return data.map((point) => ({
            ...point,
            cost: point.cost * (1 + (Math.random() - 0.5) * noiseLevel),
        }));
    }
    addOutliersToData(data, outlierCount) {
        const result = [...data];
        for (let i = 0; i < outlierCount; i++) {
    ...
    calculateMAPE(actual, predicted) {
        ...
        const ape = actual.map((a, i) => a !== 0 ? Math.abs((a - predicted[i]) / a) * 100 : 0);
    ...
    calculateRMSE(actual, predicted) {
        ...
        const mse = actual.reduce((sum, a, i) => sum + Math.pow(a - predicted[i], 2), 0) /
            actual.length;
    ...
    calculateMAE(actual, predicted) {
        ...
        return (actual.reduce((sum, a, i) => sum + Math.abs(a - predicted[i]), 0) /
            actual.length);
    }
    calculateMSE(actual, predicted) {
        ...
        return (actual.reduce((sum, a, i) => sum + Math.pow(a - predicted[i], 2), 0) /
            actual.length);
    }
    ...
    calculateSMAPE(actual, predicted) {
        if (actual.length !== predicted.length || actual.length === 0) {
            return 100;
        }
    ...
    calculateMedianAPE(actual, predicted) {
        if (actual.length !== predicted.length || actual.length === 0) {
            return 100;
        }
        const ape = actual
    ...
    async testEdgeCaseHandling(algorithm) {
        let score = 0;
        const testCases = [
            [], // Empty data
    ...
    async testPatternConsistency(algorithm, baseData) {
        try {
    ...
    calculateDataQuality(data) {
        ...
        const hasValidCosts = data.every((point) => typeof point.cost === 'number' &&
            !isNaN(point.cost) &&
            isFinite(point.cost));
        if (!hasValidCosts)
            score -= 0.3;
        ...
        const stdDev = Math.sqrt(costs.reduce((sum, cost) => sum + Math.pow(cost - mean, 2), 0) /
            costs.length);
    ...
    evaluateTestSuccess(testCase, metrics) {
        switch (testCase.category) {
            case 'accuracy':
                return metrics.accuracy.mape < 20 && metrics.accuracy.rSquared > 0.7;
            case 'performance':
                return (metrics.performance.executionTimeMs < 5000 &&
                    metrics.performance.complexityScore <= 3);
            case 'robustness':
                return (metrics.robustness.noiseResistance > 0.7 &&
    ...
    extractAlgorithmParameters(algorithm) {
    ...
    calculateQualityAssessment(results) {
        if (results.length === 0) {
            return {
                overallScore: 0,
                categoryScores: {
        ...
        const overallScore = (accuracyScore + performanceScore + robustnessScore + reliabilityScore) /
            4;
        // Generate recommendations
        const recommendations = [];
        if (accuracyScore < 80)
            recommendations.push('Improve prediction accuracy by enhancing algorithm parameters');
        if (performanceScore < 70)
            recommendations.push('Optimize algorithm performance for large datasets');
        if (robustnessScore < 75)
            recommendations.push('Enhance robustness to handle noisy data and outliers');
...
//# sourceMappingURL=algorithm-validator.js.map