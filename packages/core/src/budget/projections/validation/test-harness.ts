...
export class TestHarness {
  ...
  generateTestData(configs: TestDataConfig[]): Map<string, CostDataPoint[]> {
    ...
    const testDataSets = new Map<string, CostDataPoint[]>();
  ...
  private async runTestSuite(
    ...
    if (config.enablePerformanceBenchmarking) {
      const testDataConfigs: TestDataConfig[] = [
  ...
  private generateDataByType(config: TestDataConfig): CostDataPoint[] {
  ...
  private createCacheKey(config: TestDataConfig): string {
  ...
  private extractIssues(result: TestExecutionResult): string[] {
    const issues: string[] = [];
