...
export interface TrendAnalysis {
  ...
  seasonalPattern: {
    weeklyPattern: number[]; // Usage pattern by day of week (0-6)
    monthlyPattern: number[]; // Usage pattern by month (1-12)
    quarterlyPattern: number[]; // Usage pattern by quarter (1-4)
  };
  averageDailyUsage: number;
  projectedUsage: number;
  volatility: number; // Standard deviation of usage
  cyclicality: {
    detected: boolean;
...
export interface BudgetForecast {
  ...
  riskAssessment: {
    overageRisk: number; // 0-1, probability of exceeding budget
...
export interface AnomalyDetection {
  ...
  anomalyScore: number; // Higher means more anomalous
...
export class HistoricalBudgetAnalyzer {
  ...
  private readonly historicalDataPath: string;
  ...
  async recordDailyUsage(
    ...
    const record: HistoricalBudgetRecord = {
      date,
      dailyUsage: usage,
      dailyLimit: limit,
   ...
   */
  async analyzeTrends(lookbackDays: number = 30): Promise<TrendAnalysis> {
    const historicalRecords = await this.loadHistoricalData();
   ...
   */
  async getHistoricalInsights(lookbackDays: number = 90): Promise<{
    summary: {
      totalDays: number;
      averageUsage: number;
      peakUsage: number;
      ...
      efficiency: number; // Average usage as percentage of limit
    };
    trends: TrendAnalysis;
    recentAnomalies: AnomalyDetection[];
    recommendations: string[];
    dataQuality: {
      completeness: number; // Percentage of days with data
      consistency: number; // Measure of data consistency
      reliability: number; // Overall data reliability score
    };
  }> {
    const historicalRecords = await this.loadHistoricalData();
    const cutoffDate = new Date();
  ...
  private async loadHistoricalData(): Promise<HistoricalBudgetRecord[]> {
    try {
  ...
  private calculateSeasonalPatterns(records: HistoricalBudgetRecord[]): {
  ...
  private detectCyclicality(records: HistoricalBudgetRecord[]): {
    detected: boolean;
    period?: number;
    amplitude?: number;
  } {
  ...
  private calculateDataConsistency(records: HistoricalBudgetRecord[]): number {
 ...
 */
export function createHistoricalBudgetAnalyzer(
  projectRoot: string,
  options?: { maxHistoryDays?: number; anomalyThreshold?: number },
