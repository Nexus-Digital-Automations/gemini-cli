...
export function requirePermissions(requiredPermissions) {
    return (req, res, next) => {
        ...
        try {
            ...
            const hasAllPermissions = requiredPermissions.every((requiredPerm) => req.user.permissions.includes(requiredPerm.toString()) ||
                req.user.roles.includes('admin'));
            if (!hasAllPermissions) {
                const missingPermissions = requiredPermissions.filter((requiredPerm) => !req.user.permissions.includes(requiredPerm.toString()) &&
                    !req.user.roles.includes('admin'));
 ...
 */
function extractAuthenticationCredentials(req) {
 ...
 */
function validateCredentials(credentials, type) {
    switch (type) {
        case 'bearer':
 ...
 */
export function logAuthEvents(req, res, next) {
