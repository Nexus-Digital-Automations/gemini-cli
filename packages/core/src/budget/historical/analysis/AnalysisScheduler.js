 ...
 */
export class AnalysisSchedulerImpl {
    logger;
    trendEngine;
    dataQueryBuilder;
    // Job management
     ...
     */
    constructor(trendEngine, dataQueryBuilder, options = {}) {
    ...
    async executeJob(context) {
        ...
        // Fetch data
        const queryBuilder = this.dataQueryBuilder().timeRange(job.dataQuery.startTime, job.dataQuery.endTime);
     ...
     */
    async executeTrendAnalysis(context, data) {
     ...
     */
    async executeSeasonalAnalysis(context, data) {
        this.updateProgress(context, 30);
        this.checkCancellation(context);
        ...
        const trends = seasonalPatterns.map((pattern) => ({
            metric: 'cost',
            period: pattern.patternType,
     ...
     */
    async executeAnomalyAnalysis(context, data) {
        this.updateProgress(context, 30);
        this.checkCancellation(context);
        const result = await this.trendEngine.detectAnomalies(data, context.job.config.anomalySensitivity);
        this.updateProgress(context, 90);
        return result;
    }
    /**
     ...
     */
    async executeEfficiencyAnalysis(context, data) {
        this.updateProgress(context, 30);
        this.checkCancellation(context);
     ...
     */
    async executeForecastAnalysis(context, data) {
        this.updateProgress(context, 30);
        this.checkCancellation(context);
        const forecasts = await this.trendEngine.forecast(data, 'cost', context.job.config.forecastHorizon || 30);
        this.updateProgress(context, 90);
        // Convert to trend analysis format
        const trend = {
            metric: 'cost',
     ...
     */
    async executeInsightsAnalysis(context, data) {
        this.updateProgress(context, 30);
        this.checkCancellation(context);
     ...
     */
    calculateEstimatedCompletion(context) {
     ...
     */
    async deletePersistedJob(jobId) {
 ...
 */
export function createAnalysisScheduler(trendEngine, dataQueryBuilder, options) {
