 ...
 */
class AdvancedQueryBuilder implements QueryBuilder {
  private filters: QueryFilter[] = [];
  private sorts: SortSpec[] = [];
  private groups: GroupSpec[] = [];
  private limitCount?: number;
  private skipCount?: number;
  private selectedFields?: string[];
  private range?: QueryRange;

  constructor(private engine: AdvancedQueryEngine) {}
  ...
  async execute(): Promise<BudgetUsageTimeSeriesPoint[]> {
  ...
  async aggregate(window: AggregationWindow): Promise<AggregationResult[]> {
 ...
 */
export class AdvancedQueryEngine implements QueryEngine {
  ...
  private async loadIndexes(): Promise<void> {
    try {
      ...
      for (const [field, spec] of Object.entries(indexes)) {
        this.indexes.set(field, spec as IndexSpec);
      }
  ...
  async query(
    ...
    // Build query range
    const queryRange: QueryRange = options.timeRange || {
      start: 0,
      end: Date.now(),
    };
  ...
  async aggregateQuery(
    ...
    return {
      data: grouped as AggregationResult[], // Type conversion for compatibility
      stats: {
        ...rawResult.stats,
        executionTime: Date.now() - startTime,
      },
  ...
  private applyFieldSelection(data: BudgetUsageTimeSeriesPoint[], fields: string[]): BudgetUsageTimeSeriesPoint[] {
    return data.map(dataPoint => {
      ...
      return selected as BudgetUsageTimeSeriesPoint;
    });
  }
   ...
   */
  private setFieldValue(obj: any, fieldPath: string, value: unknown): void {
    const path = fieldPath.split('.');
    let current = obj;
   ...
   */
  private generateCacheKey(filters: QueryFilter[], options: any): string {
    const keyObject = {
   ...
   */
  private calculateIndexSeeks(filters: QueryFilter[]): number {
    let seeks = 0;
   ...
   */
  private trackQueryExecution(queryKey: string, stats: QueryStats): void {
   ...
   * List all indexes
   */
  async listIndexes(): Promise<IndexSpec[]> {
  ...
  async optimize(): Promise<{
    ...
    const suggestedIndexes: IndexSpec[] = [];
   ...
   */
  async configureCache(config: QueryCacheConfig): Promise<void> {
