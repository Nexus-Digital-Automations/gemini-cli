 ...
 */
export type QueryOperator = 'eq' | 'ne' | 'gt' | 'gte' | 'lt' | 'lte' | 'in' | 'nin' | 'exists' | 'regex' | 'between';
/**
...
export interface QueryEngine {
     ...
     */
    query(filters: QueryFilter[], options?: {
        sort?: SortSpec[];
        limit?: number;
        skip?: number;
        select?: string[];
     ...
     */
    aggregateQuery(filters: QueryFilter[], groupBy: GroupSpec[], options?: {
        timeRange?: QueryRange;
...
export interface TemporalQuery {
     ...
     */
    withinTimeWindow(center: number, windowSize: number, unit: 'minutes' | 'hours' | 'days'): QueryBuilder;
    /**
     * Find data points with specific temporal patterns
     */
    withPattern(pattern: 'weekday' | 'weekend' | 'business_hours' | 'peak_hours', timezone?: string): QueryBuilder;
    /**
     * Find seasonal data (same time of year across multiple years)
     */
    seasonal(referenceDate: number, windowDays: number, years?: number[]): QueryBuilder;
    /**
     * Find data points relative to events or anomalies
     */
    relativeToEvent(eventTimestamp: number, beforeMinutes: number, afterMinutes: number): QueryBuilder;
}
/**
