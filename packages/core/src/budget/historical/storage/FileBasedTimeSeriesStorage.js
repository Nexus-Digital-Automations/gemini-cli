...
export class FileBasedTimeSeriesStorage {
    ...
    getBucketStartTime(bucketKey) {
        const date = new Date(bucketKey.split('-')[0] +
            '-' +
    ...
    getGranularityForBucket(timestamp) {
        ...
        if (daysOld < 7)
            return 'minute'; // Recent data: minute granularity
        if (daysOld < 30)
            return 'hour'; // Week-old data: hour granularity
    ...
    async query(range) {
        ...
        const relevantBuckets = Array.from(this.bucketIndex.values()).filter((bucket) => bucket.timeRange.start <= range.end &&
        ...
        for (const bucket of relevantBuckets) {
            ...
            const filteredData = bucketData.filter((dp) => dp.timestamp >= range.start && dp.timestamp <= range.end);
    ...
    async getStats() {
        ...
        for (const bucket of buckets) {
            try {
                const filePath = bucket.compressionLevel > 0
                    ? bucket.filePath + '.gz'
                    : bucket.filePath;
        ...
        const totalDays = totalDataPoints > 0
    ...
    async compact() {
        ...
        try {
            ...
            const oldBuckets = Array.from(this.bucketIndex.values()).filter((bucket) => bucket.compressionLevel === 0 &&
    ...
    async purgeOldData(olderThan) {
        ...
        try {
            await this.initialize();
            let recordsAffected = 0;
            const bucketsToDelete = Array.from(this.bucketIndex.entries()).filter(([key, bucket]) => bucket.timeRange.end < olderThan);
