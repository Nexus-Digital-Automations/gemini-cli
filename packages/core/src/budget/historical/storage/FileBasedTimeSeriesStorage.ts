 ...
 */
export class FileBasedTimeSeriesStorage implements TimeSeriesStorage {
  private readonly config: StorageConfig;
  private readonly indexPath: string;
  private bucketIndex: Map<string, StorageBucket> = new Map();
   ...
   */
  private async loadBucketIndex(): Promise<void> {
    try {
   ...
   */
  private async saveBucketIndex(): Promise<void> {
    const indexData = {
   ...
   */
  private async getOrCreateBucket(timestamp: number): Promise<StorageBucket> {
   ...
   */
  private getBucketEndTime(bucketKey: string): number {
   ...
   */
  private shouldCompressBucket(timestamp: number): boolean {
  ...
  async storeBatch(
    ...
    try {
      ...
      const bucketGroups = new Map<string, BudgetUsageTimeSeriesPoint[]>();
   ...
   */
  async query(range: QueryRange): Promise<BudgetUsageTimeSeriesPoint[]> {
    await this.initialize();
   ...
   */
  async purgeOldData(olderThan: number): Promise<StorageOperationResult> {
    const startTime = Date.now();
   ...
   */
  async backup(backupPath: string): Promise<StorageOperationResult> {
    const startTime = Date.now();
   ...
   */
  async restore(backupPath: string): Promise<StorageOperationResult> {
    const startTime = Date.now();
