...
export interface RetentionScheduler {
     ...
     */
    schedule(cronExpression: string, ruleIds: string[], options?: {
        timezone?: string;
        maxRetries?: number;
        retryDelay?: number;
    }): Promise<string>;
...
export interface LegalHoldManager {
     ...
     */
    placeLegalHold(holdId: string, criteria: RetentionCondition[], reason: string, expirationDate?: number): Promise<void>;
    /**
     * Release legal hold
     */
...
export interface ComplianceReporter {
     ...
     */
    generateComplianceReport(startDate: number, endDate: number, format: 'json' | 'csv' | 'pdf'): Promise<{
        reportId: string;
        format: string;
        generatedAt: number;
        filePath?: string;
