...
/**
 * Archive metadata for efficient indexing
 */
...
interface ArchiveIndex {
  ...
  totalBlocks: number;
  totalSize: number;
  blocks: Record<string, ArchiveMetadata>;
  timeIndex: Record<string, string[]>; // timestamp ranges to block IDs
  algorithmIndex: Record<CompressionAlgorithm, string[]>;
}
...
export class ArchiveStorageImpl implements ArchiveStorage {
   ...
   */
  async storeBlock(compressedBlock: CompressedBlock): Promise<string> {
   ...
   */
  async retrieveBlock(blockId: string): Promise<CompressedBlock | null> {
    const startTime = Date.now();
    ...
    try {
      ...
      if (filter?.startTime || filter?.endTime) {
        blockIds = blockIds.filter(blockId => {
          ...
          if (filter.startTime && blockEnd < filter.startTime) return false;
          if (filter.endTime && blockStart > filter.endTime) return false;
   ...
   */
  private updateIndex(archiveMetadata: ArchiveMetadata): void {
   ...
   */
  private removeFromIndex(blockId: string): void {
    const archiveMetadata = this.index.blocks[blockId];
   ...
   */
  private getTimeIndexKey(timestamp: number): string {
   ...
   */
  private generateFileName(compressedBlock: CompressedBlock): string {
