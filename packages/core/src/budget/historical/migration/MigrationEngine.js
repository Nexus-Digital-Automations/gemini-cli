 ...
 */
export class MigrationEngineImpl {
    logger;
    dataSource;
    migrationsPath;
    registryLocation;
     ...
     * Execute migration
     */
    async executeMigration(migrationId, direction, options) {
        const startTime = Date.now();
     ...
     */
    async executeMigrationPath(migrations, direction, options) {
        const startTime = Date.now();
    ...
    async rollbackMigration(migrationId, options) {
        ...
        try {
            ...
            const appliedMigration = this.registry.appliedMigrations.find(m => m.migrationId === migrationId);
            if (!appliedMigration) {
     ...
     */
    async validateData(data, version) {
    ...
    async loadVersionInfo() {
        try {
            ...
            try {
                ...
                const versions = JSON.parse(versionContent);
     ...
     */
    async createExecutionContext(migration, direction, options) {
     ...
     */
    async validatePreconditions(context) {
        const { migration, direction } = context;
        // Check if migration is applicable
        if (direction === 'up') {
            if (migration.fromVersion !== this.registry.currentVersion) {
                throw new Error(`Migration ${migration.id} cannot be applied: current version is ${this.registry.currentVersion}, expected ${migration.fromVersion}`);
            }
        }
        else {
            if (migration.toVersion !== this.registry.currentVersion) {
                throw new Error(`Migration ${migration.id} cannot be rolled back: current version is ${this.registry.currentVersion}, expected ${migration.toVersion}`);
            }
        }
     ...
     */
    async performMigration(context) {
        const { migration, direction } = context;
        const startTime = Date.now();
        const result = {
            migrationId: migration.id,
     ...
     */
    async updateRegistry(migration, direction) {
        if (direction === 'up') {
            ...
            this.registry.appliedMigrations = this.registry.appliedMigrations.filter(m => m.migrationId !== migration.id);
            // Update current version
