...
export type ValidationRuleType =
  | 'required'        // Field must be present and not null/empty
  | 'type'           // Field must be of specific type
  | 'range'          // Numeric field must be within range
  | 'format'         // String field must match pattern/format
  | 'enum'           // Field value must be one of allowed values
  | 'unique'         // Field value must be unique across dataset
  | 'reference'      // Field must reference existing entity
  | 'temporal'       // Time-based validation rules
...
export interface ValidationResult {
  ...
  qualityMetrics: {
    completeness: number;     // % of required fields that are present
    accuracy: number;         // % of data that passes business rules
    consistency: number;      // % of data that is internally consistent
    validity: number;         // % of data that conforms to format rules
    uniqueness: number;       // % of data that satisfies uniqueness constraints
    timeliness: number;       // % of data that is up-to-date
  };
