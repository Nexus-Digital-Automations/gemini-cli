...
export interface EnsembleForecast extends BudgetForecast {
  ...
  monteCarloResults: {
    ...
    confidenceIntervals: {
      [confidence: string]: { lower: number; upper: number };
    };
  };
  scenarioResults: {
    [scenarioName: string]: {
...
export class BudgetForecastingEngine {
  private readonly config: ForecastingModelConfig;
  ...
  async generateEnsembleForecast(
    ...
    if (historicalRecords.length < 14) {
      throw new Error(
        'Insufficient data for advanced forecasting (minimum 14 days required)',
      );
    }

    const targetDateObj = new Date(targetDate);
    const lastRecordDate = new Date(
      Math.max(...historicalRecords.map((r) => new Date(r.date).getTime())),
    );
    const forecastHorizon = Math.ceil(
      (targetDateObj.getTime() - lastRecordDate.getTime()) /
        (1000 * 60 * 60 * 24),
    );
    ...
    const modelForecasts = await this.generateMultiModelForecasts(
      historicalRecords,
      forecastHorizon,
    );
    ...
    const scenarioResults = await this.generateScenarioForecasts(
      historicalRecords,
      ensembleForecast,
      scenarios,
      forecastHorizon,
   ...
   */
  private async generateMultiModelForecasts(
    ...
    forecasts.set(
      'exponential',
      this.exponentialSmoothingForecast(records, horizon),
    );

    // Seasonal decomposition model
    forecasts.set(
      'seasonal',
      this.seasonalDecompositionForecast(records, horizon),
    );
    ...
    forecasts.set(
      'seasonal_naive',
      this.seasonalNaiveForecast(records, horizon),
    );
  ...
  private seasonalDecompositionForecast(
    ...
    return Array.from({ length: horizon }, (_, i) =>
      Math.max(0, trendForecast[i] + seasonalForecast[i]),
    );
  }
   ...
   */
  private combineModelForecasts(modelForecasts: Map<string, number[]>): number {
  ...
  private async runMonteCarloSimulation(
  ...
  ): Promise<EnsembleForecast['monteCarloResults']> {
    const values = records.map((r) => r.dailyUsage);
    const historicalMean =
      values.reduce((sum, val) => sum + val, 0) / values.length;
    const historicalStd = Math.sqrt(
      values.reduce((sum, val) => sum + Math.pow(val - historicalMean, 2), 0) /
        values.length,
    );
    ...
    for (let i = 0; i < config.iterations; i++) {
      ...
      const volatilityNoise = this.generateRandomNormal(
        0,
        historicalStd * config.varianceScaling,
      );
    ...
    const mean =
      simulations.reduce((sum, val) => sum + val, 0) / simulations.length;
    const median = simulations[Math.floor(simulations.length / 2)];
    const standardDeviation = Math.sqrt(
      simulations.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) /
        simulations.length,
    );
  ...
  private async generateScenarioForecasts(
    ...
    scenarios: ScenarioConfig[],
    horizon: number,
  ): Promise<EnsembleForecast['scenarioResults']> {
    const results: EnsembleForecast['scenarioResults'] = {};

    for (const scenario of scenarios) {
      let adjustedForecast =
        baseForecast * scenario.adjustments.usageMultiplier;
      ...
      let impact: 'low' | 'medium' | 'high' | 'extreme';

      if (impactRatio < 1.2) impact = 'low';
      else if (impactRatio < 1.5) impact = 'medium';
      else if (impactRatio < 2.0) impact = 'high';
      else impact = 'extreme';
  ...
  private async validateModelPerformance(
    records: HistoricalBudgetRecord[],
    modelForecasts: Map<string, number[]>,
    ...
    const testForecasts = this.combineModelForecastsForValidation(
      modelForecasts,
      testData.length,
    );
    ...
    for (let i = 0; i < testData.length; i++) {
      ...
      const predicted =
        testForecasts[i] || testForecasts[testForecasts.length - 1];
    ...
    const totalSumSquares = testData.reduce(
      (sum, val) => sum + Math.pow(val - testMean, 2),
      0,
    );
    const r2Score =
      totalSumSquares > 0 ? 1 - sumSquaredError / totalSumSquares : 0;
    ...
    const accurateForecasts = testForecasts.filter(
      (pred, i) => Math.abs(pred - testData[i]) <= reasonableErrorThreshold,
    ).length;
    ...
    return {
      ...
      residualAnalysis: {
        ...
        heteroscedasticity: this.testHeteroscedasticity(
          testData,
          testForecasts,
        ),
      },
  ...
  private decomposeTimeSeries(values: number[]): {
    trend: number[];
    seasonal: number[];
    residual: number[];
  } {
  ...
  private extrapolateSeasonal(seasonal: number[], horizon: number): number[] {
    ...
    return Array.from({ length: horizon }, (_, i) => {
      ...
      const seasonalValue =
        seasonal[seasonal.length - period + seasonalIndex] || 0;
  ...
  private calculateAutocorrelation(
    ...
    const mean =
      residuals.reduce((sum, val) => sum + val, 0) / residuals.length;
    let numerator = 0;
    let denominator = 0;
  ...
  private testNormality(actual: number[], predicted: number[]): number {
    const residuals = actual.map((val, i) => val - predicted[i]);
    if (residuals.length < 3) return 0.5;
  ...
  private testHeteroscedasticity(
    ...
    if (firstVariance === 0 && secondVariance === 0) return 1;
    if (firstVariance === 0 || secondVariance === 0) return 0.5;

    const ratio =
      Math.max(firstVariance, secondVariance) /
  ...
  private getPlaceholderValidationMetrics(): ModelValidationMetrics {
    return {
      modelType: 'ensemble',
      accuracy: 75,
      mse: 0,
  ...
  private calculateAdvancedRiskAssessment(
    baseForecast: number,
    monteCarloResults: EnsembleForecast['monteCarloResults'],
    scenarioResults: EnsembleForecast['scenarioResults'],
  ): BudgetForecast['riskAssessment'] {
    ...
    if (overageRisk < 1.2) severity = 'low';
    else if (overageRisk < 1.5) severity = 'medium';
    else if (overageRisk < 2.0) severity = 'high';
    else severity = 'critical';

    const mitigation = this.generateAdvancedMitigationStrategies(
      severity,
      scenarioResults,
    );
  ...
  private generateAdvancedMitigationStrategies(
    severity: string,
    scenarios: EnsembleForecast['scenarioResults'],
  ): string[] {
    const strategies: string[] = [];

    if (severity === 'critical') {
      strategies.push(
        'Implement dynamic budget allocation based on real-time usage',
      );
    ...
    Object.entries(scenarios).forEach(([scenarioName, result]) => {
      if (result.impact === 'extreme') {
        strategies.push(
          `Prepare contingency plan for ${scenarioName} scenario`,
        );
      }
    });
  ...
  private extractModelContributions(
    modelForecasts: Map<string, number[]>,
  ): EnsembleForecast['modelContributions'] {
    const weights = new Map([
      ['linear', 0.15],
      ['exponential', 0.25],
      ['seasonal', 0.25],
    ...
    const contributions: EnsembleForecast['modelContributions'] = {};
  ...
  private estimateModelConfidence(
    ...
    if (forecast.length > 1) {
      ...
      const mean =
        forecast.reduce((sum, val) => sum + val, 0) / forecast.length;
