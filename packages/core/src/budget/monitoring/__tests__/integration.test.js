...
import { TokenMonitoringIntegration, TokenTrackingContentGenerator, createTokenMonitoringIntegration, createMonitoringEnabledContentGenerator, MonitoringPresets, } from '../integration.js';
...
describe('Token Monitoring Integration', () => {
    ...
    beforeEach(async () => {
        ...
        integration = await createTokenMonitoringIntegration(mockConfig, budgetSettings, MonitoringPresets.Testing);
    });
    afterEach(async () => {
        if (integration) {
    ...
    describe('Content Generator Wrapping', () => {
        ...
        it('should track generateContentStream requests', async () => {
            const request = {
                model: 'test-model',
                contents: [
                    { parts: [{ text: 'Test streaming prompt' }], role: 'user' },
                ],
            };
        ...
        it('should handle API failures gracefully', async () => {
            ...
            await expect(wrappedGenerator.generateContent(request, 'test-prompt-id')).rejects.toThrow('Mock API failure');
    ...
    describe('Factory Functions', () => {
        it('should create monitoring-enabled content generator', async () => {
            const result = await createMonitoringEnabledContentGenerator(mockContentGenerator, mockConfig, budgetSettings, MonitoringPresets.Development);
    ...
    describe('Statistics and Health', () => {
        ...
        it('should track integration health', () => {
            const wrappedGenerator = integration.wrapContentGenerator(mockContentGenerator);
            const integrationStats = wrappedGenerator.getIntegrationStats();
    ...
    describe('Error Handling', () => {
        it('should handle initialization errors gracefully', async () => {
            const invalidConfig = {};
            await expect(createTokenMonitoringIntegration(invalidConfig, budgetSettings, MonitoringPresets.Testing)).rejects.toThrow();
        });
        it('should recover from component failures', async () => {
            ...
            await expect(wrappedGenerator.generateContent(request, 'test-prompt-id')).rejects.toThrow();
...
describe('Token Monitoring System Performance', () => {
    ...
    beforeEach(async () => {
        ...
        integration = await createTokenMonitoringIntegration(mockConfig, budgetSettings, MonitoringPresets.Production);
    });
    afterEach(async () => {
        if (integration) {
            await integration.shutdown();
    ...
    it('should handle high-frequency requests efficiently', async () => {
        ...
        for (let i = 0; i < requestCount; i++) {
            const request = {
                model: 'test-model',
                contents: [
                    { parts: [{ text: `Test prompt ${i}` }], role: 'user' },
                ],
            };
