...
export class TokenMonitoringIntegration extends EventEmitter {
    ...
    initializeComponents() {
        ...
        if (this.integrationConfig.enableStreaming) {
            this.streamingService = new RealTimeStreamingService(this.tokenTracker, this.metricsCollector, this.eventManager, {
                enableCompression: this.integrationConfig.streamingConfig?.enableCompression ?? true,
                bufferSize: this.integrationConfig.streamingConfig?.maxBufferSize ?? 10000,
                maxUpdateFrequency: this.integrationConfig.streamingConfig?.updateFrequency ?? 1000,
