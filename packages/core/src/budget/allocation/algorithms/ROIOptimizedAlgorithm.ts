...
export class ROIOptimizedAlgorithm extends BaseAllocationAlgorithm {
   ...
   */
  private analyzeROIPotential(candidates: AllocationCandidate[]): void {
   ...
   */
  private calculateROIAnalysis(candidate: AllocationCandidate): ROIAnalysis {
   ...
   */
  private calculatePotentialROI(candidate: AllocationCandidate): number {
   ...
   */
  private calculateRiskFactor(candidate: AllocationCandidate): number {
  ...
  private calculateMarginalROI(
    ...
    if (currentAllocation <= 0) return 0;
   ...
   */
  private calculateROIStability(candidate: AllocationCandidate): number {
   ...
   */
  private calculateTimeToROI(candidate: AllocationCandidate): number {
   ...
   */
  private calculateResourceUniqueness(candidate: AllocationCandidate): number {
  ...
  private allocateUsingEfficientFrontier(
    ...
    while (remainingBudget > 0 && sortedCandidates.length > 0) {
      ...
      if (maxAdditionalAllocation < 0.01) break;
    }
  ...
  private determineImplementationComplexity(
    ...
    if (complexityScore < 30 && roiStability > 0.7) return 'low';
    if (complexityScore < 70 && roiStability > 0.5) return 'medium';
    return 'high';
  }
...
/**
 * Internal ROI analysis structure
 */
interface ROIAnalysis {
  ...
  roiStability: number;
  timeToROI: number;
  roiTrend: 'increasing' | 'decreasing' | 'stable';
