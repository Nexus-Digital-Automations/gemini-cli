...
export class UsageBasedAlgorithm extends BaseAllocationAlgorithm {
   ...
   */
  private calculateUtilizationRate(candidate: AllocationCandidate): number {
    const costAnalysis = candidate.costAnalysis;
   ...
   */
  private calculateUsageTrend(candidate: AllocationCandidate): number {
   ...
   */
  private calculateDemandElasticity(candidate: AllocationCandidate): number {
   ...
   */
  private calculateSeasonalityFactor(candidate: AllocationCandidate): number {
   ...
   */
  private calculateRequestVelocity(candidate: AllocationCandidate): number {
   ...
   */
  private calculatePeakUsageRatio(candidate: AllocationCandidate): number {
   ...
   */
  private calculateUsageVolatility(candidate: AllocationCandidate): number {
   ...
   */
  private predictUsageGrowth(candidate: AllocationCandidate): number {
   ...
   */
  private calculateUtilizationAdjustment(metrics: UsageMetric): number {
   ...
   * @returns Adjustment factor
   */
  private calculateElasticityAdjustment(metrics: UsageMetric): number {
   ...
   * @returns Adjustment factor
   */
  private calculateSeasonalAdjustment(metrics: UsageMetric): number {
   ...
   * @returns Adjustment factor
   */
  private calculateGrowthAdjustment(metrics: UsageMetric): number {
  ...
  private ensureBudgetBalance(
    ...
    const balancedRecommendations = recommendations.map((recommendation) => {
      ...
      const newRecommendedAllocation = Math.max(
        candidate.constraints.minAllocation,
        Math.min(
          candidate.constraints.maxAllocation,
...
/**
 * Internal usage metric structure for algorithm calculations
 */
