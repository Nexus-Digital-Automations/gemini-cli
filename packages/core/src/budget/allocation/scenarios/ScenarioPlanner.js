...
export class ScenarioPlanner {
    ...
    generateConstraintScenario(candidates, parameters) {
        const allocations = candidates.map(candidate => ({
            resourceId: candidate.resourceId,
            currentAllocation: candidate.currentAllocation,
            recommendedAllocation: Math.min(candidate.constraints.maxAllocation, Math.max(candidate.constraints.minAllocation, candidate.currentAllocation * 0.95)),
            allocationChange: Math.min(candidate.constraints.maxAllocation, Math.max(candidate.constraints.minAllocation, candidate.currentAllocation * 0.95)) - candidate.currentAllocation,
            strategy: 'usage_based',
            confidence: 95,
            expectedImpact: {
                costImpact: Math.min(candidate.constraints.maxAllocation, Math.max(candidate.constraints.minAllocation, candidate.currentAllocation * 0.95)) - candidate.currentAllocation,
                performanceImpact: 5,
            ...
            expectedSavings: Math.max(0, candidate.currentAllocation - Math.min(candidate.constraints.maxAllocation, Math.max(candidate.constraints.minAllocation, candidate.currentAllocation * 0.95))),
            implementationComplexity: 'low',
        ...
        return {
            ...
            totalBudget: Math.min(parameters.budgetRange.max, candidates.reduce((sum, c) => sum + c.constraints.maxAllocation, 0)),
            allocations,
            expectedOutcomes: this.calculateExpectedOutcomes(allocations),
            timeline: this.config.generation.timeHorizon,
            assumptions: [
    ...
    calculateExpectedOutcomes(allocations) {
        ...
        const utilizationEfficiency = allocations.reduce((sum, alloc) => sum + alloc.expectedImpact.utilizationImpact, 0) / allocations.length;
        const businessValue = allocations.reduce((sum, alloc) => sum + alloc.expectedImpact.businessValueImpact, 0);
    ...
    selectRecommendedScenario(scenarios, comparison) {
        ...
        const bestScenarioId = Object.keys(scores).reduce((best, current) => scores[current] > scores[best] ? current : best);
    ...
    analyzeScenarioRisks(scenarios) {
        const avgRiskScore = scenarios.reduce((sum, scenario) => sum + (100 - scenario.expectedOutcomes.successProbability * 100), 0) / scenarios.length;
    ...
    performSensitivityAnalysis(scenarios, _candidates) {
        ...
        const mostRobust = Object.keys(robustnessScores).reduce((best, current) => robustnessScores[current] > robustnessScores[best] ? current : best);
        const leastRobust = Object.keys(robustnessScores).reduce((worst, current) => robustnessScores[current] < robustnessScores[worst] ? current : worst);
        return {
