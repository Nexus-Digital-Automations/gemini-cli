...
export class ScenarioPlanner {
   ...
   */
  generateScenarios(candidates, parameters) {
    ...
    const additionalScenarios = Math.max(
      0,
      this.config.generation.scenarioCount - 5,
    );
   ...
   */
  analyzeScenarios(scenarios, candidates) {
    ...
    const implementationRoadmap =
      this.generateImplementationRoadmap(recommendedScenario);
    const result = {
      scenarios,
      comparison,
      recommendedScenario,
   ...
   */
  generateWhatIfAnalysis(baseScenario, parameterChanges) {
    const results = [];
    for (const [changeType, changes] of Object.entries(parameterChanges)) {
      for (const change of changes) {
        const modifiedScenario = this.applyParameterChange(
          baseScenario,
          change,
        );
        const impact = this.calculateImpactDelta(
          baseScenario,
          modifiedScenario,
        );
        results.push({
   ...
   * Generate base scenario (current state optimized)
   */
  generateBaseScenario(candidates, parameters) {
    ...
    return {
      ...
      description:
        'Optimized version of current allocation with minimal changes',
      totalBudget:
        parameters.budgetRange.min +
        (parameters.budgetRange.max - parameters.budgetRange.min) * 0.5,
      allocations,
   ...
   * Generate optimistic scenario
   */
  generateOptimisticScenario(candidates, parameters) {
    const allocations = candidates.map((candidate) => ({
      resourceId: candidate.resourceId,
      currentAllocation: candidate.currentAllocation,
   ...
   * Generate pessimistic scenario
   */
  generatePessimisticScenario(candidates, parameters) {
    const allocations = candidates.map((candidate) => ({
      resourceId: candidate.resourceId,
      currentAllocation: candidate.currentAllocation,
    ...
    return {
      ...
      description:
        'Conservative approach assuming budget constraints and challenges',
   ...
   * Generate balanced scenario
   */
  generateBalancedScenario(candidates, parameters) {
    const allocations = candidates.map((candidate) => ({
      resourceId: candidate.resourceId,
      currentAllocation: candidate.currentAllocation,
    ...
    return {
      ...
      totalBudget:
        (parameters.budgetRange.min + parameters.budgetRange.max) / 2,
      allocations,
      expectedOutcomes: this.calculateExpectedOutcomes(allocations),
      timeline: this.config.generation.timeHorizon,
      assumptions: [
   ...
   * Generate constraint-focused scenario
   */
  generateConstraintScenario(candidates, parameters) {
    const allocations = candidates.map((candidate) => ({
      resourceId: candidate.resourceId,
      currentAllocation: candidate.currentAllocation,
      recommendedAllocation: Math.min(
        ...
        Math.max(
          ...
          candidate.currentAllocation * 0.95,
        ),
      ),
      allocationChange:
        Math.min(
    ...
    return {
      ...
      totalBudget: Math.min(
        parameters.budgetRange.max,
        candidates.reduce((sum, c) => sum + c.constraints.maxAllocation, 0),
      ),
      allocations,
      expectedOutcomes: this.calculateExpectedOutcomes(allocations),
      timeline: this.config.generation.timeHorizon,
   ...
   */
  generateCustomScenario(candidates, parameters, index) {
    ...
    const allocations = candidates.map((candidate) => {
      ...
      const recommendedAllocation =
        candidate.currentAllocation * changeMultiplier;
      return {
        resourceId: candidate.resourceId,
        currentAllocation: candidate.currentAllocation,
        ...
        riskAssessment: {
          riskLevel:
            variation > 0.3 ? 'high' : variation > 0.15 ? 'medium' : 'low',
          riskFactors: ['Scenario variation risk'],
          mitigationStrategies: ['Adaptive management'],
          maxNegativeImpact:
            Math.abs(recommendedAllocation - candidate.currentAllocation) * 0.5,
        ...
        expectedSavings: isGrowth
          ? 0
          : Math.abs(recommendedAllocation - candidate.currentAllocation),
        implementationComplexity: 'medium',
    ...
    return {
      ...
      totalBudget:
        parameters.budgetRange.min +
        (parameters.budgetRange.max - parameters.budgetRange.min) *
          (0.3 + variation),
      allocations,
      expectedOutcomes: this.calculateExpectedOutcomes(allocations),
      timeline: this.config.generation.timeHorizon,
   ...
   */
  calculateExpectedOutcomes(allocations) {
    const totalCost = allocations.reduce(
      (sum, alloc) => sum + alloc.recommendedAllocation,
      0,
    );
    const overallROI =
      allocations.reduce(
        (sum, alloc) => sum + alloc.expectedImpact.roiImpact,
        0,
      ) / allocations.length;
    const utilizationEfficiency =
      allocations.reduce(
        (sum, alloc) => sum + alloc.expectedImpact.utilizationImpact,
        0,
      ) / allocations.length;
    const businessValue = allocations.reduce(
      (sum, alloc) => sum + alloc.expectedImpact.businessValueImpact,
      0,
    );
    const avgRisk =
      allocations.reduce((sum, alloc) => {
        const riskScore =
          alloc.riskAssessment.riskLevel === 'critical'
            ? 4
            : alloc.riskAssessment.riskLevel === 'high'
              ? 3
              : alloc.riskAssessment.riskLevel === 'medium'
                ? 2
                : 1;
    ...
    const successProbability = Math.max(
      0.3,
      Math.min(0.95, 1 - (avgRisk - 1) * 0.15),
    );
    return {
      totalCost,
      overallROI,
   ...
   * Perform comparative analysis
   */
  performComparativeAnalysis(scenarios) {
    const performanceScores = scenarios.map(
      (s) => s.expectedOutcomes.overallROI,
    );
    const costs = scenarios.map((s) => s.expectedOutcomes.totalCost);
    const riskScores = scenarios.map(
      (s) => 100 - s.expectedOutcomes.successProbability * 100,
    );
    const roiValues = scenarios.map(
      (s) => s.expectedOutcomes.riskAdjustedReturn,
    );
    return {
      performance: {
        best: scenarios[
          performanceScores.indexOf(Math.max(...performanceScores))
        ].scenarioId,
        worst:
          scenarios[performanceScores.indexOf(Math.min(...performanceScores))]
            .scenarioId,
        average:
      ...
      risk: {
        lowest:
          scenarios[riskScores.indexOf(Math.min(...riskScores))].scenarioId,
        highest:
          scenarios[riskScores.indexOf(Math.max(...riskScores))].scenarioId,
        average: riskScores.reduce((sum, r) => sum + r, 0) / riskScores.length,
        distribution: scenarios.reduce((dist, scenario) => {
          dist[scenario.scenarioId] =
            100 - scenario.expectedOutcomes.successProbability * 100;
          return dist;
        }, {}),
      },
      roi: {
        highest:
          scenarios[roiValues.indexOf(Math.max(...roiValues))].scenarioId,
   ...
   * Select recommended scenario
   */
  selectRecommendedScenario(scenarios, comparison) {
    ...
    for (const scenario of scenarios) {
      ...
      // ROI weight
      score +=
        (scenario.expectedOutcomes.riskAdjustedReturn /
      ...
      const riskScore =
        100 - scenario.expectedOutcomes.successProbability * 100;
      score +=
        ((comparison.risk.average - riskScore) / comparison.risk.average) * 0.3;
      // Cost efficiency weight
      score +=
        ((comparison.cost.average - scenario.expectedOutcomes.totalCost) /
          comparison.cost.average) *
        0.3;
   ...
   */
  analyzeScenarioRisks(scenarios) {
    const avgRiskScore =
      scenarios.reduce(
        (sum, scenario) =>
          sum + (100 - scenario.expectedOutcomes.successProbability * 100),
        0,
      ) / scenarios.length;
      ...
      blackSwanEvents: [
        {
          ...
          probability: 0.05,
          impact: 0.8,
          vulnerableScenarios: scenarios
            .filter((s) => s.expectedOutcomes.totalCost > avgRiskScore)
            .map((s) => s.scenarioId),
        },
      ],
    };
   ...
   * Perform sensitivity analysis
   */
  performSensitivityAnalysis(scenarios, _candidates) {
    ...
    const interactions = [
      {
        parameters: ['budget_change', 'performance_target'],
        strength: 0.7,
        type: 'synergistic',
      },
    ];
    ...
    const robustnessScores = scenarios.reduce((scores, scenario) => {
      scores[scenario.scenarioId] =
        scenario.expectedOutcomes.successProbability * 100;
      return scores;
    }, {});
    ...
    const leastRobust = Object.keys(robustnessScores).reduce(
      (worst, current) =>
   ...
   * Generate implementation roadmap
   */
  generateImplementationRoadmap(scenario) {
    const phases = [
      {
        id: 'planning',
  ...
  applyParameterChange(baseScenario, change) {
    ...
    const modifiedAllocations = baseScenario.allocations.map((allocation) => ({
      ...
      expectedImpact: {
        ...
        performanceImpact:
          allocation.expectedImpact.performanceImpact *
          (1 + change.value * 0.5),
      },
    }));
    return {
   ...
   */
  calculateImpactDelta(baseScenario, modifiedScenario) {
    return {
      costDelta:
        modifiedScenario.expectedOutcomes.totalCost -
        baseScenario.expectedOutcomes.totalCost,
      performanceDelta:
        modifiedScenario.expectedOutcomes.utilizationEfficiency -
        baseScenario.expectedOutcomes.utilizationEfficiency,
      roiDelta:
        modifiedScenario.expectedOutcomes.overallROI -
        baseScenario.expectedOutcomes.overallROI,
      riskDelta:
        (baseScenario.expectedOutcomes.successProbability -
          modifiedScenario.expectedOutcomes.successProbability) *
        100,
    };
  }
  /**
  ...
  validateConfiguration() {
    ...
    if (
      this.config.generation.diversityFactor < 0 ||
      this.config.generation.diversityFactor > 1
    ) {
 ...
 */
export function createScenarioPlanner(config, logger) {
  const defaultLogger = {
    info: () => {},
    warn: () => {},
    error: () => {},
...
//# sourceMappingURL=ScenarioPlanner.js.map
