...
calculateROIScore(candidate, AllocationCandidate, historicalData, FeatureCostAnalysis[]);
number;
{
...
calculateCostEfficiencyScore(candidate, AllocationCandidate, historicalData, FeatureCostAnalysis[]);
number;
{
    if (historicalData.length === 0)
        return 50;
...
calculateStrategicImportanceScore(candidate, AllocationCandidate);
number;
{
...
calculateRiskFactorScore(candidate, AllocationCandidate);
number;
{
...
calculateUsageTrendsScore(candidate, AllocationCandidate, historicalData, FeatureCostAnalysis[]);
number;
{
...
calculatePerformanceImpactScore(candidate, AllocationCandidate);
number;
{
    // Base score from business impact
    let score = candidate.businessImpact;
...
calculateDependenciesScore(candidate, AllocationCandidate);
number;
{
...
calculateInnovationScore(candidate, AllocationCandidate);
number;
{
    let score = 50;
...
calculateComplianceScore(candidate, AllocationCandidate);
number;
{
    let score = 50;
...
calculateOverallScore(scoreBreakdown, ScoreBreakdown);
number;
{
...
generateJustification(candidate, AllocationCandidate, scoreBreakdown, ScoreBreakdown, historicalData, FeatureCostAnalysis[]);
...
calculateConfidence(scoreBreakdown, ScoreBreakdown, historicalData, FeatureCostAnalysis[]);
number;
{
...
performSensitivityAnalysis(scoreBreakdown, ScoreBreakdown);
SensitivityAnalysis;
{
...
groupByPriority(rankings, ResourceRankingResult[]);
...
return groups;
generatePortfolioInsights(rankings, ResourceRankingResult[], candidates, AllocationCandidate[]);
PortfolioInsights;
{
...
calculateRankingStatistics(rankings, ResourceRankingResult[]);
RankingStatistics;
{
...
recommendAllocationStrategy(rankings, ResourceRankingResult[], insights, PortfolioInsights);
AllocationStrategy;
{
...
mergeConfig(defaultConfig, ResourceRankingConfig, userConfig, (Partial));
