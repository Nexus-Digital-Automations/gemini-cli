...
export class ResourceRanking {
    ...
    generateJustification(candidate, scoreBreakdown, _historicalData) {
        ...
        const contributions = Object.entries(scoreBreakdown.weightedContributions).sort(([, a], [, b]) => b - a);
    ...
    performSensitivityAnalysis(scoreBreakdown) {
        ...
        const currentScore = Object.values(scoreBreakdown.weightedContributions).reduce((sum, contrib) => sum + contrib, 0);
    ...
    generatePortfolioInsights(rankings, _candidates) {
        ...
        const avgReward = rankings.reduce((sum, r) => sum + r.scoreBreakdown.roi, 0) /
            rankings.length;
        ...
        const opportunities = [];
        // Identify strengths
        if (priorityDistribution.critical + priorityDistribution.high >
            rankings.length * 0.4) {
    ...
    mergeConfig(defaultConfig, userConfig) {
        return {
            ...
            enableAdvancedFeatures: userConfig.enableAdvancedFeatures ??
