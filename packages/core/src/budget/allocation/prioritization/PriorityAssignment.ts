 ...
 */
export type PriorityAssignmentStrategy =
  | 'static' // Fixed priorities based on initial assessment
  | 'dynamic' // Priorities adjust based on changing conditions
 ...
 */
export const DEFAULT_PRIORITY_CONFIG: PriorityAssignmentConfig = {
  strategy: 'hybrid',
  dynamicAdjustment: {
    enabled: true,
    frequency: 'daily',
...
export class PriorityAssignment {
  private readonly config: PriorityAssignmentConfig;
  private readonly logger: AllocationLogger;
  private assignmentHistory: Map<string, AssignmentMetadata> = new Map();
  ...
  assignPriority(
    ...
    const rationale = this.generateAssignmentRationale(
      candidate,
      ranking,
      assignedPriority,
   ...
   */
  private applyBusinessRules(candidate: AllocationCandidate): {
    appliedRules: BusinessRule[];
    confidence: number;
  } {
