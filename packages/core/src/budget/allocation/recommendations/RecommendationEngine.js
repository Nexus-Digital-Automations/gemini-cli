...
getStrategyObjectives(strategy, AllocationStrategy, context, RecommendationContext);
string[];
{
...
getStrategyParameters(strategy, AllocationStrategy, context, RecommendationContext);
...
async;
generatePrimaryRecommendations(candidates, AllocationCandidate[], context, RecommendationContext, config, AllocationAlgorithmConfig);
...
async;
generateAlternativeRecommendations(candidates, AllocationCandidate[], context, RecommendationContext, algorithmConfigs, (Map));
Promise < Map < AllocationStrategy, AllocationRecommendation[] >> {
    const: alternatives = new Map(),
    for(, [strategy, config], of, algorithmConfigs) {
...
filterRecommendationsByConfidence(recommendations, AllocationRecommendation[]);
...
async;
generateScenarios(candidates, AllocationCandidate[], context, RecommendationContext, primaryRecommendations, AllocationRecommendation[]);
...
createScenario(id, string, name, string, description, string, candidates, AllocationCandidate[], recommendations, AllocationRecommendation[], multiplier, number);
AllocationScenario;
{
...
calculateScenarioOutcomes(recommendations, AllocationRecommendation[], candidates, AllocationCandidate[]);
ScenarioOutcome;
{
...
generateInsights(candidates, AllocationCandidate[], primaryRecommendations, AllocationRecommendation[], alternativeRecommendations, (Map), context, RecommendationContext);
...
generateKeyFindings(candidates, AllocationCandidate[], recommendations, AllocationRecommendation[]);
string[];
{
    const findings = [];
    const totalSavings = recommendations.reduce((sum, rec) => sum + rec.potentialSavings, 0);
...
identifyOptimizationOpportunities(recommendations, AllocationRecommendation[]);
string[];
{
    const opportunities = [];
...
identifyRisks(recommendations, AllocationRecommendation[]);
string[];
{
    const risks = [];
...
generateStrategicRecommendations(recommendations, AllocationRecommendation[], context, RecommendationContext);
string[];
{
    const strategic = [];
...
determineImplementationPriorities(recommendations, AllocationRecommendation[]);
string[];
{
    const priorities = [];
...
calculatePerformanceMetrics(candidates, AllocationCandidate[], primaryRecommendations, AllocationRecommendation[], alternativeRecommendations, (Map));
