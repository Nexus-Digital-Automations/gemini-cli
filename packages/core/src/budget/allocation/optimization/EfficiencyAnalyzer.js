...
export class EfficiencyAnalyzer {
    ...
    calculateCostEfficiency(candidate, historicalData) {
        ...
        const avgCostPerUnit = historicalData.reduce((sum, data) => sum + data.averageCostPerRequest, 0) / historicalData.length;
    ...
    calculateUtilizationEfficiency(candidate, historicalData) {
        if (historicalData.length === 0)
            return 50; // Default score
        const avgUtilization = historicalData.reduce((sum, data) => sum + data.utilizationRate, 0) /
    ...
    analyzeTrends(candidate, historicalData) {
        ...
        const trends = [];
        // Analyze cost trend
        const costTrend = this.calculateTrend(historicalData.map((data, index) => ({
        ...
        const utilizationTrend = this.calculateTrend(historicalData.map((data, index) => ({
            timestamp: new Date(Date.now() - (historicalData.length - index) * 24 * 60 * 60 * 1000),
        ...
        // Analyze ROI trend
        const roiTrend = this.calculateTrend(historicalData.map((data, index) => ({
            timestamp: new Date(Date.now() - (historicalData.length - index) * 24 * 60 * 60 * 1000),
            value: data.roi * 100,
        })));
        trends.push({
    ...
    identifyInefficiencies(candidate, historicalData, efficiencyScores) {
        ...
        if (efficiencyScores.utilization < this.config.thresholds.medium) {
            issues.push({
                ...
                severity: efficiencyScores.utilization < this.config.thresholds.low
                    ? 'high'
                    : 'medium',
        ...
        if (efficiencyScores.cost < this.config.thresholds.medium) {
            issues.push({
                ...
                severity: efficiencyScores.cost < this.config.thresholds.low
                    ? 'high'
                    : 'medium',
        ...
        if (efficiencyScores.performance < this.config.thresholds.medium) {
            issues.push({
                type: 'performance_degradation',
                severity: efficiencyScores.performance < this.config.thresholds.low
                    ? 'critical'
                    : 'medium',
     ...
     * Generate improvement recommendations
     */
    generateImprovements(candidate, inefficiencies, _trends) {
        const improvements = [];
     ...
     */
    generatePortfolioOptimizations(resourceEfficiencies, _candidates) {
        const optimizations = [];
     ...
     */
    calculateBenchmarks(resourceEfficiencies, _historicalData) {
        const currentScore = resourceEfficiencies.reduce((sum, r) => sum + r.overallScore, 0) /
    ...
    calculateAllocationEfficiency(allocation, historicalData) {
        ...
        const avgRequestCount = historicalData.reduce((sum, data) => sum + data.requestCount, 0) /
            historicalData.length;
