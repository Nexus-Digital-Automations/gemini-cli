...
export class ResourceUtilizationOptimizer {
    ...
    forecastUtilization(historicalData, horizon) {
        ...
        const anomalyRisk = Math.min(0.5, Math.abs(slope) + (Math.sqrt(utilizationData.reduce((sum, point) => sum + Math.pow(point.y - (slope * point.x + intercept), 2), 0) / n) * 2));
        return {
            horizon: `${horizon} days`,
            predictions,
    ...
    identifyCrossResourceOpportunities(resources, candidates) {
        ...
        const utilizationVariance = resources.reduce((sum, r) => sum + r.current.variance, 0) / resources.length;
    ...
    calculatePortfolioScore(resources) {
        if (resources.length === 0)
            return 0;
        const avgUtilization = resources.reduce((sum, r) => sum + r.current.average, 0) / resources.length;
        const avgEfficiency = resources.reduce((sum, r) => sum + r.current.efficiency, 0) / resources.length;
    ...
    generatePortfolioInsights(resources, crossResourceOpportunities) {
        const totalSavings = resources.reduce((sum, r) => sum + r.opportunities.reduce((oSum, opp) => oSum + opp.benefits.costSavings, 0), 0) + crossResourceOpportunities.reduce((sum, opp) => sum + opp.benefits.costSavings, 0);
        const utilizationImprovement = resources.reduce((sum, r) => sum + r.opportunities.reduce((oSum, opp) => oSum + opp.benefits.utilizationGain, 0), 0) / resources.length;
    ...
    createImplementationRoadmap(resources, crossResourceOpportunities) {
        ...
        const quickWins = allOpportunities.filter(opp => opp.complexity === 'low' && (opp.priority === 'high' || opp.priority === 'critical'));
