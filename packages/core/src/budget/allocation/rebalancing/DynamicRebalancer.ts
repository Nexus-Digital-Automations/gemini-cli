...
export type RebalancingStrategy =
  | 'reactive' // React to performance changes
  | 'proactive' // Anticipate changes based on trends
  | 'adaptive' // Continuously adjust based on conditions
  | 'threshold_based' // Trigger on specific thresholds
  | 'machine_learning' // ML-driven rebalancing decisions
 ...
 */
export const DEFAULT_REBALANCING_CONFIG: DynamicRebalancingConfig = {
  strategy: 'adaptive',
  triggers: [
    {
...
export class DynamicRebalancer {
  private readonly config: DynamicRebalancingConfig;
  private readonly logger: AllocationLogger;
  private executionHistory: RebalancingExecutionResult[] = [];
  private lastAnalysis?: RebalancingAnalysis;
  ...
  async executeRebalancing(
    ...
    const executedActions: RebalancingAction[] = [];
    const issues: string[] = [];
   ...
   */
  private calculateResourceTrends(recentData: FeatureCostAnalysis[]): {
    allocation: 'increasing' | 'decreasing' | 'stable';
    utilization: 'increasing' | 'decreasing' | 'stable';
    performance: 'improving' | 'degrading' | 'stable';
  } {
  ...
  /**
   * Check if approval is required for actions
   */
   ...
   */
  private async executeImmediate(actions: RebalancingAction[]): Promise<void> {
    for (const action of actions) {
   ...
   */
  private async executeAction(action: RebalancingAction): Promise<void> {
   ...
   */
  private async measureActualOutcomes(analysis: RebalancingAnalysis): Promise<{
    performanceChange: number;
    costChange: number;
    efficiencyChange: number;
  }> {
   ...
   * Check if rollback is needed
   */
  private shouldRollback(outcomes: {
    performanceChange: number;
    costChange: number;
    efficiencyChange: number;
  }): boolean {
    if (!this.config.riskManagement.rollback.enabled) return false;
  ...
  private async executeRollback(
    ...
    for (const action of executedActions.reverse()) {
      const rollbackAction: RebalancingAction = {
        ...action,
        changeAmount: -action.changeAmount,
  ...
  /**
   * Convert rebalancing action to allocation recommendation
   */
