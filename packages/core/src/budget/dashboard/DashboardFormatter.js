...
export class DashboardFormatter {
    ...
    formatDashboard(data, sections) {
        ...
        if (sections.optimizationRecommendations &&
    ...
    formatHeader() {
        ...
        header += ' '.repeat(this.PANEL_WIDTH -
            2 -
            title.length -
    ...
    formatSummaryPanel(data) {
        ...
        panel += this.chartRenderer.renderGauge(currentUsage.budgetUtilization, 100, { warning: 75, critical: 90 }, { title: 'Daily Budget Utilization', unit: '%' });
        return panel;
    }
    /**
     * Format real-time usage section
     */
    formatRealTimeUsage(data) {
        let section = '';
        section += chalk.bold('⚡ Real-Time Usage') + '\n';
        section += '━'.repeat(this.PANEL_WIDTH) + '\n';
        // Hourly usage sparkline
        const hourlySparkline = this.chartRenderer.renderSparkline(data.trends.hourlyUsage, 'Last 24 Hours:');
        section += hourlySparkline + '\n\n';
        // Daily usage sparkline
        const dailySparkline = this.chartRenderer.renderSparkline(data.trends.dailyUsage, 'Last 30 Days: ');
    ...
    formatBudgetAlerts(alerts) {
        ...
        alerts.forEach((alert) => {
            ...
            if (alert.threshold && alert.currentValue) {
                const percentage = ((alert.currentValue / alert.threshold) *
                    100).toFixed(1);
     ...
     */
    formatHistoricalTrends(data) {
        const { trends } = data;
        let section = '';
        section += chalk.bold('📊 Historical Trends') + '\n';
        section += '━'.repeat(this.PANEL_WIDTH) + '\n';
        // Daily usage trend chart
        section += this.chartRenderer.renderLineChart(trends.dailyUsage, Array.from({ length: 30 }, (_, i) => `${30 - i}d`), { title: 'Daily Usage (Last 30 Days)', yAxisLabel: 'Requests' });
        section += '\n';
        return section;
    }
    /**
     ...
     */
    formatFeatureAnalysis(features) {
        let section = '';
    ...
    formatOptimizationRecommendations(recommendations) {
        ...
        recommendations.slice(0, 5).forEach((rec, index) => {
            ...
            section +=
                '   ' +
                    savingsColor(`💰 Potential Savings: $${rec.potentialSavings.toFixed(4)} (${rec.savingsPercentage.toFixed(1)}%)`);
            section +=
    ...
    getFeatureColor(roi) {
        if (roi >= 2)
            return '#00ff88'; // High ROI - green
        if (roi >= 1)
            return '#00d9ff'; // Good ROI - blue
        if (roi >= 0.5)
            return '#ffdd00'; // Medium ROI - yellow
...
//# sourceMappingURL=DashboardFormatter.js.map