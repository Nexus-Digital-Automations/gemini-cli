...
export class ChartRenderer {
     ...
     */
    renderLineChart(data, labels = [], options = {}) {
     ...
     */
    renderGauge(value, max, thresholds, options = {}) {
        const { width } = this.config;
     ...
     */
    renderMultiLineChart(series, options = {}) {
        const { width } = this.config;
     ...
     */
    shouldDrawLine(prev, current, threshold, range, height) {
        const tolerance = range / (height * 2);
        return (Math.abs(prev - threshold) < tolerance &&
 ...
 */
export function createChartRenderer(config) {
