...
export class ProgressTracker {
    ...
    completeStep(operationId, stepId, finalResult) {
        ...
        const allStepsComplete = operation.steps.every((s) => s.state === ProgressState.Completed || s.state === ProgressState.Failed);
        if (allStepsComplete) {
     ...
     */
    getActiveOperations() {
        return Array.from(this.operations.values()).filter((op) => op.state !== ProgressState.Completed &&
    ...
    cleanup(olderThanMs = 300000) {
        ...
        for (const [operationId, operation] of this.operations.entries()) {
            if ((operation.state === ProgressState.Completed ||
                operation.state === ProgressState.Failed ||
                operation.state === ProgressState.Cancelled) &&
    ...
    updateOverallProgress(operationId) {
        ...
        operation.overallProgress = Math.round(totalProgress / operation.steps.length);
        this.updateTimeEstimation(operationId);
    }
    updateTimeEstimation(operationId) {
        const operation = this.operations.get(operationId);
        if (!operation || operation.overallProgress === 0)
            return;
