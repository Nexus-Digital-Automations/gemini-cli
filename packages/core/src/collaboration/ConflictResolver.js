...
export class ConflictResolver extends EventEmitter {
    ...
    getActiveConflicts(sessionId) {
        return Array.from(this.activeConflicts.values()).filter((conflict) => conflict.id.includes(sessionId));
    }
    /**
    ...
    async checkContextConflict(contextId, newContent, existingVersions) {
        ...
        const recentVersions = existingVersions.filter((version) => new Date().getTime() - version.timestamp.getTime() < 30000);
    ...
    async analyzeLocationConflict(sessionId, location, changes, participants) {
        ...
        const conflictingChanges = sortedChanges.filter((change) => change.timestamp.getTime() - firstChange.timestamp.getTime() <=
            timeWindow);
    ...
    initializeResolutionStrategies() {
        ...
        this.resolutionStrategies.set(ConflictResolutionStrategy.ROLE_PRIORITY, async (conflict, _resolvingParticipant, resolutionData) => {
            const participants = resolutionData
                ?.participants || [];
