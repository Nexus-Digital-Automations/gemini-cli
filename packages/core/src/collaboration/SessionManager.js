...
export class SessionManager extends EventEmitter {
    ...
    async joinSession(sessionId, participant) {
        ...
        const activeParticipant = existingParticipant ||
    ...
    async getSessionMetrics(sessionId) {
        ...
        const mostActiveParticipant = Array.from(participantActivities.entries()).sort(([, a], [, b]) => b - a)[0]?.[0] || session.host.id;
        ...
        return {
            ...
            participation: {
                totalParticipants: session.participants.length,
                avgDurationPerParticipant: Array.from(participantDurations.values()).reduce((sum, duration) => sum + duration, 0) / session.participants.length,
     ...
     */
    getActiveSessions() {
        return Array.from(this.activeSessions.values()).filter((session) => session.status === SessionStatus.ACTIVE);
    }
    /**
     * Get session by ID
     */
