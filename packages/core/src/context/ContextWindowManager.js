 ...
 */
export const DEFAULT_CONTEXT_WINDOW_CONFIG = {
...
export class ContextWindowManager {
     ...
     */
    allocateTokensToSections(sections) {
        const availableTokens = this.config.totalTokens * (1 - this.config.bufferPercentage);
        for (const [sectionName, section] of Object.entries(sections)) {
            const allocationRatio = this.config.sectionAllocation[sectionName];
            const allocatedTokens = Math.max(this.config.minTokensPerSection, Math.floor(availableTokens * allocationRatio));
     ...
     */
    async addToSection(sectionName, item) {
     ...
     */
    removeFromSection(sectionName, itemIds) {
     ...
     */
    async optimizeSection(sectionName) {
        const startTime = performance.now();
     ...
     */
    async compressItemsInSection(sectionName, itemsToCompress) {
        const section = this.contextWindow.sections[sectionName];
        let totalSavings = 0;
    ...
    async optimizeContextWindow() {
        ...
        try {
            ...
            for (const sectionName of stats.needsCompression) {
                await this.optimizeSection(sectionName);
            }
    ...
    performDynamicAllocation() {
        ...
        for (const [sectionName, usage] of Object.entries(usagePatterns)) {
            const baseWeight = this.config.sectionAllocation[sectionName];
        ...
        for (const [sectionName, section] of Object.entries(sections)) {
            ...
            const newAllocation = Math.max(this.config.minTokensPerSection, Math.floor(availableTokens * normalizedWeight));
     ...
     */
    calculateUsageTrend(usageHistory) {
     ...
     */
    recordSectionUsage(sectionName, _tokensDelta) {
     ...
     */
    updateContextWindowTotals() {
        const usedTokens = Object.values(this.contextWindow.sections).reduce((sum, section) => sum + section.tokens, 0);
    ...
    getAllocationStats() {
        ...
        const efficiencyScore = totalAllocated > 0 ? (totalAllocated - wastedTokens) / totalAllocated : 0;
        return {
            totalAllocated,
            bySections,
            utilizationRates,
     ...
     */
    clearAll() {
        for (const sectionName of Object.keys(this.contextWindow.sections)) {
 ...
 */
export function createContextWindowManager(config) {
