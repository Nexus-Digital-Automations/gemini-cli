...
import { CompressionStrategy, ContextType as _ContextType, ContextPriority } from './types.js';
import { performance } from 'node:perf_hooks';
const logger = getComponentLogger('compression-fallback-system');
/**
 * Fallback strategy types
 */
export let FallbackStrategy;
 ...
 */
export let CompressionErrorType;
...
export class CompressionFallbackSystem {
    ...
    async applyAggressiveTruncation(items, targetRatio) {
        ...
        const otherItems = items.filter(item => item.priority !== ContextPriority.CRITICAL && item.priority !== ContextPriority.HIGH);
        const selectedItems = [];
        let currentTokens = 0;
