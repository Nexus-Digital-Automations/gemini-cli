...
interface CompressionStrategyImpl {
  compress(content: string, targetRatio: number): Promise<CompressionResult>;
}
...
export class SemanticCompressor {
  ...
  private strategies: Map<CompressionStrategy, CompressionStrategyImpl>;
  ...
  async batchCompress(
    ...
    try {
      ...
      const duration = performance.now() - startTime;
      const totalOriginalTokens = Array.from(results.values()).reduce(
        (sum, result) => sum + result.originalTokens,
        0,
      );
      const totalCompressedTokens = Array.from(results.values()).reduce(
        (sum, result) => sum + result.compressedTokens,
        0,
      );
   ...
   */
  private selectOptimalStrategy(item: ContextItem): CompressionStrategy {
...
class SummarizationStrategy implements CompressionStrategyImpl {
  ...
  private selectSentencesByToken(
...
class KeywordExtractionStrategy implements CompressionStrategyImpl {
  ...
  private selectTopWords(
    scoredWords: Array<{ word: string; score: number }>,
    targetTokens: number,
  ): string[] {
...
class SemanticClusteringStrategy implements CompressionStrategyImpl {
  ...
  private calculateSimilarity(segment1: string, segment2: string): number {
  ...
  private compressCluster(cluster: string[], targetRatio: number): string {
    if (cluster.length === 1) {
...
class ProgressiveDetailStrategy implements CompressionStrategyImpl {
  ...
  private analyzeContentParts(content: string): Array<{
    content: string;
    type: string;
    importance: number;
    order: number;
  ...
  private identifyLineType(line: string): string {
    if (/^(function|class|interface|type)/.test(line)) return 'declaration';
    if (/^(import|export)/.test(line)) return 'import';
    if (/^(const|let|var)/.test(line)) return 'variable';
    if (/^\/\*|^\/\/|^\*/.test(line.trim())) return 'comment';
    if (/error|exception|warning/i.test(line)) return 'error';
    if (line.trim().length === 0) return 'whitespace';
    return 'code';
  }
