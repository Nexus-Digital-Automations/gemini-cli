 ...
 */
export const DEFAULT_CONTEXT_SYSTEM_CONFIG: Partial<ContextSystemConfig> = {
  autoOptimize: true,
...
export class ContextSystem {
  private config: ContextSystemConfig;
   ...
   */
  async searchSessions(query: string, limit = 10): Promise<SessionContext[]> {
    this.ensureInitialized();
   ...
   */
  async analyzeCodeChanges(changedFiles: string[]): Promise<{
   ...
   */
  async updateConfig(newConfig: Partial<ContextSystemConfig>): Promise<void> {
   ...
   */
  async importSystemData(data: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      // Import storage data
      if (data.storage) {
        await this.storage.importData(data.storage as Record<string, unknown>);
      }
