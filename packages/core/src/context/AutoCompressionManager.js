...
(function (AutoCompressionEvent) {
    ...
    AutoCompressionEvent["COMPRESSION_COMPLETED"] = "compression_completed";
    AutoCompressionEvent["COMPRESSION_FAILED"] = "compression_failed";
    AutoCompressionEvent["EMERGENCY_COMPRESSION"] = "emergency_compression";
    AutoCompressionEvent["TOKEN_LIMIT_WARNING"] = "token_limit_warning";
    AutoCompressionEvent["TOKEN_LIMIT_EXCEEDED"] = "token_limit_exceeded";
})(AutoCompressionEvent || (AutoCompressionEvent = {}));
/**
...
export class AutoCompressionManager extends EventEmitter {
    ...
    totalTokensCache = 0;
    lastCacheUpdate = 0;
    triggerEnabled = true; // Automatic trigger for 1M token limit issue
    constructor(config = {}, configurationManager, preset) {
        super();
     ...
     */
    convertToLegacyConfig(systemConfig, overrides = {}) {
        return {
    ...
    handleConfigurationChange(newConfig) {
        logger.info('Configuration change detected, updating AutoCompressionManager', {
            ...
            thresholdsChanged: newConfig.tokenLimits.thresholds.critical !==
        ...
        if (newConfig.monitoring.interval !== oldConfig.monitoringInterval &&
     ...
     */
    async triggerRobustCompression(isEmergency = false) {
    ...
    start() {
        ...
        this.monitoringTimer = setInterval(() => this.checkTokenUsage(), this.config.monitoringInterval);
        // Initial check
     ...
     */
    async triggerCompression(force = false) {
        const snapshot = this.captureTokenUsageSnapshot();
    ...
    getCompressionHistory() {
        ...
        const compressionEvents = history.filter((s) => s.timestamp.getTime() > this.lastCompressionTime);
    ...
    async checkTokenUsage() {
        try {
            ...
            if (this.shouldCompress(snapshot)) {
                await this.performCompression(snapshot, snapshot.utilizationRatio >= this.config.emergencyThreshold);
            }
        }
        catch (error) {
     ...
     */
    calculateGrowthRate() {
        if (this.tokenUsageHistory.length < 2)
            return 0;
     ...
     */
    async performRobustCompression(snapshot, isEmergency) {
        ...
        try {
            ...
            const opportunities = snapshot.compressionOpportunities.sort((a, b) => b.estimatedSavings - a.estimatedSavings);
            ...
            for (const opportunity of opportunities) {
                ...
                try {
                    ...
                    for (const item of section.items) {
                        try {
                            const enhancedResult = await this.enhancedCompressor.compressWithTypeOptimization(item, targetRatio);
                    ...
                    if (failedItems.length > 0) {
                        ...
                        try {
                            const fallbackResult = await this.fallbackSystem.applyFallbackCompression(failedItems.map((f) => f.item), targetRatio, failedItems[0].error ||
                                new Error('Enhanced compression failed'), isEmergency);
                            if (fallbackResult.success) {
                                ...
                                for (let i = 0; i < failedItems.length && i < fallbackItems.length; i++) {
                                    ...
                                    const itemIndex = section.items.indexOf(item);
                                    if (itemIndex !== -1) {
                                        if (fallbackResult.emergencyMeasuresApplied &&
                    ...
                    logger.error('Section compression completely failed', {
                        error: sectionError instanceof Error
            ...
            try {
                ...
                if (allItems.length > 0) {
                    const emergencyFallback = await this.fallbackSystem.applyFallbackCompression(allItems, this.config.targetCompressionRatio * 0.3, // Very aggressive
                    error, true);
                    const result = {
                        ...
                        error: emergencyFallback.success ? undefined : error,
                        itemsCompressed: 0,
                        itemsRemoved: 0,
                    };
                    this.emit(AutoCompressionEvent.COMPRESSION_COMPLETED, {
                ...
                logger.error('Emergency fallback also failed', {
                    emergencyError: emergencyError instanceof Error
     ...
     */
    async performCompression(snapshot, isEmergency) {
        const startTime = performance.now();
        this.isCompressing = true;
        this.compressionAttempts++;
        const eventType = isEmergency
        ...
        try {
            ...
            for (const opportunity of opportunities) {
                ...
                const compressionResult = await this.compressor.batchCompress(section.items, targetRatio);
     ...
     */
    calculateCompressionConfidence(section) {
 ...
 */
let globalAutoCompressionManager = null;
/**
 * Get or create the global AutoCompressionManager instance
 */
export function getGlobalAutoCompressionManager(config) {
 ...
 */
export function resetGlobalAutoCompressionManager() {
    if (globalAutoCompressionManager) {
        globalAutoCompressionManager.stop();
        globalAutoCompressionManager = null;
    }
}
//# sourceMappingURL=AutoCompressionManager.js.map