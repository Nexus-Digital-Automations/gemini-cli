...
import {
  ...
  ConfigurationPreset,
  CompressionSystemConfig as _CompressionSystemConfig,
  ...
  convertToLegacyConfig(systemConfig, overrides = {}) {
    return {
      ...
      compressionStrategies: Object.entries(
        ...
        priority,
        tokenThreshold:
          systemConfig.algorithms.selectionRules.tokenThresholds[strategy] || 0,
      })),
      ...overrides,
    };
  }
...
>>>>>>> Stashed changes
      ...
      try {
        ...
        for (const [_managerId, manager] of this.contextManagers.entries()) {
          const window = manager.getCurrentWindow();
          for (const [_sectionName, section] of Object.entries(
            window.sections,
          )) {
            if (section.items && section.items.length > 0) {
              allItems.push(...section.items);
