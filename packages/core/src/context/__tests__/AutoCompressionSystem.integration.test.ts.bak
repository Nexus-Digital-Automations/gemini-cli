...
>>>>>>> Stashed changes
  ...
  describe('Performance Tests', () => {
    ...
    test('should handle memory constraints gracefully', async () => {
      ...
      const largeItems = Array.from({ length: 5 }, (_, i) =>
        createContextItem(
...
function createLargeContextItems(totalTokens: number): {
...
function generateContentByType(type: ContextType, index: number): string {
  switch (type) {
