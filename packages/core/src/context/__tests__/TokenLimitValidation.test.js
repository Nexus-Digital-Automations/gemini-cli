...
describe('Token Limit Validation - 1,272,932 → 1,048,576 Issue', () => {
    ...
    describe('Exact Token Limit Scenario', () => {
        ...
        - System: ${contextSections.system.tokens.toLocaleString()} tokens (${contextSections.system.items.length} items)`);
            ...
            compressionManager.registerContextManager('exact-limit-test', mockContextManager);
        ...
        test('should maintain system stability under repeated compression cycles', async () => {
            ...
            const avgCompressionRatio = results.reduce((sum, r) => sum + r.compressionRatio, 0) /
                results.length;
...
function createExactTokenDistribution(totalTokens) {
...
function createContextItemsWithExactTokens(totalTokens, type = ContextType.CONVERSATION, priority = ContextPriority.MEDIUM) {
    const items = [];
    let remainingTokens = totalTokens;
    let itemIndex = 0;
    while (remainingTokens > 0) {
        // Create items of varying sizes to simulate realistic content
        const itemSize = Math.min(remainingTokens, Math.floor(Math.random() * 2000) + 500);
...
function generateContentWithExactTokens(tokenCount, type, index) {
...
function generateCodeContent(targetLength, index) {
    const baseContent = `
...
function generateConversationContent(targetLength, index) {
...
function generateLogContent(targetLength, index) {
    const baseLogs = `
...
function generateSystemContent(targetLength, index) {
    const baseSystem = `
...
function generateGenericContent(targetLength, index) {
...
function createMockContextManager(totalTokens, items) {
    return {
        getCurrentWindow: vi.fn(() => ({
...
//# sourceMappingURL=TokenLimitValidation.test.js.map