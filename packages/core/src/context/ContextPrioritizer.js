...
export class ContextPrioritizer {
     ...
     */
    async prioritize(items, currentContext) {
        const startTime = performance.now();
        logger.debug(`Prioritizing ${items.length} context items`);
        try {
            // Score all items
            const scoredItems = await Promise.all(items.map((item) => this.scoreContextItem(item, currentContext, items)));
     ...
     */
    async scoreContextItem(item, currentContext, allItems = []) {
     ...
     */
    async calculateRelevanceScore(item, currentContext) {
        // Check cache first
        ...
        if (currentContext) {
            ...
            const tagMatches = item.tags.filter((tag) => contextLower.includes(tag.toLowerCase())).length;
     ...
     */
    calculateInteractionScore(item) {
     ...
     */
    calculateDependencyScore(item, allItems) {
        // Count how many other items list this item as a dependency
        const dependentCount = allItems.filter((otherItem) => otherItem.dependencies.includes(item.id)).length;
    ...
    categorizeItems(scoredItems) {
        ...
        for (const item of scoredItems) {
            ...
            else if (item.scoring.finalScore < this.config.minRelevanceThreshold ||
            ...
            else if (item.scoring.finalScore < 0.6 ||
     ...
     */
    calculateStats(originalItems, result) {
        const byPriority = {
        ...
        const compressionSavings = result.toCompress.reduce((sum, item) => sum + item.tokenCount * 0.7, 0);
        const removalSavings = result.toRemove.reduce((sum, item) => sum + item.tokenCount, 0);
        return {
 ...
 */
export function createContextPrioritizer(config) {
    return new ContextPrioritizer(config);
}
//# sourceMappingURL=ContextPrioritizer.js.map