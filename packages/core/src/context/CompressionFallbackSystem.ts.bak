...
export class CompressionFallbackSystem {
   ...
   */
  async applyFallbackCompression(
    items: ContextItem[],
    targetRatio: number,
    primaryError: Error,
   ...
   */
  private classifyCompressionError(error: Error): CompressionErrorType {
  ...
  private async applyChunkBasedProcessing(items: ContextItem[], targetRatio: number): Promise<BaseCompressionResult> {
    ...
    let totalOriginalTokens = 0;
    let totalCompressedTokens = 0;
    const allCompressedContent: string[] = [];
  ...
  private async applyAggressiveTruncation(items: ContextItem[], targetRatio: number): Promise<BaseCompressionResult> {
    ...
    for (const item of highItems) {
      if (currentTokens >= targetTokens * 0.8) break; // Reserve 20% for other items
   ...
   */
  private generateSessionId(items: ContextItem[]): string {
