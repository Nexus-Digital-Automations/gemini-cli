...
export class TokenEstimator {
   ...
   */
  private static appearsToBeCode(content: string): boolean {
   ...
   */
  private static appearsToBeMarkdown(content: string): boolean {
    ...
    const matches = markdownIndicators.filter((pattern) =>
      pattern.test(content),
    ).length;
...
export class ContextItemUtils {
  ...
  private static inferPriority(
    ...
    if (
      priorityKeywords.critical.some((keyword) =>
    ...
    if (
      priorityKeywords.high.some((keyword) => contentLower.includes(keyword))
    ) {
      return ContextPriority.HIGH;
    }

    if (
      priorityKeywords.low.some((keyword) => contentLower.includes(keyword))
    ) {
    ...
    if (metadata.priority) {
      return metadata.priority as ContextPriority;
    }
   ...
   */
  private static appearsStructured(content: string): boolean {
    // Check for JSON
    try {
      JSON.parse(content);
      return true;
...
export class ContextSearchUtils {
  ...
  static groupByType(items: ContextItem[]): Map<ContextType, ContextItem[]> {
    const groups = new Map<ContextType, ContextItem[]>();
...
export class SessionAnalysisUtils {
    ...
    for (const session of sessions) {
      ...
      for (const item of session.contextItems) {
        ...
        if (
          ...
          fileFrequency.set(
            item.metadata.filePath,
            (fileFrequency.get(item.metadata.filePath) || 0) + 1,
          );
        }
  ...
  static calculateSessionSimilarity(
    ...
    const items1 = new Set(
      session1.contextItems.map((item) => item.content.substring(0, 100)),
    );
    const items2 = new Set(
      session2.contextItems.map((item) => item.content.substring(0, 100)),
    );
