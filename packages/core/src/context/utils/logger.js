...
<<<<<<< Updated upstream
    ...
    createLogFormat() {
        return winston.format.combine(winston.format.timestamp({
            ...
            const errorInfo = meta['error'] instanceof Error
                ? {
            ...
            const metaString = Object.keys(metaWithError).length > 0
    ...
    createTransports() {
        ...
        if (this.config.enableFile) {
            const logFilePath = path.join(this.config.logDir, `${this.config.logFileName}.log`);
            ...
            const errorLogPath = path.join(this.config.logDir, `${this.config.logFileName}-error.log`);
            transports.push(new winston.transports.File({
...
<<<<<<< Updated upstream
    ...
    const envConfig = {
        debugMode: process.env['NODE_ENV'] === 'development' || !!process.env['DEBUG'],
