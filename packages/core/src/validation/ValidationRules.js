...
export class ValidationRules {
  ...
  async validateTaskStatusForExecution(context) {
    ...
    const validExecutionStatuses = ['pending', 'ready', 'in_progress'];
    const invalidStatuses = ['completed', 'failed', 'cancelled'];
  ...
  async validateExecutionTimeout(context) {
    ...
    const maxExecutionTime =
      task.maxExecutionTimeMinutes * 60 * 1000 || 3600000; // 1 hour default
