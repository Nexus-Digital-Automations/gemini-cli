...
export class IntegrationValidator {
    ...
    for (const benchmark of this.config.performanceBenchmarks) {
      ...
      try {
        ...
        results.push({
          ...
          severity: ValidationSeverity.ERROR,
          status: ValidationStatus.FAILED,
          message: `Benchmark ${benchmark.name} failed: ${(error as Error).message}`,
          timestamp: new Date(),
          duration: Date.now() - startTime,
          metadata: {
   ...
   * Check resource usage
   */
  private async checkResourceUsage(): Promise<ValidationResult[]> {
    const results: ValidationResult[] = [];
    const memoryUsage = process.memoryUsage();
   ...
   * Check health endpoints
   */
  private async checkHealthEndpoints(): Promise<ValidationResult[]> {
    const results: ValidationResult[] = [];
    ...
    for (const endpoint of this.config.monitoringChecks.healthEndpoints) {
      try {
        ...
        results.push({
          ...
          severity: ValidationSeverity.ERROR,
          status: ValidationStatus.FAILED,
          message: `Health endpoint ${endpoint} check failed: ${(error as Error).message}`,
          timestamp: new Date(),
          metadata: {
            endpoint,
