...
export class ValidationFailureHandler extends EventEmitter {
    ...
    async executeCircuitBreaker(operation, context, failureRecord) {
        ...
        if (circuitBreaker.state === CircuitBreakerState.OPEN) {
            if (Date.now() - circuitBreaker.lastFailureTime.getTime() <
        ...
        try {
            ...
            if (circuitBreaker.state === CircuitBreakerState.CLOSED) {
                if (circuitBreaker.windowFailures >=
                ...
                if (circuitBreaker.halfOpenAttempts >=
    ...
    async executeEscalation(error, context, originalOperation, failureRecord) {
        ...
        const existingFailures = Array.from(this.failureRecords.values()).filter((record) => this.getTaskId(record.context) === taskId && !record.resolved).length;
