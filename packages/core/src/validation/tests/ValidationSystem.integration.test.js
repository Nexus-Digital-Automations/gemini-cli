...
describe('ValidationSystem Integration', () => {
    ...
    describe('Complete Validation Workflow', () => {
        it('should execute complete validation workflow successfully', async () => {
            ...
            const initialSnapshot = await rollbackManager.createSnapshot(testTask, 'Pre-execution snapshot for integration test', [RollbackType.TASK_STATE, RollbackType.ENVIRONMENT]);
            ...
            const qualityResult = await qualityAssurance.performQualityAssurance(completedTask, testTaskResult, executionMetrics);
        ...
        it('should handle validation failure and trigger rollback', async () => {
            ...
            const preFailureSnapshot = await rollbackManager.createSnapshot(failingTask, 'Snapshot before attempting failing task execution');
            ...
            const failingQualityResult = await qualityAssurance.performQualityAssurance(failingTask, failingTaskResult, failingExecutionMetrics);
            ...
            const rollbackOperation = rollbackManager.createRollbackOperation(failingTask.id, RollbackTrigger.VALIDATION_FAILURE, preFailureSnapshot.id, 'Rollback due to critical validation failures and quality issues');
    ...
    describe('Component Integration', () => {
        it('should integrate validation rules with framework correctly', async () => {
            ...
            const preExecutionRules = validationRules.getApplicableRules(RuleExecutionContext.PRE_EXECUTION, 'implementation', TaskStatus.PENDING);
        ...
        it('should coordinate quality assurance with task validation', async () => {
            ...
            const qaResult = await qualityAssurance.performQualityAssurance({ ...testTask, status: TaskStatus.COMPLETED }, testTaskResult, executionMetrics);
        ...
        it('should integrate rollback manager with validation failures', async () => {
            ...
            const preFailureSnapshot = await rollbackManager.createSnapshot(criticalFailureTask, 'Snapshot before critical failure');
            ...
            if (validationResult.rollbackRecommended && validationResult.snapshot) {
                const rollbackOp = rollbackManager.createRollbackOperation(criticalFailureTask.id, RollbackTrigger.VALIDATION_FAILURE, validationResult.snapshot.id, validationResult.rollbackReason || 'Validation recommended rollback');
...
describe('ValidationSystem End-to-End Scenarios', () => {
    it('should handle complete task lifecycle with validation', async () => {
        ...
        try {
            ...
            const preExecSnapshot = await rollbackManager.createSnapshot(newTask, 'Initial task state snapshot');
            ...
            const duringExecSnapshot = await rollbackManager.createSnapshot(executingTask, 'Task execution started snapshot');
            ...
            const completionSnapshot = await rollbackManager.createSnapshot(completedTask, 'Task completion snapshot');
            const taskResult = {
                taskId: newTask.id,
                success: true,
                output: {
            ...
            const finalQA = await qualityAssurance.performQualityAssurance(completedTask, taskResult, executionMetrics);
...
//# sourceMappingURL=ValidationSystem.integration.test.js.map