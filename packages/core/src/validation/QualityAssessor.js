...
export class QualityAssessor extends EventEmitter {
    ...
    async calculateQualityMetrics(context) {
        ...
        const assessorResults = await Promise.all(Array.from(this.customAssessors.entries()).map(async ([name, assessor]) => {
            try {
    ...
    calculateOverallScore(metrics) {
        ...
        return (metrics.functionalScore * weights.functional +
    ...
    async detectQualityIssues(context, metrics, criteria) {
        ...
        if (metrics.codeComplexity > criteria.maximumCodeComplexity) {
            issues.push({
                ...
                severity: metrics.codeComplexity > criteria.maximumCodeComplexity * 1.5
                    ? 'high'
                    : 'medium',
        ...
        if (metrics.technicalDebt > criteria.maximumTechnicalDebt) {
            issues.push({
                ...
                category: 'maintainability',
                severity: metrics.technicalDebt > criteria.maximumTechnicalDebt * 1.5
                    ? 'high'
                    : 'medium',
    ...
    evaluateQualityPass(metrics, issues, criteria) {
        ...
        return (metrics.functionalScore >= criteria.minimumFunctionalScore &&
     ...
     */
    getStatistics() {
        const totalEntries = Array.from(this.qualityHistory.values()).reduce((sum, history) => sum + history.length, 0);
        return {
