...
<body>
            ...
            for (const result of report.results) {
                if (result.category === 'security') {
                    sarif.runs[0].results.push({
                        ...
                        level: result.severity === 'critical'
                            ? 'error'
     ...
     */
    async sendReportNotifications(_data, _paths) {
     ...
     */
    getStatistics() {
        const recentReports = this.reportHistory.filter((report) => Date.now() - report.timestamp.getTime() < 24 * 60 * 60 * 1000).length;
        return {
