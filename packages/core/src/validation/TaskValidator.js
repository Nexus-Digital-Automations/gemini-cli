...
(function (TaskValidationType) {
    ...
    TaskValidationType["QUALITY_ASSURANCE"] = "quality_assurance";
...
(function (TaskValidationLevel) {
    ...
    TaskValidationLevel["ADVISORY"] = "advisory";
...
export class TaskValidator extends EventEmitter {
    ...
    async executeTaskValidation(context, startTime) {
        ...
        const result = {
            ...
            validationType: context.validationType,
            validationLevel: context.validationLevel,
            passed: validationReport.failedRules === 0 &&
     ...
     * Helper methods
     */
    calculateQualityScore(results, _context) {
        if (results.length === 0)
            return 1.0;
        const weights = {
    ...
    generateRecommendations(results, _context) {
    ...
    shouldRecommendRollback(results, qualityScore, context) {
        const criticalFailures = results.filter((r) => r.severity === ValidationSeverity.CRITICAL &&
