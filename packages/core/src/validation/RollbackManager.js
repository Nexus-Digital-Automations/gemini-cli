...
(function (RollbackStrategy) {
  ...
  RollbackStrategy['CONDITIONAL'] = 'conditional'; // Rollback based on conditions
...
(function (RollbackPriority) {
  ...
  RollbackPriority['LOW'] = 'low'; // Rollback during maintenance window
...
export class RollbackManager extends EventEmitter {
  ...
  getTaskSnapshots(taskId) {
    ...
    return snapshotIds.map((id) => this.snapshots.get(id)).filter(Boolean);
  }
  /**
