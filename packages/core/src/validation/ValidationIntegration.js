...
export class ValidationIntegration extends EventEmitter {
    ...
    async validateTaskExecution(task, context, executionMetrics) {
        ...
        try {
            ...
            const validationPromise = this.executeComprehensiveValidation(task, context, executionMetrics, startTime);
     ...
     */
    async executeComprehensiveValidation(task, context, executionMetrics, startTime) {
        const result = {
            taskId: task.id,
    ...
    async executeDuringExecutionMonitoring(task, executionMetrics) {
        ...
        if (executionMetrics) {
            ...
            if (executionMetrics.memoryUsage) {
                ...
                thresholds.push({
                    metric: 'memory_usage',
                    status: memoryMB > memoryThreshold * 1.5 ? 'critical' :
                        memoryMB > memoryThreshold ? 'warning' : 'ok',
                    value: memoryMB,
            ...
            if (executionMetrics.duration) {
                ...
                thresholds.push({
                    ...
                    status: durationMinutes > timeThreshold * 1.2 ? 'critical' :
                        durationMinutes > timeThreshold ? 'warning' : 'ok',
            ...
            thresholds.push({
                metric: 'error_rate',
                status: errorRate > 0.1 ? 'critical' :
                    errorRate > 0.05 ? 'warning' : 'ok',
                value: errorRate,
                threshold: 0.05
            });
        }
    ...
    async evaluateRollbackNeed(task, result, context) {
        ...
        if (this.config.phases.postExecution.autoRollbackOnFailure) {
            try {
                const rollbackOp = this.rollbackManager.createRollbackOperation(task.id, RollbackTrigger.VALIDATION_FAILURE, latestSnapshot.id, 'Automatic rollback due to validation failures');
