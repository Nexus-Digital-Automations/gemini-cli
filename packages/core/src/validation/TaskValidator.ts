...
export class TaskValidator extends EventEmitter {
  ...
  private async executeTaskValidation(
    ...
    const frameworkContext: ValidationContext = {
      taskId: context.task.id,
      metadata: {
        task: context.task,
   ...
   */
  async createTaskSnapshot(task: Task): Promise<TaskSnapshot> {
   ...
   */
  private storeSnapshot(snapshot: TaskSnapshot): void {
   ...
   */
  async executeRollback(taskId: string, snapshotId: string): Promise<boolean> {
