...
import { TaskStore, TransactionIsolationLevel as TIL, BackupStrategy as BS, CompressionAlgorithm, SerializationFormat, } from './TaskStore.js';
/**
...
export class FileBasedTaskStore extends TaskStore {
    ...
    async initialize(config) {
        ...
        try {
            ...
            if (config.indexConfig?.maintenance.maintenanceInterval) {
                setInterval(() => this.maintainIndexes(), config.indexConfig.maintenance.maintenanceInterval);
            }
            // Set up automatic backups
            if (config.backupConfig?.enabled) {
                setInterval(() => this.performAutomaticBackup(), config.backupConfig.interval);
            }
     ...
     */
    async beginTransaction(isolationLevel = TIL.READ_COMMITTED, readOnly = false) {
        ...
        const timeoutHandle = setTimeout(() => {
            this.rollbackTransaction(context).catch(error => this.emit('error', new Error(`Transaction timeout rollback failed: ${error.message}`)));
        }, context.timeout);
     ...
     */
    async updateTask(task, expectedVersion, transaction) {
        const startTime = Date.now();
        this.ensureInitialized();
        try {
            const existingTask = this.storage.tasks[task.id];
     ...
     */
    async deleteTask(taskId, softDelete = true, transaction) {
        const startTime = Date.now();
        this.ensureInitialized();
        try {
    ...
    async queryTasks(options) {
        ...
        try {
            ...
            for (const taskId of taskIds) {
                ...
                if (options.createdAfter && new Date(task.createdAt) < options.createdAfter)
                    continue;
                if (options.createdBefore && new Date(task.createdAt) > options.createdBefore)
                    continue;
                if (options.parentTaskId && task.parentTaskId !== options.parentTaskId)
                    continue;
                if (options.tags && options.tags.length > 0) {
                    if (!options.tags.every(tag => task.tags.includes(tag)))
                        continue;
                }
                // Apply custom filters
                if (options.customFilters && options.customFilters.length > 0) {
                    if (!options.customFilters.every(filter => filter(task)))
                        continue;
                }
                // Apply search query (simple text search)
                if (options.searchQuery) {
                    const searchText = `${task.name} ${task.description}`.toLowerCase();
                    if (!searchText.includes(options.searchQuery.toLowerCase()))
                        continue;
                }
                results.push(task);
            }
     ...
     */
    async verifyIntegrity(fix = false) {
        const startTime = Date.now();
        const issues = [];
        let totalChecked = 0;
        try {
        ...
        for (const [resourceId, change] of transaction.changes) {
            if (change.type === 'task') {
                if (change.operation === 'create' || change.operation === 'update') {
                    this.storage.tasks[resourceId] = change.data;
                    this.updateIndexes(change.data.data);
                }
                ...
                if (change.operation === 'create' || change.operation === 'update') {
                    this.storage.queues[resourceId] = change.data;
                }
                else if (change.operation === 'delete') {
    ...
    async restoreTransactionData(transaction) {
        for (const [resourceId, original] of transaction.originalData) {
            if (original.type === 'task') {
                if (original.data) {
                    this.storage.tasks[resourceId] = original.data;
                    this.updateIndexes(original.data.data);
                }
                else {
                ...
                if (original.data) {
                    this.storage.queues[resourceId] = original.data;
                }
                else {
                    // Original was not present, remove it
                    delete this.storage.queues[resourceId];
     ...
     */
    async createBackup(strategy = BS.FULL, destination) {
        const startTime = Date.now();
        try {
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const backupFileName = `backup-${timestamp}.json`;
            const backupPath = destination || join(this.config.backupConfig?.backupDirectory || dirname(this.storagePath), backupFileName);
            ...
            switch (strategy) {
                ...
                case BS.INCREMENTAL:
                    ...
                    backupData = {
                        ...
                        tasks: Object.fromEntries(Object.entries(this.storage.tasks).filter(([, task]) => new Date(task.metadata.lastModified) > new Date(Date.now() - 24 * 60 * 60 * 1000) // Last 24 hours
                        )),
