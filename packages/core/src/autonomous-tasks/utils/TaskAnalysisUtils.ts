...
export class TaskAnalysisUtils {
  ...
  static analyzeTextualComplexity(task: ITask): TextualComplexity {
    ...
    const technicalTermsCount = this.TECHNICAL_TERMS.filter(term =>
      text.includes(term.toLowerCase())
    ).length;

    // Find complexity keywords
    const complexityKeywords = this.COMPLEXITY_KEYWORDS.filter(keyword =>
    ...
    const readabilityScore = Math.max(0, Math.min(100,
      206.835 - (1.015 * averageWordsPerSentence) - (84.6 * averageCharactersPerWord / 4.7)
    ));
  ...
  static analyzeContextualComplexity(task: ITask): ContextualComplexity {
    ...
    const timeoutComplexity = Math.min(10, Math.max(1,
      Math.log10((context?.timeout || 30000) / 1000)
    ));
   ...
   */
  private static normalizeTextualScore(textual: TextualComplexity): number {
    let score = 0;
   ...
   */
  private static normalizeStructuralScore(structural: StructuralComplexity): number {
    let score = 0;
   ...
   */
  private static normalizeContextualScore(contextual: ContextualComplexity): number {
    let score = 0;
  ...
  private static calculateAnalysisConfidence(
    ...
    if (task.description.length > 50) confidence += 0.1;
    if (task.parameters && Object.keys(task.parameters).length > 0) confidence += 0.1;
    if (task.dependencies.length > 0) confidence += 0.1;
    if (task.context) confidence += 0.1;
    if (analysis.textualComplexity.technicalTermsCount > 0) confidence += 0.1;
   ...
   */
  private static calculateNestingDepth(obj: any): number {
   ...
   */
  private static getTypeComplexity(type: TaskType): number {
   ...
   */
  private static calculateStringSimilarity(strA: string, strB: string): number {
   ...
   */
  private static levenshteinDistance(strA: string, strB: string): number {
    const matrix: number[][] = Array(strB.length + 1).fill(null).map(() =>
   ...
   */
  private static calculateObjectSimilarity(objA: Record<string, unknown>, objB: Record<string, unknown>): number {
   ...
   */
  private static calculatePrioritySimilarity(priorityA: TaskPriority, priorityB: TaskPriority): number {
   ...
   */
  private static calculateDependencySimilarity(depsA: TaskDependency[], depsB: TaskDependency[]): number {
    if (depsA.length === 0 && depsB.length === 0) return 1.0;
    if (depsA.length === 0 || depsB.length === 0) return 0.0;
   ...
   * Group tasks by type
   */
  private static groupTasksByType(tasks: ITask[]): Map<TaskType, ITask[]> {
    const groups = new Map<TaskType, ITask[]>();
   ...
   */
  private static groupTasksByPriority(tasks: ITask[]): Map<TaskPriority, ITask[]> {
    const groups = new Map<TaskPriority, ITask[]>();
   ...
   */
  private static detectNamingPatterns(tasks: ITask[]): TaskPattern[] {
    const patterns: TaskPattern[] = [];
    const prefixGroups = new Map<string, ITask[]>();
    const suffixGroups = new Map<string, ITask[]>();
   ...
   */
  private static normalizeParameters(parameters: Record<string, unknown>): Record<string, unknown> {
