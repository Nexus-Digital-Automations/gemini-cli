...
export class TaskBreakdownEngine extends EventEmitter {
     ...
     */
    async generateSubtasks(parentTask, decomposition, context) {
        const startTime = performance.now();
        try {
            const subtasks = [];
            for (const [index, subtask] of decomposition.subtasks.entries()) {
                // Create subtask with parent relationship
                const enhancedSubtask = await this.createSubtask(parentTask, subtask, index, context);
     ...
     */
    calculateFactorImpact(task, factor) {
        switch (factor.name) {
     ...
     */
    selectOptimalStrategy(task, complexityScore) {
     ...
     */
    detectTaskDependencies(subtasks, parentTask) {
        const dependencies = [];
     ...
     */
    hasLogicalDependency(taskA, taskB) {
        ...
        if (aName.includes('setup') && bName.includes('implement'))
            return true;
        if (aName.includes('implement') && bName.includes('test'))
            return true;
        if (aName.includes('test') && bName.includes('document'))
            return true;
        if (aName.includes('analyze') && bName.includes('implement'))
            return true;
        if (aName.includes('design') && bName.includes('implement'))
            return true;
        return false;
    }
    /**
     ...
     */
    hasResourceDependency(taskA, taskB) {
     ...
     */
    createExecutionPlan(subtasks, dependencies) {
        ...
        const independentTasks = subtasks.filter(task => !dependencies.some(dep => dep.taskId === task.id));
     ...
     */
    findCriticalPath(subtasks, dependencies) {
     ...
     */
    buildDependencyChain(task, dependencyMap, allTasks) {
        const chain = [task];
     ...
     */
    calculateComplexityReduction(parentTask, subtasks) {
     ...
     */
    async createSubtask(parentTask, templateTask, index, context) {
     ...
     */
    generateSubtaskId(parentTaskId, index) {
     ...
     */
    applyIntelligentSizing(subtask, parentTask) {
     ...
     */
    createEmptyDecompositionResult() {
        return {
            subtasks: [],
            dependencies: [],
            executionPlan: {
 ...
 */
class SequentialBreakdownStrategy {
    name = 'sequential';
    description = 'Breaks down tasks into sequential steps with dependencies';
    supportedTypes = [TaskType.FEATURE, TaskType.CODE_GENERATION, TaskType.DEPLOYMENT];
    canBreakdown(task) {
    ...
    async breakdownTask(task, context) {
        ...
        for (const [index, phase] of phases.entries()) {
            subtasks.push({
                ...
                execute: async () => ({ success: true, messages: [], metrics: {} }),
                validate: () => true,
                cancel: async () => true,
                clone: () => task,
                getProgress: () => 0,
 ...
 */
class ParallelBreakdownStrategy {
    name = 'parallel';
    description = 'Breaks down tasks into parallel independent subtasks';
    supportedTypes = [TaskType.TESTING, TaskType.CODE_ANALYSIS, TaskType.DOCUMENTATION];
    canBreakdown(task) {
        return task.dependencies.length === 0 && task.type !== TaskType.DEPLOYMENT;
    }
    async breakdownTask(task, context) {
    ...
    identifyParallelComponents(task) {
 ...
 */
class FeatureBreakdownStrategy {
    name = 'feature';
    description = 'Breaks down features into logical components';
    supportedTypes = [TaskType.FEATURE, TaskType.CODE_GENERATION];
    canBreakdown(task) {
    ...
    mapComponentToTaskType(component) {
        const typeMap = {
    ...
    getComponentDependencies(component, allComponents, parentTaskId) {
        const dependencies = [];
 ...
 */
class LayeredBreakdownStrategy {
    name = 'layered';
    description = 'Breaks down tasks into architectural layers';
    supportedTypes = [TaskType.REFACTORING, TaskType.SECURITY, TaskType.PERFORMANCE];
    canBreakdown(task) {
