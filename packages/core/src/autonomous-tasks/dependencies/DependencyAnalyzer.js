...
export class DependencyAnalyzer {
    ...
    explicitDeps = this.analyzeExplicitDependencies(tasks);
    dependencies;
    // Analyze implicit dependencies if enabled
    if(config, enableImplicitDetection) {
        const implicitDeps = await this.analyzeImplicitDependencies(tasks);
        dependencies.push(...implicitDeps);
    }
    // Analyze resource dependencies
    resourceDeps = this.analyzeResourceDependencies(tasks);
    dependencies;
    // Analyze temporal dependencies
    temporalDeps = this.analyzeTemporalDependencies(tasks);
    dependencies;
    // Analyze priority-based dependencies
    priorityDeps = this.analyzePriorityDependencies(tasks);
    dependencies;
    // Remove duplicates and validate
    uniqueDependencies = this.deduplicateDependencies(dependencies);
    validatedDependencies = this.validateDependencies(uniqueDependencies, tasks);
    // Identify independent and critical tasks
    taskIds = new Set(tasks.map(t =  & gt));
    t;
    id;
    ;
    dependentTaskIds = new Set(validatedDependencies.map(d =  & gt));
    d;
    to;
    ;
    dependencyTaskIds = new Set(validatedDependencies.map(d =  & gt));
    d;
}
from;
;
const independentTasks = Array.from(taskIds).filter(id =  & gt);
!dependentTaskIds.has(id);
;
const criticalTasks = Array.from(dependencyTaskIds).filter(id =  & gt);
validatedDependencies.filter(d =  & gt);
d.from === id;
length & gt;
1;
;
...
const avgConfidence = validatedDependencies.length & gt;
0 ?
    validatedDependencies.reduce((sum, d) =  & gt) : ;
sum + d.confidence, 0;
/ validatedDependencies.length : 1;
const result = {
...
return result;
analyzeExplicitDependencies(tasks, Task[]);
DetectedDependency[];
{
    const logger = this.getLogger();
    const dependencies = [];
    const taskMap = new Map(tasks.map(t =  & gt));
    [t.id, t];
    ;
    for (const task of tasks) {
        if (task.dependencies && task.dependencies.length & gt)
            ;
        0;
        {
...
async;
analyzeImplicitDependencies(tasks, Task[]);
Promise & lt;
DetectedDependency[] & gt;
{
    const logger = this.getLogger();
    const dependencies = [];
    for (let i = 0; i & lt; tasks.length)
        ;
    i++;
    {
        for (let j = i + 1; j & lt; tasks.length)
            ;
        j++;
        {
            const taskA = tasks[i];
            ...
            if (depAB && depAB.confidence & gt)
                ;
            this.config.sensitivity.semantic;
            {
...
async;
analyzeImplicitDependency(from, Task, to, Task);
Promise & lt;
DetectedDependency | null & gt;
{
    const reasons = [];
    let confidence = 0;
    ...
    0;
    {
        return {
            ...
            blocking: confidence & gt, 0.8: ,
            estimatedDelay: this.estimateDelay(from, to) * confidence,
        };
    }
    return null;
}
analyzeKeywordDependency(from, Task, to, Task);
number;
{
    let score = 0;
    ...
    for (const [category, keywords] of this.keywordPatterns) {
        const fromHasKeywords = keywords.some(keyword =  & gt);
        fromText.includes(keyword);
        ;
        const toHasKeywords = keywords.some(keyword =  & gt);
        toText.includes(keyword);
        ;
    ...
    if (fromIndex !== -1 && toIndex !== -1 && fromIndex & lt)
        ;
    toIndex;
    {
        score += 0.4;
...
analyzeStructuralDependency(from, Task, to, Task);
number;
{
    let score = 0;
    ...
    if (fromTypeIndex !== -1 && toTypeIndex !== -1 && fromTypeIndex & lt)
        ;
    toTypeIndex;
    {
        score += 0.5;
    ...
    if (fromPriorityIndex !== -1 && toPriorityIndex !== -1 && fromPriorityIndex & gt)
        ;
    toPriorityIndex;
    {
        score += 0.3;
    ...
    if (from.estimated_effort && to.estimated_effort &&
        from.estimated_effort & gt)
        ;
    to.estimated_effort * 2;
    {
        score += 0.2;
    }
    return Math.min(score, 1.0);
}
analyzeFeatureRelationship(from, Task, to, Task);
number;
{
    let score = 0;
...
analyzeResourceDependencies(tasks, Task[]);
DetectedDependency[];
{
    const dependencies = [];
    const logger = this.getLogger();
    // Group tasks by required capabilities
    const capabilityGroups = new Map & lt;
    string, Task[] & gt;
    ();
    for (const task of tasks) {
    ...
    for (const [capability, capabilityTasks] of capabilityGroups) {
        if (capabilityTasks.length & gt)
            ;
        1;
        {
            // Sort by priority and create sequential dependencies
            const sortedTasks = capabilityTasks.sort((a, b) =  & gt);
            {
            ...
            ;
            for (let i = 0; i & lt; sortedTasks.length - 1)
                ;
            i++;
            {
...
analyzeTemporalDependencies(tasks, Task[]);
DetectedDependency[];
{
    const dependencies = [];
    const logger = this.getLogger();
    const tasksWithDeadlines = tasks.filter(t =  & gt);
    t.metadata?.deadline;
    ;
    if (tasksWithDeadlines.length & lt)
        ;
    2;
    {
        return dependencies;
    ...
    // Sort by deadline
    tasksWithDeadlines.sort((a, b) =  & gt);
    {
    ...
    for (let i = 0; i & lt; tasksWithDeadlines.length - 1)
        ;
    i++;
    {
...
analyzePriorityDependencies(tasks, Task[]);
DetectedDependency[];
{
    const dependencies = [];
    const logger = this.getLogger();
    ...
    const sortedTasks = tasks.sort((a, b) =  & gt);
    (priorityOrder[b.priority] || 0) -
        (priorityOrder[a.priority] || 0);
    ;
    // Create priority-based dependencies for tasks with significant priority differences
    for (let i = 0; i & lt; sortedTasks.length)
        ;
    i++;
    {
        for (let j = i + 1; j & lt; sortedTasks.length)
            ;
        j++;
        {
            const higherPriorityTask = sortedTasks[i];
            const lowerPriorityTask = sortedTasks[j];
            const higherPriorityValue = priorityOrder[higherPriorityTask.priority] || 0;
            const lowerPriorityValue = priorityOrder[lowerPriorityTask.priority] || 0;
            // Only create dependency if priority difference is significant (2+ levels)
            if (higherPriorityValue - lowerPriorityValue & gt)
                ;
            2;
            {
                dependencies.push({
...
deduplicateDependencies(dependencies, DetectedDependency[]);
DetectedDependency[];
{
    const seen = new Set & lt;
    string & gt;
    ();
    const unique = [];
    for (const dep of dependencies) {
        ...
        if (!seen.has(key)) {
            ...
            const existingIndex = unique.findIndex(d =  & gt);
            d.from === dep.from && d.to === dep.to;
            ;
            if (existingIndex !== -1 && dep.confidence & gt)
                ;
            unique[existingIndex].confidence;
            {
...
validateDependencies(dependencies, DetectedDependency[], tasks, Task[]);
DetectedDependency[];
{
    const taskIds = new Set(tasks.map(t =  & gt));
    t.id;
    ;
    return dependencies.filter(dep =  & gt);
    taskIds.has(dep.from) && taskIds.has(dep.to) && dep.from !== dep.to;
    ;
}
detectPotentialCircularDependencies(dependencies, DetectedDependency[]);
string[][];
{
    const graph = new Map & lt;
    string, Set & lt;
    string & gt;
     & gt;
    ();
...
getSequentialIndex(text, string, keywords, string[]);
number;
{
    for (let i = 0; i & lt; keywords.length)
        ;
    i++;
    {
...
estimateDelay(from, Task, to, Task);
number;
{
