...
(function (SequencingStrategy) {
    /** Priority-first sequencing */
    SequencingStrategy["PRIORITY_FIRST"] = "priority_first";
    /** Dependency-aware critical path */
    SequencingStrategy["CRITICAL_PATH"] = "critical_path";
    /** Resource-optimized sequencing */
    SequencingStrategy["RESOURCE_OPTIMIZED"] = "resource_optimized";
    /** Load-balanced parallel execution */
    SequencingStrategy["LOAD_BALANCED"] = "load_balanced";
    /** User-defined custom sequencing */
    SequencingStrategy["CUSTOM"] = "custom";
})(SequencingStrategy || (SequencingStrategy = {}));
/**
 * Default sequencing configuration
 */
const DEFAULT_SEQUENCING_CONFIG = {
...
(function (SequencerEvent) {
    SequencerEvent["SEQUENCE_GENERATED"] = "sequence_generated";
    SequencerEvent["CONFLICT_DETECTED"] = "conflict_detected";
    SequencerEvent["CONFLICT_RESOLVED"] = "conflict_resolved";
    SequencerEvent["OPTIMIZATION_APPLIED"] = "optimization_applied";
    SequencerEvent["SEQUENCE_VALIDATED"] = "sequence_validated";
 ...
 */
export class DependencySequencer extends EventEmitter {
    logger;
    config;
    dependencyAnalyzer;
     ...
     */
    async getParallelExecutionGroups(tasks) {
        const logger = this.getLogger();
        try {
            const sequence = await this.generateExecutionSequence(tasks);
     ...
     */
    async updateTaskDependencies(taskId, dependencies // TaskDependency[] from TaskInterfaces
    ) {
        const logger = this.getLogger();
     ...
     */
    async generateExecutionSequence(tasks) {
     ...
     */
    convertToGraph(tasks, analysisResult) {
        ...
        const edges = analysisResult.dependencies.map(dep => ({
            from: dep.from,
            to: dep.to,
     ...
     */
    async generateSequenceByStrategy(graph, tasks) {
     ...
     */
    generatePriorityFirstSequence(graph, tasks) {
        const taskMap = new Map(tasks.map(t => [t.id, t]));
        const groups = [];
     ...
     */
    generateCriticalPathSequence(graph, tasks) {
        const taskMap = new Map(tasks.map(t => [t.id, t]));
     ...
     */
    generateResourceOptimizedSequence(graph, tasks) {
     ...
     */
    generateLoadBalancedSequence(graph, tasks) {
     ...
     */
    buildSequenceResult(groups, strategy, graph) {
     ...
     */
    async optimizeSequence(sequence, graph) {
        ...
        for (const group of optimized.groups) {
            if (!currentGroup) {
                ...
                currentGroup = {
                    ...
                    resourceRequirements: this.mergeResourceRequirements(currentGroup.resourceRequirements, group.resourceRequirements),
                    priority: Math.min(currentGroup.priority, group.priority),
                    dependencies: currentGroup.dependencies,
                    parallelSafetyConfidence: Math.min(currentGroup.parallelSafetyConfidence, group.parallelSafetyConfidence)
                };
            }
            else {
     ...
     */
    async resolveConflicts(conflicts, tasks) {
    ...
    mergeResourceRequirements(reqA, reqB) {
        const merged = { ...reqA };
...
class ConflictResolver {
    ...
    async resolveConflict(conflict, tasks) {
...
//# sourceMappingURL=DependencySequencer.js.map