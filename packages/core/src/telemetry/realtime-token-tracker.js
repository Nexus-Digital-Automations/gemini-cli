...
export class RealtimeTokenTracker extends EventEmitter {
     ...
     */
    async trackTokenUsage(sessionId, promptId, model, usageMetadata, latencyMs, authType, context) {
        const trackingStart = performance.now();
        try {
            const tokenData = this.createTokenUsageData(sessionId, promptId, model, usageMetadata, latencyMs, authType, context);
     ...
     */
    getRealtimeStatistics(timeRangeMs = 3600000, // 1 hour default
    sessionId) {
        const endTimer = createTimer(logger, 'getRealtimeStatistics', LogLevel.DEBUG);
        try {
            const now = Date.now();
            ...
            if (sessionId) {
                ...
                relevantData.push(...sessionData.filter((d) => d.timestamp >= startTime));
            }
            else {
    ...
    createTokenUsageData(sessionId, promptId, model, usageMetadata, latencyMs, authType, context) {
    ...
    addToBatch(data) {
        ...
        if (this.batchBuffer.length >= this.config.batchSize) {
            ...
            this.batchTimeout = setTimeout(() => this.flushBatch(), this.config.batchTimeoutMs);
        }
    }
    flushBatch() {
    ...
    calculateStatistics(data, startTime, endTime) {
        ...
        const averageLatency = data.reduce((sum, d) => sum + d.latencyMs, 0) / data.length;
