...
export class AnalyticsStorageEngine extends EventEmitter {
    ...
    async storeTokenUsage(data) {
        ...
        if (this.writeBuffer.length >= this.writeBufferMaxSize) {
            ...
            this.writeTimeout = setTimeout(() => this.flushWriteBuffer(), this.writeBufferTimeoutMs);
        }
    }
    /**
    ...
    async getAggregatedStatistics(query) {
        ...
        for (const [groupKey, groupData] of groups.entries()) {
            ...
            const totalCost = groupData.reduce((sum, d) => sum + (d.totalCostUsd || 0), 0);
            const averageLatency = groupData.reduce((sum, d) => sum + d.latencyMs, 0) / groupData.length;
    ...
    async writeToPartition(partitionId, data) {
        ...
        try {
            ...
            const allData = [...existingData, ...data].sort((a, b) => a.timestamp - b.timestamp);
            // Compress and write
    ...
    async readPartition(partitionPath) {
        const compressed = await require('node:fs/promises').readFile(partitionPath);
    ...
    findRelevantPartitions(query) {
        ...
        for (const partition of this.partitionMetadata.values()) {
            ...
            if (query.startTime && partition.endTime < query.startTime)
                continue;
            if (query.endTime && partition.startTime > query.endTime)
                continue;
    ...
    applyQueryFilters(data, query) {
        return data.filter((item) => {
            ...
            if (query.model && item.model !== query.model)
                return false;
            if (query.authType && item.authType !== query.authType)
                return false;
            if (query.command && item.command !== query.command)
                return false;
            if (query.feature && item.feature !== query.feature)
                return false;
            ...
            if (query.endTime && item.timestamp > query.endTime)
                return false;
            if (query.minTokens && item.totalTokens < query.minTokens)
                return false;
            if (query.maxTokens && item.totalTokens > query.maxTokens)
                return false;
    ...
    calculateCompressionRatio(partitions) {
        if (partitions.length === 0)
            return 0;
        const totalCompressed = partitions.reduce((sum, p) => sum + p.compressedSize, 0);
        const totalUncompressed = partitions.reduce((sum, p) => sum + p.uncompressedSize, 0);
    ...
    loadPartitionMetadata() {
        ...
        try {
            if (existsSync(metadataPath)) {
                const data = JSON.parse(require('node:fs').readFileSync(metadataPath, 'utf8'));
                for (const [id, metadata] of Object.entries(data)) {
                    // Convert plain objects back to Maps
                    const metadataObj = metadata;
                    metadataObj.indexes = new Map(Object.entries(metadataObj.indexes || {}).map(([k, v]) => [
                        k,
                        new Set(v),
                    ]));
    ...
    savePartitionMetadata() {
        ...
        try {
            ...
            for (const [id, metadata] of this.partitionMetadata.entries()) {
                ...
                const metadataObj = { ...metadata };
                metadataObj.indexes = Object.fromEntries(Array.from(metadata.indexes.entries()).map(([k, v]) => [
                    k,
                    Array.from(v),
                ]));
                data[id] = metadataObj;
            }
            require('node:fs').writeFileSync(metadataPath, JSON.stringify(data, null, 2));
        }
        catch (error) {
