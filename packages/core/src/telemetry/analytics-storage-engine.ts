...
/**
 * Partition metadata for efficient querying
 */
...
export class AnalyticsStorageEngine extends EventEmitter {
  ...
  private readonly partitionMetadata = new Map<string, PartitionMetadata>();
  private readonly queryCache = new QueryCache();
  private readonly writeBuffer: TokenUsageData[] = [];

  private writeTimeout: NodeJS.Timeout | null = null;
  private readonly writeBufferMaxSize = 1000;
  private readonly writeBufferTimeoutMs = 30000; // 30 seconds
  ...
  async storeTokenUsage(
    data: TokenUsageData | TokenUsageData[],
  ): Promise<void> {
   ...
   */
  async queryTokenUsage(query: AnalyticsQuery): Promise<QueryResult> {
  ...
  async cleanup(): Promise<{
    ...
    const partitionsToDelete: PartitionMetadata[] = [];
  ...
  private async flushWriteBuffer(): Promise<void> {
    ...
    try {
      ...
      const partitionGroups = new Map<string, TokenUsageData[]>();
      ...
      logger.error('Failed to flush write buffer', { error: error as Error });
  ...
  private async writeToPartition(
    ...
    try {
      ...
      let existingData: TokenUsageData[] = [];
  ...
  private buildIndexes(data: TokenUsageData[]): Map<string, Set<string>> {
    ...
    for (const field of indexFields) {
      ...
      for (const item of data) {
        const value = item[field as keyof TokenUsageData];
  ...
  private findRelevantPartitions(query: AnalyticsQuery): PartitionMetadata[] {
    const relevant: PartitionMetadata[] = [];
  ...
  private canUseIndexForQuery(
    ...
    for (const field of indexableFields) {
      const queryValue = query[field as keyof AnalyticsQuery];
      if (queryValue) {
  ...
  private async queryPartition(
    partition: PartitionMetadata,
    query: AnalyticsQuery,
  ): Promise<{ data: TokenUsageData[]; indexUsed: boolean }> {
    try {
  ...
  private calculateCompressionRatio(partitions: PartitionMetadata[]): number {
  ...
  private createCsvStream(data: TokenUsageData[]): NodeJS.ReadableStream {
  ...
  private createNdjsonStream(data: TokenUsageData[]): NodeJS.ReadableStream {
    const { Readable } = require('node:stream');
  ...
  private loadPartitionMetadata(): void {
    ...
    try {
      if (existsSync(metadataPath)) {
        ...
        for (const [id, metadata] of Object.entries(data)) {
          ...
          this.partitionMetadata.set(id, metadataObj as PartitionMetadata);
        }
  ...
  private savePartitionMetadata(): void {
    const metadataPath = join(this.basePath, 'partitions.json');
 ...
 */
let globalStorageEngine: AnalyticsStorageEngine | null = null;

/**
 * Get or create the global AnalyticsStorageEngine instance
 */
export function getGlobalStorageEngine(
 ...
 */
export function resetGlobalStorageEngine(): void {
