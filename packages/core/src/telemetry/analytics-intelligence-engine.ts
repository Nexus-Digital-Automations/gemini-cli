...
class StatisticalAnalyzer {
  ...
  static detectSeasonality(
    values: number[],
    period: number = 24,
  ): { isPresent: boolean; strength: number; pattern: number[] } {
...
class PerformancePredictor {
  private readonly modelWeights = new Map<string, number>();
  private readonly historicalData: TokenUsageData[] = [];

  /**
   * Train predictor with historical data
   */
  train(data: TokenUsageData[]): void {
    ...
    for (const feature of features) {
      const values = data.map(
        (d) => (d[feature as keyof TokenUsageData] as number) || 0,
      );
  ...
  private findSimilarSamples(context: {
    model: string;
    promptLength: number;
    timeOfDay: number;
    sessionHistoryLength: number;
  ...
  private calculateSimilarityWeight(
    sample: TokenUsageData,
    context: any,
  ): number {
...
export class AnalyticsIntelligenceEngine extends EventEmitter {
  private readonly predictor = new PerformancePredictor();
  private readonly detectedPatterns: UsagePattern[] = [];
  private readonly lastAnalysisTime = new Map<string, number>();

  private analysisInterval: NodeJS.Timeout | null = null;
  private readonly minAnalysisIntervalMs = 60000; // 1 minute
  ...
  async getDashboardInsights(
    ...
    statistics: {
      ...
      costTrend: { slope: number; isIncreasing: boolean };
      anomalyCount: number;
    };
  }> {
    const endTimer = createTimer(
  ...
  private async detectTokenAnomalies(
    data: TokenUsageData[],
    sessionId?: string,
  ): Promise<UsagePattern[]> {
  ...
  private async detectModelSwitchingPatterns(
    data: TokenUsageData[],
    sessionId?: string,
  ): Promise<UsagePattern[]> {
    // Group by session to detect frequent model switching
    const sessionGroups = new Map<string, TokenUsageData[]>();
    ...
    const patterns: UsagePattern[] = [];
    const frequentSwitchers: string[] = [];
  ...
  private analyzeModelUsageEfficiency(
    ...
    const modelEfficiency = new Map<
      string,
      { costPerToken: number; avgLatency: number; usage: number }
    >();
  ...
  private analyzeBatchingOpportunities(
    ...
    const requestsByHour = new Map<number, TokenUsageData[]>();
    ...
    const highFrequencyHours: number[] = [];
  ...
  private getRecentPatterns(sessionId?: string): UsagePattern[] {
  ...
  private setupRealtimeAnalysis(): void {
    ...
    this.tokenTracker.on('token_batch', async (event) => {
      try {
        ...
        logger.error('Realtime analysis failed', { error: error as Error });
      }
    });
    ...
    this.analysisInterval = setInterval(
      async () => {
        try {
          ...
          logger.error('Periodic analysis failed', { error: error as Error });
        }
      },
      10 * 60 * 1000,
    ); // Every 10 minutes
  ...
  private setupBackgroundTraining(): void {
    ...
    setInterval(
      async () => {
        try {
          ...
          logger.error('Background training failed', { error: error as Error });
        }
      },
      60 * 60 * 1000,
    ); // Every hour
 ...
 */
let globalIntelligenceEngine: AnalyticsIntelligenceEngine | null = null;
 ...
 */
export function resetGlobalIntelligenceEngine(): void {
  if (globalIntelligenceEngine) {
