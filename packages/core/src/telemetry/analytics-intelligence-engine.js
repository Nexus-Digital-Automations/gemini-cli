...
class StatisticalAnalyzer {
    ...
    static detectSeasonality(values, period = 24) {
        ...
        const seasonalVariance = seasonalPattern.reduce((sum, val) => sum + Math.pow(val - overallMean, 2), 0) / period;
...
class PerformancePredictor {
    ...
    predict(context) {
        ...
        if (similarSamples.length === 0) {
            ...
            const avgLatency = allValues.reduce((sum, d) => sum + d.latencyMs, 0) / allValues.length;
            const avgCost = allValues.reduce((sum, d) => sum + (d.totalCostUsd || 0), 0) /
                allValues.length;
            const avgTokens = allValues.reduce((sum, d) => sum + d.totalTokens, 0) / allValues.length;
            return {
        ...
        return {
            predictedLatencyMs: totalWeight > 0 ? weightedLatency / totalWeight : 1000,
...
export class AnalyticsIntelligenceEngine extends EventEmitter {
    ...
    async analyzeUsagePatterns(sessionId, timeRangeMs = 3600000) {
        const endTimer = createTimer(logger, 'analyzeUsagePatterns', LogLevel.DEBUG);
        ...
        try {
            ...
            const { data: recentData } = await this.storageEngine.queryTokenUsage(query);
            ...
            patterns.push(...(await this.detectLatencyAnomalies(recentData, sessionId)));
            patterns.push(...(await this.detectErrorRateAnomalies(recentData, sessionId)));
            patterns.push(...(await this.detectTokenAnomalies(recentData, sessionId)));
            patterns.push(...(await this.detectModelSwitchingPatterns(recentData, sessionId)));
            ...
            this.detectedPatterns.push(...this.detectedPatterns.filter((p) => p.detectedAt > cutoff));
            return patterns;
        }
        finally {
            endTimer();
     ...
     */
    async generateOptimizationRecommendations(sessionId, timeRangeMs = 24 * 60 * 60 * 1000) {
        const endTimer = createTimer(logger, 'generateOptimizationRecommendations', LogLevel.DEBUG);
        try {
            const query = {
                sessionId,
                startTime: Date.now() - timeRangeMs,
            ...
            const { data: usageData } = await this.storageEngine.queryTokenUsage(query);
            ...
            recommendations.push(...this.analyzeModelUsageEfficiency(usageData, aggregated));
            ...
            recommendations.sort((a, b) => b.estimatedSavingsUsd - a.estimatedSavingsUsd);
    ...
    predictPerformance(context) {
        ...
        const sessionHistory = this.tokenTracker.getSessionData(context.sessionId || 'unknown', 100);
    ...
    async getDashboardInsights(timeRangeMs = 24 * 60 * 60 * 1000) {
        const endTimer = createTimer(logger, 'getDashboardInsights', LogLevel.DEBUG);
        try {
    ...
    async detectCostAnomalies(data, sessionId) {
        ...
        const anomalies = StatisticalAnalyzer.detectAnomalies(costs, this.config.alertThresholds.costSpikeMultiplier);
        ...
        return [
            {
                type: 'cost_spike',
                severity: maxAnomaly > 4 ? 'critical' : maxAnomaly > 3 ? 'high' : 'medium',
                ...
                statistics: {
                    baseline: stats.mean,
                    current: currentCost,
                    percentageChange: stats.mean > 0
                        ? ((currentCost - stats.mean) / stats.mean) * 100
                        : 0,
                    confidence: Math.min(maxAnomaly / 5, 1.0),
                },
                recommendations: await this.generateOptimizationRecommendations(sessionId, 3600000),
                metadata: {
    ...
    async detectLatencyAnomalies(data, sessionId) {
        const latencies = data.map((d) => d.latencyMs);
        if (latencies.length < 10)
            return [];
        const stats = StatisticalAnalyzer.calculateStats(latencies);
        const anomalies = StatisticalAnalyzer.detectAnomalies(latencies, this.config.alertThresholds.latencyMultiplier);
        if (anomalies.indices.length === 0)
            return [];
        const affectedSessions = new Set();
        const affectedModels = new Set();
    ...
    async detectErrorRateAnomalies(data, sessionId) {
        const errorRate = data.reduce((sum, d) => sum + d.errorCount, 0) / data.length;
        if (errorRate > this.config.alertThresholds.errorRateThreshold) {
            const affectedSessions = [
                ...new Set(data.filter((d) => d.errorCount > 0).map((d) => d.sessionId)),
            ];
            ...
            return [
                {
                    ...
                    statistics: {
                        ...
                        current: errorRate,
                        percentageChange: ((errorRate - this.config.alertThresholds.errorRateThreshold) /
    ...
    async detectTokenAnomalies(data, sessionId) {
        ...
        const anomalies = StatisticalAnalyzer.detectAnomalies(tokens, this.config.alertThresholds.tokenAnomalyStdDev);
        if (anomalies.indices.length === 0)
            return [];
        return [
            {
                ...
                detectedAt: Date.now(),
                affectedSessions: [
                    ...new Set(anomalies.indices.map((i) => data[i]?.sessionId).filter(Boolean)),
                ],
                affectedModels: [
                    ...new Set(anomalies.indices.map((i) => data[i]?.model).filter(Boolean)),
                ],
                statistics: {
                    baseline: stats.mean,
    ...
    async detectModelSwitchingPatterns(data, sessionId) {
        ...
        for (const [sessionId, sessionData] of sessionGroups.entries()) {
            if (sessionData.length < 5)
                continue; // Need minimum requests
        ...
        if (frequentSwitchers.length > 0) {
            ...
            patterns.push({
                ...
                recommendations: await this.generateOptimizationRecommendations(undefined, 3600000),
                metadata: {
    ...
    analyzeModelUsageEfficiency(data, aggregated) {
        ...
        const sortedModels = Array.from(modelEfficiency.entries()).sort((a, b) => b[1].costPerToken - a[1].costPerToken);
        if (sortedModels.length >= 2) {
            ...
            if (mostExpensive[1].costPerToken > cheapest[1].costPerToken * 2) {
                const potentialSavings = (mostExpensive[1].costPerToken - cheapest[1].costPerToken) *
                ...
                recommendations.push({
                    ...
                    estimatedSavingsPercentage: ((mostExpensive[1].costPerToken - cheapest[1].costPerToken) /
    ...
    analyzePromptEfficiency(data) {
        ...
        const avgRatio = inputOutputRatios.reduce((sum, r) => sum + r.ratio, 0) /
            inputOutputRatios.length;
        // If average ratio is very low, suggest prompt optimization
        if (avgRatio < 0.1) {
            // Less than 10% output tokens
            const potentialSavings = data.reduce((sum, d) => sum + (d.totalCostUsd || 0), 0) * 0.2; // Estimate 20% savings
            recommendations.push({
                type: 'prompt_optimization',
                priority: 'medium',
                title: 'Optimize prompt efficiency',
                description: 'Your prompts generate relatively little output compared to input. Consider more focused prompts.',
                estimatedSavingsUsd: potentialSavings,
    ...
    analyzeCachingOpportunities(data) {
        ...
        if (cacheRatio < 0.05 && data.length > 50) {
            ...
            const potentialSavings = data.reduce((sum, d) => sum + (d.totalCostUsd || 0), 0) * 0.15; // Estimate 15% savings
            recommendations.push({
                type: 'caching_strategy',
                priority: 'high',
                title: 'Implement content caching',
                description: 'Low cache usage detected. Implement caching for repeated prompts and context.',
                estimatedSavingsUsd: potentialSavings,
    ...
    analyzeRateLimitingOpportunities(data) {
        ...
        if (requestIntervals.length > 0) {
            ...
            if (stats.p95 < 1000) {
                ...
                const burstRequests = requestIntervals.filter((interval) => interval < 100).length; // Within 100ms
    ...
    getRecentPatterns(sessionId) {
        const filtered = sessionId
            ? this.detectedPatterns.filter((p) => p.affectedSessions.includes(sessionId))
    ...
    setupBackgroundTraining() {
        ...
        setInterval(async () => {
            try {
                const { data: trainingData } = await this.storageEngine.queryTokenUsage({
 ...
 */
export function getGlobalIntelligenceEngine(tokenTracker, storageEngine, config) {
    if (!globalIntelligenceEngine) {
        ...
        globalIntelligenceEngine = new AnalyticsIntelligenceEngine(tokenTracker, storageEngine, defaultConfig);
    }
