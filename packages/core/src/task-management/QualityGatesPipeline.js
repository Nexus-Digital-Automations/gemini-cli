...
export class QualityGatesPipeline {
    ...
    constructor(config, validationEngine, pipelineConfig) {
        this.config = config;
        ...
        this.builtInRules.set('code_complexity', {
            ...
            validator: async (task, result, context) => {
                ...
                try {
                    ...
                    return {
                        passed: averageComplexity <= complexityThreshold &&
                            highComplexityFiles === 0,
                        status: averageComplexity <= complexityThreshold &&
                            highComplexityFiles === 0
                            ? 'passed'
                            : 'warning',
        ...
        this.builtInRules.set('sast_scan', {
            ...
            validator: async (task, result, context) => {
                ...
                try {
                    ...
                    const semgrepResult = await this.runCommand('semgrep --config=p/security-audit --json .', { timeout: 300000, ignoreExitCode: true });
                    let findings;
                    try {
                    ...
                    const criticalFindings = findings.filter((f) => f.extra?.severity === 'ERROR').length;
                    const highFindings = findings.filter((f) => f.extra?.severity === 'WARNING').length;
     ...
     */
    evaluateCompletionCriterion(criterion, task, result) {
...
//# sourceMappingURL=QualityGatesPipeline.js.map