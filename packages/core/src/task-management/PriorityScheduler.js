...
(function (SchedulingAlgorithm) {
    ...
    SchedulingAlgorithm["HYBRID_ADAPTIVE"] = "hybrid";
...
export class PriorityScheduler extends EventEmitter {
    ...
    async applySJFScheduling(tasks, slots, context) {
        ...
        const weightedTasks = sortedTasks.map((task) => ({
            task,
            adjustedDuration: (task.estimatedDuration || 60000) /
    ...
    getHistoricalSuccessRate(task) {
        const similarDecisions = this.learningData.filter((data) => data.decision.selectedTasks.some((t) => t.category === task.category && t.priority === task.priority));
    ...
    scoreSchedulingOption(option, context) {
        ...
        // Risk penalty
        const riskPenalty = option.expectedOutcome.riskAssessment === 'high'
            ? 50
        ...
        const highRiskTasks = tasks.filter((t) => t.priority === TaskPriority.CRITICAL ||
            ...
            (t.deadline &&
                t.deadline.getTime() - Date.now() <
                    (t.estimatedDuration || 60000) * 2)).length;
        const riskAssessment = highRiskTasks > tasks.length * 0.5
            ? 'high'
            : highRiskTasks > 0
                ? 'medium'
                : 'low';
    ...
    generatePriorityReasoning(scoredTasks, selectedTasks) {
        ...
        const avgSelectedScore = selectedScores.reduce((sum, st) => sum + st.score, 0) /
