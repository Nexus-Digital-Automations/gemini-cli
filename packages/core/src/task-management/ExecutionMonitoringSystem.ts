...
import { TaskComplexity } from './TaskExecutionEngine.js';
import type { Task, TaskType, TaskPriority } from './TaskExecutionEngine.js';
...
export class ExecutionMonitoringSystem {
  ...
  private readonly metricsHistory: ExecutionMetrics[] = [];
  private readonly eventLog: TaskExecutionEvent[] = [];
  ...
  // Real-time tracking
  private readonly runningTasks: Map<string, TaskExecutionEvent> = new Map();
  private readonly completedTasks: Map<string, TaskExecutionEvent> = new Map();
  private readonly failedTasks: Map<string, TaskExecutionEvent> = new Map();

  // Performance analysis
  private lastPerformanceAnalysis?: Date;
  private currentBottlenecks: BottleneckAnalysis[] = [];
   ...
   */
  async collectMetrics(tasks: Task[]): Promise<ExecutionMetrics> {
    const now = new Date();
