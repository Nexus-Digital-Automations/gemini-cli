...
export class DependencyAnalysisEngine {
  ...
  async validateGraph(
    ...
    try {
      ...
      const validationResult: GraphValidationStatus = {
  ...
  async detectCircularDependencies(
    ...
    const indices = new Map<DependencyNodeId, number>();
    const lowLinks = new Map<DependencyNodeId, number>();
    let index = 0;

    const tarjan = async (nodeId: DependencyNodeId): Promise<void> => {
      ...
      if (lowLinks.get(nodeId) === indices.get(nodeId)) {
        const stronglyConnectedComponent: DependencyNodeId[] = [];
        let w: DependencyNodeId;
  ...
  async identifyParallelGroups(
    ...
    const processed = new Set<DependencyNodeId>();
    let level = 0;

    while (processed.size < executionOrder.length) {
      const currentLevelNodes: DependencyNodeId[] = [];
  ...
  async calculateCriticalPath(
    ...
    const executionOrder = await this.topologicalSort(graph);
    const earlyStart = new Map<DependencyNodeId, number>();
    const earlyFinish = new Map<DependencyNodeId, number>();
    const lateStart = new Map<DependencyNodeId, number>();
    const lateFinish = new Map<DependencyNodeId, number>();
    ...
    const criticalPathNodes: DependencyNodeId[] = [];
  ...
  private async suggestBreakingPoints(
    ...
    for (const edgeId of cycleEdges) {
      ...
      let cost = 1;
      if (edge.constraint.strength === 'hard') cost = 10;
      else if (edge.constraint.strength === 'soft') cost = 5;
      else if (edge.constraint.strength === 'hint') cost = 1;
   ...
   */
  private async findLongestPath(graph: DependencyGraph): Promise<number> {
    const topologicalOrder = await this.topologicalSort(graph);
    const distances = new Map<DependencyNodeId, number>();
