...
(function (ResolutionStrategy) {
    ...
    ResolutionStrategy["PARALLEL_OPTIMIZED"] = "parallel_optimized"; // Optimize for maximum parallelization
...
export class DependencyResolver extends EventEmitter {
    ...
    areTasksCompatible(task1, task2) {
        ...
        const sharedResources = task1.requiredResources.filter(r => task2.requiredResources.includes(r));
        ...
        const hasConflict = task1Dependencies.some(dep => (dep.dependsOnTaskId === task2.id || dep.dependentTaskId === task2.id) &&
            dep.type === 'conflicts') || task2Dependencies.some(dep => (dep.dependsOnTaskId === task1.id || dep.dependentTaskId === task1.id) &&
            dep.type === 'conflicts');
        return !hasConflict;
    }
    /**
    ...
    calculateEstimatedDuration() {
        ...
        for (const [level, tasks] of executionLevels) {
            ...
            const levelDuration = Math.max(...tasks.map(taskId => this.dependencyGraph.get(taskId).task.estimatedDuration));
     ...
     */
    resolveDependencyConflicts(conflicts, strategy = ResolutionStrategy.BEST_EFFORT) {
