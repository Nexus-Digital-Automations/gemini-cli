...
export class QueuePersistenceManager extends EventEmitter {
  private config: Required<QueuePersistenceConfig>;
   ...
   */
  async loadQueueState(snapshotId?: string): Promise<{
    tasks: Map<string, Task>;
    dependencies: Map<string, TaskDependency>;
    executionRecords: Map<string, TaskExecutionRecord>;
    metrics: QueueMetrics;
  ...
  async getRecoveryInfo(): Promise<RecoveryInfo> {
    try {
      ...
      let lastSuccessfulSave: Date | undefined;
   ...
   */
  private async loadLatestSnapshot(): Promise<QueueSnapshot | null> {
    const snapshotsDir = join(this.config.persistenceDir, 'snapshots');
   ...
   * Validate snapshot integrity
   */
  private async validateSnapshot(snapshot: QueueSnapshot): Promise<{
    isValid: boolean;
    errors: string[];
    warnings: string[];
  }> {
   ...
   */
  private calculateSnapshotHash(snapshot: QueueSnapshot): string {
    const hashableData = {
