...
export class DependencyGraphManager {
    ...
    removeDependency(dependentTaskId, dependsOnTaskId) {
        ...
        if (dependentNode) {
            ...
            dependentNode.dependencyRelations = dependentNode.dependencyRelations.filter(rel => rel.dependsOnTaskId !== dependsOnTaskId);
        }
        if (dependencyNode) {
        ...
        const dfs = (taskId) => {
            if (recursionStack.has(taskId)) {
                ...
                for (let i = 0; i < cycle.length - 1; i++) {
                    const edge = this.graph.edges.find(e => e.dependentTaskId === cycle[i + 1] && e.dependsOnTaskId === cycle[i]);
                    if (edge) {
     ...
     */
    generateResolutionStrategies(cycle, edges) {
        const strategies = [];
        ...
        const mostComplexTask = taskComplexities.reduce((max, current) => current.complexity > max.complexity ? current : max);
