...
interface PerformanceTestConfig {
  ...
  timeoutMs: number;
  expectedThroughputMinimum: number; // tasks per second
  expectedLatencyMaximum: number; // milliseconds
}
...
class PerformanceTestRunner {
  ...
  async runPerformanceTest(
    ...
    const endMeasurement =
      this.performanceTracker.startMeasurement('total_test');
  ...
  validatePerformanceResult(
    ...
    if (result.throughput < config.expectedThroughputMinimum) {
      issues.push(
        `Throughput too low: ${result.throughput.toFixed(2)} < ${config.expectedThroughputMinimum} tasks/sec`,
      );
    }
    ...
    if (result.averageLatency > config.expectedLatencyMaximum) {
      issues.push(
        `Latency too high: ${result.averageLatency.toFixed(2)} > ${config.expectedLatencyMaximum} ms`,
      );
    }
    ...
    if (result.peakConcurrency > config.concurrency + 2) {
      // Allow some variance
      issues.push(
        `Peak concurrency exceeded limits: ${result.peakConcurrency} > ${config.concurrency + 2}`,
      );
    }
  ...
  describe('Throughput Benchmarks', () => {
    it('should handle high-throughput scenarios (1000 tasks)', async () => {
      ...
      try {
        ...
        const validation = performanceRunner.validatePerformanceResult(
          result,
          config,
        );
        ...
        expect(result.throughput).toBeGreaterThan(
          config.expectedThroughputMinimum,
        );
      } finally {
    ...
    it('should scale with increased concurrency', async () => {
      ...
      for (const concurrency of concurrencyLevels) {
        ...
        try {
          ...
          console.log(
            `Concurrency ${concurrency}: ${result.throughput.toFixed(2)} tasks/sec`,
          );
        } finally {
          await queue.shutdown();
        }
  ...
  describe('Latency Benchmarks', () => {
    it('should maintain low latency under normal load', async () => {
      ...
      try {
        ...
        const p50Latency =
          latencyMeasurements[Math.floor(latencyMeasurements.length * 0.5)];
        const p95Latency =
          latencyMeasurements[Math.floor(latencyMeasurements.length * 0.95)];
        const p99Latency =
          latencyMeasurements[Math.floor(latencyMeasurements.length * 0.99)];
    ...
    it('should cleanup resources after task completion', async () => {
      ...
      try {
        ...
        for (let cycle = 0; cycle < 3; cycle++) {
          ...
          console.log(
            `Cycle ${cycle + 1}: ${cycleStatus.tasksProcessed} tasks completed`,
          );
  ...
  describe('Monitoring Performance Impact', () => {
    it('should have minimal performance impact with monitoring enabled', async () => {
      ...
      try {
        ...
        // Test without monitoring
        const withoutMonitoringConfig: PerformanceTestConfig = {
          taskCount,
          concurrency: 10,
          executionTimeMs: 20,
          timeoutMs: 30000,
        ...
        const resultWithoutMonitoring =
          await performanceRunner.runPerformanceTest(
            queueWithoutMonitoring,
            withoutMonitoringConfig,
          );
        ...
        const resultWithMonitoring = await performanceRunner.runPerformanceTest(
          queueWithMonitoring,
          withoutMonitoringConfig,
        );
        ...
        const throughputRatio =
          resultWithMonitoring.throughput / resultWithoutMonitoring.throughput;
        const latencyRatio =
          resultWithMonitoring.averageLatency /
    ...
    it('should improve performance over time with optimization enabled', async () => {
      ...
      try {
        ...
        for (let batch = 0; batch < 3; batch++) {
          ...
          console.log(
            `Batch ${batch + 1}: ${batchThroughput.toFixed(2)} tasks/sec`,
          );
        ...
        expect(throughputMeasurements[2]).toBeGreaterThan(
          throughputMeasurements[0] * 0.8,
        );
      } finally {
        await queue.shutdown();
  ...
  describe('Stress Tests', () => {
    ...
    it('should maintain stability during rapid task additions', async () => {
      ...
      try {
        ...
        // Rapidly add tasks
        const additionPromises = Array.from(
          { length: totalTasks },
          async (_, i) => {
            try {
              await queue.addTask(
        ...
        console.log(
          `Rapid Addition: ${successfulAdditions}/${totalTasks} tasks added in ${addDuration}ms`,
        );
        ...
        expect(finalStatus.tasksProcessed).toBeGreaterThan(
          successfulAdditions * 0.9,
