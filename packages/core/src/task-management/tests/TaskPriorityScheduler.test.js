...
describe('TaskPriorityScheduler', () => {
    ...
    describe('Scheduling Algorithms', () => {
        ...
        describe('Dependency-Aware Scheduling', () => {
            ...
            it('should respect task dependencies', () => {
                ...
                const sequence = scheduler.scheduleSequence(tasks);
                expect(sequence.sequence.indexOf(tasks[0].id)).toBeLessThan(sequence.sequence.indexOf(tasks[1].id));
                expect(sequence.sequence.indexOf(tasks[1].id)).toBeLessThan(sequence.sequence.indexOf(tasks[2].id));
            });
            it('should identify parallel execution groups', () => {
                ...
                expect(sequence.parallelGroups[1]).toEqual(expect.arrayContaining([tasks[1].id, tasks[2].id]));
    ...
    describe('Edge Cases and Performance', () => {
        ...
        it('should scale efficiently with large task sets', () => {
            const largeTasks = Array.from({ length: 1000 }, (_, i) => createMockTask(`task-${i}`, ['critical', 'high', 'medium', 'low'][i % 4]));
...
function createMockTask(id, priority = 'medium') {
    return {
        id,
        title: `Task ${id}`,
...
//# sourceMappingURL=TaskPriorityScheduler.test.js.map