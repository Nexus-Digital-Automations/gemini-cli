...
describe('TaskExecutionEngine', () => {
    ...
    beforeEach(() => {
        ...
        taskEngine = new TaskExecutionEngine(mockConfig, {
    ...
    describe('Task Queueing and Creation', () => {
        it('should queue a task with default parameters', async () => {
            const taskId = await taskEngine.queueTask('Test Task', 'A simple test task description');
        ...
        it('should queue a task with custom options', async () => {
            const taskId = await taskEngine.queueTask('Complex Feature Implementation', 'Implement a comprehensive user authentication system with JWT tokens', {
                type: 'implementation',
                priority: 'high',
                expectedOutputs: {
        ...
        it('should automatically determine task complexity', async () => {
            // Simple task
            const simpleTaskId = await taskEngine.queueTask('Fix typo', 'Fix a typo in documentation');
            const simpleTask = taskEngine.getTask(simpleTaskId);
            expect(simpleTask?.complexity).toBe('trivial');
            // Complex task
            const complexTaskId = await taskEngine.queueTask('Implement Multi-Agent Distributed System', 'Build a comprehensive distributed system with microservices architecture, real-time monitoring, scalable message queues, automated failover mechanisms, comprehensive security framework, performance optimization, and enterprise-grade logging and analytics platform.');
    ...
    describe('Task Retrieval and Filtering', () => {
        ...
        it('should retrieve all tasks', () => {
            ...
            expect(allTasks.map((t) => t.title)).toEqual(expect.arrayContaining(['Task 1', 'Task 2', 'Task 3']));
        });
...
describe('TaskBreakdownAnalyzer', () => {
    let analyzer;
    beforeEach(() => {
        analyzer = new TaskBreakdownAnalyzer(mockConfig);
    });
    describe('Complexity Analysis', () => {
        it('should identify trivial complexity', async () => {
            const complexity = await analyzer.analyzeComplexity('Fix typo', 'Fix a typo in the documentation');
            expect(complexity).toBe('trivial');
        });
        it('should identify simple complexity', async () => {
            const complexity = await analyzer.analyzeComplexity('Update button color', 'Change the primary button color from blue to green');
            expect(complexity).toBe('simple');
        });
        it('should identify moderate complexity', async () => {
            const complexity = await analyzer.analyzeComplexity('Add user validation', 'Implement comprehensive user input validation for the registration form including email format validation, password strength checks, and duplicate email detection');
            expect(['moderate', 'simple']).toContain(complexity);
        });
        it('should identify complex/enterprise complexity', async () => {
            const complexity = await analyzer.analyzeComplexity('Build distributed microservices architecture', 'Design and implement a comprehensive distributed system with microservices architecture, including service discovery, load balancing, circuit breakers, distributed tracing, centralized logging, metrics collection, automated deployment pipelines, security authentication and authorization, data consistency patterns, and fault tolerance mechanisms.');
        ...
        it('should handle edge cases', async () => {
            ...
            const complexity3 = await analyzer.analyzeComplexity('This is a very long task title that describes a complex feature implementation', 'Do it');
...
describe('TaskExecutionUtils', () => {
    ...
    describe('Status Descriptions', () => {
        it('should provide human-readable status descriptions', () => {
            expect(TaskExecutionUtils.getStatusDescription('queued')).toBe('Waiting in queue for analysis');
            expect(TaskExecutionUtils.getStatusDescription('in_progress')).toBe('Currently executing');
            expect(TaskExecutionUtils.getStatusDescription('completed')).toBe('Successfully completed');
            expect(TaskExecutionUtils.getStatusDescription('failed')).toBe('Failed execution');
        });
    });
...
function createMockTask(id, status, durationMs) {
    const now = new Date();
...
//# sourceMappingURL=TaskExecutionEngine.test.js.map