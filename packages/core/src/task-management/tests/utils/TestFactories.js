...
export class TestFactories {
     ...
     */
    static createMockDependency(dependentTaskId, dependsOnTaskId, overrides = {}) {
        return {
            dependentTaskId,
            dependsOnTaskId,
            type: 'hard',
    ...
    static createMockDependencyGraph(tasks, dependencies, overrides = {}) {
        ...
        for (const task of tasks) {
            nodes.set(task.id, {
                ...
                dependencyRelations: dependencies.filter((dep) => dep.dependentTaskId === task.id || dep.dependsOnTaskId === task.id),
            });
        }
        const baseGraph = {
            nodes,
    ...
    static createMockExecutionSequence(tasks, overrides = {}) {
        ...
        const baseSequence = {
            ...
            parallelGroups,
            criticalPath,
            estimatedDuration: tasks.reduce((sum, task) => sum + (task.metadata.estimatedDuration || 60000), 0),
            metadata: {
    ...
    static createPerformanceScenarios() {
        return {
             ...
             */
            highVolume: (count = 1000) => this.createMockTasks(count, {
                category: TaskCategory.FEATURE,
                priority: TaskPriority.MEDIUM,
                metadata: {
                    createdAt: new Date(),
             ...
             */
            resourceIntensive: (count = 50) => this.createMockTasks(count).map((task) => ({
                ...task,
                executionContext: {
            ...
            mixedPriority: (count = 200) => {
                ...
                return this.createMockTasks(count).map((task, index) => ({
                    ...
                    metadata: {
                        ...task.metadata,
                        estimatedDuration: task.priority === TaskPriority.CRITICAL
                            ? 30000
             ...
             * Long-running tasks
             */
            longRunning: (count = 20) => this.createMockTasks(count).map((task) => ({
                ...task,
...
export class MockDataGenerators {
    ...
    static generateTaskDescriptions(count) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            template = template.replace('{issue}', TestUtils.randomElement([
                'performance bottleneck',
                'security vulnerability',
                'user complaints',
            ]));
            template = template.replace('{deps}', TestUtils.randomElement([
                'database migration',
                'API changes',
                'frontend updates',
            ]));
            template = template.replace('{feature}', TestUtils.randomElement([
                'authentication',
                'search',
                'reporting',
                'analytics',
            ]));
            template = template.replace('{requirement}', TestUtils.randomElement([
                'mobile support',
                'real-time updates',
                'batch processing',
            ]));
