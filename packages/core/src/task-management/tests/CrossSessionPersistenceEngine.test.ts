...
} from '../CrossSessionPersistenceEngine.js';
...
describe('CrossSessionPersistenceEngine', () => {
  let engine: CrossSessionPersistenceEngine;
  let tempDir: string;
  let config: CrossSessionConfig;
  let mockTask: ITask;
  ...
  describe('Initialization and Session Management', () => {
    ...
    it('should load existing sessions on initialization', async () => {
      ...
      mockReaddir.mockResolvedValue(['session-existing-session.json'] as any);
  ...
  describe('Checkpoint Management', () => {
    ...
    it('should create automatic checkpoints based on operation count', async () => {
      ...
      vi.spyOn(engine as any, 'shouldCreateCheckpoint').mockReturnValue(true);
    ...
    it('should restore from checkpoint', async () => {
      ...
      vi.spyOn(engine as any, 'dataIntegrityManager').mockReturnValue({
  ...
  describe('Crash Recovery', () => {
    ...
    it('should detect crashed sessions on initialization', async () => {
      ...
      const crashedSession: SessionMetadata = {
      ...
      mockReaddir.mockResolvedValue(['session-crashed-session.json'] as any);
      mockReadFile.mockImplementation(async (path) => {
    ...
    it('should create emergency checkpoint on crash', async () => {
      ...
      await (engine as any).handleCrash(error);
    ...
    it('should recover from crashed session checkpoint', async () => {
      ...
      mockReaddir.mockImplementation(async (path) => {
        if (path.toString().includes('checkpoints')) {
          return ['checkpoint-crashed-checkpoint.json'] as any;
        }
        return ['session-crashed-session.json'] as any;
      });
  ...
  describe('Data Integrity and Validation', () => {
    ...
    it('should validate checkpoint integrity during restoration', async () => {
      ...
      const corruptedCheckpoint: CheckpointData = {
        id: checkpointId,
        timestamp: new Date(),
        sessionId: 'test-uuid-1234',
        taskSnapshot: { 'task-1': mockTask },
