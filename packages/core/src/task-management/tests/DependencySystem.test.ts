...
class TestUtilities {
  ...
  static createLinearDependencyChain(length: number): Task[] {
    const tasks: Task[] = [];
  ...
  static createParallelTasks(count: number): Task[] {
    const tasks: Task[] = [];
    for (let i = 0; i < count; i++) {
      tasks.push(
        this.createMockTask(`parallel-task-${i}`, `Parallel Task ${i}`),
      );
    }
    return tasks;
  }

  static createCircularDependency(): Task[] {
    const taskA = this.createMockTask(
      'task-a',
      'Task A',
      'high',
      'moderate',
    ...
    const taskB = this.createMockTask(
      'task-b',
      'Task B',
      'high',
      'moderate',
    ...
    const taskC = this.createMockTask(
      'task-c',
      'Task C',
      'high',
      'moderate',
  ...
  static createComplexDependencyGraph(): Task[] {
...
describe('Dependency Analysis Engine', () => {
  let engine: DependencyAnalysisEngine;
  ...
  describe('Graph Validation', () => {
    test('should validate a simple linear dependency graph', async () => {
      ...
      const graph = await manager.createGraph(
        'linear-test',
        'Linear Test',
  ...
  describe('Topological Sorting', () => {
    ...
    test('should fail on circular dependencies', async () => {
      ...
      await expect(engine.topologicalSort(graph)).rejects.toThrow(
  ...
  describe('Critical Path Analysis', () => {
    ...
    test('should identify bottlenecks in critical path', async () => {
      ...
      const integrationBottleneck = criticalPath.bottlenecks.find(
        (b) => b.nodeId === 'node-integration',
      );
  ...
  describe('Parallel Group Identification', () => {
    test('should identify parallel execution opportunities', async () => {
      ...
      const level1Group = parallelGroups.find(
        (group) =>
          group.nodes.includes('node-a1') && group.nodes.includes('node-a2'),
      );
      expect(level1Group).toBeDefined();

      // Should identify that impl1 and impl2 can run in parallel
      const level3Group = parallelGroups.find(
        (group) =>
          group.nodes.includes('node-impl1') &&
  ...
  describe('Comprehensive Analysis', () => {
    ...
    test('should identify optimization opportunities', async () => {
      ...
      const parallelizationOpt = analysis.optimizations.find(
...
describe('Dependency Graph Manager', () => {
  ...
  describe('Graph Creation and Management', () => {
    ...
    test('should add dependencies correctly', async () => {
      ...
      const edge = await manager.addDependency(
        graph,
        'task1',
        'task2',
        constraint,
    ...
    test('should detect and resolve deadlocks', async () => {
      ...
      expect(
        graph.validationStatus.circularDependencies.length,
      ...
      const hasViolatedEdges = Array.from(graph.edges.values()).some(
        (edge) => edge.violations.length > 0,
      );
  ...
  describe('Deadlock Prevention', () => {
    ...
    test('should monitor for deadlocks periodically', async () => {
      ...
      const hasAnyViolations = Array.from(graph.edges.values()).some(
        (edge) => edge.violations.length > 0,
      );
...
describe('Dependency Visualizer', () => {
  ...
  beforeEach(async () => {
    ...
    const manager = new DependencyGraphManager();
    const tasks = TestUtilities.createComplexDependencyGraph();
    graph = await manager.createGraph(
      'viz-test',
      'Visualization Test',
      'Visualization test',
      tasks,
  ...
  describe('Visualization Options', () => {
    ...
    test('should show/hide critical path information', async () => {
      const withCriticalPath = await visualizer.visualizeGraph(
        graph,
        analysis,
        {
          format: VisualizationFormat.ASCII,
      ...
      const withoutCriticalPath = await visualizer.visualizeGraph(
        graph,
        analysis,
        {
          format: VisualizationFormat.ASCII,
...
describe('Resource-Aware Scheduler', () => {
  ...
  describe('Resource Management', () => {
    test('should check resource availability correctly', () => {
      ...
      expect(
        resourcePool.checkAvailability('cpu-1', startTime, endTime, 2),
      ).toBe(true);
      expect(
        resourcePool.checkAvailability('cpu-1', startTime, endTime, 5),
      ).toBe(false);
      expect(
        resourcePool.checkAvailability('nonexistent', startTime, endTime, 1),
      ).toBe(false);
    });
    ...
    test('should allocate and release resources', () => {
      ...
      expect(
        resourcePool.allocateResource('cpu-1', 'task1', startTime, endTime, 2),
      ).toBe(true);
      expect(
        resourcePool.checkAvailability('cpu-1', startTime, endTime, 3),
      ).toBe(false);
      expect(
        resourcePool.checkAvailability('cpu-1', startTime, endTime, 2),
      ).toBe(true);
      ...
      expect(
        resourcePool.checkAvailability('cpu-1', startTime, endTime, 4),
      ).toBe(true);
    });
    ...
    test('should calculate resource utilization', () => {
      ...
      const utilization = resourcePool.getUtilization(
        'cpu-1',
        startTime,
        endTime,
      );
  ...
  describe('Task Scheduling', () => {
    test('should schedule tasks with resource constraints', async () => {
      ...
      const schedule = await scheduler.scheduleTasks(
        graph,
        analysis,
        schedulingContext,
      );
    ...
    test('should detect and resolve resource conflicts', async () => {
      ...
      const scheduleByTime = schedule.schedule.sort(
        (a, b) => a.scheduledStart.getTime() - b.scheduledStart.getTime(),
      );

      for (let i = 1; i < scheduleByTime.length; i++) {
        expect(
          scheduleByTime[i].scheduledStart.getTime(),
        ).toBeGreaterThanOrEqual(scheduleByTime[i - 1].scheduledEnd.getTime());
      }
    });
...
describe('Integration Tests', () => {
  test('should integrate all components for end-to-end workflow', async () => {
    ...
    expect(schedule.schedule.map((s) => s.taskId).sort()).toEqual(
