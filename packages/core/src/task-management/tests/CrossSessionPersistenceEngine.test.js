...
describe('CrossSessionPersistenceEngine', () => {
    ...
    describe('Initialization and Session Management', () => {
        ...
        it('should create session metadata file on initialization', async () => {
            ...
            expect(mockWriteFile).toHaveBeenCalledWith(expect.stringContaining('session-test-uuid-1234.json'), expect.stringContaining('"sessionId":"test-uuid-1234"'));
        });
        ...
        it('should start heartbeat timer', async () => {
            ...
            expect(mockWriteFile).toHaveBeenCalledWith(expect.stringContaining('session-test-uuid-1234.json'), expect.any(String));
            vi.useRealTimers();
        });
    });
    ...
    describe('Checkpoint Management', () => {
        ...
        it('should save checkpoint to disk', async () => {
            ...
            expect(mockMkdir).toHaveBeenCalledWith(expect.stringContaining('checkpoints'), { recursive: true });
            // Should write checkpoint file
            expect(mockWriteFile).toHaveBeenCalledWith(expect.stringContaining('checkpoint-test-uuid-1234.json'), expect.stringContaining('"id":"test-uuid-1234"'));
        });
    ...
    describe('Crash Recovery', () => {
        ...
        it('should detect crashed sessions on initialization', async () => {
            ...
            engine = new CrossSessionPersistenceEngine(config);
            await engine.initialize(config);
            expect(crashRecoverySpy).toHaveBeenCalledWith(expect.objectContaining({
                crashedSessions: 1,
        ...
        it('should create emergency checkpoint on crash', async () => {
            ...
            expect(emergencySpy).toHaveBeenCalledWith(expect.objectContaining({
                sessionId: 'test-uuid-1234',
        ...
        it('should recover from crashed session checkpoint', async () => {
            ...
            engine = new CrossSessionPersistenceEngine(config);
            await engine.initialize(config);
            expect(recoveryCompletedSpy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Session Cleanup and Shutdown', () => {
        ...
        it('should cleanup inactive sessions', async () => {
            ...
            expect(mockUnlink).toHaveBeenCalledWith(expect.stringContaining('session-old-session.json'));
            vi.useRealTimers();
        });
