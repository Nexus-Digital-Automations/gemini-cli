...
} from '../AutonomousExecutionEngine.js';
 ...
 */
function createMockConfig(): TaskManagerConfig {
  return {
    config: {
      // Mock base config
...
function createMockCLIConfig(): CLIIntegrationConfig {
  return {
    config: {
      tempDir: '/tmp/gemini-cli-test',
...
describe('TaskManager Integration Tests', () => {
  ...
  describe('Core Task Management', () => {
    test('should create and execute a simple task', async () => {
      const taskId = await taskManager.addTask(
        'Test Task',
        'A simple test task for validation',
        {
          priority: 'medium',
          category: 'testing',
        },
    ...
    test('should handle multiple concurrent tasks', async () => {
      ...
      expect(
        systemStatus.taskCounts.autonomous +
          systemStatus.taskCounts.traditional,
    ...
    test('should demonstrate autonomous task breakdown', async () => {
      const complexTaskId = await taskManager.addTask(
        'Complex Comprehensive Task',
  ...
  describe('Error Handling and Recovery', () => {
    test('should handle task execution errors gracefully', async () => {
      const taskId = await taskManager.addTask(
        'Failing Task',
        'A task designed to fail for testing error handling',
        {
          priority: 'medium',
          category: 'testing',
          parameters: {
      ...
      expect(['failed', 'retrying', 'completed', 'in_progress']).toContain(
        status.status,
      );
    });
    ...
    test('should demonstrate error recovery strategies', async () => {
      const taskId = await taskManager.addTask(
        'Recoverable Task',
        'A task that fails initially but should recover',
        {
          priority: 'medium',
          category: 'testing',
          parameters: {
  ...
  describe('Performance and Optimization', () => {
    test('should maintain performance under load', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        taskPromises.push(
          taskManager.addTask(
            `Load Test Task ${i + 1}`,
            'A task for performance testing',
            {
              priority: 'medium',
              category: 'testing',
            },
    ...
    test('should optimize task scheduling', async () => {
      ...
      taskIds.push(
        await taskManager.addTask(
          'Dependent Task',
          'Task that depends on foundation',
          {
            priority: 'medium',
            category: 'testing',
  ...
  describe('System Status and Monitoring', () => {
    test('should provide accurate system status', async () => {
      ...
      expect(
        updatedStatus.taskCounts.autonomous +
          updatedStatus.taskCounts.traditional,
...
describe('AutonomousExecutionEngine Integration Tests', () => {
  ...
  test('should demonstrate error recovery mechanisms', async () => {
    ...
    expect(result.recoveryStrategy).toBe(
...
describe('TaskStateManager Integration Tests', () => {
  ...
  test('should manage task state transitions', async () => {
    ...
    const transition1 = await stateManager.transitionTaskState(
      taskId,
      ExtendedTaskState.PENDING,
      mockTask,
    );
    ...
    const transition2 = await stateManager.transitionTaskState(
      taskId,
      ExtendedTaskState.IN_PROGRESS,
      mockTask,
    );
    ...
    const transitions = [
      {
        ...
        task: { id: 'bulk-task-1', title: 'Bulk Task 1' } as any,
        reason: 'Bulk operation test',
      },
      {
        taskId: 'bulk-task-2',
        newState: ExtendedTaskState.PENDING,
        task: { id: 'bulk-task-2', title: 'Bulk Task 2' } as any,
        reason: 'Bulk operation test',
      },
      {
        taskId: 'bulk-task-3',
        newState: ExtendedTaskState.PENDING,
        task: { id: 'bulk-task-3', title: 'Bulk Task 3' } as any,
        reason: 'Bulk operation test',
      },
    ];
...
describe('CLITaskIntegration Integration Tests', () => {
  let cliIntegration: CLITaskIntegration;
  ...
  test('should register and execute CLI commands', async () => {
    ...
    const result = await cliIntegration.executeCommand(
      'test-command',
      ['arg1', 'arg2'],
      { option1: 'value1' },
    );
...
describe('End-to-End Integration Tests', () => {
  ...
  test('should demonstrate complete autonomous task lifecycle', async () => {
    ...
    const taskId = await taskManager.addTask(
      'Full Lifecycle Test',
      'This comprehensive task will demonstrate the complete autonomous task lifecycle including breakdown, scheduling, execution, error handling, quality gates, and completion reporting. The task involves multiple phases: analysis, implementation, testing, optimization, and documentation.',
      {
        priority: 'high',
        category: 'implementation',
        useAutonomousQueue: true,
    ...
    expect(systemStatus.isRunning).toBe(true);
    expect(
      systemStatus.taskCounts.autonomous + systemStatus.taskCounts.traditional,
    ).toBeGreaterThan(0);
  ...
  test('should handle system-wide error scenarios', async () => {
    ...
    taskIds.push(
      await taskManager.addTask(
        'Normal Task',
        'A regular task that should succeed',
    ...
    taskIds.push(
      await taskManager.addTask(
        'Error-Prone Task',
        'A task designed to test error handling',
        {
          priority: 'medium',
          category: 'testing',
    ...
    taskIds.push(
      await taskManager.addTask(
        'High Priority Task',
        'High priority task that should still execute',
  ...
  test('should demonstrate performance under realistic load', async () => {
    ...
    console.log(
      `Performance test: ${processedTasks}/${taskIds.length} tasks processed`,
    );
  });
});
