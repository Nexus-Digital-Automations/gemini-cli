...
function createMockTask(id, name, type = Type.FEATURE, priority = Priority.MEDIUM, dependencies = [], description = `Test task: ${name}`) {
    const mockContext = {
...
describe('DependencySequencer', () => {
    ...
    describe('Sequencing strategies', () => {
        it('should apply priority-first strategy', async () => {
            ...
            const criticalGroup = sequence.groups.find((g) => g.tasks.some((t) => t.id === 'critical'));
...
describe('IntelligentDependencyManager', () => {
    ...
    describe('Real-time monitoring and updates', () => {
        it('should handle dependency updates', async () => {
            ...
            await expect(manager.updateTaskDependencies(taskId, newDependencies)).resolves.not.toThrow();
        });
    ...
    describe('Adaptive learning capabilities', () => {
        ...
        it('should optimize performance over time', async () => {
            ...
            expect(optimizedMetrics.optimizationSuccessRate).toBeGreaterThanOrEqual(initialMetrics.optimizationSuccessRate);
        });
    });
});
