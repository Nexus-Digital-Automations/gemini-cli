...
import { TaskStateManager, createTaskStateManager, } from '../TaskStateManager.js';
...
export class TaskManagementTestRunner {
    ...
    async testTaskCreation() {
        ...
        try {
            ...
            const taskId = await this.taskManager.addTask('Test Task Creation', 'Testing basic task creation functionality');
    ...
    async testTaskExecution() {
        ...
        try {
            ...
            const taskId = await this.taskManager.addTask('Test Task Execution', 'Testing task execution capabilities', {
                priority: 'medium',
                category: 'testing',
            });
            // Wait for processing
    ...
    async testTaskStateManagement() {
        ...
        try {
            ...
            await stateManager.transitionTaskState('state-test', 'pending', mockTask);
    ...
    async testErrorHandling() {
        ...
        try {
            ...
            const taskId = await this.taskManager.addTask('Error Test Task', 'Testing error handling capabilities', {
    ...
    async testAutonomousBreakdown() {
        ...
        try {
            ...
            const taskId = await this.taskManager.addTask('Complex Autonomous Task', 'This is an extremely complex task that requires multiple phases: comprehensive analysis, detailed planning, full implementation, extensive testing, performance optimization, security validation, thorough documentation, and final deployment. Each phase has multiple sub-components and dependencies.', {
                priority: 'high',
    ...
    async testPriorityScheduling() {
        ...
        try {
            ...
            for (const priority of priorities) {
                const taskId = await this.taskManager.addTask(`${priority} Priority Task`, `Task with ${priority} priority`, { priority });
                taskIds.push(taskId);
            }
    ...
    async testTaskCreationPerformance() {
        ...
        try {
            ...
            for (let i = 0; i < numTasks; i++) {
                promises.push(this.taskManager.addTask(`Performance Test ${i}`, `Performance testing task ${i}`));
            }
            await Promise.all(promises);
            const createEndTime = performance.now();
            const totalCreationTime = createEndTime - createStartTime;
            const avgCreationTime = totalCreationTime / numTasks;
            const passed = avgCreationTime < this.config.performanceThresholds.taskCreationTimeMs;
            return {
    ...
    async testSystemResponseTime() {
        ...
        try {
            ...
            const passed = responseTime < this.config.performanceThresholds.systemResponseTimeMs;
            return {
    ...
    async testMemoryUsage() {
        ...
        try {
            ...
            const promises = [];
            for (let i = 0; i < 20; i++) {
                promises.push(this.taskManager.addTask(`Memory Test ${i}`, 'Memory usage testing task'));
            }
            await Promise.all(promises);
            await new Promise((resolve) => setTimeout(resolve, 200));
            const memAfter = process.memoryUsage();
            const memDifferenceMB = (memAfter.heapUsed - memBefore.heapUsed) / 1024 / 1024;
            const passed = memDifferenceMB < this.config.performanceThresholds.memoryUsageMB;
            return {
                name: 'Memory Usage',
                passed,
                duration: performance.now() - startTime,
    ...
    async testConcurrentTaskHandling() {
        const startTime = performance.now();
        try {
            if (!this.taskManager) {
                throw new Error('TaskManager not initialized');
            }
            const concurrentTasks = this.config.performanceThresholds.maxConcurrentTasks;
            const promises = [];
            for (let i = 0; i < concurrentTasks; i++) {
                promises.push(this.taskManager.addTask(`Concurrent Test ${i}`, 'Concurrent task handling test'));
            }
    ...
    async testThroughput() {
        ...
        try {
            ...
            while (performance.now() - throughputStartTime < testDuration) {
                await this.taskManager.addTask(`Throughput Test ${tasksCreated}`, 'Throughput testing task');
                tasksCreated++;
    ...
    async testHighVolumeTasks() {
        ...
        try {
            ...
            const promises = [];
            for (let i = 0; i < numTasks; i++) {
                promises.push(this.taskManager.addTask(`Stress Test ${i}`, `High volume task ${i}`, {
    ...
    async testSystemStability() {
        ...
        try {
            ...
            for (let i = 0; i < 30; i++) {
                ...
                promises.push(this.taskManager.addTask(`Stability Test ${i}`, isComplex
     ...
     * Helper methods
     */
    async executeTests(tests, category) {
        const results = [];
    ...
    countQualityGatesPassed(results) {
        return results.filter((r) => r.passed &&
            (r.name.includes('Validation') || r.name.includes('Quality'))).length;
    }
    async setupTestEnvironment() {
        ...
        if (this.config.enableIntegrationTests) {
            try {
                ...
                console.warn('⚠️ CLI integration setup failed, continuing without it:', error.message);
            }
        }
...
//# sourceMappingURL=TaskManagementTestRunner.js.map