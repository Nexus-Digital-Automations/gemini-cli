...
import { TaskFactories, MockTaskStore, TestUtilities, PerformanceMetrics, SafetyMonitor } from './utils/TestFactories';
describe('Infinite Loop Protection System', () => {
    ...
    describe('Circular Dependency Detection', () => {
        ...
        it('should detect complex multi-node circular dependencies', async () => {
            ...
            ].map(({ id, deps }) => TaskFactories.createTask({ id, dependencies: deps }));
        ...
        it('should prevent execution of tasks with circular dependencies', async () => {
            ...
            await expect(taskStore.executeTask('circular-a')).rejects.toThrow('Cannot execute task with circular dependencies');
    ...
    describe('System State Protection', () => {
        ...
        it('should recover system stability after infinite loop detection', async () => {
            const multipleInfiniteLoops = Array.from({ length: 5 }, (_, i) => TaskFactories.createTask({
                id: `infinite-loop-${i}`,
                executionConfig: {
                    simulateInfiniteLoop: true,
            ...
            const executionResults = await Promise.allSettled(multipleInfiniteLoops.map(task => taskStore.executeTask(task.id)));
            // All should be terminated
            executionResults.forEach(result => {
                expect(result.status).toBe('fulfilled');
                expect(result.value.success).toBe(false);
                expect(result.value.terminationReason).toMatch(/infinite|exceeded|limit/);
            });
