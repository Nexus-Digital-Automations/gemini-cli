...
import { TaskManagementSystemIntegrator, SystemConfigFactory, createIntegratedTaskManagementSystem, } from '../TaskManagementSystemIntegrator.js';
import { TaskManagementConfigManager, ConfigUtils, } from '../TaskManagementConfig.js';
describe('Task Management System Integration Tests', () => {
    ...
    describe('Task Execution Integration', () => {
        ...
        it('should queue and execute tasks through integrated system', async () => {
            const result = await integrator.queueTask('Test Integration Task', 'This is a test task for integration testing', {
                type: 'implementation',
                priority: 'high',
                expectedOutputs: {
        ...
        it('should handle task execution with autonomous breakdown', async () => {
            const result = await integrator.queueTask('Complex Integration Task', 'This is a complex task that should be broken down autonomously', {
                type: 'implementation',
                priority: 'high',
                complexity: 'high',
        ...
        it('should track task metrics and performance', async () => {
            ...
            const tasks = [];
            for (let i = 0; i < 3; i++) {
                const result = await integrator.queueTask(`Metrics Test Task ${i + 1}`, `Test task ${i + 1} for metrics collection`, {
                    type: 'implementation',
                    priority: 'normal',
                });
                tasks.push(result);
    ...
    describe('Error Handling and Recovery Integration', () => {
        ...
        it('should handle task execution errors gracefully', async () => {
            ...
            const result = await integrator.queueTask('', // Invalid empty title
