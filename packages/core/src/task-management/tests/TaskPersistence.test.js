...
describe('TaskPersistence', () => {
    ...
    describe('State Serialization', () => {
        it('should serialize queue state correctly', async () => {
            ...
            await persistence.saveQueueState(mockTasks, mockDependencies, new Map([['metric1', 100]]), { version: '1.0.0', lastSave: new Date() });
        ...
        it('should handle function serialization with registry', async () => {
            ...
            mockMkdir.mockResolvedValue(undefined);
            mockWriteFile.mockResolvedValue();
            await persistence.saveQueueState(taskWithCustomFunction, new Map(), new Map(), {});
            const writeCall = mockWriteFile.mock.calls[0];
            const serializedData = JSON.parse(writeCall[1]);
        ...
        it('should compress data when compression is enabled', async () => {
            ...
            mockMkdir.mockResolvedValue(undefined);
            mockWriteFile.mockResolvedValue();
            await compressionPersistence.saveQueueState(mockTasks, mockDependencies, new Map(), {});
    ...
    describe('State Deserialization', () => {
        ...
        it('should decompress data when needed', async () => {
            ...
            vi.spyOn(compressionPersistence, 'decompressData').mockReturnValue(JSON.stringify({
                version: '1.0.0',
                timestamp: new Date(),
                tasks: [],
                dependencies: [],
    ...
    describe('Backup Management', () => {
        it('should create backups when enabled', async () => {
            ...
            expect(calls.some((call) => call[0].includes('backup'))).toBe(true);
        });
        it('should clean old backups based on retention policy', async () => {
            ...
            mockReaddir.mockResolvedValue(oldBackupFiles);
    ...
    describe('Performance and Scalability', () => {
        ...
        it('should use streaming for large datasets', async () => {
            ...
            const streamingSpy = vi.spyOn(streamingPersistence, 'saveWithStreaming');
    ...
    describe('Event Emission', () => {
        ...
        it('should emit error events for save failures', async () => {
            ...
            await expect(persistence.saveQueueState(mockTasks, mockDependencies, new Map(), {})).rejects.toThrow();
