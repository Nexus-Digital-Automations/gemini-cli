...
import {
  SelfManagingTaskQueue,
  type SelfManagingQueueConfig,
} from '../SelfManagingTaskQueue.js';
import {
  AdvancedSchedulingSystem,
  type SchedulingConfig,
} from '../AdvancedSchedulingSystem.js';
import {
  EnhancedPersistenceEngine,
  type PersistenceConfig,
} from '../EnhancedPersistenceEngine.js';
import {
  RealtimeQueueMonitor,
  type MonitoringConfig,
  AlertSeverity,
} from '../RealtimeQueueMonitor.js';
import {
  AutomaticQueueOptimizer,
  type OptimizationConfig,
  OptimizationType,
} from '../AutomaticQueueOptimizer.js';
...
describe('SelfManagingTaskQueue', () => {
  let queue: SelfManagingTaskQueue;
  let config: SelfManagingQueueConfig;
  ...
  describe('Basic Queue Operations', () => {
    ...
    it('should respect concurrency limits', async () => {
      const concurrentExecutions = new Set<string>();
      ...
      expect(maxConcurrent.value).toBeLessThanOrEqual(
  ...
  describe('Autonomous Task Breakdown', () => {
    ...
    it('should respect breakdown depth limits', async () => {
      ...
      expect(metrics.averageBreakdownDepth).toBeLessThanOrEqual(
        config.maxBreakdownDepth,
      );
    });
  });
  ...
  describe('Adaptive Scheduling', () => {
    ...
    it('should prioritize high-priority tasks', async () => {
      const executionOrder: TaskPriority[] = [];
  ...
  describe('Performance Optimization', () => {
    it('should optimize performance continuously', async () => {
      ...
      expect(optimizationMetrics.optimizationsApplied).toBeGreaterThanOrEqual(
        0,
      );
    });
  });
...
describe('AdvancedSchedulingSystem', () => {
  let scheduler: AdvancedSchedulingSystem;
  ...
  describe('Algorithm Adaptation', () => {
    it('should adapt scheduling algorithm based on performance', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        await scheduler.recordExecutionResults(
          batch.map((task) => ({
            taskId: task.id,
            success: true,
...
describe('EnhancedPersistenceEngine', () => {
  let persistence: EnhancedPersistenceEngine;
  ...
  describe('State Management', () => {
    ...
    it('should handle versioning correctly', async () => {
      const initialState = {
        ...
        queueMetrics: { totalTasks: 3 } as any,
        resourceState: new Map(),
        learningData: {
          executionHistory: [],
          adaptationHistory: [],
      ...
      const updatedState = {
        ...
        queueMetrics: { totalTasks: 5 } as any,
      };
    ...
    it('should validate state data integrity', async () => {
      const invalidState = {
        ...
        queueMetrics: { totalTasks: 'invalid' } as any, // Invalid - should be number
        resourceState: new Map(),
        learningData: {
          executionHistory: [],
          adaptationHistory: [],
      ...
      await expect(
        persistence.saveState(invalidState as any),
      ).rejects.toThrow();
    });
  });
  ...
  describe('Cross-Session Recovery', () => {
    ...
    it('should clean up old session data', async () => {
      ...
      const state = {
        ...
        queueMetrics: { totalTasks: 1 } as any,
        resourceState: new Map(),
        learningData: {
          executionHistory: [],
          adaptationHistory: [],
  ...
  describe('Performance Optimization', () => {
    it('should compress large state data', async () => {
      ...
      const largeState = {
        tasks: largeTasks,
        queueMetrics: { totalTasks: 1000 } as any,
        resourceState: new Map(),
        learningData: {
          executionHistory: [],
          adaptationHistory: [],
...
describe('RealtimeQueueMonitor', () => {
  let monitor: RealtimeQueueMonitor;
...
describe('AutomaticQueueOptimizer', () => {
  let optimizer: AutomaticQueueOptimizer;
  ...
  describe('Performance Analysis', () => {
    it('should analyze current performance', async () => {
      ...
      if (analysis) {
        ...
        expect(analysis.healthStatus).toMatch(
    ...
    it('should track performance trends', async () => {
      ...
      if (analysis) {
        ...
        expect(analysis.trends.overallTrend).toMatch(
    ...
    it('should generate optimization recommendations', async () => {
      ...
      if (recommendations.length > 0) {
        ...
        expect(recommendation.type).toMatch(
          /algorithm_switch|concurrency_tuning|resource_reallocation/,
        );
  ...
  describe('Optimization Execution', () => {
    ...
    it('should support manual rollback', async () => {
      ...
      if (history.length > 0 && history[0].success) {
        const result = await optimizer.rollbackOptimization(
          history[0].recommendationId,
        );
...
describe('Queue Management System Integration', () => {
  ...
  beforeEach(() => {
    ...
    const queueConfig: SelfManagingQueueConfig = {
  ...
  describe('Complete Workflow Integration', () => {
    it('should handle end-to-end queue management workflow', async () => {
      const endMeasurement =
        performanceTracker.startMeasurement('e2e_workflow');
      ...
      const taskExecutionTimes: number[] = [];
      ...
      const addTasksMeasurement =
        performanceTracker.startMeasurement('add_tasks');
      for (const task of tasks) {
        await queue.addTask(task);
      }
      addTasksMeasurement();
    ...
    it('should handle high-load scenarios', async () => {
      ...
      const highLoadMeasurement = performanceTracker.startMeasurement(
      ...
      while (
        queue.getQueueStatus().pendingTasks > 0 &&
    ...
    it('should integrate monitoring and optimization systems', async () => {
      ...
      console.log(
        `- Queue Status: ${queueStatus.tasksProcessed}/${queueStatus.totalTasks} processed`,
      );
      console.log(
        `- Monitoring: ${monitor.getMonitoringStats().totalSnapshots} snapshots`,
      );
      console.log(
        `- Optimization: ${optimizationStatus.totalOptimizations} optimizations`,
      );
    ...
    it('should handle component failures gracefully', async () => {
      ...
      if (dashboardData && dashboardData.realtimeData.length > 0) {
        const latestSnapshot =
          dashboardData.realtimeData[dashboardData.realtimeData.length - 1];
  ...
  describe('Performance Benchmarks', () => {
    it('should meet performance benchmarks', async () => {
      ...
      const benchmarkMeasurement =
        performanceTracker.startMeasurement('benchmark');
