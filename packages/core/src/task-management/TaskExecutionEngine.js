...
export { TaskComplexity, TaskStatus, TaskPriority, TaskType, TaskCategory, } from './types.js';
...
Focus on creating a practical, executable breakdown that enables efficient autonomous execution.`;
    ...
    createTaskFromData(data) {
        ...
        return {
            ...
            name: data.title || 'Untitled Task', // Use title as name for compatibility
...
export class TaskExecutionEngine {
    ...
    async analyzeAndBreakdownTask(taskId) {
        ...
        try {
            ...
            await this.handleTaskFailure(task, `Breakdown analysis failed: ${errorMessage}`);
        }
    }
    /**
    ...
    async scheduleTaskExecution(taskId) {
        ...
        if (!TaskExecutionUtils.areDependenciesSatisfied(taskId, this.taskQueue, this.completedTasks, this.taskDependencies)) {
    ...
    async executeTask(taskId) {
        ...
        try {
            ...
            const executionContext = {
                ...
                dependencies: TaskExecutionUtils.getDependencyTasks(taskId, this.taskQueue, this.completedTasks, this.taskDependencies),
            ...
            if (result.terminate_reason === SubagentTerminateMode.GOAL) {
                ...
                await TaskExecutionUtils.scheduleDependentTasks(taskId, this.taskQueue, this.taskDependencies, (taskId) => this.scheduleTaskExecution(taskId));
            }
            else {
    ...
    async handleTaskFailure(task, errorMessage) {
        ...
        if (task.retryCount <= task.maxRetries) {
            ...
            await TaskExecutionUtils.handleDependentTasksOnFailure(task.id, this.taskQueue, this.taskDependencies, (task, status) => this.updateTaskStatus(task, status), (task, error) => this.handleTaskFailure(task, error));
        }
    }
