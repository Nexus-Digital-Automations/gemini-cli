...
export class TaskLifecycle extends EventEmitter {
    ...
    getAggregateMetrics() {
        ...
        for (const context of this.lifecycleContexts.values()) {
            ...
            if (context.currentState === LifecycleState.COMPLETED)
                completedTasks++;
            if (context.currentState === LifecycleState.FAILED)
                failedTasks++;
            // Retry count
    ...
    initializeDefaultHooks() {
        ...
        this.registerHook({
            ...
            timing: 'before',
            condition: async (task, _context) => task.preConditions.length > 0,
            action: async (task, _context) => {
        ...
        this.registerHook({
            ...
            timing: 'before',
            condition: async (task, _context) => task.postConditions.length > 0,
            action: async (task, _context) => {
    ...
    updateMetrics(taskId, event) {
        ...
        if (event.metadata.duration) {
            ...
            const totalTime = metrics.averageStateTransitionTime * (totalTransitions - 1) +
     ...
     */
    async evaluateCondition(_condition, _context) {
