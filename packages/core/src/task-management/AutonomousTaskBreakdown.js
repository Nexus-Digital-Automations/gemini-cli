...
(function (BreakdownStrategy) {
    ...
    BreakdownStrategy["DEPENDENCY"] = "dependency";
    BreakdownStrategy["RESOURCE"] = "resource";
    BreakdownStrategy["RISK_BASED"] = "risk_based";
...
export class AutonomousTaskBreakdown extends EventEmitter {
     ...
     */
    async analyzeAndBreakdown(task) {
    ...
    analyzeTaskComplexity(task) {
        ...
        const overallScore = durationScore * 0.4 +
            ...
            dependencyScore * 0.2 +
            riskScore * 0.2;
        const overallComplexity = overallScore >= 0.8
            ? 'extreme'
            : overallScore >= 0.6
                ? 'high'
                : overallScore >= 0.3
     ...
     */
    shouldBreakdownTask(task, complexity) {
        ...
        if (complexity.parallelizationOpportunity >
     ...
     */
    async performBreakdown(task, strategy, complexity) {
        let subtasks = [];
        switch (strategy) {
            case BreakdownStrategy.TEMPORAL:
                ...
                break;
            case BreakdownStrategy.FUNCTIONAL:
                subtasks = await this.performFunctionalBreakdown(task, _complexity);
                break;
            case BreakdownStrategy.DEPENDENCY:
                subtasks = await this.performDependencyBreakdown(task, _complexity);
                break;
            case BreakdownStrategy.RESOURCE:
                subtasks = await this.performResourceBreakdown(task, _complexity);
                break;
            case BreakdownStrategy.RISK_BASED:
                subtasks = await this.performRiskBasedBreakdown(task, _complexity);
                break;
            case BreakdownStrategy.HYBRID:
     ...
     */
    async performTemporalBreakdown(task, complexity) {
        const subtasks = [];
     ...
     */
    async performHybridBreakdown(task, complexity) {
        ...
        const breakdownOptions = await Promise.all(strategies.map(async (strategy) => {
            switch (strategy) {
                case BreakdownStrategy.FUNCTIONAL:
                    return {
                        strategy,
    ...
    calculateParallelizationOpportunity(task) {
    ...
    calculateOptimalPhaseCount(duration) {
        if (duration < 30 * 60 * 1000)
            return 2; // < 30 min: 2 phases
        if (duration < 2 * 60 * 60 * 1000)
            return 3; // < 2 hours: 3 phases
        if (duration < 8 * 60 * 60 * 1000)
            return 4; // < 8 hours: 4 phases
        return Math.min(6, Math.floor(duration / (2 * 60 * 60 * 1000))); // Max 6 phases
    }
    // Scoring methods for strategy selection...
    scoreTemporalStrategy(task, complexity) {
        let score = 0.5; // Base score
        // Good for long-running tasks
        if (complexity.estimatedDuration > 2 * 60 * 60 * 1000)
            score += 0.3;
        ...
        if (complexity.dependencyComplexity > 0.7)
            score -= 0.2;
        return Math.max(0, Math.min(1, score));
    }
    scoreFunctionalStrategy(task, complexity) {
        ...
        if (complexity.resourceRequirements > 0.5)
            score += 0.2;
        // Good parallelization opportunity
        if (complexity.parallelizationOpportunity > 0.6)
            score += 0.2;
        return Math.max(0, Math.min(1, score));
    }
    scoreDependencyStrategy(task, complexity) {
        let score = 0.4; // Lower base score
        // Excellent for highly dependent tasks
        if (complexity.dependencyComplexity > 0.6)
            score += 0.4;
    ...
    scoreResourceStrategy(task, complexity) {
        let score = 0.3; // Lower base score
        // Excellent for resource-intensive tasks
        if (complexity.resourceRequirements > 0.8)
            score += 0.5;
    ...
    scoreRiskBasedStrategy(task, complexity) {
        let score = 0.3; // Lower base score
        // Excellent for high-risk tasks
        if (complexity.riskFactors > 0.7)
            score += 0.5;
    ...
    scoreHybridStrategy(task, complexity) {
        let score = 0.7; // High base score
        // Excellent for extremely complex tasks
        if (complexity.overallComplexity === 'extreme')
            score += 0.3;
    ...
    createPhaseExecuteFunction(originalTask, phaseName, phaseIndex) {
    ...
    createPhaseRollbackFunction(originalTask, phaseName) {
        return async (task, context) => {
    ...
    createComponentExecuteFunction(originalTask, component) {
        return async (task, context) => {
    ...
    createDependencyExecuteFunction(originalTask, link) {
        return async (task, context) => {
            const linkContext = {
                ...context,
                dependencyLink: {
    ...
    createResourceExecuteFunction(originalTask, group) {
        return async (task, context) => {
    ...
    createRiskExecuteFunction(originalTask, component) {
        return async (task, context) => {
            const riskContext = {
                ...context,
                riskComponent: {
    ...
    identifyFunctionalComponents(task) {
    ...
    groupByResourceRequirements(task) {
    ...
    calculateComponentPriority(basePriority, criticality) {
        if (criticality === 'high') {
    ...
    calculateLinkPriority(basePriority, importance) {
        if (importance === 'high') {
    ...
    calculateResourcePriority(basePriority, criticality) {
        return this.calculateComponentPriority(basePriority, criticality);
    }
    calculateRiskPriority(basePriority, riskLevel) {
        if (riskLevel === 'high') {
    ...
    generatePhaseQualityGates(phaseName) {
        const gateMap = {
    ...
    calculatePhaseRiskLevel(phaseName, complexity) {
        if (phaseName.includes('Implementation') &&
            complexity.overallComplexity === 'high') {
            return 'high';
        }
    ...
    generatePhaseValidationCriteria(phaseName) {
        const criteriaMap = {
    ...
    calculateExpectedImprovement(originalTask, subtasks) {
        ...
        const avgDuration = subtasks.reduce((sum, st) => sum + st.estimatedDuration, 0) /
            totalSubtasks;
    ...
    generateBreakdownMetadata(originalTask, subtasks) {
        const totalSubtasks = subtasks.length;
    ...
    scoreBreakdownOption(originalTask, subtasks, complexity) {
        let score = 0.5; // Base score
        ...
        const avgDuration = subtasks.reduce((sum, st) => sum + st.estimatedDuration, 0) /
            subtasks.length;
        const durationVariance = subtasks.reduce((sum, st) => sum + Math.pow(st.estimatedDuration - avgDuration, 2), 0) / subtasks.length;
        const normalizedVariance = durationVariance / (avgDuration * avgDuration);
        score += Math.max(0, 0.2 - normalizedVariance);
        return Math.max(0, Math.min(1, score));
    }
    calculateIdealSubtaskCount(complexity) {
        if (complexity.overallComplexity === 'extreme')
            return 8;
        if (complexity.overallComplexity === 'high')
            return 6;
        if (complexity.overallComplexity === 'medium')
            return 4;
        return 3;
    }
    applyHybridOptimizations(originalTask, subtasks) {
    ...
    async optimizeBreakdown(breakdown) {
    ...
    mergeExcessSubtasks(subtasks) {
        ...
        while (subtasks.length > this.config.maxSubtasks && subtasks.length > 2) {
            ...
            const merged = {
                ...
                validationCriteria: Array.from(new Set([
                ...
                riskLevel: toMerge[0].riskLevel === 'high' || toMerge[1].riskLevel === 'high'
                    ? 'high'
    ...
    applyDurationConstraints(subtasks) {
        for (const subtask of subtasks) {
    ...
    updatePerformanceMetrics(breakdown) {
     ...
     */
    recordExecutionOutcome(breakdown, executionResults) {
        const totalDuration = executionResults.reduce((sum, result) => sum + result.duration, 0);
        const successRate = executionResults.filter((r) => r.success).length /
            executionResults.length;
        const avgParallelization = executionResults.reduce((sum, r) => sum + r.parallelization, 0) /
            executionResults.length;
        const avgResourceEfficiency = executionResults.reduce((sum, r) => sum + r.resourceEfficiency, 0) /
            executionResults.length;
        const actualOutcome = {
...
//# sourceMappingURL=AutonomousTaskBreakdown.js.map