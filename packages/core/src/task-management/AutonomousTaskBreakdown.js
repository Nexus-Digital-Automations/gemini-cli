...
(function (BreakdownStrategy) {
    ...
    BreakdownStrategy["HYBRID"] = "hybrid"; // Combination approach
...
export class AutonomousTaskBreakdown extends EventEmitter {
    ...
    analyzeTaskComplexity(task) {
        ...
        const overallScore = (durationScore * 0.4) +
            ...
            (dependencyScore * 0.2) +
            (riskScore * 0.2);
        const overallComplexity = overallScore >= 0.8 ? 'extreme' :
    ...
    scoreTemporalStrategy(task, complexity) {
        ...
        if (task.category === TaskCategory.FEATURE || task.category === TaskCategory.INFRASTRUCTURE)
            score += 0.2;
    ...
    scoreFunctionalStrategy(task, complexity) {
        ...
        if (task.category === TaskCategory.FEATURE || task.category === TaskCategory.REFACTOR)
            score += 0.3;
    ...
    scoreDependencyStrategy(task, complexity) {
        ...
        if (task.category === TaskCategory.TEST || task.category === TaskCategory.INFRASTRUCTURE)
            score += 0.2;
        return Math.max(0, Math.min(1, score));
    }
    scoreResourceStrategy(task, complexity) {
        ...
        if (task.category === TaskCategory.INFRASTRUCTURE || task.category === TaskCategory.PERFORMANCE)
            score += 0.3;
        return Math.max(0, Math.min(1, score));
    }
