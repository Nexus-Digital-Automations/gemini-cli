...
export class InfiniteHookIntegration {
    ...
    canHandleTask(task) {
        ...
        if (requiredCapabilities.length === 0)
            return true;
        return requiredCapabilities.some(cap => this.hookConfig.capabilities.includes(cap) || this.hookConfig.capabilities.includes('general'));
    }
    /**
    ...
    mapFeaturePriorityToTaskPriority(feature) {
        ...
        if (businessValue.includes('critical'))
            return 'critical';
        if (businessValue.includes('essential') || businessValue.includes('important'))
            return 'high';
        return 'normal';
    }
    /**
