...
import { TaskQueue, TaskPriority, TaskStatus, TaskCategory, } from './TaskQueue.js';
/**
...
export class SelfManagingTaskQueue extends EventEmitter {
    ...
    async predictTaskComplexity(taskDefinition) {
    ...
    async predictTaskFailureProbability(taskDefinition) {
    ...
    handleTaskFailure(task, record, error) {
        // Release reserved resources
        this.releaseTaskResources(task);
    ...
    handleTaskBreakdown(originalTask, breakdown) {
    ...
    async performIntelligentOptimization(strategy) {
