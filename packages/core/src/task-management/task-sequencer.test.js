...
describe('TaskSequencer', () => {
    ...
    beforeEach(() => {
        ...
        const taskDefinitions = [
            { id: 'task1', title: 'Setup Environment', category: 'implementation', priority: 'high', cpu: 2, memory: 1024 },
            { id: 'task2', title: 'Database Migration', category: 'implementation', priority: 'critical', cpu: 4, memory: 2048 },
            { id: 'task3', title: 'Unit Tests', category: 'testing', priority: 'medium', cpu: 1, memory: 512 },
            { id: 'task4', title: 'Documentation', category: 'documentation', priority: 'low', cpu: 1, memory: 256 },
            { id: 'task5', title: 'Performance Analysis', category: 'analysis', priority: 'medium', cpu: 6, memory: 4096 },
        ];
        taskDefinitions.forEach(def => {
            const task = createTestTask(def.id, def.title, def.category, def.priority, [
        ...
        test('should handle dependencies with resource optimization', () => {
            ...
            for (const group of sequence.parallelGroups) {
                ...
                for (const taskId of group) {
                    const task = sampleTasks.get(taskId);
                    if (task.executionContext?.resourceConstraints) {
                        const cpuConstraint = task.executionContext.resourceConstraints.find(c => c.resourceType === 'cpu');
                        if (cpuConstraint) {
    ...
    describe('Resource Management', () => {
        ...
        test('should calculate timing correctly in allocations', () => {
            ...
            for (let i = 1; i < allocations.length; i++) {
                ...
                expect(currentAllocation.allocatedAt.getTime()).toBeGreaterThanOrEqual(prevAllocation.allocatedAt.getTime());
            }
        });
    ...
    describe('Edge Cases', () => {
        ...
        test('should handle tasks without resource constraints', () => {
            const taskWithoutResources = createTestTask('no-resources', 'Task Without Resources', 'documentation', 'low');
        ...
        test('should handle unknown algorithm gracefully', () => {
            expect(() => {
                sequencer.generateExecutionSequence(sampleTasks, 'unknown');
            }).toThrow('Unknown sequencing algorithm: unknown');
        });
    });
});
// Helper function to create test tasks
function createTestTask(id, title, category, priority, resourceConstraints) {
    const now = new Date();
    return {
        id,
        title,
...
//# sourceMappingURL=task-sequencer.test.js.map