...
export class QueuePersistence extends EventEmitter {
     ...
     */
    async saveSnapshot(tasks, dependencies, executionRecords, lifecycleContexts, lifecycleEvents, metrics, customData, description) {
        ...
        try {
            ...
            // Create snapshot
            const snapshot = await this.createSnapshot(snapshotId, tasks, dependencies, executionRecords, lifecycleContexts, lifecycleEvents, metrics, customData, description);
            // Save to storage
    ...
    async createBackup(description) {
        ...
        try {
            ...
            const backupPath = await this.writeBackup(backupId, latestSnapshot, description);
     ...
     */
    recordTransaction(operation, entityType, entityId, beforeState, afterState) {
        ...
        const entry = {
            ...
            beforeState,
            afterState,
            checksum: this.calculateChecksum(JSON.stringify({
                operation,
                entityType,
                entityId,
                beforeState,
    ...
    getPersistenceStats() {
        ...
        const totalBytes = this.operationHistory.reduce((sum, op) => sum + op.bytesTransferred, 0);
    ...
    async verifyIntegrity() {
        const results = {
            snapshots: [],
            backups: [],
            transactionLog: { valid: true, error: undefined },
        };
        // Verify snapshots
     ...
     */
    async createSnapshot(snapshotId, tasks, dependencies, executionRecords, lifecycleContexts, lifecycleEvents, metrics, customData, description) {
        const snapshotData = {
            tasks,
            dependencies,
            executionRecords,
     ...
     */
    async writeBackup(backupId, snapshot, description) {
        if (!this.config.backupDirectory) {
        ...
        if (this.recoveryPoints.length > this.config.maxBackups) {
            ...
            for (const backup of toDelete) {
                try {
                    ...
                    const index = this.recoveryPoints.findIndex((rp) => rp.id === backup.id);
                    if (index > -1) {
...
//# sourceMappingURL=QueuePersistence.js.map