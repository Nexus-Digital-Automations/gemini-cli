...
export class TaskExecutionEngine {
    ...
    async savePersistedState() {
        try {
            const state = {
                taskQueue: Object.fromEntries(Array.from(this.taskQueue.entries()).map(([id, task]) => [id, this.serializeTask(task)])),
                completedTasks: Object.fromEntries(Array.from(this.completedTasks.entries()).map(([id, task]) => [id, this.serializeTask(task)])),
