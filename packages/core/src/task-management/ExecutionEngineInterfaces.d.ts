...
export interface PerformanceAnalyticsRequest {
    ...
    metrics: Array<'throughput' | 'latency' | 'success_rate' | 'resource_utilization' | 'queue_depth' | 'execution_time' | 'wait_time' | 'retry_rate'>;
...
export interface IExecutionEngine {
    ...
    getResourceUtilization(): Promise<Record<string, {
        used: number;
        total: number;
        utilization: number;
    }>>;
...
export interface ITaskQueueIntegration {
    ...
    requestScheduling(eligibleTasks: Task[], availableSlots: number, context: SchedulingContext): Promise<SchedulingDecision>;
    applySchedulingDecision(decision: SchedulingDecision): Promise<void>;
    onLifecycleTransition(taskId: TaskId, fromState: LifecycleState, toState: LifecycleState, context: LifecycleContext): Promise<void>;
...
export interface IPrioritySchedulerIntegration {
    selectSchedulingAlgorithm(tasks: Task[], context: SchedulingContext): Promise<SchedulingAlgorithm>;
    configureAlgorithm(algorithm: SchedulingAlgorithm, configuration: Record<string, unknown>): Promise<void>;
    recordExecutionOutcome(decision: SchedulingDecision, outcomes: TaskExecutionResult[]): Promise<void>;
...
export interface ILifecycleManagerIntegration {
    ...
    transitionTaskState(taskId: TaskId, newState: LifecycleState, metadata?: Record<string, unknown>): Promise<boolean>;
    registerLifecycleHook(state: LifecycleState, timing: 'before' | 'after' | 'during', handler: (task: Task, context: LifecycleContext) => Promise<void>): string;
...
export interface IPersistenceManagerIntegration {
    ...
    recordTransaction(operation: 'create' | 'update' | 'delete' | 'transition', entityType: 'task' | 'dependency' | 'record' | 'context', entityId: string, beforeState?: unknown, afterState?: unknown): void;
...
export interface IntegrationEvents {
    ...
    'persistence:snapshotLoaded': {
        snapshotId: string;
        taskCount: number;
    };
    'persistence:backupCreated': {
        backupId: string;
        description: string;
    };
    'persistence:integrityCheck': {
        valid: boolean;
        issues: number;
    };
    'resources:allocated': {
        allocation: ResourceAllocationResponse;
    };
    'resources:released': {
        allocationId: string;
    };
