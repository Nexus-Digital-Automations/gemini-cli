...
export class DependencyManager {
    ...
    autoConfig;
    executionHistory;
    dependencyLearningData; // pattern -> confidence score
     ...
     */
    addDependency(dependentTaskId, dependsOnTaskId, type = 'hard', options) {
        const dependency = {
            dependentTaskId,
            dependsOnTaskId,
            type,
     ...
     */
    wouldCreateCircularDependency(dependentTaskId, dependsOnTaskId) {
     ...
     */
    resolveCircularDependencies(strategy) {
     ...
     */
    attemptCycleResolution(cycle, interactive) {
     ...
     */
    generateExecutionSequence(algorithm) {
     ...
     */
    generateExecutionSequenceForTasks(taskIds, algorithm) {
     ...
     */
    generateExecutionPlan(taskIds) {
        logger.debug('Generating comprehensive execution plan');
        const tasks = taskIds
            ? new Map(taskIds.map(id => [id, this.tasks.get(id)]).filter(([, task]) => task))
            : this.tasks;
        ...
        if (circularDependencies.length > 0) {
            ...
            resolutionActions = resolved.flatMap(cycle => cycle.resolutionStrategies
                .filter(strategy => strategy.impact !== 'high')
                .map(strategy => ({
                type: strategy.strategy === 'remove_edge' ? 'remove_dependency' :
                    strategy.strategy === 'split_task' ? 'split_task' :
                        'merge_tasks',
    ...
    suggestAutoDependencies(task) {
        ...
        for (const [otherTaskId, otherTask] of this.tasks) {
            ...
            if (suggestions.length >= this.autoConfig.maxSuggestions)
                break;
        }
    ...
    calculatePatternSimilarity(pattern1, pattern2) {
        ...
        if (Math.abs(priority1Index - priority2Index) <= 1)
            similarityScore += 0.5;
     ...
     */
    updateDependencyLearning(dependentTaskId, dependsOnTaskId, type) {
     ...
     */
    recordTaskExecution(taskId, startTime, endTime, success) {
    ...
    getTaskExecutionStats(taskId) {
        ...
        const totalDuration = history.reduce((sum, record) => sum + (record.endTime.getTime() - record.startTime.getTime()), 0);
        return {
     ...
     */
    updateAutoDependencyConfiguration(config) {
