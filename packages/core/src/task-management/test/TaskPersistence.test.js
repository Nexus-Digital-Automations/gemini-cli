...
describe('TaskPersistence', () => {
    ...
    describe('Session Management', () => {
        it('should create a session successfully', async () => {
            const session = await persistence.createSession('test-agent', '/test/directory', { NODE_ENV: 'test' });
    ...
    describe('Task Management', () => {
        const createTestTask = (id = 'test-task') => ({
            id,
            title: 'Test Task',
    ...
    describe('Task Ownership and Locking', () => {
        it('should acquire exclusive lock on task', async () => {
            ...
            const session = await persistence.createSession('test-agent');
            await persistence.saveTask(task, session.sessionId);
            const ownership = await persistence.acquireTaskLock(task.id, session.sessionId, 'exclusive', 60000);
        ...
        it('should prevent concurrent exclusive locks', async () => {
            ...
            await expect(persistence.acquireTaskLock(task.id, session2.sessionId)).rejects.toThrow(`Task ${task.id} is already locked`);
        });
    ...
    describe('Execution Plans', () => {
        it('should save and load execution plans', async () => {
            ...
            const planId = await persistence.saveExecutionPlan(executionPlan, 'Test Plan', 'A test execution plan');
