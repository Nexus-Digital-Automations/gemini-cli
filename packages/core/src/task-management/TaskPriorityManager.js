...
(function (PriorityAlgorithm) {
    ...
    PriorityAlgorithm["DEPENDENCY_CRITICAL_PATH"] = "dependency_critical_path";
    PriorityAlgorithm["RESOURCE_OPTIMIZATION"] = "resource_optimization";
    PriorityAlgorithm["MACHINE_LEARNING"] = "machine_learning";
...
export class TaskPriorityManager extends EventEmitter {
    config;
    priorityHistory = new Map();
    factorLearningData = new Map();
    statistics;
    constructor(config = {}) {
        super();
        this.config = {
     ...
     */
    async calculatePriority(task, allTasks, dependencyGraph) {
     ...
     */
    calculateWeightedFactorsPriority(task, allTasks, reasons) {
     ...
     */
    calculateUrgencyImpactPriority(task, allTasks, reasons) {
     ...
     */
    calculateCriticalPathPriority(task, allTasks, dependencyGraph, reasons) {
        let priority = task.basePriority;
        if (!dependencyGraph) {
     ...
     */
    calculateResourceOptimizedPriority(task, allTasks, reasons) {
        let priority = task.basePriority;
     ...
     */
    async calculateMLPriority(task, allTasks, reasons) {
        ...
        Object.entries(featureWeights).forEach(([feature, weight]) => {
            const factorValue = task.priorityFactors[feature] || 1.0;
     ...
     */
    calculateCriticalPathLength(taskId, dependencyGraph, allTasks, visited = new Set()) {
        ...
        for (const dependentId of dependents) {
            ...
            if (dependentTask && dependentTask.status === 'PENDING') {
                const pathLength = 1 +
                    this.calculateCriticalPathLength(dependentId, dependencyGraph, allTasks, new Set(visited));
     ...
     */
    calculateResourceContention(task, allTasks) {
        const conflicts = [];
     ...
     */
    calculateMLFeatureWeights(data) {
        const weights = {};
        const factorNames = [
            'age',
            'userImportance',
     ...
     */
    storePriorityCalculation(result) {
     ...
     */
    learnFromExecution(taskId, outcome, task) {
    ...
    updateStatistics() {
        ...
        const recentAdjustments = allResults.filter((r) => r.adjustmentReasons.some((reason) => reason.timestamp.getTime() > hourAgo));
...
//# sourceMappingURL=TaskPriorityManager.js.map