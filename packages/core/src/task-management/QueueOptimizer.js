...
export class QueueOptimizer extends EventEmitter {
    ...
    generateGeneralOptimizations(tasks, dependencyAnalysis) {
        ...
        const longRunningTasks = Array.from(tasks.values()).filter(task => task.estimatedDuration > 30 * 60 * 1000 // More than 30 minutes
        );
    ...
    calculateHistoricalAccuracy() {
        if (this.optimizationHistory.length === 0)
            return 0.5;
        const accurateOptimizations = this.optimizationHistory.filter(opt => opt.actualImprovement > opt.recommendations.reduce((sum, r) => sum + r.expectedImprovement, 0) * 0.7);
    ...
    applyPriorityAdjustment(tasks, recommendation) {
        const priorityBoost = recommendation.metadata?.priorityBoost || TaskPriority.HIGH;
    ...
    applyResourceRebalancing(tasks, recommendation) {
        const loadBalancing = recommendation.metadata?.loadBalancing;
        if (!loadBalancing)
            return false;
