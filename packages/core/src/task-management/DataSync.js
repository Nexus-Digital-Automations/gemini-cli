...
export class DataSync extends EventEmitter {
    ...
    setupChangeTracking() {
        ...
        this.persistence.on('task-saved', (event) => {
            this.trackChange('update', 'task', event.taskId, 'system', // TODO: get actual session ID
            'system', // TODO: get actual agent ID
            undefined, event, { source: 'task-saved' });
        });
        ...
        this.sessionManager.on('session-created', (session) => {
            this.trackChange('create', 'session', session.sessionId, session.sessionId, session.agentId, undefined, session, { source: 'session-created' });
        });
        this.sessionManager.on('session-terminated', (event) => {
            this.trackChange('update', 'session', event.session.sessionId, event.session.sessionId, event.session.agentId, { status: 'active' }, { status: event.session.status }, { source: 'session-terminated', reason: event.reason });
        });
        // Task ownership changes
        this.sessionManager.on('task-locked', (ownership) => {
            this.trackChange('create', 'ownership', `${ownership.taskId}:${ownership.sessionId}`, ownership.sessionId, ownership.agentId, undefined, ownership, { source: 'task-locked' });
        });
        this.sessionManager.on('task-unlocked', (event) => {
            this.trackChange('delete', 'ownership', `${event.taskId}:${event.sessionId}`, event.sessionId, 'unknown', // Agent ID not available in unlock event
            event, undefined, { source: 'task-unlocked' });
        });
