 ...
 */
export var LoadBalanceAlgorithm;
 ...
 */
export var StarvationPreventionMode;
...
export class TaskPriorityScheduler extends EventEmitter {
    ...
    constructor(config = {}) {
        ...
        this.config = {
            ...
            loadBalanceAlgorithm: config.loadBalanceAlgorithm ??
    ...
    calculateDeadlineDrivenPriority(task, context) {
        ...
        const urgencyFactor = timeToDeadline <= 0
            ? 2.0
     ...
     */
    calculateDependencyAwarePriority(task, _context) {
    ...
    calculateHybridPriority(task, context) {
        ...
        if (this.starvationTracker.has(task.id)) {
            const starvationBoost = Math.min(this.config.maxPriorityBoost, (ageMs - this.config.maxStarvationTime) / 60000); // 1 point per minute
    ...
    fairQueuing() {
        ...
        for (const queue of Array.from(this.priorityQueues.values())) {
            if (queue.length === 0)
                continue;
            const totalWaitTime = queue.reduce((sum, task) => sum + (now - task.createdAt.getTime()), 0);
    ...
    getPriorityLevel(dynamicPriority) {
        if (dynamicPriority >= TaskPriority.CRITICAL)
            return TaskPriority.CRITICAL;
        if (dynamicPriority >= TaskPriority.HIGH)
            return TaskPriority.HIGH;
        if (dynamicPriority >= TaskPriority.MEDIUM)
            return TaskPriority.MEDIUM;
        if (dynamicPriority >= TaskPriority.LOW)
    ...
    buildAdjustmentContext() {
        ...
        const queueDepth = Array.from(this.priorityQueues.values()).reduce((sum, queue) => sum + queue.length, 0);
    ...
    calculateSystemLoad() {
        ...
        const totalTasks = Array.from(this.priorityQueues.values()).reduce((sum, queue) => sum + queue.length, 0);
        return Math.min(1.0, totalTasks / 100); // Normalize to 0-1 scale
    }
    calculateExecutionRate(_priority) {
        // Would be calculated from actual execution history
        return 0.85; // Placeholder
    }
    calculatePriorityFairness(_priority) {
        // Would be calculated based on actual execution fairness
        return 0.8; // Placeholder
    }
    calculateResourceUsage(_priority) {
    ...
    selectCategoryByQuota(categoryQueues) {
        ...
        for (const [category, tasks] of Array.from(categoryQueues.entries())) {
            ...
            const avgPriority = tasks.reduce((sum, task) => sum + task.dynamicPriority, 0) /
                tasks.length;
