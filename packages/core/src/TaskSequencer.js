...
(function (SequencingAlgorithm) {
    ...
    SequencingAlgorithm["RESOURCE_AWARE"] = "resource_aware";
    SequencingAlgorithm["DEADLINE_DRIVEN"] = "deadline_driven";
    SequencingAlgorithm["ADAPTIVE_HYBRID"] = "adaptive_hybrid";
    SequencingAlgorithm["MACHINE_LEARNING"] = "machine_learning";
...
export class TaskSequencer {
    ...
    dependencyAnalyzer;
    executionHistory;
    learningModel; // Simplified ML model
    sequenceCache;
    constructor(config = {}, dependencyAnalyzer) {
        this.config = {
     ...
     */
    async generateSequence(tasks, existingDependencies = [], context) {
        ...
        const dependencyAnalysis = await this.dependencyAnalyzer.analyzeDependencies(tasks, existingDependencies, context);
        // Generate sequence based on selected algorithm
        const primarySequence = await this.executeAlgorithm(this.config.algorithm, tasks, dependencyAnalysis, context);
        // Generate alternative sequences for comparison
        const alternativeSequences = await this.generateAlternativeSequences(tasks, dependencyAnalysis, context);
        // Calculate performance metrics
        const metrics = this.calculateSequenceMetrics(primarySequence, tasks, dependencyAnalysis);
        ...
        const decision = {
            ...
            reasoning: this.generateSequencingReasoning(primarySequence, metrics, dependencyAnalysis),
            evidence: {
                dependencyAnalysis,
                metrics,
     ...
     */
    async executeAlgorithm(algorithm, tasks, dependencyAnalysis, context) {
        switch (algorithm) {
     ...
     */
    async generateCriticalPathSequence(tasks, dependencyAnalysis) {
     ...
     */
    async generatePriorityWeightedSequence(tasks, dependencyAnalysis) {
        ...
        const sortedTasks = Array.from(tasks.values()).sort((a, b) => (taskScores.get(b.id) || 0) - (taskScores.get(a.id) || 0));
        const sequence = sortedTasks.map((task) => task.id);
        const parallelGroups = this.generateParallelGroupsRespectingDependencies(sequence, tasks, dependencyAnalysis.suggestedDependencies);
     ...
     */
    async generateResourceAwareSequence(tasks, dependencyAnalysis) {
        ...
        const criticalPath = this.findCriticalPathInSequence(optimizedSequence.sequence, tasks, dependencyAnalysis.suggestedDependencies);
        return {
     ...
     */
    async generateDeadlineDrivenSequence(tasks, dependencyAnalysis) {
        ...
        const tasksWithoutDeadlines = Array.from(tasks.values()).filter((task) => !this.hasDeadline(task));
     ...
     */
    async generateAdaptiveHybridSequence(tasks, dependencyAnalysis, context) {
        ...
        const optimizedGroups = this.optimizeTaskGrouping(sortedTasks, dependencyAnalysis, adaptedWeights);
     ...
     */
    async generateMLBasedSequence(tasks, dependencyAnalysis) {
     ...
     */
    calculateSchedulingFactors(task, dependencyAnalysis) {
        ...
        const dependentTasks = dependencyAnalysis.suggestedDependencies.filter((dep) => dep.dependsOnTaskId === task.id).length;
     ...
     */
    calculateCompositeScore(factors, weights) {
        let score = 0;
     ...
     */
    calculateSequenceMetrics(sequence, tasks, dependencyAnalysis) {
        const totalExecutionTime = sequence.estimatedDuration;
        const criticalPathDuration = this.calculateCriticalPathDuration(sequence.criticalPath, tasks);
        // Calculate resource utilization
        const resourceUtilization = this.calculateResourceUtilization(sequence.parallelGroups, tasks);
        ...
        const dependencySatisfaction = this.calculateDependencySatisfaction(sequence, dependencyAnalysis.suggestedDependencies);
     ...
     */
    convertTaskPriorityToScore(priority) {
        switch (priority) {
    ...
    calculateSequenceConfidence(metrics, alternatives) {
        // Simplified confidence calculation
        return Math.min(0.5 +
            metrics.dependencySatisfaction * 0.2 +
            metrics.resourceUtilization * 0.2 +
            (1 - metrics.riskAssessment) * 0.1, 1);
    }
    generateSequencingReasoning(sequence, metrics, dependencyAnalysis) {
    ...
    estimateSuccessProbability(metrics) {
        // Simplified success probability calculation
        return Math.min(0.7 +
            metrics.dependencySatisfaction * 0.2 +
            (1 - metrics.riskAssessment) * 0.1, 0.99);
    }
    ...
    async generateAlternativeSequences(tasks, dependencyAnalysis, context) {
        const alternatives = [];
        const algorithms = [
    ...
    calculateAlternativeScore(metrics) {
    ...
    generateParallelGroupsFromGraph(tasks, graph) {
    ...
    calculateParallelGroupsDuration(groups, tasks) {
        let totalDuration = 0;
    ...
    calculatePriorityScore(task, dependencyAnalysis) {
        const baseScore = this.convertTaskPriorityToScore(task.priority);
        // Add bonus for tasks with many dependents
        const dependents = dependencyAnalysis.suggestedDependencies.filter((dep) => dep.dependsOnTaskId === task.id).length;
        return baseScore + dependents * 0.5;
    }
    generateParallelGroupsRespectingDependencies(sequence, tasks, dependencies) {
    ...
    findCriticalPathInSequence(sequence, tasks, dependencies) {
        // Simplified critical path finding
        return sequence.slice(0, Math.min(sequence.length, 3));
    }
    groupTasksByResourceRequirements(tasks) {
    ...
    optimizeResourceAllocation(resourceGroups, tasks, dependencyAnalysis) {
    ...
    hasDeadline(task) {
        ...
        return (!!task.parameters?.deadline ||
            !!task.metadata.custom?.deadline);
    }
    getDeadline(task) {
        // Extract deadline timestamp
        const deadline = task.parameters?.deadline ||
            task.metadata.custom?.deadline;
        return typeof deadline === 'number'
            ? deadline
            : Date.now() + 24 * 60 * 60 * 1000; // Default 1 day
    }
    adaptWeightsToContext(context) {
        if (!context)
    ...
    optimizeTaskGrouping(sortedTasks, dependencyAnalysis, weights) {
    ...
    canGroupTogether(task, group, dependencyAnalysis) {
        // Check if task can be grouped with others (no dependencies between them)
        for (const groupTask of group) {
            const hasDependency = dependencyAnalysis.suggestedDependencies.some((dep) => (dep.dependentTaskId === task.id &&
    ...
    predictOptimalSequence(tasks, dependencyAnalysis) {
    ...
    generateParallelGroupsFromPredictions(predictions, tasks) {
    ...
    calculateCriticalPathDuration(criticalPath, tasks) {
        return criticalPath.reduce((total, taskId) => {
            const task = tasks.get(taskId);
            return total + (task?.metadata.estimatedDuration || 60000);
        }, 0);
    }
    calculateResourceUtilization(parallelGroups, tasks) {
        // Simplified resource utilization calculation
        if (parallelGroups.length === 0)
            return 0;
        const avgGroupSize = parallelGroups.reduce((sum, group) => sum + group.length, 0) /
    ...
    calculateDependencySatisfaction(sequence, dependencies) {
    ...
    calculateRiskAssessment(sequence, tasks, dependencyAnalysis) {
        ...
        const criticalTasks = Array.from(tasks.values()).filter((t) => t.priority === TaskPriority.CRITICAL).length;
        riskScore += (criticalTasks / tasks.size) * 0.3;
        return Math.min(riskScore, 1);
    }
    updateLearningModel(tasks, decision) {
     ...
     */
    recordExecutionHistory(taskId, actualDuration, resourcesUsed, success) {
        const history = {
            taskId,
...
//# sourceMappingURL=TaskSequencer.js.map