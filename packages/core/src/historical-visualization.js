...
export class HistoricalVisualizationEngine {
    ...
    async generateTrendAnalysisCharts(records, analysis, config = {}) {
        ...
        const decompositionChart = await this.createTrendDecompositionChart(records, analysis, {
        ...
        const volatilityChart = await this.createVolatilityChart(records, analysis, {
    ...
    async generateForecastCharts(records, forecasts, config = {}) {
        ...
        const timelineChart = await this.createForecastTimelineChart(records, forecasts, {
        ...
        const confidenceChart = await this.createConfidenceIntervalChart(forecasts, {
    ...
    async generateAnomalyCharts(records, anomalies, config = {}) {
        ...
        const timelineChart = await this.createAnomalyTimelineChart(records, anomalies, {
    ...
    async createDashboard(records, analysis, forecasts, anomalies, widgets = []) {
        ...
        const dashboardHtml = await this.generateDashboardHtml(records, analysis, forecasts, anomalies, activeWidgets);
        // Create dashboard artifact
        const artifact = {
            id: `dashboard-${Date.now()}`,
            type: 'dashboard',
            title: 'Budget Usage Analytics Dashboard',
            description: 'Comprehensive overview of budget usage patterns, trends, and forecasts',
            format: 'html',
            data: dashboardHtml,
            metadata: {
                ...
                charts: activeWidgets
                    ...
                    .map((w) => w.title),
                insights: this.generateKeyInsights(records, analysis, forecasts, anomalies),
            },
        };
     ...
     */
    async generateHistoricalReport(records, analysis, forecasts, anomalies, config) {
        let reportData;
        switch (config.format) {
            case 'html':
                reportData = await this.generateHtmlReport(records, analysis, forecasts, anomalies, config);
                break;
            case 'json':
                reportData = this.generateJsonReport(records, analysis, forecasts, anomalies, config);
                break;
            case 'csv':
    ...
    async createWeeklyPatternChart(records, config) {
        ...
        const weeklyAverages = weeklyData.map((sum, i) => weeklyCounts[i] > 0 ? sum / weeklyCounts[i] : 0);
        const series = [
            {
                name: 'Average Usage',
        ...
        const series = [
            {
                ...
                data: records
                    ...
                    metadata: {
                        usage: r.dailyUsage,
                        limit: r.dailyLimit,
                        efficiency: r.usagePercentage < 80
                            ? 'good'
    ...
    generateSvgChart(series, config) {
        const { width, height, title, xLabel, yLabel, showGrid, showLegend } = config;
        ...
        series.forEach((s, seriesIndex) => {
            const color = s.color || config.colors[seriesIndex % config.colors.length];
            if (s.data.length === 0)
                return;
            if (config.type === 'line' || s.type === 'line') {
                ...
                s.data.forEach((point, i) => {
                    ...
                    const barHeight = ((point.y - chartMinY) / (chartMaxY - chartMinY)) * chartHeight;
    ...
    generateHeatmapSvg(data, config) {
        ...
        const maxValue = Math.max(...Object.values(data).flatMap((monthData) => Object.values(monthData)));
    ...
    async generateDashboardHtml(records, analysis, forecasts, anomalies, widgets) {
        const summary = this.calculateSummaryStats(records);
        const insights = this.generateKeyInsights(records, analysis, forecasts, anomalies);
        return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    calculateSummaryStats(records) {
        ...
        const efficiency = validRecords.length > 0
            ? Math.round(validRecords.reduce((sum, r) => sum + r.usagePercentage, 0) /
                validRecords.length)
            : 0;
    ...
    generateKeyInsights(records, analysis, forecasts, anomalies) {
        ...
        insights.push(`Peak usage typically occurs on ${days[peakDay]}, lowest on ${days[lowDay]}`);
        // Trend insights
        if (analysis.trendStrength > 0.3) {
            insights.push(`Strong ${analysis.trend} trend detected with ${Math.round(analysis.confidence * 100)}% confidence`);
        }
        // Seasonality insights
        if (analysis.cyclicality.detected) {
            insights.push(`Cyclical pattern detected with ${analysis.cyclicality.period}-day cycles`);
        }
        // Efficiency insights
        const avgEfficiency = records
            .filter((r) => r.dailyLimit > 0)
            .reduce((sum, r) => sum + r.usagePercentage, 0) /
            records.filter((r) => r.dailyLimit > 0).length;
        if (avgEfficiency < 50) {
            insights.push('Budget utilization is low - consider optimizing limits or reallocating resources');
        }
        else if (avgEfficiency > 90) {
            insights.push('High budget utilization - consider increasing limits to avoid constraints');
        }
        // Anomaly insights
        const criticalAnomalies = anomalies.filter((a) => a.severity === 'critical').length;
        if (criticalAnomalies > 0) {
            insights.push(`${criticalAnomalies} critical usage anomalies require immediate attention`);
        }
        // Forecast insights
        if (forecasts.length > 0) {
            const avgForecast = forecasts.reduce((sum, f) => sum + f.forecastUsage, 0) /
                forecasts.length;
            const currentAvg = analysis.averageDailyUsage;
            if (avgForecast > currentAvg * 1.2) {
                insights.push('Forecasts indicate potential 20%+ increase in usage - prepare budget adjustments');
            }
        }
    ...
    async saveToDisk(artifact) {
        try {
            ...
            console.warn(`Failed to save visualization artifact ${artifact.id}:`, error);
        }
    }
}
/**
