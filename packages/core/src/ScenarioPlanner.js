...
export class ScenarioPlanner {
    ...
    generateConstraintScenario(candidates, parameters) {
        const allocations = candidates.map((candidate, index) => {
            const recommendedAllocation = Math.min(candidate.constraints.maxAllocation, Math.max(candidate.constraints.minAllocation, candidate.currentAllocation * 0.95));
    ...
    generateCustomScenario(candidates, parameters, index) {
        ...
        const allocations = candidates.map((candidate, candidateIndex) => {
