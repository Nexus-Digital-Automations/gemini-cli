...
const logger = {
    info: (message, meta) => console.info(`[AnalyticsController] ${message}`, meta),
    warn: (message, meta) => console.warn(`[AnalyticsController] ${message}`, meta),
    error: (message, meta) => console.error(`[AnalyticsController] ${message}`, meta),
};
/**
...
export class AnalyticsController {
    ...
    async getPredictions(req, res) {
        ...
        try {
            ...
            const forecast = await mlAPI.generateForecast({
                projectRoot: process.cwd(),
                settings: budgetTracker.getBudgetSettings(),
                forecastHours: parseInt(forecastHours, 10),
            });
            ...
            const response = {
                ...
                data: {
                    ...
                    metadata: {
                        timestamp: new Date().toISOString(),
                        responseTime,
                        forecastHorizon: parseInt(forecastHours, 10),
                    },
                },
            };
        ...
        try {
            ...
            const response = {
                ...
                data: {
                    ...
                    summary: {
                        totalCost: enrichedBreakdown.totalCost || 0,
                        totalRequests: enrichedBreakdown.totalRequests || 0,
                        averageCostPerRequest: (enrichedBreakdown.totalCost || 0) /
                            Math.max(enrichedBreakdown.totalRequests || 1, 1),
     ...
     */
    async runCustomAnalytics(req, res) {
        const startTime = Date.now();
     ...
     */
    generateTrendInsights(trendData, anomalyData) {
        const insights = [];
        const typedTrendData = trendData;
        ...
        const typedAnomalyData = anomalyData;
        if (typedAnomalyData?.patterns?.seasonality?.detected) {
            insights.push(`Seasonal usage pattern detected with ${typedAnomalyData.patterns.seasonality.period} period`);
        }
     ...
     */
    enrichBreakdownData(breakdown) {
        const typedBreakdown = breakdown;
        if (!typedBreakdown.categories || !Array.isArray(typedBreakdown.categories)) {
            return typedBreakdown;
        }
        const totalCost = typedBreakdown.categories.reduce((sum, cat) => sum + (cat.cost || 0), 0);
        const totalRequests = typedBreakdown.categories.reduce((sum, cat) => sum + (cat.requests || 0), 0);
        const enrichedCategories = typedBreakdown.categories
            .map((category, index) => ({
            ...category,
            percentage: totalCost > 0 && category.cost ? (category.cost / totalCost) * 100 : 0,
            rank: index + 1,
            averageCostPerRequest: category.requests && category.requests > 0 && category.cost ? category.cost / category.requests : 0,
        }))
            .sort((a, b) => (b.cost || 0) - (a.cost || 0)) // Sort by cost descending
            .map((category, index) => ({
            ...category,
            rank: index + 1,
        }));
