...
/**
 * Represents a Gaxios HTTP error with response data.
 * Used for handling HTTP errors from the Gaxios HTTP client library.
 */
interface GaxiosError {
  /** Optional response object containing error data */
  response?: {
    /** Raw response data from the HTTP request */
    data?: unknown;
  };
}
...
/**
 * Represents the structure of error response data from HTTP requests.
 * Used for parsing structured error responses from API calls.
 */
interface ResponseData {
  /** Optional error object containing details about the failure */
  error?: {
