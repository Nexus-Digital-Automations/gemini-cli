...
import { MemoryEfficientStringBuilder, StreamTokenCounter, ContextWindowManager, MemoryOptimizationUtils, memoryOptimizer, } from './memoryOptimizer.js';
...
describe('StreamTokenCounter', () => {
    ...
    test('should handle moderate files efficiently', async () => {
        ...
        const moderateContent = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. '.repeat(1000);
    ...
    test('should handle chunked large strings', () => {
        const largeText = 'This is a test sentence that will be repeated many times. '.repeat(50000);
...
describe('MemoryOptimizationUtils', () => {
    test('should optimize string replacement for large strings', () => {
        ...
        const startTime = Date.now();
        const result = MemoryOptimizationUtils.optimizeStringReplace(largeString, 'world', 'universe');
        const endTime = Date.now();
    ...
    test('should measure performance accurately', async () => {
        ...
        const { result, duration, memoryDelta } = await MemoryOptimizationUtils.measurePerformance(testOperation, 'test operation');
    ...
    test('should handle large array concatenation efficiently', () => {
        const largeArrays = Array.from({ length: 100 }, (_, i) => Array.from({ length: 1000 }, (_, j) => i * 1000 + j));
        const startTime = Date.now();
        const startMemory = process.memoryUsage();
...
describe('Integration Tests', () => {
    test('should handle realistic chat session memory usage', async () => {
        ...
        try {
            ...
            const chatHistory = [];
            const longMessage = 'This is a detailed response that contains a lot of information about various topics including technology, science, literature, and many other subjects that might come up in a typical conversation with an AI assistant. '.repeat(100);
            for (let i = 0; i < 50; i++) {
                chatHistory.push({
                    role: 'user',
