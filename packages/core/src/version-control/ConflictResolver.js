...
import { ConflictResolutionStrategy, ConflictType, ConflictCategory, } from './types.js';
/**
...
export class ConflictResolver {
     ...
     */
    async resolveConflicts(conflicts, strategy = ConflictResolutionStrategy.AUTO_WITH_FALLBACK) {
        const results = [];
    ...
    async analyzeConflicts(conflicts) {
        ...
        for (const conflict of conflicts) {
            ...
            if (complexity.risks.functionalBreakage >
     ...
     */
    generateResolutionReport(conflicts, results, analysis) {
     ...
     */
    async parseFileConflicts(filePath) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('<<<<<<<')) {
                ...
                const conflict = await this.classifyConflict(currentConflict);
     ...
     */
    async analyzeSemantics(conflict) {
     ...
     */
    async interactiveResolveConflict(conflict) {
     ...
     */
    async prepareManualResolution(conflict) {
     ...
     */
    async applyResolution(conflict, resolvedContent) {
     ...
     */
    extractContext(lines, conflictLine) {
        const contextSize = 5;
    ...
    findEnclosingFunction(lines, line) {
    ...
    async calculateConflictComplexity(conflict) {
    ...
    generateNextSteps(failedResolutions) {
        const steps = [];
    ...
    calculateEfficiencyScore(analysis, results) {
    ...
    isDataStructureConflict(conflict) {
    ...
    calculateSeverity(conflict, semantic) {
        if (conflict.type === ConflictType.FORMATTING)
            return 'low';
        if (semantic.riskFactors.length > 2)
            return 'high';
        if (conflict.type === ConflictType.LOGIC)
            return 'medium';
        return 'low';
    }
    ...
    calculateSemanticSimilarity(code1, code2) {
    ...
    checkFunctionalEquivalence(code1, code2) {
    ...
    identifyRiskFactors(conflict) {
        ...
        const theirsContent = Array.isArray(conflict.theirsContent)
            ...
            : conflict.theirsContent ? [conflict.theirsContent] : [];
        const allContent = [...oursContent, ...theirsContent].join('\n');
        if (/delete|remove|drop/i.test(allContent))
            risks.push('data_deletion');
        if (/password|key|secret|token/i.test(allContent))
            risks.push('security_sensitive');
        if (/api|endpoint|url/i.test(allContent))
            risks.push('api_breaking');
        if (/database|sql|query/i.test(allContent))
            risks.push('data_access');
        return risks;
    }
    calculateCodeComplexity(code1, code2) {
    ...
    async applyResolutionRule(conflict, rule) {
        return rule.resolver(conflict);
    }
    async semanticResolution(conflict) {
        const semantic = await this.analyzeSemantics(conflict);
    ...
    async patternBasedResolution(conflict) {
    ...
    generateManualResolutionGuidance(conflict, analysis) {
        const guidance = [];
    ...
    mergeImportStatements(conflict) {
        const oursContent = Array.isArray(conflict.oursContent)
            ? conflict.oursContent
            : conflict.oursContent ? [conflict.oursContent] : [];
        const theirsContent = Array.isArray(conflict.theirsContent)
    ...
    calculateFormattingScore(lines) {
        let score = 0;
