 ...
 */
export const DEFAULT_COMMIT_MESSAGE_CONFIG = {
...
export class CommitMessageGenerator {
     ...
     */
    async generateCommitMessage(options = { includeFileAnalysis: true, includeTestAnalysis: true, includePerformanceImpact: false }) {
        try {
     ...
     */
    generateDescription(type, analysis) {
        let description = '';
        switch (type) {
     ...
     */
    extractIssueReferences(analysis) {
        const references = [];
        try {
     ...
     */
    generateFooter(isBreakingChange, references) {
        const footerParts = [];
        if (isBreakingChange) {
    ...
    async analyzeFunctionChanges(files) {
        const functions = [];
    ...
    async analyzeClassChanges(files) {
        const classes = [];
        for (const file of files) {
            try {
                const diff = execSync(`git diff --cached -- "${file}"`, { encoding: 'utf8' });
    ...
    async analyzeImportChanges(files) {
        const changes = [];
        for (const file of files) {
            try {
                const diff = execSync(`git diff --cached -- "${file}"`, { encoding: 'utf8' });
    ...
    isOnlyConfig(analysis) {
        return analysis.configChanges.length === analysis.modifiedFiles.length;
    }
    hasPerformanceChanges(analysis) {
        const perfKeywords = ['perf', 'performance', 'optimize', 'faster', 'cache', 'lazy'];
        return analysis.functionsModified.some(func => perfKeywords.some(keyword => func.toLowerCase().includes(keyword)));
    }
    isRefactoring(analysis) {
    ...
    isBugFix(analysis) {
        ...
        return analysis.functionsModified.some(func => fixKeywords.some(keyword => func.toLowerCase().includes(keyword)));
    }
    isStyleChange(analysis) {
        // Check if only formatting/style files changed
        const styleFiles = analysis.modifiedFiles.filter(file => file.includes('.prettier') || file.includes('.eslint') ||
            file.includes('style') || file.includes('.css'));
        return styleFiles.length === analysis.modifiedFiles.length;
    }
    isCIChange(analysis) {
        return analysis.modifiedFiles.some(file => file.includes('.github/workflows') || file.includes('.gitlab-ci') ||
            file.includes('Jenkinsfile') || file.includes('.travis.yml'));
    }
    isBuildChange(analysis) {
        return analysis.modifiedFiles.some(file => file.includes('webpack') || file.includes('rollup') ||
            file.includes('build.') || file.includes('Dockerfile'));
    }
    generateFeatureDescription(analysis) {
        if (analysis.functionsModified.length > 0) {
            return `add ${analysis.functionsModified[0].toLowerCase()}`;
        }
        return `add new functionality`;
    }
    generateFixDescription(analysis) {
        if (analysis.functionsModified.length > 0) {
            return `fix issue in ${analysis.functionsModified[0]}`;
        }
        return `fix bug`;
    }
    generateDocsDescription(analysis) {
        return `update documentation`;
    }
    generateRefactorDescription(analysis) {
        if (analysis.functionsModified.length > 0) {
            return `refactor ${analysis.functionsModified[0]}`;
        }
        return `refactor code structure`;
    }
    generateTestDescription(analysis) {
        return `add tests`;
    }
    generateChoreDescription(analysis) {
        if (analysis.modifiedFiles.some(f => f.includes('package.json'))) {
            return `update dependencies`;
        }
        return `update configuration`;
    }
    generatePerfDescription(analysis) {
    ...
    getMostFrequent(array) {
        ...
        const mostFrequent = Object.entries(frequency).reduce((a, b) => a[1] > b[1] ? a : b)[0];
     ...
     */
    formatCommitMessage(commitMessage) {
