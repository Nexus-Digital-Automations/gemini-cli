 ...
 */
export const DEFAULT_BRANCH_MANAGER_CONFIG = {
...
export class BranchManager {
    ...
    generateBranchName(options) {
        ...
        if (options.type === BranchType.RELEASE ||
     ...
     */
    async applyProtectionRules(branchName) {
        try {
     ...
     */
    async switchToBranch(branchName, createIfNotExists = false) {
        try {
    ...
    async ensureBaseBranchReady(baseBranch) {
    ...
    ensureUniqueBranchName(baseName) {
    ...
    async applyProtectionRule(branchName, rule) {
    ...
    inferBranchType(branchName) {
        ...
        if (branchName.startsWith('experimental/'))
    ...
    async getUnprotectedCriticalBranches() {
        const allBranches = await this.getAllBranches();
