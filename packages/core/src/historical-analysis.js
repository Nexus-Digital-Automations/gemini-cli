...
export class HistoricalBudgetAnalyzer {
    ...
    constructor(projectRoot, options = {}) {
        ...
        this.historicalDataPath = path.join(projectRoot, '.gemini', 'budget-history.json');
    ...
    async recordDailyUsage(usageData, settings) {
        ...
        const existingIndex = historicalRecords.findIndex((record) => record.date === date);
        ...
        const trimmedRecords = historicalRecords.filter((record) => record.date >= cutoffDateString);
    ...
    async analyzeTrends(lookbackDays = 30) {
        ...
        if (historicalRecords.length < 7) {
            throw new Error('Insufficient historical data for trend analysis (minimum 7 days required)');
        }
        ...
        if (recentRecords.length < 7) {
            throw new Error(`Insufficient data in lookback period (${recentRecords.length} days, minimum 7 required)`);
        }
        ...
        const volatility = Math.sqrt(usageValues.reduce((sum, val) => sum + Math.pow(val - avgUsage, 2), 0) /
            usageValues.length);
        // Detect cyclicality
    ...
    async generateForecast(targetDate, confidence = 0.95, settings) {
        ...
        const daysAhead = Math.ceil((targetDateObj.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        if (daysAhead <= 0) {
        ...
        const seasonalAdjustment = this.calculateSeasonalAdjustment(targetDateObj, trendAnalysis);
        const trendAdjustment = this.calculateTrendAdjustment(daysAhead, trendAnalysis);
        ...
        const forecastVariance = this.calculateForecastVariance(daysAhead, trendAnalysis, historicalRecords);
        ...
        const overageRisk = this.calculateOverageRisk(forecastUsage, forecastVariance, currentLimit);
        ...
        return {
            ...
            riskAssessment: {
                ...
                mitigation: this.generateMitigationStrategies(riskSeverity, overageRisk),
            },
            seasonalAdjustment,
            trendAdjustment,
        };
    ...
    async detectAnomalies(lookbackDays = 30) {
        ...
        for (let i = windowSize; i < recentRecords.length; i++) {
            ...
            const windowAvg = window.reduce((sum, r) => sum + r.dailyUsage, 0) / window.length;
            const windowStd = Math.sqrt(window.reduce((sum, r) => sum + Math.pow(r.dailyUsage - windowAvg, 2), 0) / window.length);
            const zScore = windowStd > 0 ? (record.dailyUsage - windowAvg) / windowStd : 0;
            const anomalyScore = Math.abs(zScore);
            if (anomalyScore >= this.anomalyThreshold) {
                const anomaly = this.classifyAnomaly(record, windowAvg, zScore, anomalyScore);
                anomalies.push(anomaly);
                // Update historical record with anomaly score
                const historicalIndex = historicalRecords.findIndex((r) => r.date === record.date);
    ...
    async getHistoricalInsights(lookbackDays = 90) {
        ...
        const avgWeekdayUsage = weekdayUsage.map((sum, i) => weekdayCounts[i] > 0 ? sum / weekdayCounts[i] : 0);
        const mostActiveWeekdayIndex = avgWeekdayUsage.indexOf(Math.max(...avgWeekdayUsage));
        const weekdayNames = [
            'Sunday',
            'Monday',
            'Tuesday',
        ...
        const efficiency = validLimits.length > 0
        ...
        const recommendations = this.generateInsightRecommendations(recentRecords, trends, recentAnomalies, efficiency);
        // Assess data quality
    ...
    async saveHistoricalData(records) {
        try {
            ...
            await fs.writeFile(this.historicalDataPath, JSON.stringify(records, null, 2));
        }
        catch (error) {
    ...
    calculateSeasonalPatterns(records) {
        ...
        // Calculate averages
        const weeklyPattern = weeklyUsage.map((sum, i) => weeklyCounts[i] > 0 ? sum / weeklyCounts[i] : 0);
        const monthlyPattern = monthlyUsage.map((sum, i) => monthlyCounts[i] > 0 ? sum / monthlyCounts[i] : 0);
        const quarterlyPattern = quarterlyUsage.map((sum, i) => quarterlyCounts[i] > 0 ? sum / quarterlyCounts[i] : 0);
    ...
    detectCyclicality(records) {
        ...
        let bestPeriod = 0;
        for (let period = 2; period <= Math.floor(usageValues.length / 3); period++) {
    ...
    projectUsage(records, trend) {
        ...
        const avgRecentUsage = recentUsage.reduce((sum, val) => sum + val, 0) / recentUsage.length;
    ...
    calculateSeasonalAdjustment(targetDate, analysis) {
        ...
        const quarterlyEffect = analysis.seasonalPattern.quarterlyPattern[quarter] || 0;
        // Weight the effects (weekly has more impact than monthly/quarterly)
        const weightedAdjustment = weeklyEffect * 0.6 +
            monthlyEffect * 0.3 +
            quarterlyEffect * 0.1 -
    ...
    calculateTrendAdjustment(daysAhead, analysis) {
        ...
        const trendMagnitude = analysis.trendStrength * analysis.averageDailyUsage * 0.1; // 10% max per day
    ...
    calculateForecastVariance(daysAhead, analysis, _records) {
    ...
    calculateOverageRisk(forecastUsage, variance, limit) {
        ...
        const zScore = standardDeviation > 0 ? (limit - forecastUsage) / standardDeviation : 0;
        // Convert z-score to probability (simplified normal CDF approximation)
        return Math.max(0, Math.min(1, 0.5 * (1 - Math.tanh(zScore * Math.sqrt(2 / Math.PI)))));
    }
    ...
    classifyAnomaly(record, expectedUsage, zScore, anomalyScore) {
        ...
        return {
            ...
            description,
            confidence,
            suggestedAction: this.generateAnomalyAction(type, severity, actualUsage, expectedUsage),
        };
    }
    generateAnomalyAction(type, severity, _actualUsage, _expectedUsage) {
    ...
    generateInsightRecommendations(records, trends, anomalies, efficiency) {
        ...
        if (trends.trend === 'increasing' && trends.trendStrength > 0.5) {
            recommendations.push('Usage is trending upward - consider increasing budget limits proactively');
        }
        if (trends.volatility > trends.averageDailyUsage * 0.5) {
            recommendations.push('High usage variability detected - implement more granular monitoring');
        }
        // Efficiency recommendations
        if (efficiency < 30) {
            recommendations.push('Low budget utilization - consider reducing limits or reallocating budget');
        }
        else if (efficiency > 90) {
            recommendations.push('High budget utilization - consider increasing limits to avoid constraints');
        }
        ...
        if (weeklyPattern[maxWeekday] > weeklyPattern[minWeekday] * 2) {
            ...
            recommendations.push(`Strong weekly pattern detected - highest usage on ${days[maxWeekday]}, lowest on ${days[minWeekday]}`);
        }
        // Anomaly-based recommendations
        if (anomalies.length > 3) {
            recommendations.push('Multiple anomalies detected - review usage patterns and consider model recalibration');
        }
        // Cyclicality recommendations
        if (trends.cyclicality.detected) {
            recommendations.push(`Cyclical pattern detected with ${trends.cyclicality.period}-day period - optimize budgets accordingly`);
        }
        // Data quality recommendations
        if (records.length < 30) {
            recommendations.push('Limited historical data - continue collecting data for better forecasting accuracy');
        }
