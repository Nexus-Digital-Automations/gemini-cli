...
export class TestReporter {
  ...
  private performanceData: Array<{ test: string; duration: number; memory: number }> = [];
   ...
   */
  async finishSuite(suiteName: string): Promise<TestSuite> {
...
<body>
    ...
    <div class="performance">
        ...
        <table class="performance-table">
            ...
            <tbody>
                ${suite.metrics.performance.slowestTests.map(test =>
                    `<tr><td>${test.name}</td><td>${test.duration}ms</td></tr>`
                ).join('')}
            </tbody>
        </table>
...
<testsuites>
   ...
   */
  private async generatePerformanceCSV(suite: TestSuite): Promise<void> {
   ...
   */
  private async generateSummaryReport(suite: TestSuite): Promise<void> {
...
`;
   ...
   */
  async generateAggregateReport(): Promise<void> {
