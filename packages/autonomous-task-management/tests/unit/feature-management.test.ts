...
describe('Feature Management - Autonomous Task Management', () => {
  let AutonomousTaskManagerAPI: any;
  let api: any;
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      AutonomousTaskManagerAPI = class MockTaskManagerAPI {
        ...
        async _saveFeatures(features: any) {
          fsHelper.writeFile(this.featuresPath, JSON.stringify(features, null, 2));
        }

        _validateFeatureData(featureData: any) {
        ...
        async approveFeature(featureId: string, approvalData: any = {}) {
          try {
            await this._ensureFeaturesFile();
            const features = await this._loadFeatures();
        ...
        async rejectFeature(featureId: string, rejectionData: any = {}) {
          try {
            await this._ensureFeaturesFile();
            const features = await this._loadFeatures();
            const feature = features.features.find((f: any) => f.id === featureId);
        ...
        async listFeatures(filter: any = {}) {
          try {
            ...
            if (filter.status) {
              filteredFeatures = filteredFeatures.filter((f: any) => f.status === filter.status);
            }

            if (filter.category) {
              filteredFeatures = filteredFeatures.filter((f: any) => f.category === filter.category);
            }
