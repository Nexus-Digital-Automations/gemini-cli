...
describe('End-to-End Autonomous Workflows', () => {
  ...
  describe('Feature Lifecycle Workflows', () => {
    it('should execute complete feature suggestion-to-implementation workflow', async () => {
      ...
      const featureData = {
        ...
        description: 'Complete user profile management system with CRUD operations, avatar upload, preferences, and privacy settings for comprehensive testing',
        business_value: 'Enables user personalization, improves user engagement, and provides foundation for advanced user features',
        category: 'new-feature'
      };

      const suggestResult = await execTaskManagerCommand('suggest-feature', [JSON.stringify(featureData)]);
      ...
      const taskGenResult = await execTaskManagerCommand('create-task-from-feature', [
        featureId,
        JSON.stringify({
          created_by: agentId,
          metadata: { e2e_test: true, workflow_id: 'complete-feature-lifecycle' }
        })
      ]);
      ...
      const taskQueueResult = await execTaskManagerCommand('get-task-queue', [JSON.stringify({ feature_id: featureId })]);
      ...
      const reinitResult = await execTaskManagerCommand('reinitialize', [agentId]);
      ...
      console.log(`Complete workflow executed in ${totalDuration.toFixed(2)}ms`);
    ...
    it('should handle workflow interruption and recovery', async () => {
      ...
      const featureData = {
        ...
        description: 'Feature for testing workflow interruption and recovery mechanisms',
      ...
      const recoveryFeatures = await execTaskManagerCommand('list-features', [JSON.stringify({ status: 'approved' })]);
      expect(recoveryFeatures.success).toBe(true);
      expect(recoveryFeatures.features.some((f: any) => f.id === featureId)).toBe(true);
    });
    ...
    it('should support bulk workflow operations', async () => {
      ...
      for (let i = 0; i < bulkSize; i++) {
        ...
        const result = await execTaskManagerCommand('suggest-feature', [JSON.stringify(featureData)]);
        if (result.success) {
      ...
      const bulkApproveResult = await execTaskManagerCommand('bulk-approve-features', [
      ...
      const taskGenResult = await execTaskManagerCommand('generate-tasks-from-approved-features', []);
      expect(taskGenResult.success).toBe(true);
      expect(taskGenResult.generated_tasks.length).toBeGreaterThanOrEqual(bulkSize);
  ...
  describe('Multi-Agent Coordination Workflows', () => {
    it('should coordinate multiple specialized agents on complex project', async () => {
      const projectAgents = [
        ...
        { id: 'DEVOPS_SPECIALIST', capabilities: ['deployment', 'performance'] }
      ];
      ...
      const complexFeatureData = {
        ...
        description: 'Complete e-commerce platform with frontend UI, backend API, payment processing, inventory management, user authentication, admin dashboard, testing suite, and deployment pipeline',
        business_value: 'Comprehensive e-commerce solution enabling online sales, inventory management, and customer engagement across multiple channels',
        category: 'new-feature'
      };

      const featureResult = await execTaskManagerCommand('suggest-feature', [JSON.stringify(complexFeatureData)]);
      ...
      console.log(`Multi-agent project setup: ${projectAgents.length} agents, ${queueResult.tasks.length} tasks`);

      // Test agent coordination through statistics
      const finalStats = await execTaskManagerCommand('get-initialization-stats', []);
      expect(finalStats.success).toBe(true);
      expect(finalStats.stats.total_initializations).toBeGreaterThanOrEqual(projectAgents.length);
    }, 45000);

    it('should handle agent handoff and task dependencies', async () => {
      // Create sequential workflow requiring agent handoff
      const workflowAgents = ['ANALYSIS_AGENT', 'IMPLEMENTATION_AGENT', 'TESTING_AGENT'];
      ...
      const sequentialFeature = {
        ...
        description: 'API integration requiring analysis, implementation, and comprehensive testing phases with proper handoff between specialized agents',
        business_value: 'Enables third-party service integration with proper validation and error handling',
        category: 'enhancement'
      };

      const featureResult = await execTaskManagerCommand('suggest-feature', [JSON.stringify(sequentialFeature)]);
      const featureId = featureResult.feature.id;
      await execTaskManagerCommand('approve-feature', [featureId]);

      // Create task with dependencies
      const mainTaskResult = await execTaskManagerCommand('create-task-from-feature', [
        featureId,
        JSON.stringify({
          type: 'implementation',
      ...
      const progressResult = await execTaskManagerCommand('update-task-progress', [
        taskId,
        JSON.stringify({
          status: 'in_progress',
    ...
    it('should maintain agent heartbeats and availability tracking', async () => {
      const monitoringAgents = ['MONITOR_AGENT_1', 'MONITOR_AGENT_2', 'MONITOR_AGENT_3'];
          ...
          while (Date.now() - startTime < monitoringDuration) {
            try {
              ...
              if (reinitResult.success) {
                beats.push({ timestamp: Date.now(), sessionId: reinitResult.agent.sessionId });
              }
            } catch (error) {
      ...
      heartbeatResults.forEach(result => {
        ...
        console.log(`Agent ${result.agentId}: ${result.beats.length} heartbeats`);
      });

      // Verify system tracked the activity
      const stats = await execTaskManagerCommand('get-initialization-stats', []);
  ...
  describe('System Integration Workflows', () => {
    it('should integrate with Gemini CLI project structure', async () => {
      ...
      const initResult = await execTaskManagerCommand('initialize', [projectAgentId]);
      expect(initResult.success).toBe(true);

      // Create project-specific feature
      const projectFeature = {
        title: 'Integration Test - TypeScript Build Pipeline',
        description: 'Implement TypeScript compilation and build pipeline for the E2E test project with proper source maps and type checking',
        business_value: 'Enables production-ready TypeScript builds with comprehensive error checking and optimization',
        category: 'enhancement'
      };

      const featureResult = await execTaskManagerCommand('suggest-feature', [JSON.stringify(projectFeature)]);
      expect(featureResult.success).toBe(true);

      await execTaskManagerCommand('approve-feature', [featureResult.feature.id]);
    ...
    it('should handle CI/CD pipeline integration scenarios', async () => {
      ...
      try {
        ...
        const initResult = await execTaskManagerCommand('initialize', [ciAgentId]);
        expect(initResult.success).toBe(true);
        ...
        const ciFeatures = [
          {
            ...
            description: 'Implement automated testing pipeline with parallel test execution, coverage reporting, and failure notifications',
            business_value: 'Ensures code quality and reduces manual testing overhead in continuous integration',
            category: 'enhancement'
          },
          {
            title: 'CI/CD Integration - Deployment Automation',
            description: 'Implement automated deployment pipeline with staging, production environments, and rollback capabilities',
            business_value: 'Enables reliable and fast deployment cycles with minimal manual intervention',
            category: 'new-feature'
          }
        ];
        ...
        for (const feature of ciFeatures) {
          const result = await execTaskManagerCommand('suggest-feature', [JSON.stringify(feature)]);
          if (result.success) {
    ...
    it('should support external system webhook integration', async () => {
      ...
      const webhookPayloads = [
        {
          ...
          data: {
            ...
            description: 'Implement JWT authentication middleware with refresh token support',
        ...
        },
        {
          ...
          data: {
            ...
            description: 'Memory usage continuously increases during batch processing operations',
            priority: 'high',
            type: 'bug'
          }
        }
      ...
      for (const payload of webhookPayloads) {
        const featureData = {
          ...
          category: payload.source === 'jira' && payload.data.type === 'bug' ? 'bug-fix' : 'enhancement',
          metadata: {
      ...
      const externalFeatures = features.features.filter((f: any) =>
        f.metadata && f.metadata.external_source
      );
      expect(externalFeatures.length).toBe(webhookPayloads.length);

      console.log(`External system integration: ${externalFeatures.length} webhook features processed`);
    });
  });
  ...
  describe('Stop Authorization Workflows', () => {
    it('should execute complete stop authorization workflow', async () => {
      ...
      const completionFeature = {
        ...
        description: 'Feature created to validate stop authorization workflow after task completion',
        business_value: 'Validates proper stop authorization workflow execution',
        category: 'enhancement'
      };

      const featureResult = await execTaskManagerCommand('suggest-feature', [JSON.stringify(completionFeature)]);
      const featureId = featureResult.feature.id;

      // Approve and work on feature
      await execTaskManagerCommand('approve-feature', [featureId]);
      const taskResult = await execTaskManagerCommand('create-task-from-feature', [featureId]);
      ...
      // Authorize stop
      const stopReason = 'All TodoWrite tasks complete and project perfect: linter✅ build✅ start✅ tests✅';
      const stopResult = await execTaskManagerCommand('authorize-stop', [agentId, stopReason]);
    ...
    it('should handle multiple agents attempting stop authorization', async () => {
      ...
      const successful = stopResults.filter(r => r.status === 'fulfilled' && (r.value as any).success);
      ...
      console.log(`Stop authorization race condition: ${successful.length}/${competingAgents.length} successful`);
    });
  });
   ...
   */
  async function execTaskManagerCommand(command: string, args: string[] = []): Promise<any> {
      ...
      child.on('close', (code) => {
        if (code === 0) {
          try {
            ...
            reject(new Error(`Failed to parse JSON response: ${stdout.substring(0, 300)}...`));
          }
        } else {
          reject(new Error(`Command '${command}' failed with code ${code}. stderr: ${stderr || 'none'}, stdout: ${stdout.substring(0, 200)}...`));
        }
      });
