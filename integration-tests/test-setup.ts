...
let testMemoryBaseline: NodeJS.MemoryUsage;
let currentTestFile: string = 'unknown';
 ...
 */
export function registerTestAgent(agentId: string): void {
 ...
 */
export function registerTestFile(filePath: string): void {
 ...
 */
export function registerTestDirectory(dirPath: string): void {
 ...
 */
export function registerTestProcess(pid: number): void {
...
export async function executeTaskManagerCommand(
  ...
  try {
    const output = execSync(
      `timeout ${timeout / 1000}s node "${TASK_MANAGER_API}" ${command} ${args.join(' ')}`,
      {
        encoding: 'utf8',
        timeout,
        stdio: 'pipe',
