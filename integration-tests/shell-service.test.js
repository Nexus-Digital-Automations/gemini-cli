...
describe('ShellExecutionService programmatic integration tests', () => {
    ...
    beforeAll(async () => {
        ...
        testDir = path.join(process.env['INTEGRATION_TEST_FILE_DIR'], 'shell-service-tests');
    ...
    it('should execute a simple cross-platform command (echo)', async () => {
        ...
        const handle = await ShellExecutionService.execute(command, testDir, onOutputEvent, abortController.signal, false);
    ...
    it.runIf(process.platform === 'win32')('should execute "dir" on Windows', async () => {
    ...
    it.skipIf(process.platform === 'win32')('should execute "ls -l" on Unix', async () => {
    ...
    it('should propagate environment variables to the child process', async () => {
        ...
        try {
            const command = process.platform === 'win32' ? `echo %${varName}%` : `echo $${varName}`;
            const onOutputEvent = vi.fn();
            const abortController = new AbortController();
            const handle = await ShellExecutionService.execute(command, testDir, onOutputEvent, abortController.signal, false);
            const result = await handle.result;
...
//# sourceMappingURL=shell-service.test.js.map